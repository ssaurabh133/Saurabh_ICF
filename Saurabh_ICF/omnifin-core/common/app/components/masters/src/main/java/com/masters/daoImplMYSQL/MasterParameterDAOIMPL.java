...
public class MasterParameterDAOIMPL implements masterParameterDAO
{
	...
	{ 
		...
		String query="insert into cr_scorecard_values(SCORE_CARD_ID,SCORING_PARAM_CODE," +
	      		" NUMERIC_VALUE_FROM,NUMERIC_VALUE_TO,CHARACTER_VALUE,PARAMETER_SCORE,MAKER_ID,MAKER_DATE)values(?,?,?,?,?,?,?,DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND))"; 
...
//			
	...
	public int updateParameterList(ParameterScoreDefVO vo)
	{
		...
		try 
		{
			...
			String query="update cr_scorecard_values set SCORE_CARD_ID=?,SCORING_PARAM_CODE=?," +
					"  NUMERIC_VALUE_FROM=?,NUMERIC_VALUE_TO=?,CHARACTER_VALUE=?,PARAMETER_SCORE=?," +
					" MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)" +
					" where SCORE_CARD_ID=? and SCORING_PARAM_CODE=?" ;
