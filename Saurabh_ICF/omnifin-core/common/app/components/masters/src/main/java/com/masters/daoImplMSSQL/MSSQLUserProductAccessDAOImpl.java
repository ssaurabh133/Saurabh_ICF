...
public class MSSQLUserProductAccessDAOImpl implements UserProductAccessDAO{

	static final Logger logger = Logger.getLogger(MSSQLUserProductAccessDAOImpl.class.getName());
	...
	public boolean saveUserProductAccess(UserProductAccessVo vo, String [] productId) 
	{
		...
		try {
			...
			if(vo.getRecStatus() != null && vo.getRecStatus().equalsIgnoreCase("on")){
				...
				bufInsSql.append("UPDATE sec_user_m SET PRODUCT_ACCESS=? , MAKER_ID=?, MAKER_DATE=");
...
//				ArrayList updateList=new ArrayList();
	...
	public ArrayList<UserProductAccessVo> getRecordAtSearch(int currentPageLink) 
	{
		...
		try
		{
			...
			bufInsSql.append("select  a.USER_ID,  b.USER_NAME, a.PRODUCT_ID, c.PRODUCT_DESC,case when a.REC_STATUS='A' then 'Active' when a.REC_STATUS='X' then 'Inactive' end as status ");
			bufInsSql.append(" from cr_user_product_mapping_m a, sec_user_m b,cr_product_m c where a.USER_ID=b.USER_ID and a.product_id=c.product_id");
			...
			if(currentPageLink>1)
			{
				...
				bufInsSql.append(" ORDER BY a.product_id OFFSET ");
	...
	public ArrayList<UserProductAccessVo> searchUserProductRecord(UserProductAccessVo vo)
	{
		...
		try
		{
			...
			bufInsSql.append("select  a.USER_ID,  b.USER_NAME, a.PRODUCT_ID, c.PRODUCT_DESC, case when a.REC_STATUS='A' then 'Active' when a.REC_STATUS='X' then 'Inactive' end as status ");
			bufInsSql.append(" from cr_user_product_mapping_m a, sec_user_m b,cr_product_m c where a.USER_ID=b.USER_ID and a.product_id=c.product_id ");
			bufInsSqlTempCount.append(" select count(1) from (select  a.USER_ID,  b.USER_NAME, a.PRODUCT_ID, c.PRODUCT_DESC, case when a.REC_STATUS='A' then 'Active' when a.REC_STATUS='X' then 'Inactive' end as status from cr_user_product_mapping_m a, sec_user_m b,cr_product_m c where a.USER_ID=b.USER_ID and a.product_id=c.product_id) as b where 'A'='A' ");
