...
public class MSSQLMakeModelMasterDAOImpl implements MakeModelMasterDAO
{
	static final Logger logger = Logger.getLogger(MSSQLMakeModelMasterDAOImpl.class.getName());
...
//changed by abhimanyu on 18/07/2012
	public boolean checkRecord(MakeModelmasterVO vo) 
	{
		...
		{				
			...
			bufInsSql.append("select count(1) from cr_make_model_master a " +
					...
					" and MAKE='"+vo.getMake()+"' and MODEL='"+vo.getModel()+"' and MAKE_TYPE='"+vo.getType()+"'and USES_TYPE='"+vo.getUsesTypeId()+"' " +
					" and MANUFACTURER_ID='"+vo.getManufactId()+"' and b.state_id in ("+vo.getStateId()+");");
	...
	public int getMakeModelID(MakeModelmasterVO vo) 
	{
		...
		{				
			StringBuffer bufInsSql=new StringBuffer();			
			bufInsSql.append("select MAKE_MODEL_ID from cr_make_model_master a " +
					" inner join cr_make_model_master_state_mapping b on a.make_model_id=b.make_model_id " +
	...
	public ArrayList<MakeModelmasterVO> getMakeModelrecords(int currentPageLink) 
	{
		...
		try
		{
			...
			if(currentPageLink>1)			{
				...
				bufInsSql.append(" ORDER BY cr_make_model_master.MAKE_MODEL_ID OFFSET ");
	...
	public String updateMakeModelRecord(MakeModelmasterVO vo,String[] stateId)
	{
	   ...
	   //String makeModelId=""; 
		...
		try 
		{
				...
				StringBuffer bufInsSql = new StringBuffer();
				bufInsSql.append(" update cr_make_model_master set PRODUCT_CATEGORY=?,MAKE=?,MODEL=? , MAKE_TYPE=?, LTV=?, USES_TYPE=?,REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
				...
				bufInsSql.append(dbo);
				bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9),");
				bufInsSql.append("MANUFACTURER_ID=?,VEHICLE_CATEGORY=?,PRODUCT_ID=?,SCHEME_ID=? where MAKE_MODEL_ID=?");
