...
public class MSSQLruleMasterDAOImpl implements ruleMasterDAO {
	private static final Logger logger = Logger.getLogger(MSSQLruleMasterDAOImpl.class.getName());
...
//		{
		...
		bufInsSql.append(" ORDER BY RULE_PARAM_CODE OFFSET ");
...
public boolean updateRuleMaster(Object ob) {
	...
	try {
		...
		bufInsSql.append(" UPDATE cr_rule_param_m set RULE_PARAM_NAME=?,SOURCE_TABLE =?,SOURCE_COLUMN=?,SOURCE_DATA_TYPE=?, REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
		bufInsSql.append(dbo);
		bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
		bufInsSql.append(" where RULE_PARAM_CODE=?");
