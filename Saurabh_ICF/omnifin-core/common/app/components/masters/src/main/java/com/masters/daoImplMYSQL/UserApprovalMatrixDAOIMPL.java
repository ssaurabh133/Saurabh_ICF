...
public class UserApprovalMatrixDAOIMPL implements UserApprovalMatrixDAO{
	
	static final Logger logger = Logger.getLogger(UserApprovalMatrixDAOIMPL.class.getName());
	...
	{
		logger.info("In getApprovedUser()---->");
		DecimalFormat myFormatter = new DecimalFormat("###,##0.00######");
	...
	{		
		...
		if(CommonFunction.checkNull(flag).equalsIgnoreCase("Y"))
		{
			query.append(" select distinct a.USER_ID,b.USER_NAME,USER_ROLE,LEVEL,AMOUNT_FROM,AMOUNT_TO,a.REC_STATUS,"+
		...
		try
		{
			ArrayList header = ConnectionDAOforEJB.sqlSelect(query.toString());
			for(int i=0;i<header.size();i++)
			{	ArrayList header1=(ArrayList)header.get(i);
				if(header1!=null && header1.size()>0)
				{
	...
	{
		logger.info("In getsearchApprovedUser");		
		DecimalFormat myFormatter = new DecimalFormat("###,##0.00######");
		ArrayList list=new ArrayList();
		StringBuffer bufInsSql=new StringBuffer();
		StringBuffer bufInsSqlTempCount = new StringBuffer();
