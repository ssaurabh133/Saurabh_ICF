...
public class MSSQLScoreParamMasterDAOImpl implements ScoreParamMasterDAO {
	private static final Logger logger = Logger.getLogger(MSSQLScoreParamMasterDAOImpl.class.getName());
	...
	public boolean insertScoreMaster(Object ob) {
		...
		try {
			...
			if (!st) {
				...
				bufInsSql.append("INSERT INTO cr_scoring_param(SCORING_PARAM_CODE,SCORING_PARAM_NAME,SOURCE_TABLE,SOURCE_COLUMN,DATA_TYPE,REC_STATUS,MAKER_ID,MAKER_DATE)");
	...
	public ArrayList<scoreMasterVo> searchScoreMaster(Object ob,HttpServletRequest request) {
		...
		try {
			...
			bufInsSql.append(" ORDER BY SCORING_PARAM_CODE OFFSET ");
...
//		}
	...
	public boolean updateScoreMaster(Object ob) {
		...
		try {
			...
			bufInsSql.append(" UPDATE cr_scoring_param set SCORING_PARAM_NAME=?,SOURCE_TABLE =?,SOURCE_COLUMN=?,DATA_TYPE=?, REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
			bufInsSql.append(" dbo.STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
			bufInsSql.append(" where SCORING_PARAM_CODE=?");
	...
	public ArrayList<scoreCardMasterVo> searchCardScoreMaster(Object ob,HttpServletRequest request) {
		...
		try {
			...
			bufInsSql.append("case when sm.REC_STATUS='A' then 'Active' else 'Inactive' end as REC_STATUS,sm.product_id ");
			...
			bufInsSql.append(" ORDER BY SCORE_CARD_ID OFFSET ");
			bufInsSql.append(startRecordIndex);
			bufInsSql.append(" ROWS FETCH next ");
			bufInsSql.append(endRecordIndex);
			bufInsSql.append(" ROWS ONLY ");
...
//		}
	...
	public boolean updateCardScoreMaster(Object ob) {
				...
				try{	
					...
					bufInsSql.append(" UPDATE cr_scorecard_m set SCORE_CARD_DESC=?,PRODUCT_ID=?, REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
					bufInsSql.append(" dbo.STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
