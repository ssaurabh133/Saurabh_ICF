...
public class MSSQLPCDMasterDAOImpl implements PCDMasterDAO {
	private static final Logger logger = Logger.getLogger(MSSQLPCDMasterDAOImpl.class.getName());
	...
	public ArrayList<pcdMasterVo> searchScreenPolicyListData(Object ob) {
		...
		try {
			...
			bufInsSql.append("select DISTINCT pm.PRODUCT_DESC,pcm.product_id,sm.SCHEME_DESC,pcm.scheme_id,cs.STAGE_DESCRIPTION," +
							"case when pcm.REC_STATUS='A' then 'Active' else 'Inactive' end as REC_STATUS,pcm.STAGE_ID  from cr_policy_check_m pcm inner join cr_product_m pm on " +
							"pcm.PRODUCT_ID=pm.PRODUCT_ID inner join cr_scheme_m sm on pcm.SCHEME_ID=sm.SCHEME_ID " +
							"inner join policy_check_stages cs on cs.STAGE_ID=pcm.stage_id ");
			//bufInsSql.append("group by pcm.product_id,pcm.scheme_id,pcm.stage_id ");
...
//			}
			...
			bufInsSqlTmp.append("select count(1) from (select DISTINCT pm.PRODUCT_DESC,pcm.product_id,sm.SCHEME_DESC,pcm.scheme_id,cs.STAGE_DESCRIPTION," +
					"case when pcm.REC_STATUS='A' then 'Active' else 'Inactive' end as REC_STATUS,pcm.STAGE_ID  from cr_policy_check_m pcm inner join cr_product_m pm on " +
					"pcm.PRODUCT_ID=pm.PRODUCT_ID inner join cr_scheme_m sm on pcm.SCHEME_ID=sm.SCHEME_ID " +
					"inner join policy_check_stages cs on cs.STAGE_ID=pcm.stage_id )  tabl ");
...
//
//			}
		    logger.info("bufInsSqlTmp: "+bufInsSqlTmp.toString());
			...
			if(pcdMasterVo.getCurrentPageLink()>1)
			{
				...
				bufInsSql.append(" ORDER BY pcm.product_id OFFSET ");
...
//		}
	...
	public ArrayList<pcdMasterVo> searchPolicyListData(Object ob) {
			...
			try {
				...
				StringBuffer bufInsSql = new StringBuffer();
				bufInsSql.append("select DISTINCT pm.PRODUCT_DESC,pcm.product_id,sm.SCHEME_DESC,pcm.scheme_id,cs.STAGE_DESCRIPTION," +
						"case when pcm.REC_STATUS='A' then 'Active' else 'Inactive' end as REC_STATUS,pcm.STAGE_ID from cr_policy_check_m pcm left outer join cr_product_m pm on " +
				...
				bufInsSqlTmp.append("SELECT COUNT(1) FROM ( select DISTINCT pm.PRODUCT_DESC,pcm.product_id,sm.SCHEME_DESC,pcm.scheme_id,cs.STAGE_DESCRIPTION," +
						...
						"pcm.PRODUCT_ID=pm.PRODUCT_ID left outer join cr_scheme_m sm on pcm.SCHEME_ID=sm.SCHEME_ID " +
						"left outer join policy_check_stages cs on cs.STAGE_ID=pcm.stage_id ");
				//bufInsSqlTmp.append("SELECT COUNT(1) FROM ( select distinct pm.PRODUCT_DESC,pcm.product_id,sm.SCHEME_DESC,pcm.scheme_id,cs.STAGE_DESCRIPTION,pcm.rule_code,pcm.policy_type,pcm.approval_level,case when pcm.REC_STATUS='A' then 'Active' else 'Inactive' end as REC_STATUS from cr_policy_check_m pcm left outer join cr_product_m pm on pcm.PRODUCT_ID=pm.PRODUCT_ID left outer join cr_scheme_m sm on pcm.SCHEME_ID=sm.SCHEME_ID left outer join policy_check_stages cs on cs.STAGE_ID=pcm.stage_id ");
	...
	public ArrayList<pcdMasterVo> searchPolicyListDataForEditting(Object ob,String product,String scheme, String stageId) {
		...
		try {
			...
			bufInsSql.append("select pm.PRODUCT_DESC,pcm.product_id,sm.SCHEME_DESC,pcm.scheme_id,cs.STAGE_DESCRIPTION,pcm.rule_code," +
							"pcm.policy_type,pcm.approval_level,case when pcm.REC_STATUS='A' then 'on' else 'off' end as REC_STATUS,rm.RULE_DESCRIPTION,pcm.STAGE_ID,pcm.RULE_DTL_ID from cr_policy_check_m pcm left outer join cr_product_m pm on " +
							"pcm.PRODUCT_ID=pm.PRODUCT_ID left outer join cr_scheme_m sm on pcm.SCHEME_ID=sm.SCHEME_ID " +
							"left outer join policy_check_stages cs on cs.STAGE_ID=pcm.stage_id " +
