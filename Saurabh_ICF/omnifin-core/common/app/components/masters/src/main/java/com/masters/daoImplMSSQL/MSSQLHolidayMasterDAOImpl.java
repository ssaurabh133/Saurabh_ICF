...
public class MSSQLHolidayMasterDAOImpl implements HolidayMasterDAO  {
	private static final Logger logger = Logger.getLogger(MSSQLHolidayMasterDAOImpl.class.getName());
	...
	public boolean insertHolidayMaster(HolidayMasterVo ob) {
		...
		try{
			...
			query.append("select HOLIDAY_DATE from com_holiday_m where ");
			query.append(dbo);
			query.append("date(HOLIDAY_DATE)=");
			query.append(dbo);
			query.append("STR_TO_DATE('"+StringEscapeUtils.escapeSql(vo.getHolidayDate()).trim()+"','"+dateFormat+"')");
			logger.info("In insertHolidayMaster............inside ejb server file..........Dao Impl"+ query);
		...
		bufInsSql.append("insert into com_holiday_m(HOLIDAY_DATE,HOLIDAY_DESC,REC_STATUS,MAKER_ID,MAKER_DATE,AUTHOR_ID,AUTHOR_DATE,HOLIDAY_TYPE)");
...
//		else
	...
	public ArrayList searchHolidayData(HolidayMasterVo ob) {
		...
		try{
			...
			bufInsSql.append("SELECT HOLIDAY_ID,");
			bufInsSql.append(dbo);
			bufInsSql.append("DATE_FORMAT(HOLIDAY_DATE,'"+dateFormat+"') as HOLIDAY_DATE,HOLIDAY_DESC,");
			bufInsSql.append("case when REC_STATUS='A' then 'Active' else 'Inactive' end as REC_STATUS , HOLIDAY_TYPE");
			...
			if (!(holidaySearchDate.equals("")) && !(holidaySearchDes.equals("")) && !(holidayTypeSearch.equals(""))) {
				...
				bufInsSql.append(dbo);
				bufInsSql.append("date(HOLIDAY_DATE)=" );
				bufInsSql.append(dbo);
				bufInsSql.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')" +" AND HOLIDAY_DESC like '%" + holidaySearchDes + "%' " +"AND HOLIDAY_TYPE='"+holidayTypeSearch+"'");
				bufInsSqlTempCount.append(" WHERE ");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("date(HOLIDAY_DATE)=");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')"  + " AND HOLIDAY_DESC like '%" + holidaySearchDes + "%' "+"AND HOLIDAY_TYPE='"+holidayTypeSearch+"'");
			}
				...
				bufInsSql.append("date(HOLIDAY_DATE)=");
				bufInsSql.append(dbo);
				bufInsSql.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')" +" AND HOLIDAY_TYPE ='" + holidayTypeSearch +"'");
				bufInsSqlTempCount.append(" WHERE ");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("date(HOLIDAY_DATE)=");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')"  + "AND HOLIDAY_TYPE ='" + holidayTypeSearch +"'");
			}
				...
				bufInsSql.append("date(HOLIDAY_DATE)=");
				bufInsSql.append(dbo);
				bufInsSql.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')" +" AND HOLIDAY_DESC like '%" + holidaySearchDes + "%' ");
				bufInsSqlTempCount.append(" WHERE ");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("date(HOLIDAY_DATE)=");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')"  + " AND HOLIDAY_DESC like '%" + holidaySearchDes + "%' ");
			}
				...
				bufInsSql.append("date(HOLIDAY_DATE)=");
				bufInsSql.append(dbo);
				bufInsSql.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')");
				bufInsSqlTempCount.append(" WHERE ");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("date(HOLIDAY_DATE)=");
				bufInsSqlTempCount.append(dbo);
				bufInsSqlTempCount.append("STR_TO_DATE('"+holidaySearchDate+"','"+dateFormat+"')");
			}
...
//				{
				...
				bufInsSql.append(" ORDER BY HOLIDAY_ID OFFSET ");
...
//				}
				...
				logger.info("IN searchHolidayData() search query1 ### "+ bufInsSql.toString());
				logger.info("In searchHolidayData............inside ejb server file..........Dao Impl");
				logger.info("searchHolidayData " + searchlist.size());
...
//				+ "HOLIDAY_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where HOLIDAY_ID=?");
		...
		bufInsSql.append(dbo);
		bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
		bufInsSql.append(" where HOLIDAY_ID=?");
	...
	public boolean searchForHolidayMaster(String holidayDate){		
		...
		selquery.append("select HOLIDAY_DATE from com_holiday_m where ");
		selquery.append(dbo);
		selquery.append("date(HOLIDAY_DATE)=");
		selquery.append(dbo);
		selquery.append("STR_TO_DATE('"+holidayDate+"','"+dateFormat+"')");
		 logger.info("In SearchHolidayMethod.....................................Dao "+selquery.toString());
		 
		 try{
		  flag = ConnectionDAOforEJB.checkStatus(selquery.toString());
		 }catch(Exception e){
			 logger.info("******* Exception in Holiday Search Query *****************"+selquery.toString());
