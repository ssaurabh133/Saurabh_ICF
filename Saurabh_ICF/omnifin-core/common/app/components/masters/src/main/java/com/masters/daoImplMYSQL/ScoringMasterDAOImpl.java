...
public class ScoringMasterDAOImpl implements ScoringMasterDAO{
	private static final Logger logger = Logger.getLogger(ScoringMasterDAOImpl.class.getName());
	...
	public String saveScoringMaster(ScoringMasterVO vo) {
		...
		try {
			...
			if (!st) {
				...
				if(status){
					String query1="select max(SCORE_CARD_ID) FROM cr_scorecard_m for update";
	...
	public ArrayList<ScoringMasterVO> editScoringMasterHeader(ScoringMasterVO vo) {
		...
		try {
			...
			bufInsSql.append("select a.SCORE_CARD_ID,a.SCORE_CARD_DESC,a.PRODUCT_ID,b.product_desc,a.SCHEME_ID,c.scheme_desc,if(a.REC_STATUS='A','Active','Inactive') from cr_scorecard_m a JOIN cr_product_m b");
	...
	public ArrayList<ScoringMasterVO> editScoringMasterdtl(ScoringMasterVO vo) {
		...
		try {
			...
			bufInsSql.append("Select a.SCORING_PARAM_CODE,b.SCORING_PARAM_NAME,ifnull(a.SCORING_WEIGHTAGE,0),ifnull(a.SCORING_DEFAULT_SCORE,0) from cr_scorecard_weightage a join cr_scoring_param b");
	...
	public ArrayList<ScoringMasterVO> getScoringMasterList(ScoringMasterVO vo) {
		...
		try {
			...
			bufInsSql.append("select a.SCORE_CARD_ID,a.SCORE_CARD_DESC,b.product_desc,c.scheme_desc,if(a.REC_STATUS='A','Active','Inactive') from cr_scorecard_m a JOIN cr_product_m b");
			...
			bufInsSql.append(" ORDER BY a.SCORE_CARD_DESC");
	...
	public String updateScoringMaster(ScoringMasterVO vo) {
		...
		try {
			...
			if (!st) {
				...
				StringBuffer bufInsSql = new StringBuffer();
				bufInsSql.append("UPDATE cr_scorecard_m SET SCORE_CARD_DESC=?,PRODUCT_ID=?,SCHEME_ID=?,REC_STATUS=?,MAKER_ID=?,");
				bufInsSql.append("  MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,");
	...
	public String saveScoringMasterParamValue(ScoringMasterVO vo) {
		...
		try {
			...
			boolean st = ConnectionDAOforEJB.checkStatus(query);
			logger.info("st -----" + st);
			String recStatus=" Select ifnull(REC_STATUS,'X') REC_STATUS from cr_scorecard_m where SCORE_CARD_ID='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getScoringId()).trim()) + "'";
