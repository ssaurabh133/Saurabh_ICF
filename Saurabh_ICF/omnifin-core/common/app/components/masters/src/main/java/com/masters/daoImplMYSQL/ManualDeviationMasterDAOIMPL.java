...
public class ManualDeviationMasterDAOIMPL implements ManualDeviationMasterDAO {
	static final Logger logger = Logger.getLogger(ManualDeviationMasterDAOIMPL.class.getName());
	...
	public boolean saveManualDeviation(ManulaDeviationVO vo) {
		...
		try {
			...
			else
			{
				...
				bufInsSql.append("update cr_manual_deviation_m set PRODUCT_ID=?,SCHEME_ID=?,STAGE_ID=?,RULE_DESC=?,SUB_RULE_TYPE=?,APPROVAL_LEVEL=?,REC_STATUS=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where MANUAL_DEVIATION_ID=?");
				if (CommonFunction.checkNull(vo.getLbxProductID()).equalsIgnoreCase(""))
					insertPrepStmtObject.addNull();
				else
					insertPrepStmtObject.addString(vo.getLbxProductID().trim());
	...
	public ArrayList<ManulaDeviationVO> getManualDeviationRecords(int currentPageLink) 
	{
		...
		try
		{
			...
			bufInsSql.append("select m.MANUAL_DEVIATION_ID,m.PRODUCT_ID,p.PRODUCT_DESC,m.SCHEME_ID,s.SCHEME_DESC,m.STAGE_ID,st.STAGE_DESC,m.APPROVAL_LEVEL,m.RULE_DESC," +
					" if(m.REC_STATUS='A','Active','Inactive')as status  from cr_manual_deviation_m m" +
