package com.customerService.daoImplMSSQL;
...
public class MSSQLCustomerServiceDAOImpl implements CustomerServiceDAO {
	
	private static final Logger logger = Logger.getLogger(MSSQLCustomerServiceDAOImpl.class.getName());
	...
	public ArrayList<CustomerServiceVo> caseMarkingDetail(CustomerServiceVo ob) {
		...
		try {
				query.append(" SELECT DISTINCT G.DESCRIPTION,IIF(M.CASE_MARKING_FLAG='Y','YES','NO') AS CASE_MARKING_FLAG," );
				query.append(" IIF(M.CASE_MARKING_FLAG='Y',"+dbo+"DATE_FORMAT(M.CASE_MARKING_DATE,'"+dateFormat+"'),''),A.AGENCY_NAME,M.OTHER_DETAILS,M.REMARKS, " );
				query.append(" IIF(M.CASE_MARKING_FLAG='N',"+dbo+"DATE_FORMAT(HIST.CASE_MARKING_DATE,'"+dateFormat+"'),'') " );
	...
	public ArrayList<CustomerServiceVo> closureDetail(CustomerServiceVo ob) {
		...
		ArrayList innerList = new ArrayList();
		try {
				query.append(" SELECT DISTINCT IIF(T.TERMINATION_TYPE='X','CANCEL', IIF(T.TERMINATION_TYPE='T','TERMINATION',IIF(T.TERMINATION_TYPE='C','CLOSURE','NA'   ))) AS TERMINATION_TYPE ,  " );
				query.append(dbo);
	...
	public ArrayList<CustomerServiceVo> loanSummaryViewer(CustomerServiceVo ob) {
		...
		ArrayList innerList = new ArrayList();
		try {
		query.append("SELECT DISTINCT LOAN_NO,C.CUSTOMER_NAME,IIF(CLD.REC_STATUS='P','Pending' ,IIF(CLD.REC_STATUS='A','Active', IIF(CLD.REC_STATUS='L','Cancelled', IIF(CLD.REC_STATUS='X','Rejected',     IIF(CLD.REC_STATUS='C','Closed', 'NA'))))) AS REC_STATUS," );
		query.append(" IIF(CLD.DISBURSAL_STATUS='N','Not Disbursed', IIF(CLD.DISBURSAL_STATUS='P','Partially Disbursed',IIF(CLD.DISBURSAL_STATUS='F','Fully Disbursed','NA'   ))) AS DISBURSAL_STATUS,  " );
		query.append(" IIF(CLD.FILE_STATUS='SO','Send To OPS', IIF(CLD.FILE_STATUS='RO','Received By OPS',IIF(CLD.FILE_STATUS='RU','Received By User',IIF(CLD.FILE_STATUS='H','Hold',IIF(CLD.FILE_STATUS='C','Clear',IIF(CLD.FILE_STATUS='SS','Send To Store',IIF(CLD.FILE_STATUS='RS','Received By Store','NA'   )    )   )   )   ))) AS FILE_STATUS , " );
		query.append(dbo);
		query.append("DATE_FORMAT(LOAN_APPROVAL_DATE,'"+dateFormat+"'), ");
		query.append(" IIF(CLD.LOAN_REPAYMENT_MODE='O','OTC', IIF(CLD.LOAN_REPAYMENT_MODE='P','PDC',IIF(CLD.LOAN_REPAYMENT_MODE='E','ECS',IIF(CLD.LOAN_REPAYMENT_MODE='N','NPDC',IIF(CLD.LOAN_REPAYMENT_MODE='R','RPDC',IIF(CLD.LOAN_REPAYMENT_MODE='D','DIRECT DEBIT',IIF(CLD.LOAN_REPAYMENT_MODE='S','ESCROW','NA'   )   )   )   )   ))) AS LOAN_REPAYMENT_MODE ,LOAN_FLAT_RATE,LOAN_EFF_RATE, ");
		query.append(" GI.DESCRIPTION,IIF(CLD.LOAN_INSTALLMENT_MODE='A','ADVANCE', IIF(CLD.LOAN_INSTALLMENT_MODE='R','ARREAR','NA')) AS LOAN_INSTALLMENT_MODE ,G.DESCRIPTION,NPA_FLAG,LOAN_DPD,LOAN_DPD_STRING,LOAN_TENURE," );
		query.append(dbo);
		query.append("DATE_FORMAT(LOAN_MATURITY_DATE,'"+dateFormat+"')  ");
	...
	public ArrayList<CustomerServiceVo> reshcedulingDetail(CustomerServiceVo ob) {
		...
		ArrayList innerList = new ArrayList();
		try {
				query.append(" SELECT DISTINCT IIF(R.RESCH_TYPE='R','Repricing', IIF(R.RESCH_TYPE='P','Part Payment',IIF(R.RESCH_TYPE='D','EMI Deferral',IIF(R.RESCH_TYPE='U','Due date change',IIF(R.RESCH_TYPE='A','Additional Disbursal','NA'   )   )   ))) AS RESCH_TYPE ,  " );
				query.append(dbo);
	...
	public ArrayList<CustomerServiceVo> secuitizationDetail(CustomerServiceVo ob) {
		...
		ArrayList innerList = new ArrayList();
		try {
				query.append(" SELECT DISTINCT IIF(PM.POOL_TYPE='S','Securitized','Re-finance') AS POOL_TYPE," );
				query.append(dbo);
				query.append("DATE_FORMAT(PM.POOL_CREATION_DATE,'"+dateFormat+"') AS POOL_CREATION_DATE,A.AGENCY_NAME  " );
			...
			for (int i = 0; i < size; i++) {
				...
				if (innerList.size() > 0) {
					...
					vo.setSecPoolType((CommonFunction.checkNull(innerList.get(0)).trim()));
					vo.setSecPoolMarkingDate((CommonFunction.checkNull(innerList.get(1)).trim()));
					vo.setSecAgencyName((CommonFunction.checkNull(innerList.get(2)).trim()));
					list.add(vo);
	...
	public ArrayList<CustomerServiceVo> customerExposureListViewer(
		...
		ArrayList innerList = new ArrayList();
		try {
				query.append(" SELECT DISTINCT L.LOAN_NO,IIF(L.REC_STATUS='A','Approved',IIF(L.REC_STATUS='X','Rejected',IIF(L.REC_STATUS='P','Pending',IIF(L.REC_STATUS='F','Forward',IIF(L.REC_STATUS='C','Closed',IIF(L.REC_STATUS='L','Canceled','NA'))))))AS 'LOAN STATUS',  " );
				query.append(" IIF(L.DISBURSAL_STATUS='P','Partially Disbursed',IIF(L.DISBURSAL_STATUS='F','Full Disbursed',IIF(L.DISBURSAL_STATUS='N','Not Disbursed','NA')))AS 'DISBURSAL STATUS' ,"+dbo+"DATE_FORMAT(L.NEXT_DUE_DATE,'"+dateFormat+"') AS 'FIRST EMI' ,"+dbo+"DATE_FORMAT(DATEADD(MONTH,LOAN_NO_OF_INSTALLMENT,L.NEXT_DUE_DATE),'"+dateFormat+"') AS 'LAST EMI' ,  " );
				query.append(" IIF(R.LOAN_CUSTOMER_ROLE_TYPE='PRAPPL','Applicant',IIF(R.LOAN_CUSTOMER_ROLE_TYPE='COAPPL','Co-Applicant',IIF(R.LOAN_CUSTOMER_ROLE_TYPE='GUARANTOR','Guarantor','NA'))) AS 'CUSTOMER ROLE',ISNULL(LOAN_BALANCE_PRINCIPAL,0),ISNULL(LOAN_OVERDUE_AMOUNT,0),(SUM(ISNULL(ADVICE_AMOUNT,0))-SUM(ISNULL(TXN_ADJUSTED_AMOUNT,0))) AS 'OVERDUE CHARGES',LOAN_DPD,  " );
				query.append(" NPA_FLAG,IIF(L.REPO_FLAG='A','YES',IIF(L.REPO_FLAG='X','NO','NA')) AS 'REPO FLAG',IIF(L.LEGAL_FLAG='A','YES',IIF(L.LEGAL_FLAG='X','NO','NA')) AS 'LEGAL FLAG','SOA LINK',L.LOAN_ID,G.CUSTOMER_NAME  " );
				...
				{
					query.append(" (SELECT ISNULL(A.GCD_ID,0)  FROM CR_DEAL_CUSTOMER_ROLE A WHERE A.DEAL_ID='"+CommonFunction.checkNull(ob.getTxnId())+"')  ");
				}
				else
				{
					query.append(" (SELECT ISNULL(A.GCD_ID,0) FROM CR_LOAN_CUSTOMER_ROLE A WHERE A.LOAN_ID='"+CommonFunction.checkNull(ob.getTxnId())+"')  ");
				}
				...
				query.append(" GROUP BY L.LOAN_ID,L.LOAN_NO,L.REC_STATUS,L.DISBURSAL_STATUS,L.NEXT_DUE_DATE,LOAN_NO_OF_INSTALLMENT,R.LOAN_CUSTOMER_ROLE_TYPE,LOAN_BALANCE_PRINCIPAL,LOAN_OVERDUE_AMOUNT,LOAN_DPD,NPA_FLAG,REPO_FLAG,LEGAL_FLAG,G.CUSTOMER_NAME  ");
	...
	public ArrayList<CustomerServiceVo> notePadListLoanSummaryDetail(CustomerServiceVo ob) {
		...
		try{
		  ...
		  query.append("select distinct b.description ,"+dbo+"DATE_FORMAT(notepad_date,'"+dateFormatWithTime+"'),person_met,meeting_location,iif(followup_reqd='N','NO','YES') as followup_reqd,"+
						" "+dbo+"DATE_FORMAT(followup_date,'"+dateFormatWithTime+"'),followup_location,notepad_comments,followup_remarks,FOLLOWUP_PERSON, " +
						" s.USER_NAME,"+dbo+"DATE_FORMAT(a.maker_date,'"+dateFormatWithTime+"'),'LOAN INITIATION MAKER' from cr_notepad_dtl a,generic_master b,sec_user_m s"
		  ...
		  query.append("select distinct b.description ,"+dbo+"DATE_FORMAT(notepad_date,'"+dateFormatWithTime+"'),person_met,meeting_location,iif(followup_reqd='N','NO','YES') as followup_reqd,"+
					" "+dbo+"DATE_FORMAT(followup_date,'"+dateFormatWithTime+"'),followup_location,notepad_comments,followup_remarks,FOLLOWUP_PERSON, " +
					" s.USER_NAME,"+dbo+"DATE_FORMAT(a.maker_date,'"+dateFormatWithTime+"'),'DEAL CAPTURE' from cr_notepad_dtl a,generic_master b,sec_user_m s"
					+" where notepad_code=b.value and " +
		  ...
		  query.append("select distinct b.description ,"+dbo+"DATE_FORMAT(notepad_date,'"+dateFormatWithTime+"'),person_met,meeting_location,iif(followup_reqd='N','NO','YES') as followup_reqd,"+
					" "+dbo+"DATE_FORMAT(followup_date,'"+dateFormatWithTime+"'),followup_location,notepad_comments,followup_remarks,FOLLOWUP_PERSON, " +
					" s.USER_NAME,"+dbo+"DATE_FORMAT(a.maker_date,'"+dateFormatWithTime+"'),'LEAD TRACKING' from cr_notepad_dtl a,generic_master b,sec_user_m s"
					+" where notepad_code=b.value and " +
		  ...
		  query.append("select distinct b.description ,"+dbo+"DATE_FORMAT(notepad_date,'"+dateFormatWithTime+"'),person_met,meeting_location,iif(followup_reqd='N','NO','YES') as followup_reqd,"+
					" "+dbo+"DATE_FORMAT(followup_date,'"+dateFormatWithTime+"'),followup_location,notepad_comments,followup_remarks,FOLLOWUP_PERSON, " +
					" s.USER_NAME,"+dbo+"DATE_FORMAT(a.maker_date,'"+dateFormatWithTime+"'),'LEAD PROCESSING' from cr_notepad_dtl a,generic_master b,sec_user_m s"
					+" where notepad_code=b.value and " +
