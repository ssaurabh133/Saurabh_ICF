package com.commonFunction.daoImplMYSQL;
...
public class commonFunctionDaoImpl implements commonDao
{
	...
	try 
	{
		...
		StringBuilder query=new StringBuilder();
		 ...
		 query.append("select cr_product_m.REPAYMENT_TYPE,cr_product_m.ASSET_FLAG,cr_deal_loan_dtl.DEAL_ASSET_COST as astcstLD," +
		 		...
		 		"  left outer join (select a.deal_id,sum(ifnull(b.ASSET_COLLATERAL_VALUE,0)) as ac,sum(ifnull(b.LOAN_AMOUNT,0)) as lm " +
	...
	public ArrayList<Object> getIndividualDetails(String code, Object pageStatus,String updateInMaker,String statusCase,String updateFlag,String pageStatuss)  
{
	...
	if((pageStatuss!=null && pageStatuss.equals("fromLeads"))  || (updateFlag!=null && updateFlag.equals("updateFlag")) ||(updateFlag!=null && updateFlag.equals("notEdit")))
	{
		...
		try
		{
			...
			logger.info("customer status ................................................. "+custStatus);
			 query="select CUSTOMER_ID, CUSTOMER_FNAME,CUSTOMER_MNAME,CUSTOMER_LNAME,CUSTOMER_EMAIL,DATE_FORMAT(CUSTOMER_DOB,'"+dateFormat+"'),CUSTMER_PAN,CUSTOMER_REFERENCE,CUSTOMER_BLACKLIST,CUSTOMER_BLACKLIST_REASON,CUSTOMER_CATEGORY,CUSTOMER_TYPE,CUSTOMER_CONSTITUTION,FATHER_HUSBAND_NAME,CASTE_CATEGORY,MARITAL_STATUS,DRIVING_LICENSE,VOTER_ID,PASSPORT_NUMBER " +
	...
	else
	{
			...
			{	
				...
				query="select CUSTOMER_ID, CUSTOMER_FNAME,CUSTOMER_MNAME,CUSTOMER_LNAME,CUSTOMER_EMAIL,DATE_FORMAT(CUSTOMER_DOB,'"+dateFormat+"'),CUSTMER_PAN,CUSTOMER_REFERENCE,CUSTOMER_BLACKLIST,CUSTOMER_BLACKLIST_REASON,CUSTOMER_CATEGORY,CUSTOMER_TYPE,CUSTOMER_STATUS,CUSTOMER_CONSTITUTION,FATHER_HUSBAND_NAME,CASTE_CATEGORY,MARITAL_STATUS,DRIVING_LICENSE,VOTER_ID,PASSPORT_NUMBER " +
				      "from "+tableName+" where CUSTOMER_ID="+code+"" ;
				 logger.info("In getIndividualDetails() of CortateDAOImpl.  Query : "+query);
				ArrayList individualDetails = ConnectionDAO.sqlSelect(query);
				logger.info("getIndividualDetails "+individualDetails.size());
	...
	public int setApproveStatus(int cId, String statusCase,String userId,String businessDate)
	{
		...
		boolean qrystatus=false;
			try{
				...
				{
					query="update gcd_customer_m set CUSTOMER_STATUS='A',MAKER_ID='"+CommonFunction.checkNull(userId)+"'," +
						  "MAKER_DATE=DATE_ADD(STR_TO_DATE('"+CommonFunction.checkNull(businessDate)+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where CUSTOMER_ID=?";
				}
	...
	public boolean saveCorporateUpdate(Object ob, int id, String recStatus, String statusCase,String updateFlag,String pageStatuss) 
	{
		...
		{				
			if(pageStatuss!=null && pageStatuss.equals("fromLeads")||updateFlag!=null && updateFlag.equals("updateFlag"))
			{
					...
					queryUpdate="update "+tableName+" set CUSTOMER_NAME =?,CUSTOMER_FNAME=?,CUSTOMER_MNAME=?,CUSTOMER_LNAME=?,CUSTOMER_CATEGORY=?,CUSTOMER_DOB=STR_TO_DATE(?,'"+dateFormat+"')," +
								...
								" CUSTOMER_BLACKLIST_REASON=?,CUSTOMER_TYPE=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),CUSTOMER_GROUP_ID=?,AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),FATHER_HUSBAND_NAME=?,CASTE_CATEGORY=?,MARITAL_STATUS=?,DRIVING_LICENSE=?,VOTER_ID=?,PASSPORT_NUMBER=? where CUSTOMER_ID=?";
				...
				else
				{
					...
					bufInsUpdSql.append(" INSERT INTO GCD_CUSTOMER_HST ");
					  ...
					  bufInsUpdSql.append("MAKER_DATE,GROUP_ID,AUTHOR_ID,");
					  bufInsUpdSql.append("AUTHOR_DATE,FATHER_HUSBAND_NAME,CASTE_CATEGORY,");
					  bufInsUpdSql.append("MARITAL_STATUS,DRIVING_LICENSE,VOTER_ID,PASSPORT_NUMBER,DATE_ADD(STR_TO_DATE('"+CommonFunction.checkNull(corporateDetailVo.getMakerDate())+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),'"+CommonFunction.checkNull(corporateDetailVo.getMakerId())+"'");
					...
					{				
						queryUpdate="update "+tableName+" set CUSTOMER_NAME =?,CUSTOMER_FNAME=?,CUSTOMER_MNAME=?,CUSTOMER_LNAME=?,CUSTOMER_CATEGORY=?,CUSTOMER_DOB=STR_TO_DATE(?,'"+dateFormat+"')," +
									" CUSTOMER_CONSTITUTION=?,CUSTOMER_REGISTRATION_TYPE=?,CUSTOMER_REGISTRATION_NO=?,CUSTMER_PAN=?,CUSTOMER_VAT_NO=?,CUSTOMER_INDUSTRY=?," +
									" CUSTOMER_SUB_INDUSTRY=?,CUSTOMER_BUSINESS_SEGMENT=?,CUSTOMER_EMAIL=?,CUSTOMER_WEBSITE=?,CUSTOMER_REFERENCE=?,CUSTOMER_BLACKLIST=?," +
									" CUSTOMER_BLACKLIST_REASON=?,CUSTOMER_TYPE=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),FATHER_HUSBAND_NAME=?,CASTE_CATEGORY=?,MARITAL_STATUS=?,DRIVING_LICENSE=?,VOTER_ID=?,PASSPORT_NUMBER=? ,GROUP_ID=? where CUSTOMER_ID=?";
				...
				ArrayList qryList = new ArrayList();
				logger.info("CorporateDAOImpl saveCorporateUpdate().............fromLeads..........................");
				queryUpdate1="update cr_deal_customer_m set CUSTOMER_NAME =?,CUSTOMER_FNAME=?,CUSTOMER_MNAME=?,CUSTOMER_LNAME=?,CUSTOMER_CATEGORY=?,CUSTOMER_DOB=STR_TO_DATE(?,'"+dateFormat+"')," +
							" CUSTOMER_CONSTITUTION=?,CUSTOMER_REGISTRATION_TYPE=?,CUSTOMER_REGISTRATION_NO=?,CUSTMER_PAN=?,CUSTOMER_VAT_NO=?,CUSTOMER_INDUSTRY=?," +
							" CUSTOMER_SUB_INDUSTRY=?,CUSTOMER_BUSINESS_SEGMENT=?,CUSTOMER_EMAIL=?,CUSTOMER_WEBSITE=?,CUSTOMER_REFERENCE=?,CUSTOMER_BLACKLIST=?," +
							" CUSTOMER_BLACKLIST_REASON=?,CUSTOMER_TYPE=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),CUSTOMER_GROUP_ID=?,AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),FATHER_HUSBAND_NAME=?,CASTE_CATEGORY=?,MARITAL_STATUS=?,DRIVING_LICENSE=?,VOTER_ID=?,PASSPORT_NUMBER=? where GCD_CUSTOMER_ID=?";
				...
				insertPrepStmtObject6.setSql(queryUpdate1);
...
//										insertPrepStmtObject1.addNull();
								...
								else if(cv.getHiApplType()!=null && cv.getHiApplType().equals("PRAPPL"))
								{
						            ...
						            logger.info("In saveCorporateDetails update deal table  "+status);
							            ...
							            bufInsSql2.append(" ?," );
							            bufInsSql2.append(" ?," );
							            bufInsSql2.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) )" );
...
//											insertPrepStmtObject2.addNull();
			...
			//logger.info("*********************************"+corporateDetailVo.getMakerId());
				...
				qryList.add(insertPrepStmtObject);
				 ...
				 String gcdID=ConnectionDAO.singleReturn("select max(CUSTOMER_ID) from gcd_customer_m for update");
		...
		try
		{
			if(pageStatuss!=null && pageStatuss.equals("fromLeads")||updateFlag!=null && updateFlag.equals("updateFlag"))
			{
				...
				String query="update "+tableName+" set ADDRESS_TYPE=?,BPTYPE=?,BPID=?,ADDRESS_LINE1=?, " +
				     ...
				     "MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),TAHSIL=?,BRANCH_DISTANCE=?,COMMUNICATION_ADDRESS=? where ADDRESS_ID=?";
			...
			else
			{
		        ...
		        bufInsUpdSql.append(" ( ");
		          ...
		          bufInsUpdSql.append(" PINCODE,PRIMARY_PHONE,ALTERNATE_PHONE,TOLLFREE_NUMBER,FAX, LANDMARK,NO_OF_YEARS,NO_OF_MONTHS,");
				  bufInsUpdSql.append("ADDRESS_DETAIL,COMMUNICATION_ADDRESS ,MAKER_ID,MAKER_DATE,AUTHOR_ID,AUTHOR_DATE ,TAHSIL,BRANCH_DISTANCE,ADDRESS_ID,DATE_ADD(STR_TO_DATE('"+CommonFunction.checkNull(vo.getMakerDate())+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),'"+CommonFunction.checkNull(vo.getMakerId())+"' ");
				...
				{
					subQuery=",MAKER_ID='"+CommonFunction.checkNull(vo.getMakerId())+"'," +
					"MAKER_DATE=DATE_ADD(STR_TO_DATE('"+CommonFunction.checkNull(vo.getMakerDate())+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)"+
					",AUTHOR_ID='"+CommonFunction.checkNull(vo.getMakerId())+"',AUTHOR_DATE=DATE_ADD(STR_TO_DATE('"+CommonFunction.checkNull(vo.getMakerDate())+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) ";	
				}
				else
					subQuery="";	
				String query="update "+tableName+" set ADDRESS_TYPE=?,BPTYPE=?,BPID=?,ADDRESS_LINE1=?, " +
				...
				String query1="update cr_deal_address_m set ADDRESS_TYPE=?,BPTYPE=?,BPID=?,ADDRESS_LINE1=?, " +
				     "ADDRESS_LINE2=?, ADDRESS_LINE3=?, COUNTRY=?, STATE=?, " +
				     "DISTRICT=?, PINCODE=?,PRIMARY_PHONE=?,ALTERNATE_PHONE=?,TOLLFREE_NUMBER=?,FAX=?, "+
				     "LANDMARK=?,NO_OF_YEARS=?,NO_OF_MONTHS=?,ADDRESS_DETAIL=?,MAKER_ID=?," +
				     "MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),TAHSIL=?,BRANCH_DISTANCE=?,COMMUNICATION_ADDRESS=? where BPID=?";
...
//					insertPrepStmtObject1.addInt(addId);
	...
	public ArrayList<Object> getReferenceAll(String code, Object pageStatus,String updateFlag) {
		...
		try{
				String query="select REF_ID, concat(F_NAME,' ',IFNULL(M_NAME,''),' ',L_NAME)NAME, RELATIONSHIP, KNOWING_SINCE, MOBILE_NUMBER, LANDLINE_NUMBER from "+tableName+"  where BPID="+code;
...
//					" where DEAL_ID="+dealId;
	...
	public ArrayList<Object> getStakeDetails(int code,Object pageStatus,String statusCase,String updateFlag,String updateInMaker) {
		...
		if(updateFlag!=null && updateFlag.equals("updateFlag") ||(updateFlag!=null && updateFlag.equals("notEdit")))
		{
			...
			try{
				String query="SELECT distinct cs.STAKEHOLDER_ID, cs.STAKEHOLDER_SALUTATION, cs.STAKEHOLDER_NAME, s.STAKEHOLDER_ID, " +
				"cs.STAKEHOLDER_PERCENTAGE, DATE_FORMAT(cs.STAKEHOLDER_DOB,'"+dateFormat+"'), cs.STAKEHOLDER_EXPERIENCE, cs.STAKEHOLDER_DIN, " +
				"cs.STAKEHOLDER_IDENTIFICATION_NO, p.DESCRIPTION,DATE_FORMAT(cs.STAKEHOLDER_JOINING_DATE,'"+dateFormat+"') , cs.ELIGIBLE_FOR_COMPUTATION," +
	 ...
	 if((pageStatuss!=null && pageStatuss.equals("fromLeads"))  || (updateFlag!=null && updateFlag.equals("updateFlag")) ||(updateFlag!=null && updateFlag.equals("notEdit")))
		{
		 logger.info("In Credit Processing , Customer Entry.,getStakeDetailsAll().......");
			...
			try{
				String query="SELECT c.STAKEHOLDER_ID,d.DESCRIPTION, c.STAKEHOLDER_NAME, c.STAKEHOLDER_PERCENTAGE, "+ 
					" DATE_FORMAT(c.STAKEHOLDER_JOINING_DATE,'"+dateFormat+"')" +
			...
			//added by neeraj tripathi start space
				String perQuery="select sum(ifnull(STAKEHOLDER_PERCENTAGE,0)) from cr_deal_customer_stakeholder_m where CUSTOMER_ID="+id;
...
//			"  and d.GENERIC_KEY='DESIGNATION' and c.CUSTOMER_ID="+id;
			...
			logger.info("getStakeDetailsAll "+stakeDetailsAll.size());
			//added by neeraj tripathi start space
			String perQuery="select sum(ifnull(STAKEHOLDER_PERCENTAGE,0)) from "+tableName+" where CUSTOMER_ID="+id;
		 	logger.info("Query for getting total percentage : "+perQuery);
		 	String percentage=ConnectionDAO.singleReturn(perQuery);
		//end space
			for(int i=0;i<stakeDetailsAll.size();i++){
...
return list;
	...
	public String getDOBfromDCM(int id) {
		...
		String incDateQ="select DATE_FORMAT(CUSTOMER_DOB,'"+dateFormat+"')  from cr_deal_customer_m where CUSTOMER_ID="+id;
	 	String incDate=ConnectionDAO.singleReturn(incDateQ);
		return incDate;
	}
	...
	public String getDOBfromGcdCM(int id) {
		String incDateQ="select DATE_FORMAT(CUSTOMER_DOB,'"+dateFormat+"') from gcd_customer_m where CUSTOMER_ID="+id;
		String incDate=ConnectionDAO.singleReturn(incDateQ);
		return incDate;
	}
	...
	public String getCustomerWithDoj(String tableName,String getDoj, int id) {
		String dojCheck = ConnectionDAO.singleReturn("select CUSTOMER_ID from "+tableName+" where STR_TO_DATE('"+getDoj+"','%d-%m-%Y') >= (select CUSTOMER_DOB from "+tableName+" where CUSTOMER_ID="+id+") limit 1");
		return dojCheck;
	}
	...
	public ArrayList getStackHolderDetail(String stackHolderId ,Object pageStatus,String statusCase,String updateInMaker,String updateFlag,String pageStatuss)
	{
		...
		if((pageStatuss!=null && pageStatuss.equals("fromLeads"))  || (updateFlag!=null && updateFlag.equals("updateFlag")) ||(updateFlag!=null && updateFlag.equals("notEdit")))
		{
			...
			tableName="cr_deal_customer_stakeholder_m";
			try{
				String query="SELECT cs.STAKEHOLDER_ID, cs.STAKEHOLDER_SALUTATION, cs.STAKEHOLDER_NAME, cs.STAKEHOLDER_TYPE, " +
						"cs.STAKEHOLDER_PERCENTAGE, DATE_FORMAT(cs.STAKEHOLDER_DOB,'"+dateFormat+"'), cs.STAKEHOLDER_EXPERIENCE, " +
								"cs.STAKEHOLDER_DIN, cs.STAKEHOLDER_IDENTIFICATION_NO,cs.STAKEHOLDER_POSITION, " +
								"DATE_FORMAT(cs.STAKEHOLDER_JOINING_DATE,'"+dateFormat+"'), cs.ELIGIBLE_FOR_COMPUTATION," +
								" cs.STAKEHOLDER_PRIMARY_PHONE, cs.STAKEHOLDER_ALTERNATE_PHONE, cs.STAKEHOLDER_PRIMARY_EMAIL," +
			...
			//added by neeraj tripathi start space
				...
				String perQuery="select sum(ifnull(STAKEHOLDER_PERCENTAGE,0)) from cr_deal_customer_stakeholder_m where CUSTOMER_ID="+cid;
			 	logger.info("Query for getting total percentage : "+perQuery);
			 	String percentage=ConnectionDAO.singleReturn(perQuery);
			//end space
		...
		String query="SELECT cs.STAKEHOLDER_ID, cs.STAKEHOLDER_SALUTATION, cs.STAKEHOLDER_NAME, cs.STAKEHOLDER_TYPE, cs.STAKEHOLDER_PERCENTAGE, DATE_FORMAT(cs.STAKEHOLDER_DOB,'"+dateFormat+"'), cs.STAKEHOLDER_EXPERIENCE, cs.STAKEHOLDER_DIN, cs.STAKEHOLDER_IDENTIFICATION_NO, cs.STAKEHOLDER_POSITION, DATE_FORMAT(cs.STAKEHOLDER_JOINING_DATE,'"+dateFormat+"'), cs.ELIGIBLE_FOR_COMPUTATION, cs.STAKEHOLDER_PRIMARY_PHONE, cs.STAKEHOLDER_ALTERNATE_PHONE, cs.STAKEHOLDER_PRIMARY_EMAIL, cs.STAKEHOLDER_ALTERNATE_EMAIL, cs.STAKEHOLDER_WEBSITE FROM "+tableName+" cs WHERE cs.STAKEHOLDER_ID="+stackHolderId;
	...
	//added by neeraj tripathi start space
		...
		logger.info("Query for getting customerId : "+custQuery);
		String cid=ConnectionDAO.singleReturn(custQuery);
		String perQuery="select sum(ifnull(STAKEHOLDER_PERCENTAGE,0)) from "+tableName+" where CUSTOMER_ID="+cid;
	 	logger.info("Query for getting total percentage : "+perQuery);
	 	String percentage=ConnectionDAO.singleReturn(perQuery);
	//end space
		logger.info("getStackHolderDetail "+stackHolderDetail.size());
	...
	public int updateStakeHolder(Object ob, int id, int stackId, String recStatus,String statusCase,String updateFlag,String pageStatuss) {
		...
		try{
			...
			if(pageStatuss!=null && pageStatuss.equals("fromLeads")||updateFlag!=null && updateFlag.equals("updateFlag"))
			{
				...
				tableName="cr_deal_customer_stakeholder_m";
				String query="update "+tableName+" set STAKEHOLDER_SALUTATION=?," +
				" STAKEHOLDER_NAME=?, STAKEHOLDER_TYPE=?, STAKEHOLDER_PERCENTAGE=?, STAKEHOLDER_DOB=STR_TO_DATE(?,'"+dateFormat+"'), " +
				...
				"STAKEHOLDER_JOINING_DATE=STR_TO_DATE(?,'"+dateFormat+"'), ELIGIBLE_FOR_COMPUTATION=?," +
				"MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where STAKEHOLDER_ID=?";
			...
			else
			{
				...
				"STAKEHOLDER_PRIMARY_PHONE=? ,STAKEHOLDER_ALTERNATE_PHONE=?, STAKEHOLDER_PRIMARY_EMAIL=?," +
				" STAKEHOLDER_ALTERNATE_EMAIL=?, STAKEHOLDER_WEBSITE=?, STAKEHOLDER_POSITION=?, " +
				"STAKEHOLDER_JOINING_DATE=STR_TO_DATE(?,'"+dateFormat+"'), ELIGIBLE_FOR_COMPUTATION=? "+subQuery+"  where STAKEHOLDER_ID=?";
				...
				//----------Ritu Code Start----------
				PrepStmtObject insertPrepStmtObject1 = new PrepStmtObject();
				String query1="update cr_deal_customer_stakeholder_m set STAKEHOLDER_SALUTATION=?," +
				" STAKEHOLDER_NAME=?, STAKEHOLDER_TYPE=?, STAKEHOLDER_PERCENTAGE=?, STAKEHOLDER_DOB=STR_TO_DATE(?,'"+dateFormat+"'), " +
				"STAKEHOLDER_EXPERIENCE=?, STAKEHOLDER_DIN=?, STAKEHOLDER_IDENTIFICATION_NO=?, " +
				"STAKEHOLDER_PRIMARY_PHONE=? ,STAKEHOLDER_ALTERNATE_PHONE=?, STAKEHOLDER_PRIMARY_EMAIL=?," +
				" STAKEHOLDER_ALTERNATE_EMAIL=?, STAKEHOLDER_WEBSITE=?, STAKEHOLDER_POSITION=?, " +
				"STAKEHOLDER_JOINING_DATE=STR_TO_DATE(?,'"+dateFormat+"'), ELIGIBLE_FOR_COMPUTATION=?," +
				"MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where CUSTOMER_ID=?";
			 ...
			 logger.info("In Credit Processing , Customer Entry..,getcreditRatingDetails()......");
				...
				try{
					String query="SELECT r.CUSTOMER_RATING_ID,a.AGENCY_NAME,r.RATING_AGENCY,r.CUSTOMER_RATING," +
							"DATE_FORMAT(r.RATING_DATE,'"+dateFormat+"') from com_agency_m a,"+tableName+" r" +
	...
	public int updateCreditRating(Object ob, int id,int crId, String recStatus,String statusCase,String updateFlag,String pageStatuss)
	{
		...
		try{
			...
			if(pageStatuss!=null && pageStatuss.equals("fromLeads")||updateFlag!=null && updateFlag.equals("updateFlag"))
			{
				...
				"RATING_DATE=STR_TO_DATE(?,'"+dateFormat+"'),MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where CUSTOMER_RATING_ID=?";
			...
			//--------Ritu Code Start--------
			PrepStmtObject insertPrepStmtObject1 = new PrepStmtObject();
			String query1="update cr_deal_customer_rating set RATING_AGENCY=?, CUSTOMER_RATING=?, " +
			"RATING_DATE=STR_TO_DATE(?,'"+dateFormat+"'),MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where CUSTOMER_ID=?";
	...
	public ArrayList getCreditRatingDetail(String creditRatingId,Object pageStatus, String statusCase,String updateInMaker,String updateFlag,String pageStatuss)
	{
		...
		if((pageStatuss!=null && pageStatuss.equals("fromLeads"))  || (updateFlag!=null && updateFlag.equals("updateFlag")) ||(updateFlag!=null && updateFlag.equals("notEdit")))
		{
			...
			tableName="cr_deal_customer_rating";
			try{
				String query="SELECT a.AGENCY_CODE, DATE_FORMAT(r.RATING_DATE,'"+dateFormat+"'), r.CUSTOMER_RATING, r.CUSTOMER_RATING_ID " +
	...
	public int updateIndReference(Object ob, int id, int refId, String recStatus,String statusCase, String updateFlag, String pageStatuss) {
		...
		try
		{
			...
			{
				tableName="cr_deal_reference_m";
				String query="update "+tableName+" set BPID=?,BPTYPE=?,F_NAME=?,M_NAME=?,L_NAME=?, RELATIONSHIP=?, KNOWING_SINCE=?, MOBILE_NUMBER=?,LANDLINE_NUMBER=?, " +
				"MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where REF_ID=?";
			...
			else
			{
				...
				String query1="update cr_deal_reference_m set BPID=?,BPTYPE=?,F_NAME=?,M_NAME=?,L_NAME=?, RELATIONSHIP=?, KNOWING_SINCE=?, MOBILE_NUMBER=?,LANDLINE_NUMBER=?, " +
				"MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?,'"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where BPID=?";
