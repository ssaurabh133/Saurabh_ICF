package com.login.daoImplMYSQL;
...
public class LoginDaoImpl implements LoginDao {
	...
	public String isUserExist(String userName, String userPassword) throws SQLException {
		...
		//System.out.println("Enter for DB isUserExist action"); // comment by saorabh on 17/02/2014
		logger.info("Enter for DB isUserExist action"); //add by saorabh on 17/02/2014
		try {
...
//			
			...
			user=null;// add by saorabh on 17/02/2014
...
//				
	...
	public String getLogindate(String id) throws SQLException {
		...
		try {
				...
				String login="select DATE_FORMAT(LOGIN_DATE,'"+dateFormatWithTime+"') FROM sec_user_log where Login_status='N' ORDER BY  USER_LOG_ID DESC LIMIT 0,1";
				
				logindate=CommonFunction.checkNull(ConnectionDAO.singleReturn(login));
	
			if(logindate.equalsIgnoreCase("")){
				String newdate="select DATE_FORMAT(SYSDATE(),'"+dateFormatWithTime+"')";
				
				logindate=CommonFunction.checkNull(ConnectionDAO.singleReturn(newdate));
				
			}
			login=null;// add by saorabh on 17/02/2014	
		}
			catch(Exception e){
				e.getStackTrace();
	...
	public String isforcepassword(String userid)  throws SQLException{
		...
		try {
		      ...
		      ArrayList modList = new ArrayList();
				...
				String passflag="select FORCED_PASSWORD_FLAG,PASS_COUNT from sec_user_m where USER_ID='"+StringEscapeUtils.escapeHtml(CommonFunction.checkNull(userid).trim())+"'";
				//userlogin=CommonFunction.checkNull(ConnectionDAO.singleReturn(passflag));
				list = ConnectionDAO.sqlSelect(passflag);
			      for (int i = 0; i < list.size(); i++)
			      {
			        modList = (ArrayList)list.get(i);
			        if (modList.size() > 0)
			        {
			          userlogin = StringEscapeUtils.escapeSql(CommonFunction.checkNull((String)modList.get(0)).trim());
			          user.setPassCount(StringEscapeUtils.escapeSql(CommonFunction.checkNull((String)modList.get(1)).trim()));
			        }
				...
				passflag=null;// add by saorabh on 17/02/2014	
...
//				
	...
	public String totalexpirydays()  throws SQLException{
		...
		try {
					...
					sql=null;// add by saorabh on 17/02/2014
	...
	public String businessdate() {
		...
		try {
					...
					String sql ="SELECT DATE_FORMAT(PARAMETER_VALUE,'"+dateFormat+"') FROM parameter_mst WHERE PARAMETER_KEY='BUSINESS_DATE'";					
					businessdays=CommonFunction.checkNull(ConnectionDAO.singleReturn(sql));
					sql=null;// add by saorabh on 17/02/2014
			}catch(Exception e){
				e.getStackTrace();
	...
	public UserVO saveLoginDetails(String userid,String sessId,String ipaddress) throws RemoteException, SQLException {
		...
		try {
			...
			String query="SELECT DATE_FORMAT(PARAMETER_VALUE,'"+dateFormat+"') FROM parameter_mst WHERE PARAMETER_KEY='BUSINESS_DATE'";
			String businessdate=CommonFunction.checkNull(ConnectionDAO.singleReturn(query));
...
bufInsSql.append(" SET LOGOUT_DATE =sysdate(),LOGIN_STATUS ='N'");
bufInsSql.append(" WHERE USER_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(userid).trim())+"'  AND LOGIN_STATUS='Y'");
	...
	public boolean passcount(String id, int count) throws SQLException {
		...
		logger.info("Enter passcount");
			...
			StringBuffer bufInsSql =	new StringBuffer();
		try{	bufInsSql.append("UPDATE sec_user_m SET PASS_COUNT='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(count).trim())+"' WHERE USER_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(id).trim())+"'");
	...
	public boolean statuslock(String userid) throws SQLException {
		...
		StringBuffer bufInsSql =	new StringBuffer();
		try{bufInsSql.append("UPDATE sec_user_m SET ACCOUNT_STATUS='L' WHERE USER_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(userid).trim())+"'");
	...
	public String loggedonce(String userid) throws SQLException {
			...
			try {
			//String sqlname="select USER_ID from sec_user_m where USER_ID=='"+StringEscapeUtils.escapeHtml(CommonFunction.checkNull(userid).trim())+"'";
		//	String userName=CommonFunction.checkNull(ConnectionDAO.singleReturn(sqlname));
	...
	public ArrayList getallbranch() throws SQLException {
		...
		//String userbranch=""; // comment by saorabh on 17/02/2014
	...
	public int matchAnswer(String userid,LoginActionForm loginForm) throws SQLException {
		...
		int count=0;
		//ArrayList qryList = new ArrayList(); //comment by saorabh on 17/20/2014
		boolean status = false;
		PrepStmtObject insertPrepStmtObject = new PrepStmtObject();
		...
		try {
			...
			querBuilder.append("select count(1) from sec_user_m where SECURITY_ANSWER1=? and SECURITY_ANSWER2=? and USER_ID=?");
			...
			logger.info("IN insertPrepStmtObject() select query ### "+ insertPrepStmtObject.printQuery());
...
//			
//
//				String query="select count(1) from sec_user_m where USER_ID='"+StringEscapeUtils.escapeHtml(CommonFunction.checkNull(userid).trim())+"'";
//		
//				
...
//			} 
	...
	public String portno() throws SQLException {
		...
		try {
				...
				query=null;// add by saorabh on 17/02/2014
			}catch(Exception e){
				e.getStackTrace();
...
//       session = Session.getInstance(props);
	...
	public boolean adminPwdCheck(String adminPwd) throws SQLException {
		...
		logger.info("Enter statuslock");
		try {
		String query = "select USER_ID from sec_user_m WHERE USER_PASSWORD=MD5('"+adminPwd+"') and USER_ID='ADMIN'";
			...
			query=null;// add by saorabh on 17/02/2014
			userId=null;// add by saorabh on 17/02/2014
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	public String checkForBusinessDateOpen(Object ob) {
		...
		try {
			...
			//manaulId=Integer.parseInt(StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getManaulId()).trim()));
				...
				logger.info("new date................................................. "+CommonFunction.checkNull(vo.getNewBusinessDate()));			

				cst = con.prepareCall("call GetYearPeriodID (?,STR_TO_DATE(?,'"+dateFormat+"'),?,?,?,?,?,?)");
	...
	public boolean saveNewBusinessDate(ManualAdviceCreationVo vo)
	{
		...
		PrepStmtObject insertPrepStmtObject = new PrepStmtObject();
		boolean status=false;
		String curDateTime = ConnectionDAO.singleReturn("SELECT sysDate()");
		String stat="";
		try{
				...
				if(CommonFunction.checkNull(vo.getNewBusinessDate()).equalsIgnoreCase(""))
					...
					insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(curDateTime));
				...
				insertPrepStmtObject.setSql(bufInsSql.toString());	  
	             ...
	             qryList.add(insertPrepStmtObject);		           
	              ...
	              if(status)
				  {
	          		...
	          		String queryUpdate="UPDATE parameter_mst set PARAMETER_VALUE=STR_TO_DATE(?,'"+dateFormat+"'),MAKER_ID=?,MAKER_DATE=STR_TO_DATE(?,'"+dateFormatWithTime+"') WHERE PARAMETER_KEY='BUSINESS_DATE'";
	        		...
	        		updatePrepStmtObject.setSql(queryUpdate);
	...
	public String getuserEmailId(String userid)
	{
		...
		try {	
				...
				emailquery=null;// add by saorabh on 17/02/2014
			}catch(Exception e){
					e.getStackTrace();	 
			}
			return  usermailId;
	...
	public String getSmtpHost()
	{
		...
		try {	
				...
				hostquery=null;// add by saorabh on 17/02/2014
			}catch(Exception e){
					e.getStackTrace();	 
			}
			return  host;
	...
	public String getSmtpPort()
	{
		...
		try {	
				...
				portquery=null;// add by saorabh on 17/02/2014
			}catch(Exception e){
					e.getStackTrace();	 
			}
			return  port;
	...
	public String getSmtpMailAddress()
	{
		...
		try {	
				...
				mailquery=null;// add by saorabh on 17/02/2014
			}catch(Exception e){
					e.getStackTrace();	 
			}
			return  mailAddress;
	...
	public String accountExpiryDate(String userid) {
		...
		String days="";
			...
			try {
				...
				String dayquery="select DATEDIFF('"+accountExpiryDate+"',SYSDATE()) as value from dual";
	...
	public ArrayList getAboutCompany() {
		...
		try {
			 ...
			 query.append("SELECT  COMPANY_DESC,PHONE_NO,ITHELP_EMAIL_ID from com_company_m WHERE REC_STATUS='A' LIMIT 1");
	 ...
	 public String getuserName(String userid)
	  {
	    ...
	    try {
	      String emailquery = new StringBuilder().append(" SELECT USER_NAME FROM sec_user_m WHERE User_ID='").append(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(userid).trim())).append("'").toString();
	      userName = CommonFunction.checkNull(ConnectionDAO.singleReturn(emailquery));
	      emailquery = null;
	    } catch (Exception e) {
	      e.getStackTrace();
	    }
	  ...
	  public String isUserExistPass(String userName)
	    throws SQLException
	  {
	    String userid = "";
	    try
	    {
	      ...
	      String user = new StringBuilder().append(" SELECT USER_PASSWORD FROM sec_user_m WHERE User_ID='").append(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(userName)).trim()).append("' ").toString();
	      userid = CommonFunction.checkNull(ConnectionDAO.singleReturn(user));
	    }
	    catch (Exception e) {
	      e.getStackTrace();
