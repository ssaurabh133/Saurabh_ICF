...
public class ConnectionReportDumpsDAO {
	
	private static final Logger logger = Logger.getLogger(ConnectionReportDumpsDAO.class.getName());
	private static int count =0;
	private ConnectionReportDumpsDAO()
	{
	...
	public static Connection getConnection() {
		...
		try {
			...
			logger.info("get connection is for Reports dump -->");
 // --------------------- For Jboss Application Server 4.x ------------------------	


			 DATASOURCE_CONTEXT = "java:FinCraftDB3";
...
// -------------------- For Tomcat server------------------------------------
		...
		//logger.info("count create :::::::::: "+count);
		return connection;
	}
...
//	                stmt.close();
	...
	public static ArrayList sqlSearchResultPrepStmt(PrepStmtObject prepStmtObject, int maxRows, int startRows) throws  java.sql.SQLException,java.rmi.RemoteException {
	    ...
	    public static Collection callSP(String spName, Collection inParam, Collection outParam) throws Exception {
	        ...
	        //  logger.info("Flag ConnectionDAO ::-- "+s1);
	          if(!s1.equalsIgnoreCase("E")){
	        	  
	        	//  logger.info("Success Flag ConnectionDAO ::-- "+s1);	
	    ...
	    //sidharth----sidharth space for column name with query result start
	    
	    public static ArrayList sqlColumnWithResult(String sql) throws java.sql.SQLException {
	        Connection conn = null;
	        Statement stmt = null;
		...
		public static Collection callSPForHiberNate(String spName, Collection inParam, Collection outParam) throws Exception {
	        CallableStatement sp = null;
	        boolean status=false;
	        ArrayList outMessages = new ArrayList();
	        StringBuffer sql = new StringBuffer();
	        ...
	        try {
	            ...
	            sql.append(") };");
	            logger.info("the procedure is..."+sql.toString());
	            sp = session.connection().prepareCall(sql.toString());
	            ...
	            status=sp.execute();
	            logger.info("the procedure is..."+sp);
	            int sizeOutParam=outParam.size();
	            for (count = 1; count <= sizeOutParam; count++) {
	                outMessages.add(new String(CommonFunction.checkNull(sp.getString(inParam.size()+count))));
	            }
	    ...
	    //sidharth space for column name with query result end 
