...
public class MSSQLNhbMappingDAOImpl implements NhbMappingDAO {
	private static final Logger logger = Logger.getLogger(MSSQLNhbMappingDAOImpl.class.getName());
	...
	logger.info("In getSearchNhbMapping...................Dao Impl");
		...
		try
		{
			...
			StringBuilder nhbCategory=new StringBuilder();
			 ...
			 bufInsSql.append("select d.DEAL_ID,d.DEAL_NO,");
			 bufInsSql.append("(SELECT STUFF((SELECT '|'+ISNULL(DESCRIPTION,'') from cr_deal_nhb_mapping_dtl A JOIN GENERIC_MASTER B ON(A.NHB_CATEGORY=B.VALUE AND B.GENERIC_KEY='NHB_CATEGORY' and B.REC_STATUS='A') where A.DEAL_ID=D.DEAL_ID FOR XML PATH ('')),1,1,''))NHB_CATEGORY_DESC,");
			 bufInsSql.append("deal.CUSTOMER_NAME,p.PRODUCT_DESC,s.SCHEME_DESC,");
			 bufInsSql.append(" (SELECT STUFF((SELECT '|'+ISNULL(NHB_CATEGORY,'') from cr_deal_nhb_mapping_dtl A where A.DEAL_ID=D.DEAL_ID FOR XML PATH ('')),1,1,''))NHB_CATEGORY");
			...
			bufInsSql.append(" join cr_scheme_m s on n.DEAL_SCHEME=s.SCHEME_ID ");
			 bufInsSql.append(" Where d.REC_STATUS='A' ");
			...
			bufInsSqlTempCount.append(" Where d.REC_STATUS='A'  ");
...
//			}
			...
			bufInsSql.append(" GROUP BY D.DEAL_ID,D.DEAL_NO,deal.CUSTOMER_NAME,p.PRODUCT_DESC,s.SCHEME_DESC");
			 bufInsSqlTempCount.append(" GROUP BY D.DEAL_ID,D.DEAL_NO,deal.CUSTOMER_NAME,p.PRODUCT_DESC,s.SCHEME_DESC)AS b ");
