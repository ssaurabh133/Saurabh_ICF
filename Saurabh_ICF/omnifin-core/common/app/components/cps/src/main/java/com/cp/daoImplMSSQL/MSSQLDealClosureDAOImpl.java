...
public class MSSQLDealClosureDAOImpl implements DealClosureDAO 
{
	private static final Logger logger = Logger.getLogger(MSSQLDealClosureDAOImpl.class.getName());
	...
	public ArrayList fetchDealCanData(String dealId) 
	{
	    ...
	    try{
			...
			bufInsSql.append("DATE_FORMAT(a.DEAL_DATE,'%d-%m-%Y')" );
			bufInsSql.append(",a.DEAL_APPLICATION_FORM_NO,c.PRODUCT_DESC, d.SCHEME_DESC, b.DEAL_ASSET_COST, b.DEAL_LOAN_AMOUNT, b.DEAL_MARGIN_RATE, b.DEAL_MARGIN_AMOUNT," +
	...
	public boolean insertDealCancellation(Object ob) {
		...
		try {
			...
			selectQuery.append("select count(1) from cr_deal_termination_dtl where deal_id="+vo.getLbxDealNo());	
			
			String recordFound = ConnectionDAO.singleReturn(selectQuery.toString());
			
			logger.info("CommonFunction.checkNull(recordFound)"+CommonFunction.checkNull(recordFound));
			
			if(!CommonFunction.checkNull(recordFound).equalsIgnoreCase("0")){
				bufInsSql.append("Delete from cr_deal_termination_dtl where deal_id=?");
	...
	public ArrayList<DealCancellationVo> searchDealCancellation(DealCancellationVo vo,
		...
		DecimalFormat myFormatter = new DecimalFormat("###,##0.00######");
			...
			logger.info("Customer Name :----"+custName );
				bufInsSql.append("select b.DEAL_NO, a.CUSTOMER_NAME, c.PRODUCT_DESC, d.SCHEME_DESC, a.LOAN_AMOUNT, ");
				bufInsSql.append(dbo); 
				bufInsSql.append("DATE_FORMAT(a.DEAL_DATE,'%d-%m-%Y')" );

				bufInsSql.append(", a.MAKER_ID, b.DEAL_ID  ");

				bufInsSql.append(",ROW_NUMBER() OVER (ORDER BY b.DEAL_NO) AS Row from cr_deal_termination_dtl a left join cr_deal_dtl b on(a.DEAL_ID=b.DEAL_ID) ");
			...
			bufInsSql.append(" ORDER BY b.DEAL_NO OFFSET ");
			...
			logger.info("Search searchDealCancellation query for SQL SERVER : " + bufInsSql.toString());
	...
	public ArrayList dealCancellationValues(String dealId,String Status) {
		...
		DealCancellationVo vo= null;
			try
			{
				...
				query.append("select b.DEAL_NO, a.CUSTOMER_NAME,");
				query.append(dbo); 
				query.append("DATE_FORMAT(a.DEAL_DATE,'%d-%m-%Y')");
		
				query.append(", a.DEAL_APPLICATION_FORM_NUMBER, c.PRODUCT_DESC, d.SCHEME_DESC, a.ASSET_COST, a.LOAN_AMOUNT, " +
	...
	public boolean modifySaveDeal(Object ob) {
		...
		try {
				...
				StringBuffer bufInsSql = new StringBuffer();
				bufInsSql.append("UPDATE cr_deal_termination_dtl SET REASON_FOR_CLOSURE=?,MAKER_ID=?,MAKER_DATE=" );
	...
	public boolean modifyDealForward(Object ob) {
		...
		try {
				...
				StringBuffer bufInsSql = new StringBuffer();
				bufInsSql.append("UPDATE cr_deal_termination_dtl SET REASON_FOR_CLOSURE=?,REC_STATUS='F', MAKER_ID=?,MAKER_DATE=" );
				// bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) ");
					bufInsSql.append(dbo);
					bufInsSql.append(" STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
