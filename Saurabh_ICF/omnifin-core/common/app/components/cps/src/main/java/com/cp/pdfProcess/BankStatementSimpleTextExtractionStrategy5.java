...
public class BankStatementSimpleTextExtractionStrategy5 implements
     ...
     */
    public BankStatementSimpleTextExtractionStrategy5() 
    {
    ...
    public void renderText(TextRenderInfo renderInfo) {
        ...
        float yStart = 600.000f;
        float yEnd = 65.000f;
        float yTolerance = 10f;
        float yPageChangeTolerance = -100f;
        String str = renderInfo.getText();
        String delimiterPrefix = "";
        
        if(str.contains("141224265GN00001"))
        {
        	System.out.println("------------");
        }
        
        if(renderInfo.getText().equals("Page") && ".".equalsIgnoreCase(""+result.charAt(result.length()-3)))
        {
        	startData = false;
        }
        
        if(renderInfo.getText().equals("STATEMENT") && ".".equalsIgnoreCase(""+result.charAt(result.length()-3)))
        {
        	startData = false;
        }
        
        if(start.get(1)>=(yStart+yTolerance) || start.get(1)<=(yEnd-yTolerance)){
        	return;
        }
