...
public class DealReassignDAOImpl implements DealReassignDAO {

		private static final Logger logger=Logger.getLogger(DealReassignDAOImpl.class.getName());
	...
	public boolean UpdateInsertDealMovemenDtl(ReprocessingDealVo reprocessingDealVo, String companyId,String businessDate, String userId) 
		{
			...
			try {
				...
				if(updstatus)
				{
					...
					insQuery.append("'DC',");
					insQuery.append("'I',");
					insQuery.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," );
					insQuery.append(" '0000-00-00', " );
					insQuery.append("?,");
					insQuery.append("'',");
					insQuery.append("'A' ");
					insQuery.append(")");
