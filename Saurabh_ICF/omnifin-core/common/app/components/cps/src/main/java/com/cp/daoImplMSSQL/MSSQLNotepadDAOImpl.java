...
public class MSSQLNotepadDAOImpl implements NotepadDAO{
	private static final Logger logger = Logger.getLogger(MSSQLNotepadDAOImpl.class.getName());
...
//			}
			...
			try
			{
				...
				bufInsSql.append(" select distinct d.DEAL_ID,d.DEAL_NO,dbo.DATE_FORMAT(d.DEAL_INITIATION_DATE,'%d-%m-%Y'),g.CUSTOMER_NAME,p.PRODUCT_DESC, s.SCHEME_DESC,dbo.DATE_FORMAT(dl.DEAL_SANCTION_VALID_TILL,'%d-%m-%Y') from cr_deal_dtl d");
				...
				if((!(vo.getAgrementDate().equalsIgnoreCase("")))) {
			   	  bufInsSql.append(" d.LOAN_AGREEMENT_DATE =dbo.STR_TO_DATE('"+vo.getAgrementDate()+"','"+dateFormatWithTime+"') AND");
			   	  bufInsSqlTempCount.append(" d.LOAN_AGREEMENT_DATE =dbo.STR_TO_DATE('"+vo.getAgrementDate()+"','"+dateFormatWithTime+"') AND");
			   	  appendSQL=true;
			     }
...
//						fetchVo.setReportingToUserId(userName);
	...
	public ArrayList getDealHeader(String dealId) {
		...
		try
		{
			StringBuilder query =new StringBuilder();
			 query.append(" select TOP 1 L.DEAL_ID,L.DEAL_NO,L.DEAL_CUSTOMER_ID,G.CUSTOMER_NAME,dbo.DATE_FORMAT(L.MAKER_DATE,'%d-%m-%Y'),p.PRODUCT_DESC,s.SCHEME_DESC,D.DEAL_PRODUCT_CATEGORY from cr_deal_dtl L "+
							...
							" where L.DEAL_ID="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(dealId)).trim()+"");
	...
	public ArrayList getLeadHeader(String leadId) {
		...
		{
			StringBuilder query =new StringBuilder();
			 query.append(" select TOP 1 L.LEAD_ID,L.CUSTOMER_NAME,dbo.DATE_FORMAT(L.MAKER_DATE,'%d-%m-%Y'),p.PRODUCT_DESC,s.SCHEME_DESC,p.PRODUCT_CATEGORY from cr_lead_dtl L "+
							" left join cr_product_m p on L.PRODUCT=p.PRODUCT_ID"+
							" left join cr_scheme_m s on L.SCHEME=s.SCHEME_ID "+
							" where L.LEAD_ID="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(leadId)).trim()+" ");
