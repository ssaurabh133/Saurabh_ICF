package com.cp.dealDaoImplMYSQL;
...
public class DealProcessingDaoImpl implements DealProcessingDao 
{
	private static final Logger logger = Logger.getLogger(DealProcessingDaoImpl.class.getName());
	...
	public ArrayList getDealHeader(String id)
	{
		...
		try
		{
			StringBuilder query=new StringBuilder();
			 query.append(" select d.deal_id, deal_no,d.DEAL_CUSTOMER_ID,deal.CUSTOMER_NAME,DATE_FORMAT(deal_date,'"+dateFormat+"'),p.PRODUCT_DESC,s.SCHEME_DESC,l.DEAL_PRODUCT_CATEGORY,d.REC_STATUS  from cr_deal_dtl d "+
...
//		String checkSDAm="";
		...
		//String q6="select CUSTOMER_TYPE from cr_deal_customer_role,cr_deal_customer_m where DEAL_CUSTOMER_ID=CUSTOMER_ID and DEAL_ID="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(dealId)).trim();
		 ...
		 checkSDQ.append("SELECT IFNULL(S.SD_AMOUNT,0) FROM cr_deal_sd_m s  WHERE  S.DEAL_ID ="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(dealId)).trim());
		checkSDAm.append(ConnectionDAO.singleReturn(checkSDQ.toString()));
		logger.info("sdAmount: "+sdAmount+" Query: "+checkSDQ);
		
		StringBuilder q12=new StringBuilder();
		 q12.append("SELECT IFNULL(C.DEAL_CHARGE_FINAL_AMOUNT,0) FROM cr_deal_txncharges_dtl c WHERE  c.DEAL_CHARGE_CODE='103' AND c.DEAL_ID ="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(dealId)).trim());
		...
		if(sdAmount!=null && !CommonFunction.checkNull(sdAmount).equalsIgnoreCase("0"))
		{
			...
			logger.info("tenure: "+tenure+" Query: "+q11);
			logger.info("sdAmount: "+sdAmount+" Query: "+q12);
		}
		...
		logger.info("amount: "+guram+" Query: "+q8);
		...
		StringBuilder q7=new StringBuilder();
			...
			logger.info("q13: "+q13+"status "+gCheckAddr);
		   ...
		   if(!CommonFunction.checkNull(coCheckAddr).equalsIgnoreCase("") && coCheckAddr.size()>0)
			{
			    ...
			    for(int j=0;j<ff;j++)
				{
					...
					{		
						...
						logger.info("q16: "+q16+"status "+coAddr);
...
//		 }
	  try
	  {
			...
			StringBuffer bufInsSql=new StringBuffer();
			if(vo.getLoanNO().equalsIgnoreCase(""))
				bufInsSql.append("select CUSTOMER_ID,CUSTOMER_NAME,CUSTOMER_CONSTITUTION,CUSTOMER_BUSINESS_SEGMENT,if(CUSTOMER_TYPE='I','INDIVIDUAL','CORPORATE'),DEAL_APPLICATION_FORM_NO,loan_no from cr_deal_customer_m left outer join cr_deal_dtl on(cr_deal_dtl.DEAL_CUSTOMER_ID=cr_deal_customer_m.CUSTOMER_ID) left outer join cr_loan_dtl on(cr_loan_dtl.LOAN_DEAL_ID=cr_deal_dtl.DEAL_ID) where true ");
			else
				bufInsSql.append("select CUSTOMER_ID,CUSTOMER_NAME,CUSTOMER_CONSTITUTION,CUSTOMER_BUSINESS_SEGMENT,if(CUSTOMER_TYPE='I','INDIVIDUAL','CORPORATE'),LOAN_REFERENCE_NO,loan_no from gcd_customer_m left outer join cr_loan_dtl on(cr_loan_dtl.LOAN_CUSTOMER_ID=gcd_customer_m.CUSTOMER_ID) where true ");
...
//	       }
	...
	public ArrayList getAllLoanDetails(String dealLoanId) 
	{
		...
		//con= (Connection) ConnectionDAO.getConnection();
			StringBuilder query=new StringBuilder();
		 query.append("SELECT DEAL_PRODUCT,DEAL_SCHEME,DATE_FORMAT(DEAL_DISBURSAL_DATE,'"+dateFormat+"'),DEAL_ASSET_COST," +
				" DEAL_MARGIN_RATE,DEAL_MARGIN_AMOUNT,DEAL_LOAN_AMOUNT,DEAL_TENURE,DEAL_REPAYMENT_FREQ,DEAL_RATE_TYPE," +
				" DEAL_RATE_METHOD,DEAL_BASE_RATE_TYPE,DEAL_BASE_RATE,DEAL_MARKUP,DEAL_FINAL_RATE,DEAL_REPAYMENT_TYPE,DEAL_INSTALLMENT_TYPE," +
				" DEAL_INSTALLMENT_MODE,DEAL_REPAYMENT_MODE,DEAL_LOAN_PURPOSE,DEAL_LOAN_ID,p.PRODUCT_DESC,s.SCHEME_DESC,s.MIN_AMT_FIN,s.MAX_AMT_FIN,s.MIN_MARGIN_RATE,s.MAX_MARGIN_RATE,s.MIN_TENURE,s.MAX_TENURE,p.ASSET_FLAG,d.DEAL_NO_OF_INSTALLMENT,p.ASSET_FLAG,DEAL_SECTOR_TYPE,DEAL_ADVANCE_INSTL,DATE_FORMAT(DEAL_REPAY_EFF_DATE,'"+dateFormat+"') ,DATE_FORMAT(DEAL_MATURITY_DATE,'"+dateFormat+"'),DEAL_DUE_DAY,DATE_FORMAT(NEXT_DUE_DATE,'"+dateFormat+"'),g.value,S.RATE_METHOD from cr_deal_loan_dtl d left join cr_product_m p on d.DEAL_PRODUCT=p.PRODUCT_ID left join cr_scheme_m s on d.DEAL_SCHEME=s.SCHEME_ID left join generic_master g on g.VALUE=d.LOAN_TYPE where d.DEAL_LOAN_ID="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(dealLoanId)).trim());
	...
	public ArrayList getLoanTypeList() {
		...
		try {
			...
			logger.info("In getSourceDetailList...............query...........DAOImpl"+ query);
			LoanDetailVo loanVo = null;
	...
	public ArrayList getProductSchemeDetailsFromLead(String product, String scheme,
			...
			try{
				...
				StringBuilder query=new StringBuilder();
			 query.append("SELECT DEFAULT_MARGIN_RATE,RATE_TYPE,RATE_METHOD,S.BASE_RATE_TYPE,B.BASE_RATE,DEF_FLAT_RATE,"+
	                      " DEF_EFF_RATE,DEF_TENURE,REPAYMENT_FREQ,INSTALLMENT_TYPE,REPAYMENT_MODE,INSTALLMENT_MODE ,P.REPAYMENT_TYPE,P.REVOLVING_FLAG"+
	                      " ,MIN_MARGIN_RATE,MAX_MARGIN_RATE,MIN_TENURE,MAX_TENURE,MIN_AMT_FIN,MAX_AMT_FIN,P.ASSET_FLAG,p.PRODUCT_DESC,s.SCHEME_DESC,p.PRODUCT_CATEGORY from cr_scheme_m S " +
	                      " LEFT JOIN cr_base_rate_m B ON S.BASE_RATE_TYPE=B.BASE_RATE_TYPE AND B.rec_status='A' and b.EFFECTIVE_FROM <= STR_TO_DATE('"+bdate+"','"+dateFormat+"') "+
	                      " LEFT JOIN cr_product_m P ON S.PRODUCT_ID=P.PRODUCT_ID"+
	                      " where SCHEME_ID="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(scheme)).trim()+" AND S.REC_STATUS='A' " +
	...
	public boolean deleteChargeInstallmentRepay(String dealId, String[] dealLoanId) {
		...
		for(int k=0;k<dealLoanId.length;k++)
		{
			...
			logger.info("delete query:deleteChatge "+deleteChatge);
			logger.info("delete query:deleteSecurity "+deleteSecurity);
			logger.info("delete query:deleteInstallment "+deleteInstallment);
			logger.info("delete query:deleteRepay "+deleteRepay);
			qryList.add(deleteChatge);
			...
			if(upd)
			{
				...
				StringBuilder query=new StringBuilder();
				 query.append("update cr_deal_loan_dtl set DEAL_PRODUCT=?, " +
						" DEAL_SCHEME=?,DEAL_DISBURSAL_DATE=STR_TO_DATE(?,'"+dateFormat+"')," +
						" DEAL_ASSET_COST=?,DEAL_MARGIN_RATE=?, " +
						" DEAL_MARGIN_AMOUNT=?,DEAL_LOAN_AMOUNT=?,DEAL_TENURE=?," +
						" DEAL_REPAYMENT_FREQ=?, " +
						" DEAL_RATE_TYPE=?,DEAL_RATE_METHOD=?,DEAL_BASE_RATE_TYPE=?,DEAL_BASE_RATE=?, " +
						" DEAL_MARKUP=?,DEAL_FINAL_RATE=?,DEAL_REPAYMENT_TYPE=?,DEAL_INSTALLMENT_TYPE=?, DEAL_INSTALLMENT_MODE=?, " +
						" DEAL_REPAYMENT_MODE=?,DEAL_NO_OF_INSTALLMENT=?,DEAL_LOAN_PURPOSE=?,DEAL_SECTOR_TYPE=?,DEAL_ADVANCE_INSTL=?,DEAL_REPAY_EFF_DATE=STR_TO_DATE(?,'"+dateFormat+"'),DEAL_MATURITY_DATE=STR_TO_DATE(?,'"+dateFormat+"'),DEAL_DUE_DAY=?,NEXT_DUE_DATE=STR_TO_DATE(?,'"+dateFormat+"'),DEAL_FLAT_RATE=?,DEAL_EFF_RATE=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),LOAN_TYPE=? where DEAL_LOAN_ID=?");
		...
		{			
			...
			StringBuilder query=new StringBuilder();
			 query.append("select DEAL_CHARGE_DTL_ID,DEAL_CHARGE_TYPE,DEAL_CHARGE_CODE,CHARGE_DESC," +
						...
						" DEAL_CHARGE_CALCULATED_ON,if(DEAL_CHARGE_METHOD='P','PERCENTAGE','FLAT'),if(DEAL_CHARGE_TAX_INCLUSIVE='N','NO','YES')," +
						" DEAL_CHARGE_TAX_RATE1,DEAL_CHARGE_TAX_RATE2,DEAL_CHARGE_BP_TYPE,if(DEAL_MIN_CHARGE_METHOD='P','PERCENTAGE','FLAT'),DEAL_CHARGE_MIN_CHARGE_AMOUNT," +
	...
	public String callRefreshChargesDetailPro(String dealCap, String dealId)
	{
		...
		logger.info("supllierIdQ: "+supllierIdQ);
		try
		{
