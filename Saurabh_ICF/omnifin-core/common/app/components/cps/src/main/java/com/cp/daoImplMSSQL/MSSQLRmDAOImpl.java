...
public class MSSQLRmDAOImpl  implements RmDAO {
	
	private static final Logger logger = Logger.getLogger(MSSQLRmDAOImpl .class.getName());
	...
	public ArrayList<RmChangeVo> searchRmData(Object ob) {
		...
		try {
			...
			bufInsSql.append("select a.DEAL_NO,d.CUSTOMER_NAME,c.USER_NAME ,a.DEAL_ID,b.DEAL_ID,isnull((select user_name from SEC_USER_M where user_id=b.NEW_RM),(select user_name from SEC_USER_M where user_id=b.EXISTING_RM))as new_rm,P.product_desc,S.scheme_desc,b.REC_STATUS, sm.USER_NAME,st.USER_NAME,isnull((select user_name from SEC_USER_M where user_id=b.NEW_RO),(select user_name from SEC_USER_M where user_id=b.EXISTING_RO))as new_ro,isnull((select user_name from SEC_USER_M where user_id=b.NEW_MAKER),(select user_name from SEC_USER_M where user_id=b.EXISTING_MAKER))as new_maker from cr_deal_dtl a ");
	...
	public ArrayList<RmChangeVo> searchRmMakerAuthor(Object ob, String recStatus) {
		...
		try {
			...
			searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());
			logger.info("IN searchRmMakerAuthor() search query1 ### "+ bufInsSql.toString());
			totalRecordSize = searchlist.size();
	...
	public boolean insertRmChangeData(Object ob, String[] checkbox) {
		...
		try {
			...
			String rmQuery = "select DEAL_RM from cr_deal_dtl where deal_id='"+checkbox[0]+"'";
			String rmQuery1 = "select DEAL_RO from cr_deal_dtl where deal_id='"+checkbox[0]+"'";
			String makerQuery = "select MAKER_ID from cr_deal_dtl where deal_id='"+checkbox[0]+"'";
			String dealRm = ConnectionDAO.singleReturn(rmQuery);
			String dealRm1 = ConnectionDAO.singleReturn(rmQuery1);
			String dealRm2 = ConnectionDAO.singleReturn(makerQuery);
			rmQuery=null;
			rmQuery1=null;
			makerQuery=null;
				...
				for (int j = 0; j <length ; j++) {
					...
					bufInsSql.append(" CAST(CAST(dbo.STR_TO_DATE(?,'%d-%m-%Y %H:%i') AS DATE) AS DATETIME) + CAST(CONVERT (time, SYSDATETIME()) AS TIME))");//MAKER_DATE
	...
	try {
			...
			for (int j = 0; j <length ; j++) {
				...
				bufInsSql.append("UPDATE cr_rm_change_dtl SET REC_STATUS='F', " +
								" MAKER_DATE=CAST(CAST(dbo.STR_TO_DATE(?,'%d-%m-%Y %H:%i') AS DATE) AS DATETIME) + CAST(CONVERT (time, SYSDATETIME()) AS TIME) WHERE DEAL_ID=? AND REC_STATUS='P'");
			...
			for (int j = 0; j <length ; j++) {
				...
				if (decesion.equalsIgnoreCase("A")) {				
					...
					bufInsSql.append("UPDATE cr_rm_change_dtl SET REC_STATUS='A',AUTHOR_REMARKS=? ,AUTHOR_ID=?," +
						" AUTHOR_DATE=CAST(CAST(dbo.STR_TO_DATE(?,'%d-%m-%Y %H:%i') AS DATE) AS DATETIME) + CAST(CONVERT (time, SYSDATETIME()) AS TIME) WHERE DEAL_ID=? AND REC_STATUS='F'");
					if(!CommonFunction.checkNull(dealRm).equalsIgnoreCase(""))
					{
