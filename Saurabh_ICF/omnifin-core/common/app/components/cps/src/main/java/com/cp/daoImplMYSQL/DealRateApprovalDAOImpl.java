...
public class DealRateApprovalDAOImpl implements DealRateApprovalDAO 
{
	private static final Logger logger = Logger.getLogger(DealRateApprovalDAOImpl.class.getName());
	...
	{
		logger.info("In fetchDealRateApprovalMakerDetail() of DealRateApprovalDAOImpl.");		
 		...
 		try
 		{
 			...
 			bufInsSql.append("select d.DEAL_ID,DEAL_NO,DEAL_APPLICATION_FORM_NO,");
 			...
 			logger.info("In fetchDealRateApprovalMakerDetail() of DealRateApprovalDAOImpl Count Qery :  "+bufInsSqlTempCount.toString());
			...
			logger.info("In fetchDealRateApprovalMakerDetail() of DealRateApprovalDAOImpl Search Qery : " + bufInsSql.toString());
	...
	{
		logger.info("In fetchRecordDetail() of DealRateApprovalDAOImpl.");		
		...
		// For customer Detail's
 		try
 		{
 			...
 			bufInsSql.append(" IF(ADDRESS_LINE1 is null,IF(ADDRESS_LINE2 is null,IF(ADDRESS_LINE3 is null,' ',concat(upper(ADDRESS_LINE3),' ')), IF(ADDRESS_LINE3 is null,concat(upper(ADDRESS_LINE2),' '),concat(concat(upper(ADDRESS_LINE2),','),concat(upper(ADDRESS_LINE3),' ')))), IF(ADDRESS_LINE2 is null,IF(ADDRESS_LINE3 is null,concat(upper(ADDRESS_LINE1),' '),concat(concat(upper(ADDRESS_LINE1),','), concat(upper(ADDRESS_LINE3),' '))),IF(ADDRESS_LINE3 is null,concat(concat(upper(ADDRESS_LINE1),','),concat(upper(ADDRESS_LINE2),' ')), concat(concat(concat(upper(ADDRESS_LINE1),','),concat(upper(ADDRESS_LINE2),',')),concat(upper(ADDRESS_LINE3),' '))))),'\n',");
 			bufInsSql.append(" concat(IF(DISTRICT_DESC is null,IF(STATE_DESC is null,IF(COUNTRY_DESC is null,' ',concat('(',upper(COUNTRY_DESC),') ')), IF(COUNTRY_DESC is null,concat(upper(STATE_DESC),' '),concat(concat(upper(STATE_DESC),','),concat('(',upper(COUNTRY_DESC),') ')))), IF(STATE_DESC is null,IF(COUNTRY_DESC is null,concat(upper(DISTRICT_DESC),' '),concat(concat(upper(DISTRICT_DESC),','), concat('(',upper(COUNTRY_DESC),') '))),IF(COUNTRY_DESC is null,concat(concat(upper(DISTRICT_DESC),','),concat(upper(STATE_DESC),' ')), concat(concat(concat(upper(DISTRICT_DESC),','),concat(upper(STATE_DESC),',')),concat('(',upper(COUNTRY_DESC),') '))))), IF(PINCODE is null,' ',concat('- ',PINCODE))),'\n',");
 			bufInsSql.append(" IF(PRIMARY_PHONE is null,IF(ALTERNATE_PHONE is null,' ', concat('PHONE NO.-',concat(ALTERNATE_PHONE,' '))),IF(ALTERNATE_PHONE is null,concat('PHONE NO.-',concat(PRIMARY_PHONE, ' ')),concat('PHONE NO.-',concat(concat(PRIMARY_PHONE,','),concat(ALTERNATE_PHONE,' ')))))) location,");
 			...
 			logger.info("In fetchRecordDetail() of DealRateApprovalDAOImpl Customer Detail Qery : " + bufInsSql.toString());
			header = ConnectionDAO.sqlSelect(bufInsSql.toString());
 		...
 		try
 		{
 			...
 			bufInsSql.append(" (((IFNULL(pro.DEAL_CHARGE_CALCULATED_AMOUNT,0)+IFNULL(refundable.DEAL_CHARGE_CALCULATED_AMOUNT,0))/b.deal_loan_amount)*100) processingFeePer,(IFNULL(pro.DEAL_CHARGE_CALCULATED_AMOUNT,0)+IFNULL(refundable.DEAL_CHARGE_CALCULATED_AMOUNT,0)) processingFee,");
 			bufInsSql.append(" (IFNULL(pro.DEAL_CHARGE_TAX_AMOUNT1,0)+IFNULL(refundable.DEAL_CHARGE_TAX_AMOUNT1,0)) taxOnProcFee,(IFNULL(pro.DEAL_CHARGE_NET_AMOUNT,0)+IFNULL(refundable.DEAL_CHARGE_NET_AMOUNT,0))totalProcFee,");
 			bufInsSql.append(" b.DEAL_TENURE,((IFNULL(seq.SD_AMOUNT,0)/b.deal_loan_amount)*100) MMD_ICD_AMT_PER ,IFNULL(seq.SD_AMOUNT,0)MMD_ICD_AMT,");
 			bufInsSql.append(" IFNULL(seq.SD_INTEREST_RATE,0)INTEREST_ON_MMD_ICD,");
 			bufInsSql.append(" (IFNULL(b.deal_loan_amount,0)-IFNULL(seq.SD_AMOUNT,0))NET_LOAN_AMT,IFNULL(b.deal_irr1,0)net_acc_irr,IFNULL(b.deal_irr2,0)gross_acc_irr,A.DEAL_ID ");
 			...
 			bufInsSql.append(" ((b.DEAL_LOAN_AMOUNT/b.DEAL_ASSET_COST)*100)LtvOnAsset,");
 		 	bufInsSql.append(" if(LTVONASSET.ASSET_COLLATERAL_VALUE is null,0.00,((b.DEAL_LOAN_AMOUNT/LTVONASSET.ASSET_COLLATERAL_VALUE)*100))LtvOnCombined ,");
 		 	bufInsSql.append(" if(b.DEAL_ASSET_COST is null,0.00,(((b.DEAL_LOAN_AMOUNT-ifnull(seq.SD_AMOUNT,0))/b.DEAL_ASSET_COST)*100))NetLtvOnAsset ,");
 		 	bufInsSql.append(" if(LTVONASSET.ASSET_COLLATERAL_VALUE is null,0.00,(((b.DEAL_LOAN_AMOUNT-ifnull(seq.SD_AMOUNT,0))/LTVONASSET.ASSET_COLLATERAL_VALUE)*100))NetLtvOnCombined   ");
 			bufInsSql.append(" from cr_deal_dtl a");
 			bufInsSql.append(" join cr_deal_loan_dtl b on(a.deal_id=b.deal_id)");
 			...
 			logger.info("In fetchRecordDetail() of DealRateApprovalDAOImpl Loan Detail Qery : " + bufInsSql.toString());
			header = ConnectionDAO.sqlSelect(bufInsSql.toString());
  		  	ArrayList header1=(ArrayList)header.get(0);
		...
		{
			fetchVo.setInterestOnMmdIcd("0.00");
		}
...
//    		}
 		...
 		{
 			bufInsSql=new StringBuffer(); 
 			bufInsSql.append(" select IFNULL(legal.DEAL_CHARGE_CALCULATED_AMOUNT,0)legalCharges,IFNULL(legal.DEAL_CHARGE_FINAL_AMOUNT,0)total_leagal,");
 			bufInsSql.append(" IFNULL(roc.DEAL_CHARGE_CALCULATED_AMOUNT,0)rocCharges,IFNULL(roc.DEAL_CHARGE_FINAL_AMOUNT,0)total_roc,");
 			bufInsSql.append(" IFNULL(valuation.DEAL_CHARGE_CALCULATED_AMOUNT,0)valuationCharges,IFNULL(valuation.DEAL_CHARGE_FINAL_AMOUNT,0)total_valuation,");
 			bufInsSql.append(" IFNULL(stamp.DEAL_CHARGE_CALCULATED_AMOUNT,0)stampCharges,IFNULL(stamp.DEAL_CHARGE_FINAL_AMOUNT,0)total_stamp,");
 			bufInsSql.append(" IFNULL(other.DEAL_CHARGE_CALCULATED_AMOUNT,0)otherCharges,IFNULL(other.DEAL_CHARGE_FINAL_AMOUNT,0)total_other");
 			...
 			logger.info("In fetchRecordDetail() of DealRateApprovalDAOImpl Other Charges Detail Qery : " + bufInsSql.toString());
			header = ConnectionDAO.sqlSelect(bufInsSql.toString());
  		  	ArrayList header1=(ArrayList)header.get(0);
 		...
 		try
 		{
 			...
 			bufInsSql.append("select IFNULL(dealer.DEAL_CHARGE_FINAL_AMOUNT,0)dealerCharges,");
 			bufInsSql.append("ifnull(manufacture.DEAL_CHARGE_FINAL_AMOUNT,0)+ifnull(dealer.DEAL_CHARGE_FINAL_AMOUNT,0)manufactureCharges ");
 			bufInsSql.append("from "); 
 			bufInsSql.append("cr_deal_dtl a ");
 			bufInsSql.append("left join ");
 			bufInsSql.append("(");
 			...
 			logger.info("In fetchRecordDetail() of DealRateApprovalDAOImpl subvention Detail Qery : " + bufInsSql.toString());
			header = ConnectionDAO.sqlSelect(bufInsSql.toString());
  		  	ArrayList header1=(ArrayList)header.get(0);
 		...
 		try
 		{
 			...
 			bufInsSql.append("SELECT IFNULL(A.RACK_PROCESSING_FEE,0),IFNULL(A.RACK_RATE,0) from cr_deal_loan_dtl B ");  
 			...
 			logger.info("In fetchRecordDetail() of DealRateApprovalDAOImpl cross sell-life insurance Detail Qery : " + bufInsSql.toString());
			header = ConnectionDAO.sqlSelect(bufInsSql.toString());
  		  	ArrayList header1=(ArrayList)header.get(0);
	...
	public boolean insertRecordDetail(DealRateApprovalVO ob) {
		...
		try {
			...
			bufInsUpdSql.append("PRODUCT_SOLD ,CSOI_PREMIUM_AMOUNT ,TRANSACTION_FEE ,RACK_PROCESSING_FEE ,RACK_RATE ,APPROVAL_BY ,REC_STATUS ,MAKER_ID ,MAKER_DATE,ROI_TYPE,CHANNEL_TYPE,CHANNEL_NAME,PAYOUT_APPLICABLE,PAYOUT_DIFF_FROM_ABOVE_CELL,PSL,IS_THIS_A_BALANCE_TRANSFER,EXISTING_ROI_OF_THE_CUSTOMER,LTV_PERCENTAGE_ON_ASSET,LTV_PERCENTAGE_ON_COMBINED,NET_LTV_PERCENTAGE_ON_ASSET,NET_LTV_PERCENTAGE_ON_COMBINED  )");
			  ...
			  bufInsUpdSql.append(" STR_TO_DATE(?,'"+dateFormatWithTime+"'),");//DATE_OF_BIRTH
			  ...
			  bufInsUpdSql.append(" STR_TO_DATE(?,'"+dateFormatWithTime+"'),");//MAKER_DATE
	...
	{
		logger.info("In fetchDealRateApprovalAuthorDetail() of DealRateApprovalDAOImpl.");		
 		ArrayList list=new ArrayList();
 		try
 		{
 			ArrayList header=null;
 			...
 			StringBuffer bufInsSqlTempCount = new StringBuffer(); 			
 						...
 						bufInsSql.append("select d.DEAL_ID,d.DEAL_NO,DEAL_APPLICATION_FORM_NO,");
 			 			bufInsSql.append("DATE_FORMAT(n.DEAL_DATE,'"+dateFormat+"') AS DEAL_DATE,d.CUSTMER_NAME,d.PRODUCT_DESC,d.SCHEME_DESC,(SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=n.MAKER_ID) MAKER_ID ");
		 ...
		 if(result.equalsIgnoreCase("A") && status)
		  {
			...
			query.append(" STR_TO_DATE(?,'"+dateFormatWithTime+"'),DEAL_FORWARD_USER=? ");
			...
			bufInsUpdSql.append("INSERT INTO CR_DEAL_MOVEMENT_DTL(DEAL_ID, DEAL_STAGE_ID,DEAL_ACTION, DEAL_RECEIVED,DEAL_RECEIVED_USER,REC_STATUS)");
				  ...
				  bufInsUpdSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"'),");//DEAL_RECEIVED
	...
	public String isItMachineOrProperty(String dealId) {
	      ...
	      try
	     {
			...
			if(!CommonFunction.checkNull(assetClass).equalsIgnoreCase("0"))
			{
				...
				logger.info("In isItMachineOrProperty(): "+query2.toString());
