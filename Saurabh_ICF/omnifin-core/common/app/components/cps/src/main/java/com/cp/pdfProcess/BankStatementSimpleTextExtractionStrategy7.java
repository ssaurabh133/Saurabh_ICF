...
public class BankStatementSimpleTextExtractionStrategy7 implements
		TextExtractionStrategy {

    private Vector lastStart;
    private Vector lastEnd;
    private float lastPagePositionStart = 37.0f;
     ...
     */
    public BankStatementSimpleTextExtractionStrategy7() 
    {
    ...
    public void renderText(TextRenderInfo renderInfo) {
        ...
        float yStart = 480.000f;
        float yEnd = 52.000f;
        if(currentPageNo!=	1){
        	yStart=765.00f;
        }
        
        float yTolerance = 28f;
        float sameLineThreshold = 19f;
        float yPageChangeTolerance = -100f;
        String str = renderInfo.getText();
        String delimiterPrefix = "";

        if(str.equalsIgnoreCase("NEFTINW-0026815117"))
        {
        	System.out.println("------------");
        }
        
        if((str.contains("Opening balance")) || (str.contains("Closing balance")) || (str.contains("as on")) || (str.contains("Opening balance")) || (str.contains("Write to us at Customer Contact Centr"))
        		|| str.contains("Call 1800 102 6022"))
        {
        	return;
        }
