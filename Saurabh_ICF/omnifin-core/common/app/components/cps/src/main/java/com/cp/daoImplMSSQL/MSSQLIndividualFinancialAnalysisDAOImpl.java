...
public class MSSQLIndividualFinancialAnalysisDAOImpl implements IndividualFinancialAnalysisDAO {
	
	private static final Logger logger = Logger.getLogger(MSSQLIndividualFinancialAnalysisDAOImpl.class.getName());
	...
	public ArrayList<CommonDealVo> individualFinancialSearchGetDetail(CommonDealVo vo) {
		...
		try
		{
			...
			logger.info("Search individualFinancialSearchGetDetail query for SQL SERVER : " + bufInsSql.toString());
...
//			{
			...
			bufInsSql.append(endRecordIndex);
			bufInsSql.append(" ROWS ONLY ");
			logger.info("Search individualFinancialGetDetailBehind query for SQL SERVER : " + bufInsSql.toString());
	    	...
	    	//{
	   		 if(!check){
    			 ...
    			 bufInsSql =	new StringBuffer();
    	    			...
    	    			bufInsSql.append("insert into cr_financial_data_dtl(DEAL_ID,SOURCE_TYPE,DEAL_CUSTOMER_ROLE_TYPE,DEAL_CUSTOMER_ID,FINANCIAL_MONTH,FINANCIAL_YEAR,VERIFICATION_METHOD,REMARKS,REC_STATUS,MAKER_ID,MAKER_DATE)");
...
//		    	    			for(int y=currBusinessDateYear, pVal=0;y >currBusinessDateYear-5;y--,pVal++)
	...
	public ArrayList getdealAllParamDeatils(CommonDealVo vo1) {
		...
		try{
			...
			query.append("Select cfdd.FINANCIAL_DATA_ID,cfp.PARAM_CODE,");
			query.append("ISNULL(cfdd.FINANCIAL_MONTH,0),");
			query.append("ISNULL(cfdd.PARAM_VALUE,0),cfp.PARAM_NAME" +
					...
					" where cfp.SOURCE_TYPE='"+vo1.getSourceType()+"' and (cfdd.DEAL_ID='"+vo1.getLbxDealNo()+"' or ");
			query.append("ISNULL(cfdd.DEAL_ID,'') = '') ");
			
//			query ="Select ISNULL(cfdd.FINANCIAL_DATA_ID,0),cfp.PARAM_CODE,ISNULL(cfdd.FINANCIAL_MONTH,0),ISNULL(cfdd.PARAM_VALUE,0)" +
//			" from cr_financial_param cfp  left join cr_financial_data_dtl cfdd on cfp.PARAM_CODE=cfdd.PARAM_CODE and cfdd.REC_STATUS='P'" +
//			" where cfp.SOURCE_TYPE='"+vo1.getSourceType()+"' and (cfdd.DEAL_ID='"+vo1.getLbxDealNo()+"' or ISNULL(cfdd.DEAL_ID,'') = '') ";
	...
	public String saveIndividualObligation(Object ob)
	{
		...
		String result="";
			...
			//Nishant Space end
				...
				if(status)
				{
					 String query3="Select max(OBLIGATION_ID) from cr_obligation_analysis_dtl  WITH (ROWLOCK) ";
		...
		try
		{
			...
			Boolean check=ConnectionDAO.checkStatus(query);
			 ...
			 queryUpdate.append("STR_TO_DATE(?,'"+dateFormatWithTime+"'),OUTSTANDING_AMOUNT=?," +
					"EMI=?,FINANCED_AMOUNT=?,OBLIGATION_TYPE=?,VERIFICATION_METHOD=?,MAKER_ID=?,MAKER_DATE=");
	...
	public int saveBankAccountAnalysis(Object ob)
		...
		public boolean updateBankAccountAnalysis(Object ob)
		  {
			...
			try
			{
				...
				queryUpdate.append("update cr_bank_analysis_dtl set DEAL_ID=?,BANK_NAME=?,BANK_BRANCH=?,ACCOUNT_NO=?,ACCOUNT_TYPE=?,STATEMENT_YEAR=?,STATEMENT_MONTH=?,TOTAL_CR=?," +
						...
						"OVER_LIMIT_AMOUNT=?,BOUNCING_INWARD_OUTWARD=?,LIMIT_OBLIGATION=?,TOTAL_CR_TXN=?,TOTAL_DR_TXN=? ,VERIFICATION_METHOD=?,BALANCE_AMOUNT=?,REMARKS=?,MAKER_ID=?,MAKER_DATE=");
				//queryUpdate.append("DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)");
				//Saurabh space starts
				queryUpdate.append(dbo);
				...
				queryUpdate.append(" where BANK_ANALYSIS_ID=?"); 
				PrepStmtObject prepStmt = new PrepStmtObject();
		...
		public ArrayList getIncomeDetailList(Object ob) {
			...
			query.append(" (select description from generic_master where GENERIC_KEY='VERIFICATION_METHOD' and value=VERIFICATION_METHOD) VERIFICATION_METHOD");
			query.append("  from cr_financial_data_dtl a where deal_id='"+CommonFunction.checkNull((obVo.getLbxDealNo()))+"'");
		...
		public ArrayList getIncomeDetaisForEdit(Object ob) {
			...
			query.append("select FINANCIAL_DATA_ID,DEAL_ID,SOURCE_TYPE,DEAL_CUSTOMER_ROLE_TYPE,DEAL_CUSTOMER_ID,");
			query.append("FINANCIAL_MONTH,FINANCIAL_YEAR,VERIFICATION_METHOD,remarks  from cr_financial_data_dtl ") ;
		   	 ...
		   	 try {
		   		 ...
		   		 if(!check){
	    			 ...
	    			 bufInsSql =	new StringBuffer();
	    	    			...
	    	    			bufInsSql.append("update  cr_financial_data_dtl set SOURCE_TYPE=?,DEAL_CUSTOMER_ROLE_TYPE=?,DEAL_CUSTOMER_ID=?,FINANCIAL_MONTH=?,FINANCIAL_YEAR=?,VERIFICATION_METHOD=?,REMARKS=?,MAKER_ID=?,");
...
/*Added By arun deleteRatioAnalysis Ends Here*/		
		...
		public String updateStatus(String dealId){
			...
			try
			{
			bufIns.append("Update cr_deal_dtl set REC_STATUS='A' where DEAL_ID='"+CommonFunction.checkNull(dealId).trim()+"' ");
