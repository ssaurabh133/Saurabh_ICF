...
public class DedupeDAOImpl implements DedupeDAO 
{
	private static final Logger logger = Logger.getLogger(DedupeDAOImpl.class.getName());
	...
	public ArrayList getDupRecord(DedupeVO vo) 
	{
		...
		String aadhaar = CommonFunction.checkNull(vo.getAadhaar()).trim();
		...
		try
		{
			...
			out.add(s1);
			out.add(s2);
			 logger.info(new StringBuilder().append("deDupe('").append(customerID).append("','").append(customerName).append("','").append(customerName).append("','").append(panNo).append("','").append(aadhaar).append("','").append(regNo).append("','").append(dlNo).append("','").append(passportNo).append("','").append(voterID).append("','").append(phNo).append("','").append(groupName).append("','").append(dealCustomer).append("','").append(loanCustomer).append("','").append(s1).append("','").append(s2).append("'").toString());
	        ...
	        {
	        	resultList.add("");
	        	bufInsSql.append("select DISTINCT " +
	        			" a.deal_id,b.deal_no,dl.DEAL_LOAN_AMOUNT,case b.REC_STATUS when 'A' then 'APPROVED ' when 'X' then if((select count(1) from cr_deal_movement_dtl where REC_STATUS='A' and deal_id=b.DEAL_ID)>0,'REJECTED ','CANCELLED ')when 'P' then 'PENDING 'when 'F' then 'FORWARDED 'end as deal_STATUS," +
	        			...
	        			" a.deal_customer_id,c.customer_name,CUSTMER_PAN,CUSTOMER_REGISTRATION_NO,DRIVING_LICENSE,PASSPORT_NUMBER,VOTER_ID,(select group_concat(primary_phone separator ',') from cr_deal_address_m where bpid=c.customer_id)phone," +
	        			" l.loan_no,l.loan_loan_amount,date_format(l.LOAN_REPAY_EFF_DATE,'%d-%m-%Y'),date_format(l.LOAN_MATURITY_DATE,'%d-%m-%Y'),case l.rec_status when 'P' then 'PENDING' when 'A' then 'ACTIVE' when 'L' then 'CANCELLED' when 'X' then 'REJECTED' when 'C' then 'CLOSED' end as loan_status," +
	        			" case l.DISBURSAL_STATUS when 'P' then 'PARTIALLY DISBURSED' when 'F' then 'FULLY DISBURSED' when 'N' then 'NOT DISBURSED' END AS DISBURSAL_STATUS," +
	        			" LOAN_BALANCE_PRINCIPAL,LOAN_OVERDUE_PRINCIPAL,LOAN_DPD,gr.GROUP_DESC_PRINT,UID_NO" +
	        	...
	        	if(customerID.trim().equalsIgnoreCase(""))
	        	{
	        		bufInsSql.append("   union  " + 
	        			" select DISTINCT " +   
	        			" b.deal_id,b.deal_no,dl.DEAL_LOAN_AMOUNT,case b.REC_STATUS when 'A' then 'APPROVED ' when 'X' then if((select count(1) from cr_deal_movement_dtl where REC_STATUS='A' and deal_id=b.DEAL_ID)>0,'REJECTED ','CANCELLED ')when 'P' then 'PENDING 'when 'F' then 'FORWARDED 'end as deal_STATUS," + 
	        			" CASE a.LOAN_CUSTOMER_ROLE_TYPE WHEN 'PRAPPL' THEN 'APPLICANT' WHEN 'COAPPL' THEN 'CO-APPLICANT' WHEN 'GUARANTOR' THEN 'GUARANTOR' END AS CUST_TYPE," + 
	        			" a.gcd_id,c.customer_name,CUSTMER_PAN,CUSTOMER_REGISTRATION_NO,DRIVING_LICENSE,PASSPORT_NUMBER,VOTER_ID,(select group_concat(primary_phone separator ',') from com_address_m where bpid=c.customer_id)phone," + 
	        			" l.loan_no,l.loan_loan_amount,date_format(l.LOAN_REPAY_EFF_DATE,'%d-%m-%Y'),date_format(l.LOAN_MATURITY_DATE,'%d-%m-%Y'),case l.rec_status when 'P' then 'PENDING' when 'A' then 'ACTIVE' when 'L' then 'CANCELLED' when 'X' then 'REJECTED' when 'C' then 'CLOSED' end as loan_status," + 
	        			" case l.DISBURSAL_STATUS when 'P' then 'PARTIALLY DISBURSED' when 'F' then 'FULLY DISBURSED' when 'N' then 'NOT DISBURSED' END AS DISBURSAL_STATUS," + 
	        			" LOAN_BALANCE_PRINCIPAL,LOAN_OVERDUE_PRINCIPAL,LOAN_DPD,gr.GROUP_DESC_PRINT,UID_NO" + 
	        	...
	        	bufInsSql.append(" order by deal_id,deal_customer_id");
	           	...
	           	bufInsSqlTempCount.append("select count(1) from ( select DISTINCT " +
	    	        	" a.deal_id,b.deal_no,dl.DEAL_LOAN_AMOUNT,case b.REC_STATUS when 'A' then 'APPROVED ' when 'X' then if((select count(1) from cr_deal_movement_dtl where REC_STATUS='A' and deal_id=b.DEAL_ID)>0,'REJECTED ','CANCELLED ')when 'P' then 'PENDING 'when 'F' then 'FORWARDED 'end as deal_STATUS," +
	    	        	" CASE a.DEAL_CUSTOMER_ROLE_TYPE WHEN 'PRAPPL' THEN 'APPLICANT' WHEN 'COAPPL' THEN 'CO-APPLICANT' WHEN 'GUARANTOR' THEN 'GUARANTOR' END AS CUST_TYPE," +
	    	        	" a.deal_customer_id,c.customer_name,CUSTMER_PAN,CUSTOMER_REGISTRATION_NO,DRIVING_LICENSE,PASSPORT_NUMBER,VOTER_ID,(select group_concat(primary_phone separator ',') from cr_deal_address_m where bpid=c.customer_id)phone," +
	    	        	" l.loan_no,l.loan_loan_amount,date_format(l.LOAN_REPAY_EFF_DATE,'%d-%m-%Y'),date_format(l.LOAN_MATURITY_DATE,'%d-%m-%Y'),case l.rec_status when 'P' then 'PENDING' when 'A' then 'ACTIVE' when 'L' then 'CANCELLED' when 'X' then 'REJECTED' when 'C' then 'CLOSED' end as loan_status," +
                ...
                for (int i=0;i<formatlist.size();i++)
                {
					...
					dvo.setGroupName(CommonFunction.checkNull(data.get(22)).toString());
					 dvo.setDupAadhaar(CommonFunction.checkNull(data.get(23)).toString());
