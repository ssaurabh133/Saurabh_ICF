...
public class MSSQLTradeCheckDAOImpl implements TradeCheckDAO {
	private static final Logger logger = Logger.getLogger(MSSQLTradeCheckDAOImpl.class.getName());
	...
	/* Trade Check Search Method */
		...
		ArrayList<TradeCheckInitVo> list = new ArrayList<TradeCheckInitVo>();
			...
			if(appendSQL){
				...
				bufInsSql.append(" WHERE A.DEAL_BRANCH='"+StringEscapeUtils.escapeSql(vo.getBranchId()).trim()+"' and A.Rec_status='"+StringEscapeUtils.escapeSql(vo.getStage()).trim()+"' and dv.DEAL_ID=c.deal_id and dv.VERIFICATION_TYPE='T' and dv.VERIFICATION_ACTION='I' and dv.REC_STATUS='F'");
				bufInsSqlTempCount.append(" WHERE A.DEAL_BRANCH='"+StringEscapeUtils.escapeSql(vo.getBranchId()).trim()+"' and A.Rec_status='"+StringEscapeUtils.escapeSql(vo.getStage()).trim()+"' and dv.DEAL_ID=c.deal_id and dv.VERIFICATION_TYPE='T' and dv.VERIFICATION_ACTION='I' and dv.REC_STATUS='F'");
			}
	...
	public ArrayList<TradeCheckInitVo> searchTradeCheckCapDataAuto(TradeCheckInitVo vo, String recStatus, HttpServletRequest request) {
		...
		ArrayList<TradeCheckInitVo> list = new ArrayList<TradeCheckInitVo>();
			...
			if(recStatus.equals("R"))
			{
				...
				bufInsSql.append(" (SELECT COUNT(*) D_TRADE_COUNT,X.DEAL_ID FROM cr_deal_verification_dtl X WHERE X.VERIFICATION_TYPE='T' and X.VERIFICATION_ACTION='I' GROUP BY X.DEAL_ID) AS G,");
				bufInsSqlTempCount.append(" (SELECT COUNT(*) D_TRADE_COUNT,X.DEAL_ID FROM cr_deal_verification_dtl X WHERE X.VERIFICATION_TYPE='T' and X.VERIFICATION_ACTION='I' GROUP BY X.DEAL_ID) AS G,");
				...
				bufInsSql.append(" where dv.DEAL_ID=c.deal_id and dv.VERIFICATION_TYPE='T' and dv.VERIFICATION_ACTION='I' ");
				bufInsSqlTempCount.append(" where dv.DEAL_ID=c.deal_id and dv.VERIFICATION_TYPE='T' and dv.VERIFICATION_ACTION='I' ");
			...
			else
			{
				...
				bufInsSql.append(" left join com_agency_user_mapping agm on ag.AGENCY_CODE=agm.AGENCY_CODE ");
				bufInsSqlTempCount.append(" left join com_agency_user_mapping agm on ag.AGENCY_CODE=agm.AGENCY_CODE ");
				bufInsSql.append(" where dv.DEAL_ID=c.deal_id and dv.VERIFICATION_TYPE='T' and dv.VERIFICATION_ACTION='I' and dv.REC_STATUS='"+StringEscapeUtils.escapeSql(recStatus)+"'");
				bufInsSqlTempCount.append(" where dv.DEAL_ID=c.deal_id and dv.VERIFICATION_TYPE='T' and dv.VERIFICATION_ACTION='I' and dv.REC_STATUS='"+StringEscapeUtils.escapeSql(recStatus)+"'");
...
//						vo.setRelation((CommonFunction.checkNull(header1.get(3))).trim());
	...
	public boolean modifyTradecheck(Object ob, String dealId,String verificationID) {
		...
		try {
				...
				bufInsSql.append(" where a.deal_id=b.deal_id and a.VERIFICATION_ID=b.VERIFICATION_ID and b.verification_type='T' and a.DEAL_ID=? and a.VERIFICATION_ID in ("+verificationID+")");
				//---------------
		    	if((CommonFunction.checkNull(vo.getMakerId()).trim()).equalsIgnoreCase(""))
				insertPrepStmtObject.addNull();
				else
