...
public class MSSQLRepoTransactionDaoImpl implements RepoTransactionDAO{

	private static final Logger logger = Logger.getLogger(MSSQLRepoTransactionDaoImpl.class.getName());
 ...
 // Repo Details By Stockyard Starts
	...
	logger.info("repoID:-1---"+vo.getRepoId());
		...
		try {
				...
				bufInsSql.append(dbo);
				bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9), ");
				bufInsSql.append(" STOCKYARD_ENTRY_TIME=?,STOCKYARD_MAKER_ID = ?,REC_STATUS=?");
				bufInsSql.append(" where REPO_ID=?");
	...
	public ArrayList searchRepoMarkingAuthor(Object ob) {
		...
		try {
			...
			bufInsSql.append(" AND RMD.MAKER_ID <> '" + repoMarkingMakerVo.getMakerId() + "' ");
			bufInsSqlTempCount.append(" AND RMD.MAKER_ID <> '" + repoMarkingMakerVo.getMakerId() + "' ");
...
// nazia workspace starts for  REPO CONFIRMATION ------>>>>>
	...
	public ArrayList searchRepoConfirmation(Object ob) {
		...
		try {
			...
			bufInsSql.append(" WHERE RCD.AGENCY_FLAG='Y' AND RCD.STOCKYARD_FLAG = 'Y' AND ISNULL(RCD.REPO_CONFIRMATION_FLAG,'N')='N' AND RCD.BRANCH_ID='"+CommonFunction.checkNull(repoVo.getBranchId())+"'  ");
			...
			bufInsSqlTempCount.append("SELECT COUNT(1) FROM REPO_CASE_DTL RCD WHERE RCD.AGENCY_FLAG='Y' AND RCD.STOCKYARD_FLAG = 'Y' AND ISNULL(RCD.REPO_CONFIRMATION_FLAG,'N')='N' AND RCD.BRANCH_ID='"+CommonFunction.checkNull(repoVo.getBranchId())+"'  ");
			...
			//bufInsSql.append("AND RCD.AGENCY_MAKER_ID <> '" + repoVo.getMakerId() + "' ");
			//bufInsSqlTempCount.append(" AND RCD.AGENCY_MAKER_ID <> '" + repoVo.getMakerId() + "' ");
			
			
			//bufInsSql.append("AND RCD.STOCKYARD_MAKER_ID <> '" + repoVo.getMakerId() + "' ");
			//bufInsSqlTempCount.append(" AND RCD.STOCKYARD_MAKER_ID <> '" + repoVo.getMakerId() + "' ");
...
public boolean saveRepoConfirmationDetail(Object ob) {
	...
	try {
		...
		logger.info("## In saveRepoConfirmationDetail() : update query (FOR REPO_CASE_DTL) "+insertPrepStmtObject.printQuery());
		...
		bufInsSql.append(" UPDATE CR_LOAN_DTL SET REPO_FLAG=?,REPO_REMARKS = ?, " );
		bufInsSql.append("REPO_DATE=");
		bufInsSql.append(dbo);
		bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
		bufInsSql.append(" where LOAN_ID = (SELECT LOAN_ID FROM REPO_CASE_DTL WHERE REPO_ID = ?)");
		...
		logger.info("## In saveRepoConfirmationDetail() : update query (FOR CR_LOAN_DTL) : ==>> "+insertPrepStmtObject.printQuery());
