package com.legal.daoImplMYSQL;
...
import com.legal.vo.ReasonMasterVo;


public class LegalTransactionDaoImpl implements LegalTransactionDAO{

	private static final Logger logger = Logger.getLogger(LegalTransactionDaoImpl.class.getName());
	...
	public ArrayList searchLegalNoticeInitiationMakerData(Object ob) {
		...
		try {
			...
			/*bufInsSql.append("SELECTDistinct  NOTICE_ID,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID=LND.LOAN_NO) AS LAN,(SELECT DISTINCT NOTICE_TYPE_DESC FROM LEG_NOTICE_TYPE_DEF_M WHERE NOTICE_TYPE_CODE = LND.NOTICE_CODE) AS NOTICE,");
			...
			bufInsSql.append("SELECT Distinct NOTICE_ID,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID=LND.LOAN_NO) AS LAN,GCD.CUSTOMER_NAME,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,(SELECT DISTINCT NOTICE_TYPE_DESC FROM LEG_NOTICE_TYPE_DEF_M WHERE NOTICE_TYPE_CODE = LND.NOTICE_CODE) AS NOTICE,");
			      bufInsSql.append("case lnd.REC_STATUS when 'A' then 'Approved' when 'F' then 'Forwarded' when 'X' then 'Reject' else 'Pending' end as REC_STATUS ");
			      bufInsSql.append(" FROM LEGAL_NOTICE_DTL LND JOIN GCD_CUSTOMER_M GCD ON GCD.CUSTOMER_ID=LND.CUSTOMER_ID left join cr_loan_customer_role cr on cr.GCD_ID=gcd.customer_id WHERE LND.REC_STATUS = 'P' ");
			 
			      bufInsSqlTempCount.append("SELECT COUNT(1) FROM LEGAL_NOTICE_DTL LND JOIN GCD_CUSTOMER_M GCD ON GCD.CUSTOMER_ID=LND.CUSTOMER_ID left join cr_loan_customer_role cr on cr.GCD_ID=gcd.customer_id and LND.loan_no=cr.loan_id  WHERE LND.REC_STATUS = 'P'  ");
			 
			      if (!loanNo.equals("")) {
			         bufInsSql.append(new StringBuilder().append(" AND lnd.LOAN_NO = '").append(loanNo).append("' ").toString());
			         bufInsSqlTempCount.append(new StringBuilder().append(" AND lnd.LOAN_NO = '").append(loanNo).append("' ").toString());
			       }
			 ...
			 bufInsSql.append(" ORDER BY NOTICE_ID ");
			...
			for (int i = 0; i < searchlist.size(); i++) {
				...
				if (data.size() > 0) {
					...
					noticeVo.setLoanNo(CommonFunction.checkNull(data.get(1)).toString());
					          noticeVo.setCustomerId(CommonFunction.checkNull(data.get(2)).toString());
					          noticeVo.setCustRole(CommonFunction.checkNull(data.get(3)).toString());
					         noticeVo.setNoticeDesc(CommonFunction.checkNull(data.get(4)).toString());
					           noticeVo.setRecStatus(CommonFunction.checkNull(data.get(5)).toString());
	...
	public boolean insertLegalNoticeInitiationMakerData(Object ob) {
		...
		String query = "select NOTICE_ID from LEGAL_NOTICE_DTL where NOTICE_ID='"
				+ StringEscapeUtils.escapeSql(vo.getNoticeId().trim()) + "'";
		logger.info("In insertCaseTypeMaster.......inside ejb server file..........Dao Impl"
						+ query);
		...
		try {
			...
			if (!st) {
				...
				StringBuffer bufInsSql = new StringBuffer();
				bufInsSql.append("insert into LEGAL_NOTICE_DTL(LOAN_NO,CUSTOMER_ID,NOTICE_CODE,REASON_CODE,NOTICE_AMOUNT,MODE_OF_NOTICE,TIME_OF_CONCILIATION,VENUE_OF_CONCILIATION,NAME_OF_CONCILIATOR,DESIGNATION_OF_CONCILIATOR,MAKER_REMARKS,REC_STATUS,DATE_OF_CONCILIATION,MAKER_ID,MAKER_DATE)");
				...
				if (CommonFunction.checkNull(vo.getLbxLoanId())
				 ...
				 if (CommonFunction.checkNull(vo.getLbxcustomerId()).equalsIgnoreCase(""))
				          {
				         insertPrepStmtObject.addNull();
				          }
				        else insertPrepStmtObject.addString(vo.getLbxcustomerId().toUpperCase().trim());
	...
	public ArrayList editLegalNoticeInitiationMakerData(Object ob) {
		...
		try {
			...
			StringBuffer bufInsSql = new StringBuffer();
			       bufInsSql.append(" SELECT Distinct NOTICE_ID,LOAN_NO,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LND.LOAN_NO),GCD.CUSTOMER_NAME,lnd.Customer_Id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,NOTICE_CODE,(SELECT DISTINCT NOTICE_TYPE_DESC FROM LEG_NOTICE_TYPE_DEF_M WHERE NOTICE_TYPE_CODE = LND.NOTICE_CODE),");
			       bufInsSql.append(" REASON_CODE,(select reason_desc from com_legal_reason_m where reason_id=LND.REASON_CODE) as Reason_desc,NOTICE_AMOUNT,MODE_OF_NOTICE, ");
			       bufInsSql.append(this.dbo);
			       bufInsSql.append("DATE_FORMAT(DATE_OF_CONCILIATION,'"+dateFormat+"'), ");
			      bufInsSql.append(" TIME_OF_CONCILIATION,VENUE_OF_CONCILIATION, ");
			     bufInsSql.append(" NAME_OF_CONCILIATOR,DESIGNATION_OF_CONCILIATOR,lnd.MAKER_REMARKS ");
			      bufInsSql.append(" FROM LEGAL_NOTICE_DTL LND JOIN GCD_CUSTOMER_M GCD ON GCD.CUSTOMER_ID=LND.CUSTOMER_ID  left join cr_loan_customer_role cr on cr.GCD_ID=gcd.customer_id and LND.loan_no=cr.loan_id   WHERE NOTICE_ID='"+StringEscapeUtils.escapeSql(vo.getNoticeId())+"'");
			...
			for (int i = 0; i < searchlist.size(); i++) {
				...
				if (data.size() > 0) {
					...
					caseTypeDataVo.setNoticeId(CommonFunction.checkNull(data.get(0)).toString());
					           caseTypeDataVo.setLbxLoanId(CommonFunction.checkNull(data.get(1)).toString());
					           caseTypeDataVo.setLoanNo(CommonFunction.checkNull(data.get(2)).toString());
					           caseTypeDataVo.setCustomerId(CommonFunction.checkNull(data.get(3)).toString());
					           caseTypeDataVo.setLbxcustomerId(CommonFunction.checkNull(data.get(4)).toString());
					           caseTypeDataVo.setCustRole(CommonFunction.checkNull(data.get(5)).toString());
					           caseTypeDataVo.setLbxNoticeCode(CommonFunction.checkNull(data.get(6)).toString());
					           caseTypeDataVo.setNoticeDesc(CommonFunction.checkNull(data.get(7)).toString());
					           caseTypeDataVo.setLbxReasonId(CommonFunction.checkNull(data.get(8)).toString());
					           caseTypeDataVo.setReasonDesc(CommonFunction.checkNull(data.get(9)).toString());
					           ...
					           {
					             caseTypeDataVo.setNoticeAmount(this.myFormatter.format(this.myFormatter.parse(CommonFunction.checkNull(data.get(10)))));
					           }
					 
					           caseTypeDataVo.setModeOfNotice(CommonFunction.checkNull(data.get(11)).toString());
					          caseTypeDataVo.setDateOfConciliation(CommonFunction.checkNull(data.get(12)).toString());
					          caseTypeDataVo.setTimeOfConciliation(CommonFunction.checkNull(data.get(13)).toString());
					          caseTypeDataVo.setVenueOfConciliation(CommonFunction.checkNull(data.get(14)).toString());
					           caseTypeDataVo.setNameOfConciliator(CommonFunction.checkNull(data.get(15)).toString());
					           caseTypeDataVo.setDesignationOfConciliator(CommonFunction.checkNull(data.get(16)).toString());
					          caseTypeDataVo.setMakerRemarks(CommonFunction.checkNull(data.get(17)).toString());
	...
	public boolean updateLegalNoticeInitiationMakerData(Object ob) {
		...
		try {
			...
			bufInsSql.append(" UPDATE LEGAL_NOTICE_DTL SET LOAN_NO=?,Customer_id=?,NOTICE_CODE=?,REASON_CODE=?,NOTICE_AMOUNT=?,");
	...
	public ArrayList searchLegalNoticeInitiationCheckerData(Object ob) {
		...
		try {
			...
			logger.info("In searchCaseTypeMasterData()..............inside ejb server file.......................Dao Impl");
			logger.info("Maker Id :::"+legalNoticeVo.getMakerId());
			loanNo = (StringEscapeUtils.escapeSql(CommonFunction.checkNull(legalNoticeVo.getLbxLoanId())).trim());
			noticeCode = (StringEscapeUtils.escapeSql(CommonFunction.checkNull(legalNoticeVo.getLbxNoticeCode())).trim());
           	StringBuffer bufInsSql = new StringBuffer();
			StringBuffer bufInsSqlTempCount = new StringBuffer();
	 ...
	 if (!loanNo.equals("")) {
			...
			bufInsSql.append(" SELECT Distinct NOTICE_ID,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID=LND.LOAN_NO) LOAN_NO,GCD.CUSTOMER_NAME,lnd.Customer_Id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
			      bufInsSql.append(" (SELECT DISTINCT NOTICE_TYPE_DESC FROM LEG_NOTICE_TYPE_DEF_M WHERE NOTICE_TYPE_CODE = LND.NOTICE_CODE) NOTICE, ");
			       bufInsSql.append(this.dbo);
			       bufInsSql.append("DATE_FORMAT(lnd.MAKER_DATE,'"+this.dateFormat+"'), ");
			       bufInsSql.append(" (SELECT DISTINCT USER_NAME FROM SEC_USER_M WHERE USER_ID = LND.MAKER_ID) NOTICE_INITIATED_BY, ");
			      bufInsSql.append(" (SELECT REASON_DESC FROM COM_LEGAL_REASON_M  WHERE REASON_ID = LND.REASON_CODE) AS REASON,MAKER_REMARKS,MODE_OF_NOTICE FROM LEGAL_NOTICE_DTL LND  JOIN GCD_CUSTOMER_M GCD ON GCD.CUSTOMER_ID=LND.CUSTOMER_ID left join cr_loan_customer_role cr on cr.GCD_ID=gcd.customer_id and LND.loan_no=cr.loan_id   ");
			      bufInsSql.append(" WHERE lnd.REC_STATUS='F' and lnd.maker_id <> '"+StringEscapeUtils.escapeSql(legalNoticeVo.getMakerId())+"' ");
			       bufInsSqlTempCount.append("SELECT COUNT(1) FROM LEGAL_NOTICE_DTL WHERE REC_STATUS='F' ");
			 
			       if (!loanNo.equals("")) {
			         bufInsSql.append(" AND lnd.LOAN_NO = '"+loanNo+"' ");
			         bufInsSqlTempCount.append(" AND lnd.LOAN_NO = '"+loanNo+"' ");
			       }
...
//			}
			...
			for (int i = 0; i < searchListSize; i++) {
				...
				if (data.size() > 0) {
					...
					noticeVo.setLoanNo(CommonFunction.checkNull(data.get(1)).toString());
					           noticeVo.setCustomerId(CommonFunction.checkNull(data.get(2)).toString());
					           noticeVo.setLbxcustomerId(CommonFunction.checkNull(data.get(3)).toString());
					           noticeVo.setCustRole(CommonFunction.checkNull(data.get(4)).toString());
					           noticeVo.setNoticeDesc(CommonFunction.checkNull(data.get(5)).toString());
					           noticeVo.setMakerDate(CommonFunction.checkNull(data.get(6)).toString());
					           noticeVo.setMakerId(CommonFunction.checkNull(data.get(7)).toString());
					           noticeVo.setReasonDesc(CommonFunction.checkNull(data.get(8)).toString());
					           noticeVo.setMakerRemarks(CommonFunction.checkNull(data.get(9)).toString());
	...
	public ArrayList editLegalNoticeInitiationCheckerData(Object ob) {
		...
		try {
			...
			bufInsSql.append(" FROM LEGAL_NOTICE_DTL LND WHERE NOTICE_ID='"+StringEscapeUtils.escapeSql(vo.getNoticeId())+"'");*/
			 ...
			 bufInsSql.append(" SELECT  NOTICE_ID,LOAN_NO,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LND.LOAN_NO), gcd.Customer_name,lnd.Customer_Id, ");
			        bufInsSql.append(" (select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,NOTICE_CODE,(SELECT DISTINCT NOTICE_TYPE_DESC FROM LEG_NOTICE_TYPE_DEF_M WHERE NOTICE_TYPE_CODE = LND.NOTICE_CODE),");
			        bufInsSql.append(" REASON_CODE,(SELECT DISTINCT REASON_DESC FROM COM_LEGAL_REASON_M WHERE REASON_ID = LND.REASON_CODE),NOTICE_AMOUNT,MODE_OF_NOTICE, ");
			        bufInsSql.append(this.dbo);
			        bufInsSql.append("DATE_FORMAT(DATE_OF_CONCILIATION,'"+this.dateFormat+"'), ");
			        bufInsSql.append(" TIME_OF_CONCILIATION,VENUE_OF_CONCILIATION, ");
			        bufInsSql.append(" NAME_OF_CONCILIATOR,DESIGNATION_OF_CONCILIATOR,LND>MAKER_REMARKS ");
			        bufInsSql.append(" FROM LEGAL_NOTICE_DTL LND JOIN GCD_CUSTOMER_M GCD ON GCD.CUSTOMER_ID=LND.CUSTOMER_ID left join cr_loan_customer_role cr on cr.GCD_ID=gcd.customer_id and LND.loan_no=cr.loan_id   WHERE NOTICE_ID='"+StringEscapeUtils.escapeSql(vo.getNoticeId())+"'");
			...
			for (int i = 0; i < searchlist.size(); i++) {
				...
				if (data.size() > 0) {
					...
					caseTypeDataVo.setNoticeId(CommonFunction.checkNull(data.get(0)).toString());
					           ...
					           caseTypeDataVo.setLbxReasonId(CommonFunction.checkNull(data.get(8)).toString());
					           caseTypeDataVo.setReasonDesc(CommonFunction.checkNull(data.get(9)).toString());
					           caseTypeDataVo.setNoticeAmount(CommonFunction.checkNull(data.get(10)).toString());
					           caseTypeDataVo.setModeOfNotice(CommonFunction.checkNull(data.get(11)).toString());
					           caseTypeDataVo.setDateOfConciliation(CommonFunction.checkNull(data.get(12)).toString());
					           caseTypeDataVo.setTimeOfConciliation(CommonFunction.checkNull(data.get(13)).toString());
					           caseTypeDataVo.setVenueOfConciliation(CommonFunction.checkNull(data.get(14)).toString());
...
/*				bufInsSql.append(" SELECT NOTICE_ID,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID=LND.LOAN_NO) LOAN_NO, ");
			 ...
			 if (!loanNo.equals("")) {
				...
				bufInsSql.append(" SELECT NOTICE_ID,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID=LND.LOAN_NO) LOAN_NO,gcd.Customer_name,lnd.Customer_Id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
				       bufInsSql.append(" (SELECT DISTINCT NOTICE_TYPE_DESC FROM LEG_NOTICE_TYPE_DEF_M WHERE NOTICE_TYPE_CODE = LND.NOTICE_CODE) NOTICE, ");
				       bufInsSql.append(this.dbo);
				       bufInsSql.append("DATE_FORMAT(LND.MAKER_DATE,'"+this.dateFormat+"'), ");
				       bufInsSql.append(" (SELECT DISTINCT USER_NAME FROM SEC_USER_M WHERE USER_ID = LND.MAKER_ID) NOTICE_INITIATED_BY, ");
				       bufInsSql.append(this.dbo);
				       bufInsSql.append("DATE_FORMAT(LND.AUTHOR_DATE,'"+this.dateFormat+"'), ");
				       bufInsSql.append(" (SELECT DISTINCT USER_NAME FROM SEC_USER_M WHERE USER_ID = LND.AUTHOR_ID) NOTICE_APPROVED_BY, ");
				       bufInsSql.append(" LND.REASON_CODE,MAKER_REMARKS,AUTHER_REMARKS,MODE_OF_NOTICE FROM LEGAL_NOTICE_DTL LND ");
				       bufInsSql.append(" JOIN GCD_CUSTOMER_M GCD ON GCD.CUSTOMER_ID=LND.CUSTOMER_ID left join cr_loan_customer_role cr on cr.GCD_ID=gcd.customer_id and LND.loan_no=cr.loan_id   WHERE lnd.REC_STATUS='A' ");
				       bufInsSqlTempCount.append("SELECT COUNT(1) FROM LEGAL_NOTICE_DTL WHERE REC_STATUS='A' ");
...
//				}
				...
				for (int i = 0; i < searchListSize; i++) {
					...
					if (data.size() > 0) {
						...
						noticeVo.setLoanNo(CommonFunction.checkNull(data.get(1)).toString());
						           ...
						           noticeVo.setMakerDate(CommonFunction.checkNull(data.get(6)).toString());
						           noticeVo.setMakerId(CommonFunction.checkNull(data.get(7)).toString());
						           noticeVo.setAutherDate(CommonFunction.checkNull(data.get(8)).toString());
						           noticeVo.setAutherId(CommonFunction.checkNull(data.get(9)).toString());
						           noticeVo.setReasonDesc(CommonFunction.checkNull(data.get(10)).toString());
						           noticeVo.setMakerRemarks(CommonFunction.checkNull(data.get(11)).toString());
						           noticeVo.setAuthorRemarks(CommonFunction.checkNull(data.get(12)).toString());
						           noticeVo.setModeOfNotice(CommonFunction.checkNull(data.get(13)).toString());
		...
		public ArrayList editLegalDeclineDispatchNotice(Object ob) {
			...
			try {
				...
				bufInsSql.append(" FROM LEGAL_NOTICE_DTL LND WHERE NOTICE_ID='"+StringEscapeUtils.escapeSql(vo.getNoticeId())+"' ");*/
				 ...
				 bufInsSql.append(" SELECT NOTICE_ID,LOAN_NO,(SELECT DISTINCT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LND.LOAN_NO),gcd.Customer_name,lnd.Customer_Id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,NOTICE_CODE,(SELECT DISTINCT NOTICE_TYPE_DESC FROM LEG_NOTICE_TYPE_DEF_M WHERE NOTICE_TYPE_CODE = LND.NOTICE_CODE),");
				       bufInsSql.append(" REASON_CODE,(SELECT DISTINCT REASON_DESC FROM COM_LEGAL_REASON_M WHERE REASON_ID = LND.REASON_CODE),NOTICE_AMOUNT,MODE_OF_NOTICE, ");
				        bufInsSql.append(this.dbo);
				        bufInsSql.append("DATE_FORMAT(DATE_OF_CONCILIATION,'"+this.dateFormat+"'), ");
				        bufInsSql.append(" TIME_OF_CONCILIATION,VENUE_OF_CONCILIATION, ");
		...
		public ArrayList searchLegalCaseInitiationMaker(Object ob) {
			...
			try {
				...
				bufInsSql.append(" ORDER BY LOAN_ID ");*/
				
				
				bufInsSql.append(" select LND.LOAN_NO,cld.LOAN_NO,(SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE ");
				       bufInsSql.append(" CUSTOMER_ID = LND.CUSTOMER_ID) CUST_NAME,LND.Customer_id, (select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
				       bufInsSql.append(" (SELECT DISTINCT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID = CLD.LOAN_BRANCH) BRANCH, ");
				       bufInsSql.append(" (SELECT DISTINCT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = CLD.LOAN_PRODUCT) PRODUCT, ");
				       bufInsSql.append(" (SELECT DISTINCT SCHEME_DESC FROM CR_SCHEME_M WHERE SCHEME_ID = CLD.LOAN_SCHEME) SCHEME  ");
				       bufInsSql.append("  from LEGAL_NOTICE_DTL LND join CR_LOAN_DTL CLD on  CLD.LOAN_ID = LND.LOAN_NO  ");
				       bufInsSql.append(" join cr_loan_customer_role cr on LND.loan_NO=cr.loan_id and LND.customer_id=cr.gcd_id ");
				       bufInsSql.append(" where lnd.REC_STATUS='D' AND NOT EXISTS (SELECT 1 FROM LEGAL_CASE_DTL LCD  ");
				       bufInsSql.append(" WHERE LCD.LOAN_ID = CLD.LOAN_ID and LND.customer_id=LCD.customer_id AND REC_STATUS IN ('I','A')) ");
				       bufInsSqlTempCount.append(" SELECT COUNT(1) FROM LEGAL_NOTICE_DTL LND WHERE REC_STATUS = 'D' AND  NOT EXISTS (SELECT 1 FROM LEGAL_CASE_DTL LCD WHERE LCD.LOAN_ID = LND.LOAN_NO and LND.customer_id=LCD.customer_id AND LCD.REC_STATUS IN ('I','A')) ");
				       ...
				       bufInsSql.append(" ORDER BY LND.LOAN_NO ");
				...
				for (int i = 0; i < searchlist.size(); i++) {
					...
					if (data.size() > 0) {
						...
						caseInitVo.setLoanNo("<a href=legalCaseInitiationMakerDispatch.do?method=openLegalCaseInitiationMaker&loanId="
						 ...
						 caseInitVo.setCustomerId(CommonFunction.checkNull(data.get(2)).toString());
						          caseInitVo.setLbxcustomerId(CommonFunction.checkNull(data.get(3)).toString());
						          caseInitVo.setCustRole(CommonFunction.checkNull(data.get(4)).toString());
						          caseInitVo.setBranch(CommonFunction.checkNull(data.get(5)).toString());
						          caseInitVo.setProduct(CommonFunction.checkNull(data.get(6)).toString());
						          caseInitVo.setScheme(CommonFunction.checkNull(data.get(7)).toString());
		...
		public ArrayList openEditLegalCaseInitiationMaker(Object ob) {
			...
			try {
				...
				/*bufInsSql.append(" SELECT  LOAN_ID,LOAN_NO,(SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = CLD.LOAN_CUSTOMER_ID) CUST_NAME, " );
				bufInsSql.append(" (SELECT DISTINCT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID = CLD.LOAN_BRANCH) BRANCH,(SELECT DISTINCT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = CLD.LOAN_PRODUCT) PRODUCT, ");
				bufInsSql.append(" (SELECT DISTINCT SCHEME_DESC FROM CR_SCHEME_M WHERE SCHEME_ID = CLD.LOAN_SCHEME) SCHEME,LOAN_DPD,(IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL(CLD.LOAN_BALANCE_PRINCIPAL,0)) TOS, ");
				bufInsSql.append(" (SELECT DISTINCT sum(IFNULL(cr_txnadvice_dtl.advice_amount,0))- sum(IFNULL(txn_adjusted_amount,0))- sum(IFNULL(amount_in_process,0)) ");
				...
				bufInsSql.append(" (IFNULL(CLD.LOAN_BALANCE_PRINCIPAL,0)+IFNULL(CLD.LOAN_OVERDUE_PRINCIPAL,0)) POS ");
				bufInsSql.append(" FROM CR_LOAN_DTL CLD WHERE REC_STATUS = 'A' AND LOAN_ID='"+StringEscapeUtils.escapeSql(vo.getLoanId())+"' ");
			*/
				
				bufInsSql.append(" SELECT  CLD.LOAN_ID,CLD.LOAN_NO,(SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LND.CUSTOMER_ID) CUST_NAME,LND.Customer_id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
				       bufInsSql.append(" (SELECT DISTINCT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID = CLD.LOAN_BRANCH) BRANCH,(SELECT DISTINCT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = CLD.LOAN_PRODUCT) PRODUCT, ");
				       bufInsSql.append(" (SELECT DISTINCT SCHEME_DESC FROM CR_SCHEME_M WHERE SCHEME_ID = CLD.LOAN_SCHEME) SCHEME,LOAN_DPD,(IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL(CLD.LOAN_BALANCE_PRINCIPAL,0)) TOS, ");
				       bufInsSql.append(" (SELECT DISTINCT sum(IFNULL(cr_txnadvice_dtl.advice_amount,0))- sum(IFNULL(txn_adjusted_amount,0))- sum(IFNULL(amount_in_process,0)) ");
				       bufInsSql.append(" from cr_txnadvice_dtl where advice_type = 'R' and CHARGE_CODE_ID <> 7 and CHARGE_CODE_ID <> 27 AND LOAN_ID = CLD.LOAN_ID) CHARGES,");
				       bufInsSql.append(" (IFNULL(CLD.LOAN_BALANCE_PRINCIPAL,0)+IFNULL(CLD.LOAN_OVERDUE_PRINCIPAL,0)) POS ");
				      bufInsSql.append(" FROM CR_LOAN_DTL CLD join LEGAL_NOTICE_DTL LND on cld.loan_id=LND.Loan_No  join cr_loan_customer_role cr on LND.loan_NO=cr.loan_id and LND.customer_id=cr.gcd_id  WHERE CLD.REC_STATUS = 'A' AND CLD.LOAN_ID='"+StringEscapeUtils.escapeSql(vo.getLoanId())+"' ");
				...
				for (int i = 0; i < searchlist.size(); i++) {
					...
					if (data.size() > 0) {
						...
						caseInitMakerVo.setLoanId(CommonFunction.checkNull(data.get(0)).toString());
						           caseInitMakerVo.setLoanNo(CommonFunction.checkNull(data.get(1)).toString());
						           caseInitMakerVo.setCustomerId(CommonFunction.checkNull(data.get(2)).toString());
						           caseInitMakerVo.setLbxcustomerId(CommonFunction.checkNull(data.get(3)).toString());
						           caseInitMakerVo.setCustRole(CommonFunction.checkNull(data.get(4)).toString());
						           caseInitMakerVo.setBranch(CommonFunction.checkNull(data.get(5)).toString());
						           caseInitMakerVo.setProduct(CommonFunction.checkNull(data.get(6)).toString());
						           caseInitMakerVo.setScheme(CommonFunction.checkNull(data.get(7)).toString());
						           caseInitMakerVo.setDpd(CommonFunction.checkNull(data.get(8)).toString());
						           ...
						           {
						             caseInitMakerVo.setTotalOutstanding(this.myFormatter.format(this.myFormatter.parse(CommonFunction.checkNull(data.get(9)))));
						           }
						           ...
						           {
						             caseInitMakerVo.setOtherCharges(this.myFormatter.format(this.myFormatter.parse(CommonFunction.checkNull(data.get(10)))));
						           }
						           ...
						           {
						             caseInitMakerVo.setPrincipalOutstanding(this.myFormatter.format(this.myFormatter.parse(CommonFunction.checkNull(data.get(11)))));
						           }
		...
		public boolean insertLegalCaseInitiationMaker(Object ob) {
			...
			String query = "select LOAN_ID from LEGAL_CASE_DTL where REC_STATUS IN ('I','A') AND LOAN_ID='"
					+ StringEscapeUtils.escapeSql(vo.getLoanId().trim()) + "'";
			logger.info("In insertCaseTypeMaster.......inside ejb server file..........Dao Impl"
							+ query);
			boolean st = ConnectionDAOforEJB.checkStatus(query);
			try {
				...
				if (!st) {
					...
					bufInsSql.append(" REC_STATUS) " );
					bufInsSql.append(" SELECT LOAN_ID,lnd.CUSTOMER_ID,LOAN_BRANCH,LOAN_PRODUCT,LOAN_SCHEME,LOAN_DPD, ");
					bufInsSql.append(" (IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL(CLD.LOAN_BALANCE_PRINCIPAL,0)) TOS, " );
				//	bufInsSql.append(" (SELECT SUM(OTHER_CHARGES) FROM CR_REPAYSCH_DTL WHERE LOAN_ID = CLD.LOAN_ID) CHARGES, ");
					...
					bufInsSql.append("'I'");
					bufInsSql.append(" FROM CR_LOAN_DTL CLD join LEGAL_NOTICE_DTL LND on LND.loan_no=CLD.loan_id WHERE CLD.LOAN_ID = '"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getLoanId()))+"' and LND.customer_id="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getLbxcustomerId()))+"' ");
		...
		public boolean checkingLRNStatus(Object ob) {
			...
			String query1 = "SELECT LOAN_ID FROM LEGAL_CASE_DTL WHERE CASE_TYPE = 'LRN' AND LOAN_ID = '"+StringEscapeUtils.escapeSql(vo.getLoanId().trim()) +"' AND WITHDRAWAL_FLAG = 'N' AND REC_STATUS NOT IN ('X','R')";
		...
		public boolean checkingNotice138Status(Object ob) {
			...
			String query1 = "SELECT LOAN_ID FROM LEGAL_CASE_DTL WHERE CASE_TYPE = 'NOTICE138' AND LOAN_ID = '"+StringEscapeUtils.escapeSql(vo.getLoanId().trim()) +"' AND WITHDRAWAL_FLAG = 'N' AND REC_STATUS NOT IN ('X','R')";
...
public ArrayList searchLegalCaseFileMaker(Object ob) {
	...
	try {
		...
		loanId = (StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getLbxLoanId())).trim());
       	...
       	StringBuffer bufInsSql = new StringBuffer();
		StringBuffer bufInsSqlTempCount = new StringBuffer();
		/*bufInsSql.append(" SELECT LEGAL_ID,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO, ");
		bufInsSql.append(" (SELECT CASE_TYPE_DESC FROM LEG_CASE_TYPE_DEF_M WHERE CASE_TYPE_CODE = LCD.CASE_TYPE LIMIT 1 ) CASE_TYPE, ");
		bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
		bufInsSql.append(" (SELECT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID  = LCD.BRANCH_ID LIMIT 1) BRANCH  ");
		...
		if (!loanId.equals("")) {
		 ...
		 bufInsSql.append(" SELECT LEGAL_ID,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO, (SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID) CUST_NAME,LCD.Customer_id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,");
		       bufInsSql.append(" (SELECT CASE_TYPE_DESC FROM LEG_CASE_TYPE_DEF_M WHERE CASE_TYPE_CODE = LCD.CASE_TYPE LIMIT 1 ) CASE_TYPE, ");
		       bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
		        bufInsSql.append(" (SELECT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID  = LCD.BRANCH_ID LIMIT 1) BRANCH  ");
		        bufInsSql.append(" FROM LEGAL_CASE_DTL LCD  join cr_loan_customer_role cr on LCD.loan_ID=cr.loan_id and LCD.customer_id=cr.gcd_id WHERE LCD.REC_STATUS IN ('A','CFM') AND POA = '"+StringEscapeUtils.escapeSql(vo.getMakerId())+"' and BRANCH_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getBranchId())).trim()+"'");
		  
		        bufInsSqlTempCount.append(new StringBuilder().append(" SELECT COUNT(1) FROM LEGAL_CASE_DTL LCD WHERE REC_STATUS IN ('A','CFM') AND POA = '").append(StringEscapeUtils.escapeSql(vo.getMakerId())).append("' ").toString());
		...
		bufInsSql.append(" ORDER BY LEGAL_ID ");
			...
			if (data.size() > 0) {
				...
				caseFileVo.setLegalId("<a href=legalCaseFileMakerDispatch.do?method=openLegalCaseFileMaker&legalId="
				 ...
				 caseFileVo.setLoanNo(CommonFunction.checkNull(data.get(1)).toString());
				            caseFileVo.setCustomerId(CommonFunction.checkNull(data.get(2)).toString());
				            caseFileVo.setLbxcustomerId(CommonFunction.checkNull(data.get(3)).toString());
				            caseFileVo.setCustRole(CommonFunction.checkNull(data.get(4)).toString());
				            caseFileVo.setCaseTypeDesc(CommonFunction.checkNull(data.get(5)).toString());
				            caseFileVo.setProduct(CommonFunction.checkNull(data.get(6)).toString());
				            caseFileVo.setBranch(CommonFunction.checkNull(data.get(7)).toString());
...
public ArrayList openEditLegalCaseFileMaker(Object ob) {
	...
	try {
		...
		StringBuffer bufInsSql = new StringBuffer();
		bufInsSql.append(" SELECT LEGAL_ID,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO, " );
		bufInsSql.append(" (SELECT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID LIMIT 1) CUSTOMER,   ");
		bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
		bufInsSql.append(" (SELECT SCHEME_DESC FROM CR_SCHEME_M WHERE SCHEME_ID = LCD.SCHEME_ID LIMIT 1) SCHEME, ");
		bufInsSql.append(" (SELECT CASE_TYPE_DESC FROM LEG_CASE_TYPE_DEF_M WHERE CASE_TYPE_CODE = LCD.CASE_TYPE LIMIT 1) CASE_TYPE, ");
		bufInsSql.append(" (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID = LCD.ADVOCATE_ID LIMIT 1) ADVOCATE, ");
		...
		bufInsSql.append(" (SELECT STAGE_DESC FROM LEG_STAGE_DEF_M WHERE STAGE_CODE = LCD.STAGE LIMIT 1) STAGE_DESC, ");
		bufInsSql.append(" STAGE AS STAGE_CODE,SECTION, ");
		bufInsSql.append(" (SELECT COURT_TYPE_DESC FROM LEG_COURT_TYPE_DEF_M WHERE COURT_TYPE_CODE = LCD.COURT_TYPE LIMIT 1) AS COURT_TYPE_DESC, ");
		bufInsSql.append(" COURT_TYPE,(SELECT COURT_NAME_DESC FROM LEG_COURT_NAME_DEF_M WHERE COURT_NAME_CODE = LCD.COURT_NAME LIMIT 1) COURT_NAME_DESC, ");
		bufInsSql.append(" COURT_NAME,JUDGE_NAME, ");
		bufInsSql.append(" (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID = LCD.POA LIMIT 1) POA_DESC, ");
...
public ArrayList searchLegalCaseFileAuthor(Object ob) {
	...
	try {
		...
		logger.info("In searchLegalCaseFileAuthor()..............inside ejb server file.......................Dao Impl");
		...
		if (!loanId.equals("")) {
		 ...
		 bufInsSql.append(" SELECT LEGAL_ID,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO,(SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID) CUST_NAME,LCD.Customer_id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
	       ...
	       bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
	       bufInsSql.append(" (SELECT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID  = LCD.BRANCH_ID LIMIT 1) BRANCH  ");
	       bufInsSql.append(" FROM LEGAL_CASE_DTL LCD  join cr_loan_customer_role cr on LCD.loan_ID=cr.loan_id and LCD.customer_id=cr.gcd_id WHERE LCD.REC_STATUS = 'CFF' and CASE_FILE_MAKER_ID <> '"+StringEscapeUtils.escapeSql(vo.getMakerId())+"' ");
		...
		logger.info("IN searchLegalCaseFileAuthor() search query1 ### "+ bufInsSql.toString());
		logger.info("In searchLegalCaseFileAuthor.....................................Dao Impl");
		logger.info("searchLegalCaseFileAuthor " + searchlist.size());
...
public ArrayList searchLegalCourtProcessingMaker(Object ob) {
	...
	try {
		...
		loanId = (StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getLbxLoanId())).trim());
       	...
       	StringBuffer bufInsSql = new StringBuffer();
		StringBuffer bufInsSqlTempCount = new StringBuffer();
		/*bufInsSql.append(" SELECT LEGAL_ID,CASE_TYPE,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO, ");
		bufInsSql.append(" (SELECT CASE_TYPE_DESC FROM LEG_CASE_TYPE_DEF_M WHERE CASE_TYPE_CODE = LCD.CASE_TYPE LIMIT 1 ) CASE_TYPE, ");
		bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
		bufInsSql.append(" (SELECT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID  = LCD.BRANCH_ID LIMIT 1) BRANCH  ");
		...
		if (!loanId.equals("")) {
		 ...
		 bufInsSql.append(" SELECT LEGAL_ID,CASE_TYPE,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO,(SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID) CUST_NAME,LCD.Customer_id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
	       bufInsSql.append(" (SELECT CASE_TYPE_DESC FROM LEG_CASE_TYPE_DEF_M WHERE CASE_TYPE_CODE = LCD.CASE_TYPE LIMIT 1 ) CASE_TYPE, ");
	       bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
	       bufInsSql.append(" (SELECT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID  = LCD.BRANCH_ID LIMIT 1) BRANCH  ");
	       bufInsSql.append(new StringBuilder().append(" FROM LEGAL_CASE_DTL LCD  join cr_loan_customer_role cr on LCD.loan_ID=cr.loan_id and LCD.customer_id=cr.gcd_id WHERE LCD.REC_STATUS IN ('CFA','CPM','CPA') AND POA ='").append(StringEscapeUtils.escapeSql(vo.getMakerId())).append("' ").toString());
	 
	       bufInsSqlTempCount.append(new StringBuilder().append(" SELECT COUNT(1) FROM LEGAL_CASE_DTL LCD WHERE LCD.REC_STATUS IN ('CFA','CPM','CPA') AND POA ='").append(StringEscapeUtils.escapeSql(vo.getMakerId())).append("' ").toString());
	 
	       if (!loanId.equals("")) {
	         bufInsSql.append(new StringBuilder().append(" AND LCD.LOAN_ID = '").append(loanId).append("' ").toString());
	         bufInsSqlTempCount.append(new StringBuilder().append(" AND LCD.LOAN_ID = '").append(loanId).append("' ").toString());
	       }
		...
		for (int i = 0; i < searchlist.size(); i++) {
			...
			if (data.size() > 0) {
				...
				caseFileVo.setLegalId("<a href=legalCourtProcessingMakerDispatch.do?method=openLegalCourtProcessingMaker&legalId="
								...
								+ ">"
								+ CommonFunction.checkNull(data.get(0)).toString() + "</a>");
				caseFileVo.setCustomerId(CommonFunction.checkNull(data.get(3)).toString());
		           caseFileVo.setLbxcustomerId(CommonFunction.checkNull(data.get(4)).toString());
		           caseFileVo.setCustRole(CommonFunction.checkNull(data.get(5)).toString());
		           caseFileVo.setCaseTypeDesc(CommonFunction.checkNull(data.get(6)).toString());
		           caseFileVo.setProduct(CommonFunction.checkNull(data.get(7)).toString());
		           caseFileVo.setBranch(CommonFunction.checkNull(data.get(8)).toString());
		           caseFileVo.setTotalRecordSize(count);
				detailList.add(caseFileVo);
			}
	...
	try {
		...
		if (!loanId.equals("")) {
		 ...
		 bufInsSql.append(" SELECT LEGAL_ID,CASE_TYPE,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO,(SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID) CUST_NAME,LCD.Customer_id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
	       ...
	       bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
	       bufInsSql.append(" (SELECT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID  = LCD.BRANCH_ID LIMIT 1) BRANCH  ");
	       bufInsSql.append(" FROM LEGAL_CASE_DTL LCD  join cr_loan_customer_role cr on LCD.loan_ID=cr.loan_id and LCD.customer_id=cr.gcd_id WHERE LCD.REC_STATUS IN ('CPF') and CASE_FILE_MAKER_ID <> '"+StringEscapeUtils.escapeSql(vo.getMakerId())+"' ");
...
public boolean saveCourtProcessingAuthorData(Object ob) {
	...
	try {
		...
		bufInsSql.append(dbo);
		bufInsSql.append("DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) ");
		bufInsSql.append(" WHERE LEGAL_ID=? AND IFNULL(APPROVAL_FLAG,'N') <> 'Y'");
		...
		bufInsSql.append(" UPDATE LEGAL_CASE_DTL A " );
		bufInsSql.append(" JOIN (SELECT LEGAL_ID,COURT_PROCESSING_MAKER_DATE, CASE_STAGE_ID,POA,HEARING_DATE,STAGE,CASE_NO FROM LEGAL_CASE_STAGE_DTL ");
		bufInsSql.append(" WHERE LEGAL_ID = ? ORDER BY COURT_PROCESSING_MAKER_DATE,CASE_STAGE_ID DESC LIMIT 1) B ");
		bufInsSql.append(" ON B.LEGAL_ID = A.LEGAL_ID ");
		bufInsSql.append(" SET A.POA = B.POA, A.DATE_OF_HEARING = B.HEARING_DATE, A.STAGE = B.STAGE, A.CASE_NO = B.CASE_NO, ");
...
public ArrayList searchLegalReopenClosedCase(Object ob) {
	...
	try {
		...
		bufInsSql.append(" SELECT LEGAL_ID,CASE_TYPE,(SELECT LOAN_NO FROM CR_LOAN_DTL WHERE LOAN_ID = LCD.LOAN_ID LIMIT 1) LOAN_NO,(SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID) CUST_NAME,LCD.Customer_id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role, ");
	       ...
	       bufInsSql.append(" (SELECT PRODUCT_DESC FROM CR_PRODUCT_M WHERE PRODUCT_ID = LCD.PRODUCT_ID LIMIT 1) PRODUCT, ");
	       bufInsSql.append(" (SELECT BRANCH_DESC FROM COM_BRANCH_M WHERE BRANCH_ID  = LCD.BRANCH_ID LIMIT 1) BRANCH  ");
	       bufInsSql.append(" FROM LEGAL_CASE_DTL LCD  join cr_loan_customer_role cr on LCD.loan_ID=cr.loan_id and LCD.customer_id=cr.gcd_id  WHERE WITHDRAWAL_FLAG = 'Y' ");
...
public ArrayList searchAssignRejectCaseData(Object ob) {
	...
	try {
		...
		bufInsSql.append(" SELECT LCD.LEGAL_ID,CLT.LOAN_NO,GCM.CUSTOMER_NAME,LCD.CUSTOMER_ID,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,CBM.BRANCH_DESC,CPM.PRODUCT_DESC, CSM.SCHEME_DESC,DPD, TOTAL_OUTSTANDING,CHARGES,PRINCIPAL_OUTSTANDING, CASE_TYPE, INITIATION_MAKER_IMEMO, CRM.REASON_DESC, INITIATION_MAKER_REMARKS FROM LEGAL_CASE_DTL LCD ");
	       bufInsSql.append(" INNER JOIN CR_LOAN_DTL CLT ON CLT.LOAN_ID=LCD.LOAN_ID ");
	       bufInsSql.append(" INNER JOIN GCD_CUSTOMER_M GCM ON GCM.CUSTOMER_ID=LCD.CUSTOMER_ID ");
	       bufInsSql.append(" Inner Join cr_loan_customer_role cr on cr.gcd_id=LCD.CUSTOMER_ID and cr.loan_id=lcd.Loan_Id ");
	       bufInsSql.append(" INNER JOIN COM_BRANCH_M CBM ON CBM.BRANCH_ID=LCD.BRANCH_ID ");
	       bufInsSql.append(" INNER JOIN CR_SCHEME_M CSM ON CSM.SCHEME_ID=LCD.SCHEME_ID  ");
	       bufInsSql.append("INNER JOIN CR_PRODUCT_M CPM ON CPM.PRODUCT_ID=LCD.PRODUCT_ID  ");
	       bufInsSql.append("INNER JOIN COM_LEGAL_REASON_M CRM ON CRM.REASON_ID=LCD.INITIATION_MAKER_REASON  ");
		...
		for (int i = 0; i < searchlist.size(); i++) {
			...
			if (data.size() > 0) {
				...
				caseInitVo.setCustomerId(CommonFunction.checkNull(data.get(2)).toString());
		           ...
		           caseInitVo.setDpd(CommonFunction.checkNull(data.get(8)).toString());
		           caseInitVo.setTotalOutsatanding(CommonFunction.checkNull(data.get(9)).toString());
		           caseInitVo.setOtherCharges(CommonFunction.checkNull(data.get(10)).toString());
		           caseInitVo.setPos(CommonFunction.checkNull(data.get(11)).toString());
		           caseInitVo.setCaseType(CommonFunction.checkNull(data.get(12)).toString());
		           caseInitVo.setiMemoBy(CommonFunction.checkNull(data.get(13)).toString());
		           caseInitVo.setReason(CommonFunction.checkNull(data.get(14)).toString());
		           caseInitVo.setRemark(CommonFunction.checkNull(data.get(15)).toString());
		           caseInitVo.setTotalRecordSize(count);
				detailList.add(caseInitVo);
			}
...
public boolean insertAssignRejectCase(Object ob) {
	...
	try {
		...
		StringBuffer bufInsSql = new StringBuffer();
		logger.info("In updateCountryData..........inside ejb server file............Dao Impl");
		bufInsSql.append(" UPDATE LEGAL_CASE_DTL SET ASSIGN_REJECT_IMEMO=?,ASSIGN_REJECT_REASON=?,ASSIGN_REJECT_REMARKS=?,ADVOCATE_ID=?,POA=?,REC_STATUS=?,");
		...
		if (CommonFunction.checkNull(vo.getLbxPoa()).equalsIgnoreCase(""))
	         insertPrepStmtObject.addNull();
	       else {
	         insertPrepStmtObject.addString(vo.getLbxPoa().toUpperCase().trim());
	       }
...
public ArrayList openEditAssignRejectCaseData(Object ob) {
	...
	try {
		...
		bufInsSql.append("INNER JOIN COM_REASON_M CRM ON CRM.REASON_ID=LCD.INITIATION_MAKER_REASON  ");*/
		 ...
		 bufInsSql.append(" SELECT LCD.LEGAL_ID,CLT.LOAN_NO,GCM.CUSTOMER_NAME,LCD.CUSTOMER_ID,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,CBM.BRANCH_DESC,CPM.PRODUCT_DESC, CSM.SCHEME_DESC,DPD, TOTAL_OUTSTANDING,CHARGES,PRINCIPAL_OUTSTANDING, cas.CASE_TYPE_DESC, INITIATION_MAKER_IMEMO, CRM.REASON_DESC,INITIATION_MAKER_REMARKS,CASE_TYPE FROM LEGAL_CASE_DTL LCD ");
	       bufInsSql.append(" INNER JOIN CR_LOAN_DTL CLT ON CLT.LOAN_ID=LCD.LOAN_ID ");
	       bufInsSql.append(" INNER JOIN GCD_CUSTOMER_M GCM ON GCM.CUSTOMER_ID=LCD.CUSTOMER_ID ");
	       bufInsSql.append(" Inner Join cr_loan_customer_role cr on cr.gcd_id=LCD.CUSTOMER_ID and cr.loan_id=lcd.Loan_Id ");
	       bufInsSql.append(" INNER JOIN COM_BRANCH_M CBM ON CBM.BRANCH_ID=LCD.BRANCH_ID ");
	       bufInsSql.append(" INNER JOIN CR_SCHEME_M CSM ON CSM.SCHEME_ID=LCD.SCHEME_ID  ");
	       bufInsSql.append("INNER JOIN CR_PRODUCT_M CPM ON CPM.PRODUCT_ID=LCD.PRODUCT_ID  ");
	       bufInsSql.append("INNER JOIN COM_LEGAL_REASON_M CRM ON CRM.REASON_ID=LCD.INITIATION_MAKER_REASON  inner join leg_case_type_def_m cas on lcd.CASE_TYPE=cas.CASE_TYPE_CODE ");
			...
			if (data.size() > 0) {
				...
				caseInitMakerVo.setLoanNo(CommonFunction.checkNull(data.get(1)).toString());
				 caseInitMakerVo.setCustomerId(CommonFunction.checkNull(data.get(2)).toString());
		           ...
		           caseInitMakerVo.setDpd(CommonFunction.checkNull(data.get(8)).toString());
		           caseInitMakerVo.setTotalOutsatanding(CommonFunction.checkNull(data.get(9)).toString());
		           caseInitMakerVo.setOtherCharges(CommonFunction.checkNull(data.get(10)).toString());
		           caseInitMakerVo.setPos(CommonFunction.checkNull(data.get(11)).toString());
		           caseInitMakerVo.setCaseType(CommonFunction.checkNull(data.get(12)).toString());
		           caseInitMakerVo.setiMemoBy(CommonFunction.checkNull(data.get(13)).toString());
		           caseInitMakerVo.setReason(CommonFunction.checkNull(data.get(14)).toString());
		           caseInitMakerVo.setRemark(CommonFunction.checkNull(data.get(15)).toString());
		           caseInitMakerVo.setLbxCaseTypeCode(CommonFunction.checkNull(data.get(16)).toString());
...
public ArrayList searchReassignCaseData(Object ob) {
	{
		...
		try {
			...
			/*bufInsSql.append(" (SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID) CUST_NAME,"); */
			 bufInsSql.append(" (SELECT DISTINCT CUSTOMER_NAME FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID = LCD.CUSTOMER_ID) CUST_NAME,LCD.Customer_id,(select description from generic_master where GENERIC_KEY='cust_role' and VALUE=cr.LOAN_CUSTOMER_ROLE_TYPE)as cust_role,");
			...
			bufInsSql.append(" from LEGAL_CASE_DTL LCD  join cr_loan_customer_role cr on LCD.loan_ID=cr.loan_id and LCD.customer_id=cr.gcd_id ");
			...
			if (!loanId.equals("")) {
			 ...
			 bufInsSql.append(" ORDER BY LCD.LOAN_ID ");
			...
			for (int i = 0; i < searchlist.size(); i++) {
				...
				if (data.size() > 0) {
					...
					caseInitVo.setCustomerName(CommonFunction.checkNull(data.get(2)).toString());
			           caseInitVo.setLbxcustomerId(CommonFunction.checkNull(data.get(3)).toString());
			           caseInitVo.setCustRole(CommonFunction.checkNull(data.get(4)).toString());
			           caseInitVo.setCaseType(CommonFunction.checkNull(data.get(5)).toString());
			           caseInitVo.setCaseFileDate(CommonFunction.checkNull(data.get(6)).toString());
			           caseInitVo.setDateOfhearing(CommonFunction.checkNull(data.get(7)).toString());
			           caseInitVo.setCaseNo(CommonFunction.checkNull(data.get(8)).toString());
			           caseInitVo.setAdvocateName(CommonFunction.checkNull(data.get(9)).toString());
			           caseInitVo.setStage(CommonFunction.checkNull(data.get(10)).toString());
			           caseInitVo.setSection(CommonFunction.checkNull(data.get(11)).toString());
			           caseInitVo.setCourtName(CommonFunction.checkNull(data.get(12)).toString());
			           caseInitVo.setPoa(CommonFunction.checkNull(data.get(13)).toString());
...
//				insertPrepStmtObject.addString(noticeLegalDeclineID);
    ...
    public ArrayList editReassignCase(Object ob) {
    	...
    	try {
    		...
    		bufInsSql.append(" INNER JOIN CR_SCHEME_M CSM ON CSM.SCHEME_ID=LCD.SCHEME_ID  ");
    		bufInsSql.append("INNER JOIN CR_PRODUCT_M CPM ON CPM.PRODUCT_ID=LCD.PRODUCT_ID  ");
    		bufInsSql.append("INNER JOIN COM_LEGAL_REASON CRM ON CRM.REASON_ID=LCD.INITIATION_MAKER_REASON  ");
    ...
    {
      String qry1 = "select max(notice_id) from  LEGAL_NOTICE_DTL";
      logger.info(new StringBuilder().append("query to get notice id :: ").append(qry1).toString());
      String noticeId = ConnectionDAO.singleReturn(qry1);
      return noticeId;
    }
    ...
    public String insertReasonMaster(Object ob)
    {
      ...
      try
      {
        ...
        StringBuffer bufInsSql = new StringBuffer();
        bufInsSql.append("insert into COM_LEGAL_REASON_M(REASON_TYPE,REASON_DESC,CHARGE_FLAG,REC_STATUS,REASON_DESC_L,MAKER_ID,MAKER_DATE,AUTHOR_ID,AUTHOR_DATE,REASON_SHORT_CODE,NOTICE_TYPE_CODE,CASE_TYPE_CODE)");
        bufInsSql.append(" values ( ");
        bufInsSql.append(" ?,");
        bufInsSql.append(" ?,");
        bufInsSql.append(" ?,");
        ...
        if (CommonFunction.checkNull(vo.getLbxNoticeCode()).equalsIgnoreCase(""))
        {
          insertPrepStmtObject.addNull();
        }
        else insertPrepStmtObject.addString(vo.getLbxNoticeCode().toUpperCase().trim());
  
        if (CommonFunction.checkNull(vo.getLbxCaseTypeCode()).equalsIgnoreCase(""))
        {
          insertPrepStmtObject.addNull();
        }
        else insertPrepStmtObject.addString(vo.getLbxCaseTypeCode().toUpperCase().trim());
    ...
    public ArrayList searchReasonData(Object ob)
    {
      ...
      try {
        ...
        bufInsSql.append("if(b.REC_STATUS='A','Active','Inactive')as REC_STATUS ,REASON_TYPE,REASON_SHORT_CODE,n.notice_type_desc,c.case_type_desc,b.NOTICE_TYPE_CODE,b.CASE_TYPE_CODE ");
        bufInsSql.append(" from COM_LEGAL_REASON_M B left join LEG_NOTICE_TYPE_DEF_M n on b.NOTICE_TYPE_CODE=n.notice_type_code left join LEG_CASE_TYPE_DEF_M c on b.CASE_TYPE_CODE=c.case_type_code ");
  
        bufInsSqlTempCount.append("SELECT COUNT(1) FROM COM_LEGAL_REASON_M B ");
        ...
        for (int i = 0; i < searchlist.size(); i++) {
          ...
          if (data.size() > 0) {
            ...
            deptMVO.setReasonIdModify(new StringBuilder().append("<a href=legalReasonMasterSearch.do?method=openEditReason&ReasonId=").append(CommonFunction.checkNull(data.get(0)).toString()).append(">").append(CommonFunction.checkNull(data.get(6)).toString()).append("</a>").toString());
            ...
            deptMVO.setNotice(CommonFunction.checkNull(data.get(7)).toString());
            deptMVO.setCaseType(CommonFunction.checkNull(data.get(8)).toString());
            deptMVO.setLbxNoticeCode(CommonFunction.checkNull(data.get(9)).toString());
            deptMVO.setLbxCaseTypeCode(CommonFunction.checkNull(data.get(10)).toString());
            deptMVO.setTotalRecordSize(count);
            detailList.add(deptMVO);
          }
        }
    ...
    public boolean updateReasonData(Object ob) {
      ...
      try {
        ...
        StringBuffer bufInsSql = new StringBuffer();
        bufInsSql.append("UPDATE COM_LEGAL_REASON_M set REASON_TYPE=?,REASON_DESC=?,CHARGE_FLAG=?,REASON_SHORT_CODE=?,");
        bufInsSql.append(new StringBuilder().append(" REC_STATUS=?,NOTICE_TYPE_CODE=?,CASE_TYPE_CODE=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '").append(this.dateFormatWithTime).append("'),INTERVAL CURTIME() HOUR_SECOND) ").toString());
        ...
        if (CommonFunction.checkNull(vo.getLbxCaseTypeCode()).equalsIgnoreCase(""))
          ...
          insertPrepStmtObject.addString(vo.getLbxCaseTypeCode());
        }
        if (CommonFunction.checkNull(vo.getMakerId()).equalsIgnoreCase(""))
          insertPrepStmtObject.addNull();
        else
