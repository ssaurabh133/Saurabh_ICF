...
public class LegalMasterDaoImpl implements LegalMasterDAO{

	private static final Logger logger = Logger.getLogger(LegalMasterDaoImpl.class.getName());
	...
	public ArrayList searchCaseTypeMasterData(Object ob) {
		...
		try {
			...
			bufInsSqlTempCount.append(" WHERE 5=5 ");
			 ...
			 bufInsSql.append(" ORDER BY CASE_TYPE_CODE ");
...
public ArrayList searchLawFirmMasterData(Object ob) {
		...
		try {
			...
			if (!lawFirmCode.equals("")) {
			 ...
			 bufInsSql.append(" ORDER BY LAWFIRM_CODE ");
...
public ArrayList searchAdvocateMasterData(Object ob) {
		...
		try {
			...
			bufInsSql.append(" ORDER BY ADVOCATE_CODE ");
...
//			}
	...
	public boolean insertAdvocateMaster(Object ob,String [] branchName,String [] caseTypeCode) {
		...
		try {
			...
			if (!st) {
				...
				StringBuilder bufInsSql = new StringBuilder();
				bufInsSql.append("insert into LEG_ADVOCATE_DEF_M(ADVOCATE_CODE,ADVOCATE_DESC,ADVOCATE_TYPE_FLAG,LAW_FIRM_CODE,ADDRESS1,ADDRESS2,COUNTRY,STATE,CITY,ZIPCODE,PHONE1,PHONE2,MOBILE,EMAIL,PAN,BANK_ACCNO,DATE_OF_APPOINTMENT,MEMBERSHIP_LICNO,TENURE_OF_RETAINERSHIP,REGULAR_RETAINER_FLAG,RETAINERSHIP_VALID_UP_TO,REC_STATUS,Classification,MAKER_ID,MAKER_DATE,AUTHOR_ID,AUTHOR_DATE)");
				bufInsSql.append(" values ( ");
				...
				if (CommonFunction.checkNull(stat).equalsIgnoreCase(""))
				 ...
				 if (CommonFunction.checkNull(vo.getClassification()).equalsIgnoreCase(""))
			           insertPrepStmtObject.addNull();
			         else {
			           insertPrepStmtObject.addString(vo.getClassification());
			         }
	...
	public ArrayList editAdvocateData(Object ob) {
		...
		try {
			...
			bufInsSql.append(" REC_STATUS,ADVOCATE_CODE,Classification from LEG_ADVOCATE_DEF_M LADM ");
			...
			for (int i = 0; i < searchlist.size(); i++) {
				...
				if (data.size() > 0) {
					...
					advocateVo.setClassification(CommonFunction.checkNull(data.get(26)).toString());
	...
	public boolean updateAdvocateData(Object ob,String [] branchName,String [] caseTypeCode) {
		...
		try {
			...
			bufInsSql.append(" ,MOBILE = ? ,EMAIL = ?,PAN = ?,BANK_ACCNO = ?, MEMBERSHIP_LICNO = ?,TENURE_OF_RETAINERSHIP = ?,REGULAR_RETAINER_FLAG = ?, REC_STATUS =? ,classification=?" );
	...
	public ArrayList searchCourtTypeMasterData(Object ob) {
		...
		try {
			...
			bufInsSqlTempCount.append(" WHERE 5=5 ");
			 ...
			 bufInsSql.append(" ORDER BY COURT_TYPE_CODE ");
		...
		public ArrayList searchCourtNameMasterData(Object ob) {
			...
			try {
				...
				bufInsSqlTempCount.append(" WHERE 5=5 ");
				 ...
				 bufInsSql.append(" ORDER BY COURT_NAME_CODE ");
		...
		public ArrayList editCourtNameData(Object ob) {
			...
			public ArrayList searchPOAMasterData(Object ob) {
				...
				try {
				   ...
				   if (!courtNameCode.equals("")) {
						bufInsSql.append(" AND COURT_NAME_CODE like '%" + courtNameCode + "%' ");
						bufInsSqlTempCount.append(" AND COURT_NAME_CODE like '%" + courtNameCode + "%' ");
					}
				   
				   bufInsSql.append(" ORDER BY POA_CODE ");
			...
			status = ConnectionDAOforEJB.sqlInsUpdDeletePrepStmt(updatelist);
				...
				public ArrayList searchNoticeTypeMasterData(Object ob) {
					...
					try {
						...
						bufInsSql.append(" ORDER BY NOTICE_TYPE_CODE ");
...
//						}
				...
				public ArrayList searchStageTypeMasterData(Object ob) {
					...
					try {
						...
						if (!caseType.equals("")) {
							 ...
							 bufInsSql.append(" ORDER BY a.STAGE_CODE ");
				...
				public boolean insertStageTypeMaster(Object ob,String [] productIdList,String [] paymentStageFlagList,String [] repetitiveFlagList,String productIds) {
					...
					try {
						...
						if (!st) {
							...
							StringBuilder bufInsSql = new StringBuilder();
							bufInsSql.append("insert into LEG_STAGE_DEF_M (STAGE_CODE,STAGE_DESC,SEQUENCE_NO,CASE_TYPE_CODE,REC_STATUS,CLOSURE_STAGE_FLAG,STAGE_TAT,MAKER_ID,MAKER_DATE)");
							bufInsSql.append(" values ( ");
							...
							if (CommonFunction.checkNull(vo.getTat()).equalsIgnoreCase(""))
						           ...
						           insertPrepStmtObject.addString(vo.getTat().toUpperCase().trim());
						         }
				...
				public ArrayList editStageTypeData(Object ob) {
					...
					try {
						...
						bufInsSql.append(" a.REC_STATUS,a.CLOSURE_STAGE_FLAG,b.CASE_TYPE_DESC,a.STAGE_TAT  from LEG_STAGE_DEF_M a ");
						...
						for (int i = 0; i < size; i++) {
							...
							if (data.size() > 0) {
								...
								stageTypeMasterVo.setTat(CommonFunction.checkNull(data.get(7)).toString());
				...
				public boolean updateStageTypeData(Object ob,String [] productIdList,String [] paymentStageFlagList,String [] repetitiveFlagList,String productIds) {
					...
					try {
						...
						bufInsSql.append(" REC_STATUS=?,CLOSURE_STAGE_FLAG=?,STAGE_TAT=?,MAKER_ID=?,MAKER_DATE=");
				...
				public boolean checkExistingPANAdvocate(Object ob,String insertUpdateFlag){
					...
					if(CommonFunction.checkNull(insertUpdateFlag).equalsIgnoreCase("Y"))
						...
						query = "select ADVOCATE_CODE from LEG_ADVOCATE_DEF_M where ADVOCATE_desc<>'"+ (StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getAdvocateCode())).trim()) + "' and PAN='"+ (StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getPanNo())).trim()) + "'";
