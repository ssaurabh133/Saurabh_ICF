...
public class CaseTypeMasterBehindAction extends Action  {
		private static final Logger logger = Logger.getLogger(CaseTypeMasterBehindAction.class.getName());
		
		public ActionForward execute(ActionMapping mapping, ActionForm form,
				HttpServletRequest request, HttpServletResponse response) throws Exception {
			HttpSession session=request.getSession(false);
			ServletContext context = getServlet().getServletContext();
			logger.info(" ## In CaseTypeMasterBehindAction() : .........");
			...
			CaseTypeMasterVo vo = new CaseTypeMasterVo(); //change
	  ...
	  LegalMasterBusinessSessionBeanRemote bp = (LegalMasterBusinessSessionBeanRemote) LookUpInstanceFactory.getLookUpInstance(LegalMasterBusinessSessionBeanRemote.REMOTE_IDENTITY, request);
			...
			//change from here
			list= bp.searchCaseTypeMasterData(vo);
