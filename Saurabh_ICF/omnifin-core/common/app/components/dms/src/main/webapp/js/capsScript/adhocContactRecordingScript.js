...
function searchAdhocContactRecording(message){ 
...
//    alert("ok");
	...
	document.getElementById("contactRecordingSearch").action = contextPath+"/AdhocContactRecordingBehindAction.do";
	document.getElementById("contactRecordingSearch").submit();
		 return true;
	}
}
}
function openAdhocContactRecordingTrail(loanNo,exclationFlag)
{
	var contextPath=document.getElementById("contextPath").value;
	var url=contextPath+"/AdhocContactRecordingDispatchAction.do?method=openAdhocContactRecordingTrail&loanId="+loanNo+"&exclationFlag="+exclationFlag;
	popupWin=window.open(url,'AdhocContactRecording','height=1200,width=1000,top=400,left=400, scrollbars=yes ').focus();
	if (window.focus) 
	   popupWin.focus();
	
	/*  var contextPath=document.getElementById("contextPath").value;
	document.getElementById("contactRecordingSearch").action = contextPath+"/AdhocContactRecordingDispatchAction.do?method=openAdhocContactRecordingTrail&loanId="+loanNo+"&exclationFlag="+exclationFlag;
	document.getElementById("contactRecordingSearch").submit();	*/
}
...
function openPopUpFollowUp(){
	...
	var exclationFlag=document.getElementById("exclationFlag").value;
		 var url="AdhocContactRecordingDispatchAction.do?method=openFollowUpTrail&loanId="+loanId+"&exclationFlag="+exclationFlag;
...
function saveFollowUpTrail(alert1)
{
	  ...
	  if(actionCode=='COLL'){
		if(actionDate=="")
			{
			msg14="* Action Date is required \n";
			  check=1;
			}
			if(actionTime=="")
			{
			msg16="* Action Time is required \n";
			  check=1;
			}
		if(relationshipS=="")
		{
		msg15="* RelationShip is required \n";
		  check=1;
		}
		if(paymentMode==""){
		...
		if(nxtActionDate==""){
			 ...
			 msg13="* Next Action Date is required \n";
			  check=1;		  
		 }
	  }
	  ...
	  if(check==1){
		  ...
		  alert(msg14+msg16+msg15+msg7+msg8+msg1+msg2+msg3+msg4+msg5+msg6+msg12+msg9+msg10+msg11+msg13);
		  DisButClass.prototype.EnbButMethod();
		  return false;
		 
	  }else{
	document.getElementById("contactRecFollowUpForm").action = contextPath+"/AdhocContactRecordingDispatchAction.do?method=saveFollowUpTrail";
...
function displayAddress(){ 
	//alert("In displayAddress");
	 ...
	 var loanId=document.getElementById("loanId").value;
	 var url=contextPath+"/AdhocContactRecordingDispatchAction.do?method=showAddressDetails&loanId="+loanId;
...
/*Supervisor Review Code starts here*/
 ...
 function openForeClosurePopUp()
 {
 	...
 	{
 		document.getElementById("searchForCMForm").action=sourcepath+"/searchCMBehindAction.do?method=openEarlyClosureDetail&loanId="+loanid+"&reportFormate=P";
...
//	alert("Loan no :---"+loanNo);
	//alert("exclationFlag no :---"+exclationFlag);
	  var contextPath=document.getElementById("contextPath").value;
	document.getElementById("supervisorReviewSearch").action = contextPath+"/AdhocContactRecordingDispatchAction.do?method=openAdhocContactRecordingTrail&loanId="+loanNo+"&exclationFlag="+exclationFlag+"&superReview=superReview";
	document.getElementById("supervisorReviewSearch").submit();	
}
...
//	alert("Loan no :---"+loanNo);
	//alert("exclationFlag no :---"+exclationFlag);
	  var contextPath=document.getElementById("contextPath").value;
	document.getElementById("supervisorReviewSearch").action = contextPath+"/AdhocContactRecordingDispatchAction.do.do?method=openAdhocContactRecordingTrail&loanId="+loanNo+"&caseHistory=caseHistory";
	document.getElementById("supervisorReviewSearch").submit();	
}
/*Use This method as used fro refrance of case History   */
