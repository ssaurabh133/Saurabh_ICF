...
public class CollectionDumpDAOImpl implements CollectionDumpDAO{
	private static final Logger logger = Logger.getLogger(CollectionDumpDAOImpl.class.getName());
...
try{
		...
		if(s1.toString().equalsIgnoreCase("S"))
		{
			logger.info("After proc call..commit message."+s2);
               ...
               bufInsSql.append("select LOAN_NO,APPLICATION_NO,BRANCH,PRODUCT,SCHEME,LOAN_AMOUNT,TENURE,LOAN_STATUS,LOAN_MATURITY_DATE,MAKER_ID,");
               bufInsSql.append(" MAKER_DATE,ASSET_DETAILS,CUSTOMER_NAME,CUSTOMER_CATEGORY,FATHER_NAME,RESIDENCE_ADDRESS,PINCODE,TELE1,TELE2,MOBILE, ");
               bufInsSql.append(" GAURANTOR_NAME,GAURANTOR_FATHER_NAME,GAURANTOR_ADDRES,GAURANTOR_PINCODE,GAURANTOR_TEL1,GAURANTOR_TEL2,GAURANTOR_MOBILE, ");
               bufInsSql.append(" DISURSAL_AMOUNT,DISBURSAL_STATUS,DISBURSAL_DATE,DUE_DAY,EMI_AMOUNT,OVERDUE_EMI_AMT,OD_POS, ");
               bufInsSql.append(" CURRENT_QUEUE,CURRENT_DPD,CURRENT_QUEUE_STATE,COLL_AMOUNT,LAST_COLL_DATE,MODE_OF_COLL,BKT_MGR,TEAM_LEAD,FOS, ");
               bufInsSql.append(" DEALING_EXECUTIVE,VENDOR_DEALER_NAME,CURRENT_EMI_NO,LPP,CBC,NO_OF_CHQ_BOUNCE,COLLETRAL_MONEY,POS");
               bufInsSql.append(" from coll_temp_dump_rpt_tbl	 where CURRENT_DPD>0 AND maker_id='"+makerID+"' order by BRANCH");
             ...
             /* bufInsSql.append("SELECT REGION,BRANCH,PRODUCT,SCHEME,ASSET_DETAILS,LOAN_NO,APPLICATION_NO,DEAL_CATEGORY," +
	  		...
	  		"90_MIN,OD_POS,COLL_AMOUNT,LAST_COLL_DATE,MODE_OF_COLL,BKT_MGR,TEAM_LEAD,FOS,RESIDENCE_ADDRESS," +
