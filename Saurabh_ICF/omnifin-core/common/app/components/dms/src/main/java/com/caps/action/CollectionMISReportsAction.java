...
import com.caps.actionForm.CollectionMISReportsForm;


public class CollectionMISReportsAction extends Action {
	
	private static final Logger logger = Logger.getLogger(CollectionMISReportsAction.class.getName());
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
	    ...
	    logger.info("In CollectionMISReportsAction::::::::::::::::::::::::::::::");
		...
		if(userobj!=null)
		{
			...
			logger.info("here execute method of  CollectionMISReportsAction action the session is out----------------");
			return mapping.findForward("sessionOut");
		}

		CollectionMISReportsForm collMisForm=(CollectionMISReportsForm) form;	
		String reportformat=collMisForm.getReportformat();
		String loanNo=collMisForm.getLbxLoanno();
		String product=collMisForm.getProduct();
		String customerName=collMisForm.getCustomername();
		String dpd2=collMisForm.getDpd2();
		String dpd1=collMisForm.getDpd1();
		String queue=collMisForm.getLbxQueuesearchId();
		String pos2=collMisForm.getPos2();
		String pos1=collMisForm.getPos1();
		String customerType=collMisForm.getCustype();
		String custCategory=collMisForm.getCustcategory();
		String balanceprincipal=collMisForm.getBalanceprincipal();
		//String fromdate=collMisForm.getFromdate();
		//String todate=collMisForm.getTodate();
		String user=collMisForm.getLbxUserSearchId();
		...
		String productCategory=collMisForm.getProducTCategory();
		String loanClass=collMisForm.getLoanClassification();
		String p_branch_type=collMisForm.getBranch();
		String p_branch_id=collMisForm.getLbxBranchId();
		...
		if(dbType.equalsIgnoreCase("MSSQL"))
		{
			 ...
			 bufInsSql.append("CCD.QUEUE_DATE as QUEUE_DATE,CCD.LAST_ACTION_CODE,");
			 bufInsSql.append("CCD.LAST_ACTION_DATE AS LAST_ACTION_DATE  from cr_loan_dtl CLD  ");
			 bufInsSql.append("left join coll_case_dtl CCD  ON CLD.LOAN_NO=CCD.LOAN_NO join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID  where CLD.REC_STATUS='A' AND DELINQUENCY_FLAG = 'Y' and CCD.LAST_ACTION_CODE='COLL'  ");
		
		}
		else
		{
			/* bufInsSql.append("Select A.LOAN_NO,A.LOAN_DPD,A.CUSTOMER_NAME,A.LOAN_OVERDUE_PRINCIPAL,(IFNULL(A.ACTIVEPAYMENT,0)+IFNULL(B.OTHER_CHARGE,0)+ IFNULL(C.CBC,0)+ IFNULL(D.LPP,0)) AS TOTALOVERDUE, concat(A.QUEUE_CODE,' ')QUEUE_CODE,A.USERNAME ,A.ALLOCATIONDATE,A.QUEUE_DATE,A.LAST_ACTION_CODE,A.LAST_ACTION_DATE ");   
			 ...
			 bufInsSql.append("CCD.QUEUE_CODE,CONCAT((SELECT USER_NAME FROM sec_user_m WHERE USER_ID=CCD.USER_ID),' ') as USERNAME ," +
			 		"CCD.ALLOCATION_DATE AS ALLOCATIONDATE,CCD.QUEUE_DATE as QUEUE_DATE," +
			 		"CCD.LAST_ACTION_CODE,CCD.LAST_ACTION_DATE AS LAST_ACTION_DATE  from cr_loan_dtl CLD  ");
			 bufInsSql.append("left join coll_case_dtl CCD  ON CLD.LOAN_NO=CCD.LOAN_NO join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID  where CLD.REC_STATUS='A' AND DELINQUENCY_FLAG = 'Y' and CCD.LAST_ACTION_CODE='COLL'  ");
*/	
			bufInsSql.append("Select CLD.LOAN_NO,CLD.LOAN_DPD,CCD.LOAN_DPD as COLL_LOAN_DPD,CLD.LOAN_ID,gcm.CUSTOMER_NAME,");
			bufInsSql.append("CLD.LOAN_OVERDUE_PRINCIPAL,CCD.ALLOCATION_DATE AS ALLOCATIONDATE, ");
			bufInsSql.append("IF(DATE(CCD.QUEUE_DATE)='0000-00-00',null,DATE(CCD.QUEUE_DATE)) AS QUEUE_DATE,CFU.ACTION_CODE AS LAST_ACTION_CODE,concat(CCD.QUEUE_CODE,' ')QUEUE_CODE, ");
			bufInsSql.append("CCD.LAST_ACTION_DATE AS LAST_ACTION_DATE,CONCAT((SELECT USER_NAME FROM sec_user_m WHERE USER_ID=CCD.USER_ID),' ') as USERNAME, @A:=0,@B:=0,@C:=0, ");
			...
			bufInsSql.append("AND CHARGE_CODE_ID NOT IN ('109','110','7') AND CLD.LOAN_ID=cr_txnadvice_dtl.LOAN_ID) AS OTHER_CHARGE, ");
			bufInsSql.append("(SELECT @B:=ifnull(sum(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0)), 0)  	");
			bufInsSql.append("FROM cr_txnadvice_dtl WHERE    ADVICE_TYPE='R' AND REC_STATUS='A' AND CHARGE_CODE_ID='110' AND   CLD.LOAN_ID= cr_txnadvice_dtl.LOAN_ID) AS CBC, ");
			bufInsSql.append("(SELECT @C:=ifnull(sum(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0)), 0) 	");
			bufInsSql.append("FROM cr_txnadvice_dtl WHERE    ADVICE_TYPE='R'AND REC_STATUS='A'AND CHARGE_CODE_ID='109' AND   CLD.LOAN_ID= cr_txnadvice_dtl.LOAN_ID) AS  LPP, ");
			bufInsSql.append("(IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('0.0',0)+IFNULL(@A,0)+ IFNULL(@B,0)+ IFNULL(@C,0)) AS TOTALOVERDUE,CFU.ACTION_AMOUNT as COLLECTION_AMOUNT ");
			bufInsSql.append("from cr_loan_dtl CLD  ");
			bufInsSql.append("left join coll_case_dtl CCD  ON CLD.LOAN_ID=CCD.LOAN_ID  ");
			...
			bufInsSql.append(" where CLD.REC_STATUS='A' AND DELINQUENCY_FLAG = 'Y' and CFU.ACTION_CODE='COLL' ");
		...
		String file = "collectionMISReports";
