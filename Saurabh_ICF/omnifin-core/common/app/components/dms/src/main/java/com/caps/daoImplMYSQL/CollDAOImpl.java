...
//				queueQuery.append(" AND QUEUE_DESC_L="+queueCodeMasterVo.getQueueDesc()+" ");
	...
	public String queueActionDate(String curreentdate) {
		...
		String query ="";
		try {
			query = "select count(1) from coll_case_dtl CCD join coll_queue_def_m CQDM ON CCD.QUEUE_CODE=CQDM.QUEUE_CODE "
					+ "where CQDM.REC_STATUS='A' AND " +
							"STR_TO_DATE('"+curreentdate+"','"+dateFormat+"') > date_add(CCD.ALLOCATION_DATE, INTERVAL CQDM.ACTION_DAYS DAY)";
			
			logger.info("In queueactiondate......Dao Impl:::::::::" + query);
			count = ConnectionDAO.singleReturn(query);
	...
	public ArrayList<AllocationMasterVo> searchAllocationData(Object ob,HttpServletRequest request) {
		...
		try {
			...
			bufInsSql.append("USER_ID,(SELECT A.USER_NAME FROM SEC_USER_M A WHERE A.USER_ID=B.USER_ID),ALLOCATION_PERCENTAGE FROM coll_alloc_queue_map B");
	...
	public ArrayList<AllocationMasterVo> searchQueueAllocationData(AllocationMasterVo allocationVo){
		...
		try {
			...
			str="select sum(ifnull(ALLOCATION_PERCENTAGE,0)) as percentage from coll_alloc_queue_map where  QUEUE_CODE ='"+queueId+"'";
			percent1=ConnectionDAO.singleReturn(str);
...
//				}else{
	...
	public String calcpercentage(String[] checkbox) throws SQLException{
		...
		for(int i=0;i<length;i++){
			...
			logger.info("query"+query);
			percentage=ConnectionDAO.singleReturn(query);
		}
		}catch(Exception e){
			e.printStackTrace();
		}
...
ArrayList<ReallocationGridVo> detailListGrid=new 	ArrayList<ReallocationGridVo>();
  ...
  logger.info("In reallocationGrid......balanceprincipal  "+balanceprincipal);         
      ...
      assigndatefrom=StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getAssignfrom()).trim());
       ...
       lppChargeQuery.append("Select  sum((IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0))) LPP");
       ...
       cbcquery.append("Select  sum(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0)) CBC");
       cbcquery.append(" FROM cr_txnadvice_dtl WHERE LOAN_ID='"+loanNo+"' AND ADVICE_TYPE='R' AND REC_STATUS='A' AND CHARGE_CODE_ID='110'");
       bccQuery.append("Select  sum(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0)) OTHER_CHARGE");
	   ...
	   bccQuery.append(" AND CHARGE_CODE_ID NOT IN ('109','110','7')");
		...
		otherCharge="0";
	     ...
	     bufInsSql.append("Select CLD.LOAN_NO,ifnull(CLD.LOAN_DPD,0),gcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_PRINCIPAL,");
		 bufInsSql.append(" IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,");
		 bufInsSql.append(" CCD.QUEUE_CODE,(select USER_NAME from sec_user_m where USER_ID= CCD.USER_ID)as user,");
		bufInsSql.append("	DATE_FORMAT(ALLOCATION_DATE,'"+ dateFormat + "'),DATE_FORMAT(QUEUE_DATE,'"+ dateFormat + "') ,CLD.LOAN_ID from cr_loan_dtl CLD left join coll_case_dtl CCD ");
		 bufInsSql.append(" ON CLD.LOAN_NO=CCD.LOAN_NO join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID");
		 bufInsSql.append(" where CLD.REC_STATUS='A' AND DELINQUENCY_FLAG = 'Y'  and CLD.LOAN_BRANCH in(select branch_id from sec_user_branch_dtl SUBD where SUBD.USER_ID ='"+loginuser+"' and  SUBD.REC_STATUS = 'A') ");
		...
		bufInsSqlTempCount.append(" IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,");
		bufInsSqlTempCount.append(" CCD.QUEUE_CODE,(select USER_NAME from sec_user_m where USER_ID= CCD.USER_ID)as user,");
		bufInsSqlTempCount.append("	DATE_FORMAT(ALLOCATION_DATE,'"+ dateFormat + "'),DATE_FORMAT(QUEUE_DATE,'"+ dateFormat + "') ,CLD.LOAN_ID from cr_loan_dtl CLD left join coll_case_dtl CCD ");
		bufInsSqlTempCount.append(" ON CLD.LOAN_NO=CCD.LOAN_NO join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID");
		bufInsSqlTempCount.append(" where CLD.REC_STATUS='A' AND DELINQUENCY_FLAG = 'Y' and  CLD.LOAN_BRANCH in(select branch_id from sec_user_branch_dtl SUBD where SUBD.USER_ID ='"+loginuser+"'  and SUBD.REC_STATUS = 'A') ");
		...
		if((!(loanNo.trim().equalsIgnoreCase("")))) {
	 	...
	 	if((!(assigndatefrom.trim().equalsIgnoreCase("")))) {
	 	  bufInsSql.append(" AND DATE_FORMAT(CCD.ALLOCATION_DATE,'"+dateFormat+"') >= '"+assigndatefrom+"' ");
	 	bufInsSqlTempCount.append(" AND DATE_FORMAT(CCD.ALLOCATION_DATE,'"+dateFormat+"') >= '"+assigndatefrom+"' ");
	      }
	 	if((!(assigndateto.trim().equalsIgnoreCase("")))) {
	   	   bufInsSql.append(" AND DATE_FORMAT(CCD.ALLOCATION_DATE,'"+dateFormat+"') <= '"+assigndateto+"' ");
	     	bufInsSqlTempCount.append(" AND DATE_FORMAT(CCD.ALLOCATION_DATE,'"+dateFormat+"') <= '"+assigndateto+"' ");
	     }
		...
		if((!(dpd2.trim().equalsIgnoreCase("")))) {
	    	  bufInsSql.append(" AND ifNull(CLD.LOAN_DPD,0) >= '"+dpd2+"' ");
	    	  bufInsSqlTempCount.append(" AND ifNull(CLD.LOAN_DPD,0) >= '"+dpd2+"' ");
	      }
		if((!(dpd1.trim().equalsIgnoreCase("")))) {
  	  bufInsSql.append(" AND ifNull(CLD.LOAN_DPD,0) <= '"+dpd1+"' ");
  	  bufInsSqlTempCount.append(" AND ifNull(CLD.LOAN_DPD,0) <= '"+dpd1+"' ");
        }
		...
		if((!(queue.trim().equalsIgnoreCase("")))) {
			...
			bufInsSql.append("    ORDER BY CLD.LOAN_ID");
...
try {
	...
	for (int i = 0; i < length; i++) {
		bufInsSql = new StringBuilder();
		insertPrepStmtObject = new PrepStmtObject();
    bufInsSql.append("update coll_case_dtl set USER_ID=?,REALLOCATION_REMARKS=?,REALLOCATION_FLAG='Y',ALLOCATION_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) " +
    		" WHERE LOAN_NO='"+CommonFunction.checkNull(checkbox[i])+"'");
...
//			dpd2 = CommonFunction.checkNull(vo.getDpd2()).trim();
		...
		bufInsSql.append("select  CLD.LOAN_ID,CLD.LOAN_NO,IFNULL(CLD.LOAN_DPD,0), gcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_PRINCIPAL,"
						+ "IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,"
						+ " CCD.QUEUE_CODE,CCD.USER_ID,DATEDIFF(STR_TO_DATE('"+curreentdate+"','"+dateFormat+"'),CCD.ALLOCATION_DATE) AS ActionNotTaken,DATE_FORMAT(CCD.QUEUE_DATE,'"+dateFormat+"') As QueueAssignDate"
		...
		if ((!(queueDateFrom.trim().equalsIgnoreCase("")))) {
			bufInsSql.append(" AND DATE_FORMAT(CCD.QUEUE_DATE,'" + dateFormat + "') >= '" + queueDateFrom + "' ");
			bufInsSqlTempCount.append(" AND DATE_FORMAT(CCD.QUEUE_DATE,'"+ dateFormat + "') >= '" + queueDateFrom + "' ");
		}
		if ((!(queueDateTo.trim().equalsIgnoreCase("")))) {
			bufInsSql.append(" AND DATE_FORMAT(CCD.QUEUE_DATE,'"+ dateFormat + "') <= '" + queueDateTo + "' ");
			bufInsSqlTempCount.append(" AND DATE_FORMAT(CCD.QUEUE_DATE,'"+ dateFormat + "') <= '" + queueDateTo + "' ");
		}
		if ((!(customerName.trim().equalsIgnoreCase("")))) {
			bufInsSql.append(" AND gcm.CUSTOMER_NAME like '%"+ customerName + "%'  ");
			bufInsSqlTempCount.append(" AND gcm.CUSTOMER_NAME like '%"+ customerName + "%'  ");
		...
		if ((!(actiondays.trim().equalsIgnoreCase("")))) {
			bufInsSql.append(" AND DATEDIFF(STR_TO_DATE('"+curreentdate+"','"+dateFormat+"'),CCD.ALLOCATION_DATE)  ='" + actiondays + "' ");
			bufInsSqlTempCount.append(" AND DATEDIFF(STR_TO_DATE('"+curreentdate+"','"+dateFormat+"'),CCD.ALLOCATION_DATE)  ='" + actiondays+ "' ");
		}
...
public ArrayList<ContactRecordingSearchVO> contactRecordingSearchList(ContactRecordingSearchVO vo){
	...
	try {
		...
		otherCharge="0";
	     ...
	     logger.info("sumCharges:---"+sumCharges)  ;
	     /*toni*/
	     bufInsSql.append("Select distinct CLD.LOAN_NO,CLD.LOAN_ID,gcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_AMOUNT,");
		 bufInsSql.append(" IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,");
		 bufInsSql.append(" CCD.QUEUE_CODE,CCD.USER_ID,ESCALATION_FLAG,if(CCD.LAST_ACTION_CODE is not null ,'YES','NO'),ifnull(cld.loan_dpd,0) as current_dpd,ifnull(ccd.loan_dpd,0) as old_dpd, ");
		 bufInsSql.append(" ifnull(cld.LOAN_DUE_DAY,0)LOAN_DUE_DAY,cld.NEXT_DUE_DATE,crd.bill_flag,crd.INSTL_DATE,day(last_day(STR_TO_DATE('"+bDate+"','"+dateFormat+"'))),day(STR_TO_DATE('"+vo.getBusinessdate()+"','%d-%m-%Y')) ");
		 bufInsSql.append(" From cr_loan_dtl CLD inner join coll_case_dtl CCD ");
		 bufInsSql.append(" ON CLD.LOAN_NO=CCD.LOAN_NO inner join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID left join coll_follow_up_trails cft on cft.loan_id=cld.LOAN_ID");
		 bufInsSql.append(" left join cr_repaysch_dtl crd on ( CRD.INSTL_NO<>0 AND cld.loan_id=crd.LOAN_ID and year(crd.INSTL_DATE)=year(STR_TO_DATE('"+bDate+"','"+dateFormat+"')) and month(crd.INSTL_DATE)=month(STR_TO_DATE('"+vo.getBusinessdate()+"','%d-%m-%Y'))) ");
		...
		bufInsSqlTempCount.append("Select count(1) from (Select distinct CLD.LOAN_NO,CLD.LOAN_ID,gcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_AMOUNT,");
		bufInsSqlTempCount.append(" IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,");
		bufInsSqlTempCount.append(" CCD.QUEUE_CODE,CCD.USER_ID,ESCALATION_FLAG,if(CCD.LAST_ACTION_CODE is not null ,'YES','NO') From cr_loan_dtl CLD left join coll_case_dtl CCD ");
		...
		bufInsSqlTempCount.append(" left join cr_repaysch_dtl crd on ( CRD.INSTL_NO<>0 AND cld.loan_id=crd.LOAN_ID and year(crd.INSTL_DATE)=year(STR_TO_DATE('"+bDate+"','"+dateFormat+"')) and month(crd.INSTL_DATE)=month(STR_TO_DATE('"+bDate+"','"+dateFormat+"'))) ");
		...
		/*if((!(tos2.trim().equalsIgnoreCase("")))) {
	    	  bufInsSql.append(" AND ifnull(ifnull(CTD.AMOUNT_IN_PROCESS,0) + ifnull(CTD.TXN_ADJUSTED_AMOUNT,0) + IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0) ,0) >= '"+tos2+"' ");
	    	  bufInsSqlTempCount.append(" AND ifnull(ifnull(CTD.AMOUNT_IN_PROCESS,0) + ifnull(CTD.TXN_ADJUSTED_AMOUNT,0) + IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0) ,0)   >= '"+tos2+"' ");
	      }
		if((!(tos1.trim().equalsIgnoreCase("")))) {
	    	  bufInsSql.append(" AND ifnull(ifnull(CTD.AMOUNT_IN_PROCESS,0) + ifnull(CTD.TXN_ADJUSTED_AMOUNT,0) + IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0) ,0) <= '"+tos1+"' ");
	    	  bufInsSqlTempCount.append(" AND ifnull(ifnull(CTD.AMOUNT_IN_PROCESS,0) + ifnull(CTD.TXN_ADJUSTED_AMOUNT,0) + IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0) ,0) <= '"+tos1+"' ");
	      }
		...
		if(((actionDue.trim().equalsIgnoreCase("NA")))) {
	    	  bufInsSql.append(" AND IFNULL(CCD.LAST_ACTION_CODE,'') ='' ");
	    	  bufInsSqlTempCount.append(" AND IFNULL(CCD.LAST_ACTION_CODE,'') ='' ");
	      }
		...
		if((!(dueDate.trim().equalsIgnoreCase("")))) {
		 	  bufInsSql.append(" AND DATE_FORMAT(CCD.NEXT_ACTION_DATE,'"+dateFormat+"') >= '"+dueDate+"' ");
		 	bufInsSqlTempCount.append(" AND DATE_FORMAT(CCD.NEXT_ACTION_DATE,'"+dateFormat+"') >= '"+dueDate+"' ");
		      }
					 ...
					 || vo.getCurrentPageLink()>1)
			{
				...
				logger.info("current PAge Link no .................... "+vo.getCurrentPageLink());
				if((CommonFunction.checkNull(vo.getFlag())).trim().equalsIgnoreCase("Y"))
				{
					if(vo.getCurrentPageLink()>1)
					{
					startRecordIndex = (vo.getCurrentPageLink()-1)*no;
				...
				{
					String query="select parameter_value from parameter_mst where parameter_key='MOBILE_OFFLINE_COLL_LIMIT';";
					String MobileLimit=CommonFunction.checkNull(ConnectionDAO.singleReturn(query.toString()));
					logger.info("MOBILE OFFLINE COLLECTION LIMIT::::::"+MobileLimit);
					bufInsSql.append(" limit 0,"+MobileLimit);
				}
			}
			logger.info("query : "+bufInsSql);
		...
		/*	logger.info("searchQueueEdit "
			...
			if (data.size() > 0) {
		     ...
		     if (StringUtils.equalsIgnoreCase(vo.getFlag(), "WebService"))
              conteactRecVO.setLoanno(CommonFunction.checkNull(data.get(0).toString()).trim());
            else {
              conteactRecVO.setLoanno(new StringBuilder().append("<a href=#").append(CommonFunction.checkNull(data.get(0).toString()).trim()).append(" onclick=").append("openContactRecordingTrail('").append(CommonFunction.checkNull(data.get(1).toString()).trim()).append("','").append(CommonFunction.checkNull(data.get(7))).append("')").append(" style=color:red>").append(CommonFunction.checkNull(data.get(0)).toString().trim()).append("</a>").toString());
            }
          }
          else if (StringUtils.equalsIgnoreCase(vo.getFlag(), "WebService"))
            conteactRecVO.setLoanno(CommonFunction.checkNull(data.get(0).toString()).trim());
          else {
            conteactRecVO.setLoanno(new StringBuilder().append("<a href=#").append(CommonFunction.checkNull(data.get(0).toString()).trim()).append(" onclick=").append("openContactRecordingTrail('").append(CommonFunction.checkNull(data.get(1).toString()).trim()).append("','").append(CommonFunction.checkNull(data.get(7))).append("')").append(">").append(CommonFunction.checkNull(data.get(0)).toString().trim()).append("</a>").toString());
          }
				conteactRecVO.setCustomername(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(2)).trim()));
                    ...
                    conteactRecVO.setActionFlag(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(8)).trim()));
                  //Code start For Add Loan Dpd Status
		    		 String strCurrentDPD="0";
		    		 String strOldDPD="0";
		    		 String strDueDay = "0";
		    		 ...
		    		 int dueDay=0;
		    		 int cldpd=0;
					 strCurrentDPD=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(9)).trim());
					 if(strCurrentDPD.equalsIgnoreCase("")){
						 cdpd=Integer.parseInt((strCurrentDPD));
						 logger.info("strCurrentDPD::::"+strCurrentDPD+" cdpd::"+cdpd);
					 }
					 strOldDPD=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(10)).trim());
					 if(strOldDPD.equalsIgnoreCase("")){
						 odpd=Integer.parseInt((strOldDPD));
						 logger.info("strOldDPD::::"+strOldDPD+" odpd::"+odpd);
					 }
					 strDueDay=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(11)).trim());
					 if(strDueDay.equalsIgnoreCase("")){
						 dueDay=Integer.parseInt((strDueDay));
						 logger.info("strDueDay::::"+strDueDay+" dueDay::"+dueDay);
					 }
					 int intTotalDay=0;
					 int intDate=0;
					 String strTotalDay="0";
					 strTotalDay=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(15)).trim());
					 if(strTotalDay!=null && !strTotalDay.trim().equalsIgnoreCase(""))
					 {
					 intTotalDay=Integer.parseInt(strTotalDay);
					 }
					 String strDay="0";
					 strDay=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(16)).trim());
					 if(strDay!=null && !strDay.trim().equalsIgnoreCase(""))
					 {
						 intDate=Integer.parseInt(strDay);
					 }
					// intDate=Integer.parseInt(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(16)).trim()));
					 
					 logger.info("intTotalDay::::"+intTotalDay+"intDate::::"+intDate);
					 logger.info("NEXT_DUE_DATE:::::"+StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(12)).trim()));
					 logger.info("INSTL_DATE:::::::"+StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(14)).trim()));
					 String billflag=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(13)).trim());
					 logger.info("billflag:::::::"+billflag);
					 if(cdpd == 0)
						{
							if(billflag.equalsIgnoreCase("Y"))
							{
								cldpd = 0;
							}
							else
							{
								cldpd = intTotalDay - dueDay +1;
							}
						}
					 else
					 {
						 cldpd = cdpd + (intTotalDay - intDate +1);
					 }
					 String dpdStatus = "";
					 int dpd = cldpd - odpd;
					if (cldpd == 0) 
					{
						dpdStatus = "Normalized";
					} 
					else if (dpd <= 4 && dpd >= -4) 
					{
						dpdStatus = "Stablized";
					...
					{
						dpdStatus = "Roll Forward";
					} 
					else if (dpd < -4) {
						dpdStatus = "Roll Backward";
					}
				  conteactRecVO.setLoanDpdStatus(dpdStatus);
				  
				  //Code End For Loan Dpd Status
...
public ArrayList<ContactRecordingFollowUpVO> customerInfoList(ContactRecordingFollowUpVO vo){
	...
	query.append(" DATE_FORMAT(gcm.CUSTOMER_DOB,'"+dateFormat+"') CUSTOMER_DOB ,");
	query.append(" (SELECT QUEUE_DESC from coll_queue_def_m cqdm where  cqdm.QUEUE_CODE= CCD.QUEUE_CODE) QUEUE_CODE,");
	query.append(" DATE_FORMAT(ALLOCATION_DATE,'"+dateFormat+"') ALLOCATION_DATE,");
	query.append(" DATE_FORMAT(QUEUE_DATE,'"+dateFormat+"') QUEUE_DATE,");
...
public ArrayList<ContactRecordingFollowUpVO> customerContactInfoList(ContactRecordingFollowUpVO vo){
	...
	try{
	 ...
	 query.append(" ,case cam.ADDRESS_TYPE when 'PERMANENT' then 'PERMANENT' when 'CURRENT' then 'CURRENT' when 'REI' then 'CURRENT RESIDENCE' end as add_type ");
	 ...
	 query.append(" where CLD.REC_STATUS='A'  AND DELINQUENCY_FLAG = 'Y' AND CLD.LOAN_ID='"+vo.getLoanId()+"' ");
		  ...
		  for(int i=0;i<size;i++){
			  ...
			  if(subList.size()>0){
				  ...
				  ContactRecordingVO.setAddressType(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(subList.get(8)).trim()));
				  customerInfoList.add(ContactRecordingVO);
			}
		  }
...
public ArrayList<ContactRecordingFollowUpVO> loanInfoList(ContactRecordingFollowUpVO vo){
	...
	try{
		...
		bccQuery.append("Select  SUM(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0)) OTHER_CHARGE");
		bccQuery.append(" FROM cr_txnadvice_dtl WHERE LOAN_ID='"+vo.getLoanId()+"' AND ADVICE_TYPE='R' AND REC_STATUS='A' ");
		bccQuery.append(" AND CHARGE_CODE_ID NOT IN ('109','110','7') group by LOAN_ID");
		LPP=CommonFunction.checkNull(ConnectionDAO.singleReturn(lppChargeQuery.toString()));
		CBC=CommonFunction.checkNull(ConnectionDAO.singleReturn(cbcquery.toString()));
		logger.info("bccQuery.toString()"+bccQuery.toString());
		otherCharge=CommonFunction.checkNull(ConnectionDAO.singleReturn(bccQuery.toString()));
		if(LPP.equalsIgnoreCase("")){
			LPP="0";	
		}
        ...
        query.append("Select CLD.LOAN_ID,CLD.LOAN_LOAN_AMOUNT,DATE_FORMAT(CLD.LOAN_DISBURSAL_DATE,'"+dateFormat+"'),CLD.LOAN_EMI_AMOUNT,CLD.LOAN_NO_OF_INSTALLMENT,");
        query.append(" CLD.LOAN_BILLED_INSTL_NUM,CLD.LOAN_OVERDUE_AMOUNT,CLD.LOAN_TENURE,ifnull(CLD.LOAN_DPD,0),");
        query.append(" (IFNULL(CLD.LOAN_OVERDUE_PRINCIPAL,0)+IFNULL(CLD.LOAN_BALANCE_PRINCIPAL,0)) PRINCIPAL_OUTSTANDING,");
        query.append(" (IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)-IFNULL(CLD.LOAN_OVERDUE_PRINCIPAL,0)) INTEREST_OVERDUE,");
        query.append(" CLD.LOAN_OVERDUE_PRINCIPAL,CLD.LOAN_REPAYMENT_FREQ,DATE_FORMAT(CLD.NEXT_DUE_DATE,'"+dateFormat+"'),");
        query.append(" (select DATE_FORMAT(max(INSTL_DATE),'"+dateFormat+"') from cr_repaysch_dtl where LOAN_ID= '"+vo.getLoanId()+"')EMI_END_DATE,");
        ...
        query.append(" DATE_FORMAT(CDD.DEAL_DATE,'"+dateFormat+"'),DATE_FORMAT(CDLD.DEAL_SANCTION_DATE,'"+dateFormat+"'),(CLD.LOAN_BILLED_INSTL_NUM - CLD.LOAN_OVERDUE_INSTL_NUM) NO_OF_EMI_PAID,LOAN_REPAYMENT_MODE,LOAN_RATE_TYPE,(Select user_name from sec_user_m where user_id=DEAL_RM) RM,(Select user_name from sec_user_m where user_id=(Select USER_REPORTING_TO from sec_user_m where user_id=DEAL_RM)) Super_rm,DATE_FORMAT(CID.RECEIVED_DATE,'%d-%m-%Y'), CID.INSTRUMENT_AMOUNT,CLD.LOAN_OVERDUE_AMOUNT ");
        ...
        query.append("and CID.INSTRUMENT_ID=( select max(INSTRUMENT_ID) from CR_INSTRUMENT_DTL where INSTRUMENT_TYPE='R' and REC_STATUS in ('D','R') and TXN_TYPE='LIM'AND TXNID='"+vo.getLoanId()+"')");
...
//				  }
				  ...
				  else if(repaymentMode.equalsIgnoreCase("H")){
					  repaymentMode="NACH";
				  }
...
public boolean saveFollowUpTrail(ContactRecordingFollowUpVO followUpVO){
	...
	try{
		logger.info("In saveFollowUpTrail...Dao Impl.");
		 ...
		 bufInsSql.append("insert into coll_follow_up_trails (LOAN_ID,ACTION_CODE,REMARKS,MEMO," +
		 		          "ACTION_AMOUNT,ACTION_DATE,ACTION_TIME,NEXT_ACTION_DATE,NEXT_ACTION_TIME,ACTION_BY," +
		 		          "CONTACT_PERSON,RelationShip,CONTACT_MODE,CONTACT_PLACE,PAY_MODE,RECIPT_NO,REC_STATUS," +
		 		          "ACTION_TIME_TAKEN,INSTRUMENT_NO,INSTRUMENT_DATE,CONTACT_BY, " +
		 		          "MAKER_ID,MAKER_DATE,AUTHOR_ID,AUTHOR_DATE,APPLICANT_TYPE)");
		 bufInsSql.append(" values ( ");
		 ...
		 bufInsSql.append(" STR_TO_DATE(?,'"+dateFormat+"'),");//ACTION_DATE
		 bufInsSql.append(" ?, " ); //ACTION_TIME
		 bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND), ");//NEXT_ACTION_DATE
		 ...
		 bufInsSql.append("  DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," ); //INSTRUMENT_DATE
		 bufInsSql.append(" ?," ); //CONTACT_BY
		 bufInsSql.append(" ?," ); //MAKER_ID
		 bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," ); //MAKER_DATE
		 bufInsSql.append(" ?," ); //AUTHOR_ID
		 bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) ," ); //AUTHOR_DATE
		 bufInsSql.append(" ? )" ); //APPLICANT_TYPE
		 ...
		 //Rohit Changes for action Time Starts
		 if(CommonFunction.checkNull(followUpVO.getActionTime()).equalsIgnoreCase(""))
				insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(followUpVO.getActionTime().trim()));
		//Rohit Changes for action Time end
		...
		//Rohit Changes for RelationShip starts
		 if(CommonFunction.checkNull(followUpVO.getRelationshipS()).equalsIgnoreCase(""))
				insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(followUpVO.getRelationshipS().trim()));
		//Rohit Changes for RelationShip end
		 ...
		 if (CommonFunction.checkNull(followUpVO.getBusinessDate()).equalsIgnoreCase(""))
			...
			if(CommonFunction.checkNull(followUpVO.getApplicantType()).equalsIgnoreCase(""))
				insertPrepStmtObject.addNull();
			else
			insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(followUpVO.getApplicantType().trim()));
			...
			if(exclationFleg.equalsIgnoreCase("Y")){
				 bufInsSql.append("update coll_case_dtl set ESCALATION_FLAG='N',USER_ID=?,ALLOCATION_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),LAST_ACTION_CODE=?,LAST_ACTION_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),NEXT_ACTION_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),ESCLATION_COUNT=? where LOAN_ID=? " ); //LOAN_ID
				 ...
				 bufInsSql.append("update coll_case_dtl set LAST_ACTION_CODE=?,LAST_ACTION_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),NEXT_ACTION_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),ESCLATION_COUNT=? where LOAN_ID=? " ); //LOAN_ID	
			...
			if(CommonFunction.checkNull(followUpVO.getActionCode()).equalsIgnoreCase("EC")){
				bufInsSql =	new StringBuilder();
				 ...
				 bufInsSql.append("UPDATE coll_case_dtl SET ESCALATION_FLAG='Y' , ESCLATION_DATE=STR_TO_DATE(?, '"+dateFormatWithTime+"') ,ESCALATION_REMARK=? WHERE LOAN_ID=?");
 ...
 public ArrayList<ContactRecordingFollowUpVO> followUpGridList(ContactRecordingFollowUpVO vo){
	...
	try{
     ...
     query.append("Select DATE_FORMAT(ACTION_DATE,'"+dateFormat+"'),");
     query.append(" (Select ACTION_DESC from coll_action_code_m CACM where CFUT.ACTION_CODE=CACM.ACTION_CODE) ACTION_CODE,ifnull(gm1.DESCRIPTION,'')CONTACT_MODE,CONTACT_PLACE,");
     query.append(" DATE_FORMAT(NEXT_ACTION_DATE,'"+dateFormat+"'),NEXT_ACTION_TIME,REMARKS,ACTION_AMOUNT from ");
     query.append("  coll_follow_up_trails CFUT left join generic_master gm1 on (gm1.value=CFUT.CONTACT_MODE) and gm1.generic_key='CONTACT_MODE' where LOAN_ID='"+vo.getLoanId()+"'   order by FOLLOW_UP_ID desc limit 0,3");
...
public ArrayList<CollCustomerAddressVo> searchClassificationData(String loanId){
	...
	try {
		...
		bufInsSqlTempCount.append("select count(1) from (select 'Collection',ADDRESS_TYPE,(Select DISTRICT_DESC from com_district_m where DISTRICT_ID= DISTRICT) DISTRICT,STATE,COUNTRY,PRIMARY_PHONE,FAX,PINCODE,ADDRESS_LINE1 from coll_address_dtl coll "); 
		...
		bufInsSqlTempCount.append("where crlt.LOAN_ID='"+loanId+"') A ");
	...
	try {
		...
		//Select CLD.LOAN_NO,CLD.LOAN_ID,cdcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_PRINCIPAL,CCD.QUEUE_CODE,CCD.USER_ID,ESCALATION_FLAG From cr_loan_dtl CLD left join coll_case_dtl CCD ON CLD.LOAN_NO=CCD.LOAN_NO join cr_deal_customer_m cdcm on cdcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID where CLD.REC_STATUS='A' AND DELINQUENCY_FLAG = 'Y' AND CCD.USER_ID='ANIL' or ESCALATION_FLAG='Y'
			...
			otherCharge="0";
		     ...
		     logger.info("otherCharge:---"+otherCharge)  ;
		     logger.info("sumCharges:---"+sumCharges)  ;
		     bufInsSql.append("Select distinct CLD.LOAN_NO,CLD.LOAN_ID,gcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_PRINCIPAL,");
			 bufInsSql.append(" IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,");
			 bufInsSql.append(" CCD.QUEUE_CODE,CCD.USER_ID,ESCALATION_FLAG From cr_loan_dtl CLD left join coll_case_dtl CCD ");
			 bufInsSql.append(" ON CLD.LOAN_NO=CCD.LOAN_NO join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID");
			...
			bufInsSqlTempCount.append("Select count(1) from (Select distinct CLD.LOAN_NO,CLD.LOAN_ID,gcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_PRINCIPAL,");
			bufInsSqlTempCount.append(" IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,");
...
public ArrayList<PaymentDetailsVo> paymentDetails(String loanId){
	...
	try {
		...
		bufInsSql.append("select distinct DATE_FORMAT(pmnt.PMNT_DATE,'"+dateFormat+"') as PAYMENT_DATE," );
		...
		bufInsSql.append("WHEN 'S' THEN 'SUSPENSE' WHEN 'B' THEN 'DIRECT DEBIT' WHEN 'E' THEN 'ECS' WHEN 'H' THEN 'NACH' ");
		...
		bufInsSqlTempCount.append("select distinct DATE_FORMAT(pmnt.PMNT_DATE,'"+dateFormat+"') as PAYMENT_DATE," );
		...
		bufInsSqlTempCount.append("WHEN 'S' THEN 'SUSPENSE' WHEN 'B' THEN 'DIRECT DEBIT' WHEN 'E' THEN 'ECS' WHEN 'H' THEN 'NACH' ");
		bufInsSqlTempCount.append("WHEN 'O' THEN 'OTHER' END PAYMENT_MODE from cr_pmnt_dtl pmnt join cr_txnadvice_dtl txn ");
		bufInsSqlTempCount.append(" ON pmnt.TXNADVICEID=txn.TXNADVICE_ID   JOIN cr_instrument_dtl CID ON  CID.INSTRUMENT_ID=pmnt.INSTRUMENT_ID WHERE txn.LOAN_ID='"+loanId+"' and  pmnt.rec_status <> 'X' and txn.advice_type='R' order by pmnt.PMNT_DATE desc ");
...
public ArrayList<CRFollowUpTrailsDtlVO> followUpTrailsDtlData(CRFollowUpTrailsDtlVO vo)  throws Exception{
	...
	try {
		...
		bufInsSql.append("Select DATE_FORMAT(ACTION_DATE,'"+dateFormat+"'),(Select ACTION_DESC from coll_action_code_m CACM where CFUT.ACTION_CODE=CACM.ACTION_CODE) ACTION_CODE,");
		bufInsSql.append("CONTACT_MODE,CONTACT_PLACE,DATE_FORMAT(NEXT_ACTION_DATE,'"+dateFormat+"'),NEXT_ACTION_TIME,ACTION_AMOUNT,ACTION_TIME,CONTACT_BY,CONTACT_PERSON,REMARKS,CFUT.ACTION_CODE,FOLLOW_UP_ID,CFUT.REC_STATUS,(select DESCRIPTION from generic_master	where generic_key='Relation_type'  and value=CFUT.relationShip) as RelationShip,APPLICANT_TYPE from "); 
	...
	int size = searchlist.size();
		for (int i = 0; i < size; i++) {
			...
			if (data.size() > 0) {
				...
				String strOnclick="";
				//String strOnclick = "<a href=#" +CommonFunction.checkNull(data.get(11).toString()).trim()+" onclick="+"openResolutionFollowUpDtl('"+CommonFunction.checkNull(data.get(11).toString()).trim()+"') >"+CommonFunction.checkNull(data.get(1)).toString().trim()+"</a>";
				String recStatus=CommonFunction.checkNull(data.get(13).toString().trim());
				if(recStatus.equalsIgnoreCase("A"))	
				{
					strOnclick="<a href=followUpResolutionAction.do?method=fetchActionResolution&actionId="+CommonFunction.checkNull(data.get(11).toString()).trim()+"&followupId="+CommonFunction.checkNull(data.get(12).toString()).trim()+"&loanId="+vo.getLoanId()+">"+CommonFunction.checkNull(data.get(1)).toString().trim()+"</a>";
				}
				else
				{
					strOnclick=CommonFunction.checkNull(data.get(1)).toString().trim();
				}
				logger.info("strOnclick"+strOnclick);
				Vo.setActionCode(strOnclick);
				...
				//Vo.setActionId("<a href=/collButtonAction.do?method=fetchActionResolution&ActionId="+commonFunction.checkNull(data.get(11)).totoString()+"FOLLOW_UP_ID="+commonFunction.checkNull(data.get(12)).totoString()+">"+);
				//Vo.setActionId("<a href=#" +CommonFunction.checkNull(data.get(11).toString()).trim()+" onclick="+"openResolutionFollowUpDtl('"+CommonFunction.checkNull(data.get(11).toString()).trim()+"', '"+CommonFunction.checkNull(data.get(12))+"')"+">"+CommonFunction.checkNull(data.get(1)).toString().trim()+"</a>"); 
			    Vo.setRelationshipS(CommonFunction.checkNull(data.get(14)));
			    Vo.setApplicantType(CommonFunction.checkNull(data.get(15)));
				Vo.setTotalRecordSize(count);
				detailList.add(Vo);
			}
		}
...
public ArrayList<ForeClosureVo> foreClosureData(String companyId, String loanId, String effectiveDate, String closureType)
{
	...
	try {
		...
		query = "select DATE_FORMAT(d.LOAN_REPAY_EFF_DATE,'"+dateFormat+"') from cr_loan_dtl d where d.LOAN_ID="+StringEscapeUtils.escapeSql(loanId);
		repayEffDateStr = CommonFunction.checkNull(ConnectionDAO.singleReturn(query));
...
//		bufInsSqlTempCount.append(	") A ");
		...
		bufInsSql.append("select DATE_FORMAT(a.INSTRUMENT_DATE,'"+dateFormat+"') as INSTRUMENT_DATE,a.INSTRUMENT_AMOUNT,CASE b.next_STATUS WHEN 'B' THEN 'BOUNCE' END AS STATUS from  cr_instrument_dtl a join cr_instrument_update_dtl b on a.INSTRUMENT_ID=b.INSTRUMENT_ID and NEXT_STATUS='B' and  b.rec_status='R' "); 
		bufInsSql.append(" where TXNID='"+loanId+"' and a.TXN_TYPE='LIM' group by b.INSTRUMENT_BATCH_ID ");
		bufInsSqlTempCount.append("select Count(1) from (" );
		bufInsSqlTempCount.append("select DATE_FORMAT(a.INSTRUMENT_DATE,'"+dateFormat+"') as INSTRUMENT_DATE,a.INSTRUMENT_AMOUNT,CASE b.next_STATUS WHEN 'B' THEN 'BOUNCE' END AS STATUS from  cr_instrument_dtl a join cr_instrument_update_dtl b on a.INSTRUMENT_ID=b.INSTRUMENT_ID and NEXT_STATUS='B' and  b.rec_status='R' "); 
		bufInsSqlTempCount.append(" where TXNID='"+loanId+"' and a.TXN_TYPE='LIM' group by b.INSTRUMENT_BATCH_ID ");
		bufInsSqlTempCount.append(	") A ");
		logger.info("search Query...." + bufInsSql);
		logger.info("bufInsSqlTempCount **************************** : "+bufInsSqlTempCount.toString());
        count =Integer.parseInt(ConnectionDAO.singleReturn(bufInsSqlTempCount.toString()));
...
public ArrayList<ViewpayableRecievableVo> viewPayableReceivable(ViewpayableRecievableVo viewPayableRecievable,String loanId){
		...
		try{
			...
			bufInsSql.append("SELECT DATE_FORMAT(ADVICE_DATE,'"+dateFormat+"')  AS ADVICE_DATE, (SELECT CHARGE_DESC FROM com_charge_code_m WHERE CHARGE_CODE=CHARGE_CODE_ID) cHARGE ,");
			bufInsSql.append("(ADVICE_AMOUNT-(TXN_ADJUSTED_AMOUNT+AMOUNT_IN_PROCESS)) BALANCE_AMOUNT,if(ADVICE_TYPE='P','PAYABLE','RECEIVABLE') AS ADVICE_TYPE "); 
			...
			bufInsSqlTempCount.append("select Count(1) from (" );
			bufInsSqlTempCount.append("SELECT DATE_FORMAT(ADVICE_DATE,'"+dateFormat+"') AS ADVICE_DATE, (SELECT CHARGE_DESC FROM com_charge_code_m WHERE CHARGE_CODE=CHARGE_CODE_ID) cHARGE ,");
			bufInsSqlTempCount.append("(ADVICE_AMOUNT-(TXN_ADJUSTED_AMOUNT+AMOUNT_IN_PROCESS)) BALANCE_AMOUNT,if(ADVICE_TYPE='P','PAYABLE','RECEIVABLE') AS ADVICE_TYPE ");
			bufInsSqlTempCount.append("FROM cr_txnadvice_dtl WHERE LOAN_ID='"+loanId+"'   AND (ADVICE_AMOUNT-(TXN_ADJUSTED_AMOUNT))>0 AND  REC_STATUS in('A','F') ORDER BY ADVICE_DATE ASC");
			bufInsSqlTempCount.append(	") A ");
			logger.info("search Query...." + bufInsSql);
			logger.info("bufInsSqlTempCount **************************** : "+bufInsSqlTempCount.toString());
	        count =Integer.parseInt(ConnectionDAO.singleReturn(bufInsSqlTempCount.toString()));
...
public ArrayList<ClassificationProcessVo> getClassificationData(String businessDate){
	...
	ArrayList searchlist = new ArrayList();
	StringBuilder bufInsSql = new StringBuilder();
	StringBuilder allocationSql = new StringBuilder();
	StringBuilder bufUpsSql = new StringBuilder();
	ArrayList data = new ArrayList();
	ArrayList qryList = new ArrayList();
	String allFlag="N";
	ClassificationProcessVo VO=null;
	try{
		//Code Start for alloaction Upload
		allocationSql.append("SELECT PARAMETER_VALUE  FROM PARAMETER_MST WHERE PARAMETER_KEY='ALLOCATION_UPLOAD_FLAG'; ");
		logger.info("allocationSql:"+allocationSql);
		allFlag=CommonFunction.checkNull(ConnectionDAO.singleReturn(allocationSql.toString()));
		logger.info("allFlag:"+allFlag);
		if(allFlag.equalsIgnoreCase("Y"))
		{
			bufUpsSql.append("UPDATE coll_classification_process_m SET REC_STATUS='X' WHERE PROCESS_NAME='Allocation'");
			logger.info("bufUpsSql:"+bufUpsSql);
			qryList.add(bufUpsSql.toString());
			ConnectionDAO.sqlInsUpdDelete(qryList);	
			
		}
		//Rohit Changes starts for collection module
		else
		{
			bufUpsSql.append("UPDATE coll_classification_process_m SET REC_STATUS='A' WHERE PROCESS_NAME='Allocation'");
			logger.info("bufUpsSql:"+bufUpsSql);
			qryList.add(bufUpsSql.toString());
			ConnectionDAO.sqlInsUpdDelete(qryList);	
			
		}
		//Rohit Changes end for collection module
		//Code End for alloaction Upload
		bufInsSql.append("SELECT A.PROCESS_NAME,DATE_FORMAT(B.START_TIME,'"+dateFormatWithTime+"'),DATE_FORMAT (B.END_TIME,'"+dateFormatWithTime+"'),B.REC_STATUS FROM coll_classification_process_m A");
		bufInsSql.append(" LEFT OUTER JOIN coll_classification_process_LOG  B ON A.PROCESS_NAME=B.PROCESS_NAME AND PROCESS_DATE=STR_TO_DATE('"+businessDate+"','"+dateFormatWithTime+"') AND");
...
public ArrayList<CollCustomerAddressVo> getGuarantorInfo(String loanId){
	...
	try{
		...
		bufInsSql.append(" CDAM.PINCODE,CDAM.PRIMARY_PHONE,CDAM.FAX,CDAM.ADDRESS_TYPE from cr_loan_dtl CLD join "); 
      ...
      mainList=ConnectionDAO.sqlSelect(bufInsSql.toString());
		...
		for(int i=0;i<size;i++)
		{
			...
			if(subList.size()>0){
				...
				vo.setAddr_type(CommonFunction.checkNull(subList.get(10)).trim());
...
public ArrayList<CollCustomerAddressVo> defaultcountry() {
	...
	public ArrayList<CustomerSaveVo> getRefrencesInfo(String loanId){
		...
		StringBuilder query=new StringBuilder();
		 try
		 {
			 ...
			 query.append("Select  concat(F_NAME,' ',IFNULL(M_NAME,''),' ',L_NAME)NAME,RELATIONSHIP,KNOWING_SINCE,MOBILE_NUMBER,LANDLINE_NUMBER ");
			 query.append(" from cr_loan_dtl CLD join  com_reference_m CFM on CLD.LOAN_CUSTOMER_ID=CFM.BPID ");
			 query.append(" WHERE CLD.LOAN_ID='"+loanId+"' and CFM.REC_STATUS='A'");
	...
	public ArrayList<CollCustomerAddressVo> getCoApplicantInfo(String loanId){
		...
		try{
			...
			bufInsSql.append(" gcd_customer_m CDCM on CDCR.GCD_ID=CDCM.CUSTOMER_ID join ");
			bufInsSql.append(" com_address_m CDAM on CDCM.CUSTOMER_ID=CDAM.BPID "); 
			bufInsSql.append("  where CLD.LOAN_ID='"+loanId+"'and CDCR.DEAL_CUSTOMER_ROLE_TYPE='COAPPL' and CDAM.BPTYPE='CS'");
	...
	public CollectionSummaryVO getCollectionSummary(CollectionSummaryVO collectionSummaryVO){
		...
		try{
			...
			qryfollowUpToday.append(" where a.user_id='"+CommonFunction.checkNull(collectionSummaryVO.getUserId())+"'  AND DELINQUENCY_FLAG='Y' AND  b.NEXT_ACTION_DATE=str_to_date('"+dateFormat+"','"+collectionSummaryVO.getBusinessDate()+"');");
			
			qryEscalatedCase.append("select count(1) from coll_case_dtl a  join coll_follow_up_trails b  on a.LOAN_ID=b.LOAN_ID " );
			qryEscalatedCase.append(" where user_id='"+CommonFunction.checkNull(collectionSummaryVO.getUserId())+"' AND DELINQUENCY_FLAG='Y'AND b.action_code='EC' and a.ESCLATION_DATE=str_to_date('"+dateFormat+"','"+collectionSummaryVO.getBusinessDate()+"');");
			...
			qryEscalatedReceive.append(" and ESCALATION_FLAG='Y'AND DELINQUENCY_FLAG='Y' AND ESCLATION_DATE=str_to_date('"+dateFormat+"','"+collectionSummaryVO.getBusinessDate()+"');");
			
			qrytotalPtpToday.append("select count(1) from coll_case_dtl a  join coll_follow_up_trails b  on a.LOAN_ID=b.LOAN_ID " );
			qrytotalPtpToday.append(" where user_id='"+CommonFunction.checkNull(collectionSummaryVO.getUserId())+"' AND DELINQUENCY_FLAG='Y'AND b.action_code='PTP' and b.MAKER_DATE=str_to_date('"+dateFormat+"','"+collectionSummaryVO.getBusinessDate()+"');");
...
/* NAZIA CODE FOR ADHOC CONTACT RECORDING */
...
public ArrayList<ContactRecordingSearchVO> adhocContactRecordingSearchList(ContactRecordingSearchVO vo){
	...
	try {
		...
		otherCharge="0";
	     ...
	     bufInsSql.append("Select distinct CLD.LOAN_NO,CLD.LOAN_ID,gcm.CUSTOMER_NAME,CLD.LOAN_OVERDUE_PRINCIPAL,");
		 bufInsSql.append(" IFNULL(CLD.LOAN_OVERDUE_AMOUNT,0)+IFNULL('"+sumCharges+"',0) as activePayment ,");
		 bufInsSql.append(" CCD.QUEUE_CODE,CCD.USER_ID,ESCALATION_FLAG,crd.INSTL_DATE,ifnull(cld.loan_dpd,0) as current_dpd,ifnull(ccd.loan_dpd,0) as old_dpd, ");
		 bufInsSql.append(" ifnull(cld.LOAN_DUE_DAY,0)LOAN_DUE_DAY,cld.NEXT_DUE_DATE,crd.bill_flag,day(last_day(STR_TO_DATE('"+vo.getBusinessdate().toString()+"','%d-%m-%Y'))),day(STR_TO_DATE('"+vo.getBusinessdate()+"','%d-%m-%Y')) ");
		 bufInsSql.append(" From cr_loan_dtl CLD inner join coll_case_dtl CCD ");		
		 bufInsSql.append(" ON CLD.LOAN_NO=CCD.LOAN_NO inner join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID");
		 bufInsSql.append(" left join cr_repaysch_dtl crd on (cld.loan_id=crd.LOAN_ID and year(crd.INSTL_DATE)=year(STR_TO_DATE('"+vo.getBusinessdate().toString()+"','%d-%m-%Y')) and month(crd.INSTL_DATE)=month(STR_TO_DATE('"+vo.getBusinessdate().toString()+"','%d-%m-%Y'))) ");		 
		...
		// bufInsSql.append(" AND CCD.LOAN_BRANCH IN (SELECT BRANCH_ID FROM SEC_USER_BRANCH_DTL WHERE USER_ID='"+vo.getMakerId()+"')");
		 //bufInsSql.append(" AND ((CCD.USER_ID='"+vo.getMakerId()+"'AND ESCALATION_FLAG='N')  ");		 
		 //bufInsSql.append(" OR (CCD.LOAN_ID IN (SELECT CCD.LOAN_ID FROM COLL_CASE_DTL CCD WHERE");
		 //bufInsSql.append(" CCD.USER_ID IN (select USER_ID from sec_user_m WHERE USER_REPORTING_TO ='"+vo.getMakerId()+"') AND ESCALATION_FLAG='Y')))");
		...
		bufInsSqlTempCount.append(" ON CLD.LOAN_NO=CCD.LOAN_NO inner join gcd_customer_m gcm on gcm.CUSTOMER_ID=CLD.LOAN_CUSTOMER_ID");
		bufInsSqlTempCount.append(" left join cr_repaysch_dtl crd on (cld.loan_id=crd.LOAN_ID and year(crd.INSTL_DATE)=year(STR_TO_DATE('"+vo.getBusinessdate().toString()+"','%d-%m-%Y')) and month(crd.INSTL_DATE)=month(STR_TO_DATE('"+vo.getBusinessdate().toString()+"','%d-%m-%Y'))) ");
		...
		//bufInsSqlTempCount.append(" AND ((CCD.USER_ID='"+vo.getMakerId()+"' AND ESCALATION_FLAG='N') ");
		//bufInsSqlTempCount.append("  OR (CCD.LOAN_ID IN (SELECT CCD.LOAN_ID FROM COLL_CASE_DTL CCD WHERE");
		//bufInsSqlTempCount.append(" CCD.USER_ID IN (select USER_ID from sec_user_m WHERE USER_REPORTING_TO ='"+vo.getMakerId()+"') AND ESCALATION_FLAG='Y')))");
		     ...
		     //    logger.info("Exclation Flag:000---"+CommonFunction.checkNull(data.get(7)));
				...
				conteactRecVO.setCustomername(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(2)).trim()));
		    		...
		    		if(StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(3))).equalsIgnoreCase("")){
					 ...
					 int intDate=0;
					 String strTotalDay="0";
					 strTotalDay=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(14)).trim());
					 if(strTotalDay!=null && !strTotalDay.trim().equalsIgnoreCase(""))
					 {
					 intTotalDay=Integer.parseInt(strTotalDay);
					 }
					 String strDay="0";
					 strDay=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(15)).trim());
					 if(strDay!=null && !strDay.trim().equalsIgnoreCase(""))
					 {
						 intDate=Integer.parseInt(strDay);
					 }
					 ...
					 logger.info("intTotalDay::::"+intTotalDay+"intDate::::"+intDate);
					 logger.info("NEXT_DUE_DATE:::::"+StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(12)).trim()));
					 logger.info("INSTL_DATE:::::::"+StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(8)).trim()));
					 String billflag=StringEscapeUtils.escapeHtml(CommonFunction.checkNull(data.get(13)).trim());
					 logger.info("billflag:::::::"+billflag);
					 if(cdpd == 0)
						{
...
/* Nazia code for adhoc Contact Recording SearchList ends here */
//Changes Start for Resolution Detail
@Override
public ArrayList<CRFollowUpTrailsDtlVO> getActionResolutionDtl(CRFollowUpTrailsDtlVO vo) {
	logger.info(" In getActionResolutionDtl:::::::");
	ArrayList<CRFollowUpTrailsDtlVO> resolutionList = new ArrayList<CRFollowUpTrailsDtlVO>();
	
    ArrayList mainlist=new ArrayList();
	ArrayList subList=new ArrayList();
	StringBuilder bufInsSql = new StringBuilder();
	StringBuilder bufInsSql1 = new StringBuilder();
	//CRFollowUpTrailsDtlVO  resVo = null;

	try {
		logger.info("In getQueueList()....................");
		
		bufInsSql.append("SELECT (select DESCRIPTION from generic_master where GENERIC_KEY='COLL_RESOLUTION_STAT' and rec_status ='A' and VALUE=CFUT.RESOLUTION_STATUS limit 1)RESOLUTION_STATUS,"
				+ "CFUT.RESOLUTION_REMARK,date_format(CFUT.RESOLUTION_DATE,'%d-%m-%Y')RESOLUTION_DATE,"
				+ "(select DESCRIPTION from generic_master where GENERIC_KEY='COLL_RESOLUTION_TYPE' and rec_status ='A' and VALUE=CFUT.RESOLUTION_TYPE limit 1)RESOLUTION_TYPE,  ");
		bufInsSql.append(" (Select ACTION_DESC from coll_action_code_m CACM where CFUT.ACTION_CODE=CACM.ACTION_CODE limit 1) ACTION_DESC  ");
		bufInsSql.append(" FROM  COLL_FOLLOW_UP_TRAILS CFUT  ");
		bufInsSql.append("WHERE rec_status='X' and CFUT.LOAN_ID='"+CommonFunction.checkNull(vo.getLoanId().toString()).trim()+"' ");
		...
		logger.info("Query: "+bufInsSql.toString());
	 	...
	 	for(int i=0;i<size;i++){
			...
			if (subList.size() > 0) {
				vo=new CRFollowUpTrailsDtlVO();
				vo.setResolutionStatus(CommonFunction.checkNull(subList.get(0)).toString());
				vo.setResolutionRemarks(CommonFunction.checkNull(subList.get(1)).toString());
				vo.setResolutionDate(CommonFunction.checkNull(subList.get(2)).toString());
				vo.setResolutionType(CommonFunction.checkNull(subList.get(3)).toString());
				vo.setActionCode(CommonFunction.checkNull(subList.get(4)).toString());
				//resVo.setTotalRecordSize(count);
				resolutionList.add(vo);
			}
...
public String SaveActionResolutionDtl(CRFollowUpTrailsDtlVO vo) {
	logger.info(" In SaveActionResolutionDtl:::::::");
	Boolean status=false;
	//PrepStmtObject insertPrepStmtObject = new PrepStmtObject();
	PrepStmtObject insertPrepStmtObject = new PrepStmtObject();
	//StringBuilder bufInsSql = new StringBuilder();
	StringBuilder bufUpdSql = new StringBuilder();
	logger.info("Follow Up Id:::::"+vo.getFollowUpId());
	String followUpId=vo.getFollowUpId()+"";
	//StringBuilder query=new StringBuilder();
...
//	ArrayList  data= new ArrayList();
	try{
//			String follId=vo.getFollowUpId()+"";
//			query.append("SELECT LOAN_ID,ACTION_CODE,REMARKS, MEMO, ACTION_AMOUNT, ACTION_TIME_TAKEN, date_format(ACTION_DATE,'%d-%m-%Y'),date_format(NEXT_ACTION_DATE,'%d-%m-%Y'), NEXT_ACTION_TIME, ACTION_BY, CONTACT_PERSON, CONTACT_MODE, CONTACT_PLACE, CONTACT_BY, "); 
//			query.append(" PAY_MODE, INSTRUMENT_NO, date_format(INSTRUMENT_DATE,'%d-%m-%Y'), RECIPT_NO,REC_STATUS,MAKER_ID,date_format(MAKER_DATE,'%d-%m-%Y'), AUTHOR_ID, date_format(AUTHOR_DATE,'%d-%m-%Y') FROM  coll_follow_up_trails WHERE LOAN_ID='"+CommonFunction.checkNull(vo.getLoanId().toString()).trim()+"' AND   FOLLOW_UP_ID='"+CommonFunction.checkNull(follId).trim()+"' limit 1 ");
//			logger.info("Query: "+query.toString());
//			qryList2 = ConnectionDAO.sqlSelect(query.toString());
//		    int size = qryList2.size();
//		    if(size>0)
//		    {
//		    	for(int i=0;i<size;i++)
//			    {
//		    		data = (ArrayList) qryList2.get(i);
//					if (data.size() > 0)
//					{
//			    	logger.info("Before Insert:::::");
//			    	bufInsSql.append("insert into coll_follow_up_trails (LOAN_ID, ACTION_CODE, REMARKS, MEMO, ACTION_AMOUNT, ACTION_TIME_TAKEN, ACTION_DATE, NEXT_ACTION_DATE, NEXT_ACTION_TIME, ACTION_BY, CONTACT_PERSON, CONTACT_MODE, CONTACT_PLACE, CONTACT_BY, ");
//			    	bufInsSql.append(" PAY_MODE, INSTRUMENT_NO, INSTRUMENT_DATE, RECIPT_NO, REC_STATUS, MAKER_ID, MAKER_DATE, AUTHOR_ID, AUTHOR_DATE, "); 
//			    	bufInsSql.append(" RESOLUTION_STATUS, RESOLUTION_REMARK, RESOLUTION_DATE, RESOLUTION_TYPE)");
//			    	bufInsSql.append(" values(");
//			    	bufInsSql.append(" ?," );//1 LOAN_ID
//					bufInsSql.append(" ?," );//2 Action Code
//					bufInsSql.append(" ?," );//3 REMARKS
//					bufInsSql.append(" ?," );//4 MEMO
//					bufInsSql.append(" ?," );//5 ACTION_AMOUNT
//					bufInsSql.append(" ?," );//6 ACTION_TIME_TAKEN
//					bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//7 ACTION_DATE
//					bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," );//8 NEXT_ACTION_DATE
//					bufInsSql.append(" ?," );//9 NEXT_ACTION_TIME
//					bufInsSql.append(" ?," );//10 ACTION_BY
//					bufInsSql.append(" ?," );//11 CONTACT_PERSON
//					bufInsSql.append(" ?," );//12 CONTACT_MODE
//					bufInsSql.append(" ?," );//13 CONTACT_PLACE
//					bufInsSql.append(" ?," );//14 CONTACT_BY
//					bufInsSql.append(" ?," );//15 PAY_MODE
//					bufInsSql.append(" ?," );//16 INSTRUMENT_NO 
//					bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," );//17 INSTRUMENT_DATE
//					bufInsSql.append(" ?," );//18 RECIPT_NO
//					bufInsSql.append(" ?," );//19 REC_STATUS
//					bufInsSql.append(" ?," );//20 MAKER_ID
//					bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," );//21 MAKER_DATE
//					bufInsSql.append(" ?," );//22 AUTHOR_ID
//					bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," );//23 AUTHOR_DATE
//					bufInsSql.append(" ?," );//24 RESOLUTION_STATUS
//					bufInsSql.append(" ?," );//25 RESOLUTION_REMARK
//					bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)," );//26 RESOLUTION_DATE
//					bufInsSql.append(" ? )" );//27 RESOLUTION_TYPE
//	
//					String amount=CommonFunction.checkNull(data.get(4).toString());
//				      if(!amount.equalsIgnoreCase("")){
//				    	  amount=myFormatter.parse(StringEscapeUtils.escapeHtml(amount).trim()).toString();  
//				      }
...
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(0)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(1)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(1)).trim()));
//					if(CommonFunction.checkNull(data.get(2)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(2)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(3)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(3)).trim()));
//					
...
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(5)).trim()));
//					if(CommonFunction.checkNull(data.get(6)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(6)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(7)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(7)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(8)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(8)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(9)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(9)).trim()));
//					if(CommonFunction.checkNull(data.get(10)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(10)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(11)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(11)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(12)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(12)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(13)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(13)).trim()));
//					if(CommonFunction.checkNull(data.get(14)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(14)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(15)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(15)).trim()));
//					if(CommonFunction.checkNull(data.get(16)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(16)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(17)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(17)).trim()));
//					if(CommonFunction.checkNull(data.get(18)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql("A"));
//					
//					if(CommonFunction.checkNull(data.get(19)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(19)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(20)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(20)).trim()));
//					
//					if(CommonFunction.checkNull(data.get(21)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(21)).trim()));
//					if(CommonFunction.checkNull(data.get(22)).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString(StringEscapeUtils.escapeSql(CommonFunction.checkNull(data.get(22)).trim()));
//					
...
//					
//					if(CommonFunction.checkNull(vo.getResolutionDate()).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString((vo.getResolutionDate()).trim());
//					if(CommonFunction.checkNull(vo.getResolutionType()).trim().equalsIgnoreCase(""))
//						insertPrepStmtObject.addNull();
//					else
//						insertPrepStmtObject.addString((vo.getResolutionType()).trim());
...
//						stat="S";
						...
						bufUpdSql.append("update coll_follow_up_trails set REC_STATUS='X',RESOLUTION_STATUS=?,RESOLUTION_REMARK=?,RESOLUTION_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),RESOLUTION_TYPE=? where LOAN_ID=? AND FOLLOW_UP_ID=? ");
						...
						if(CommonFunction.checkNull(followUpId).trim().equalsIgnoreCase(""))
							insertPrepStmtObject.addNull();
						else
							insertPrepStmtObject.addString((followUpId).trim());
						insertPrepStmtObject.setSql(bufUpdSql.toString());
						logger.info("IN SaveActionResolutionDtl() Update query1 ### "+insertPrepStmtObject.printQuery());
...
public String getActionDesc(String actionId){
	...
	logger.info(" In getActionDesc:::::::");
	String ActionDesc="";
	StringBuilder query=new StringBuilder();
	//CRFollowUpTrailsDtlVO  resVo = null;

	try {
		query.append("Select ACTION_DESC from coll_action_code_m  where ACTION_CODE='"+actionId.trim()+"' limit 1 ");
		logger.info("Query: "+query.toString());
		ActionDesc=ConnectionDAO.singleReturn(query.toString());
		logger.info("ActionDesc: "+ActionDesc);
...
public ArrayList<CRFollowUpTrailsDtlVO> getResolutionFollowUpStatus(){
	...
	CRFollowUpTrailsDtlVO resolVo=null;
	 ...
	 ArrayList<CRFollowUpTrailsDtlVO> resolStausList=new  ArrayList<CRFollowUpTrailsDtlVO>();
		try{	
              query="select VALUE,DESCRIPTION from generic_master where GENERIC_KEY='COLL_RESOLUTION_STAT' and rec_status ='A'";              		
...
public ArrayList<Object> getContactListApplicant() {
	...
	try
	{		
		String query=("select VALUE,DESCRIPTION FROM generic_master  WHERE GENERIC_KEY='CONTACT_MODE'  AND REC_STATUS='A'");
		logger.info("in getIndividualEmplomentList() of CorpotateDAOImpl  Query : "+query);
		ContactRecordingFollowUpVO vo = null;
		ArrayList source = ConnectionDAO.sqlSelect(query);
		logger.info("Size : "+source.size());
		int size =source.size();
		for(int i=0;i<size;i++)
		{
			ArrayList subApplicantDoc=(ArrayList)source.get(i);
			if(subApplicantDoc.size()>0)
			{
				vo = new ContactRecordingFollowUpVO();
				vo.setContactCodeType((CommonFunction.checkNull(subApplicantDoc.get(0)).toString()));
				vo.setContactCode((CommonFunction.checkNull(subApplicantDoc.get(1)).toString()));
				list.add(vo);
			}
		}
	}catch(Exception e){
...
//Rohit Changes end for drop down on contact mode



//Rohit Changes Starts for drop down on Place contacted 
/*@Override
public ArrayList<Object> getContactedPlace() {
	
	ArrayList<Object> mainList=new ArrayList<Object>();
	
	try{
	String query=("select VALUE,DESCRIPTION FROM generic_master  WHERE GENERIC_KEY='CONTACT_PLACE'  AND REC_STATUS='A'");
	logger.info("query for the ContactedPlace *****"+query);
	...
	for(int i=0;i<list.size();i++){
		...
		if(data.size()>0){
			ContactRecordingFollowUpVO relatVO=new ContactRecordingFollowUpVO();
			relatVO.setContactPlaceCode(CommonFunction.checkNull(data.get(0)));
			relatVO.setContactPlaceDesc(CommonFunction.checkNull(data.get(1)));
...
//Rohit Changes end for drop down on Place contacted 
*/
//}