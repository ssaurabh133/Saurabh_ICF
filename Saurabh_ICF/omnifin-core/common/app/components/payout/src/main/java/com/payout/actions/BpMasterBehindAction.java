...
public class BpMasterBehindAction extends Action{
	private static final Logger logger = Logger.getLogger(BpMasterBehindAction.class.getName());
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		logger.info("In BpMasterBehindAction................");
		HttpSession session = request.getSession();
		boolean flag=false;
		UserObject userobj=(UserObject)session.getAttribute("userobject");
		if(userobj==null){
			logger.info("here execute method of BpMasterBehindAction action the session is out----------------");
		...
		ArrayList<BPMasterVO> list=new ArrayList<BPMasterVO>();
		  ...
		  logger.info("current page link .......... "+request.getParameter("d-49520-p"));
			...
			bpMasterVO.setCurrentPageLink(currentPageLink);
			list= payMaster.searchBpMasterData(bpMasterVO);
