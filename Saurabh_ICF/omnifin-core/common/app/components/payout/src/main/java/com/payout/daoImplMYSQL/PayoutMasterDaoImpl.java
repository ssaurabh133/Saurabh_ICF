package com.payout.daoImplMYSQL;
...
public class PayoutMasterDaoImpl implements PayoutMasterDAO{

	private static final Logger logger = Logger.getLogger(PayoutMasterDaoImpl.class.getName());
	...
	try {
		...
		bufInsSql.append("if(A.REC_STATUS='A','Active','Inactive')as REC_STATUS FROM pay_activity_m a where 'a'='a' ");
	...
	public String updateActivityCode(ActivityMasterVO vo) {
	 ...
	 try {
			...
			if (vo.getRecStatus()!= null && vo.getRecStatus().equals("on")) {
				...
				bufInsSql.append(" MAKER_ID=?,MAKER_DATE= DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");
				bufInsSql.append("AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) ");
	...
	try {
		...
		bufInsSql.append("SELECT a.ACTIVITY_CODE,a.ACTIVITY_DESC,a.MAPPING_SOURCE_ID,(SELECT SOURCE_NAME FROM PAY_SOURCE_M b WHERE a.MAPPING_SOURCE_ID=b.SOURCE_ID) SOURCE_NAME,MAPPING_CODE,");
		bufInsSql.append("if(A.REC_STATUS='A','Active','Inactive')as REC_STATUS FROM pay_activity_m a where a.ACTIVITY_CODE='"+codeId+"' ");
	...
	public ArrayList<TaxMasterVO> openEditTax(TaxMasterVO vo) {
		...
		try {
			...
			bufInsSql.append(" IF(A.REC_STATUS='A','Active','Inactive')as REC_STATUS FROM PAY_TAX_M A where TAX_ID='"+CommonFunction.checkNull(vo.getTaxId())+"' ");
	...
	public String saveTaxMaster(TaxMasterVO vo) {
	 ...
	 try {
		 ...
		 String query = "Select TAX_DESC from pay_tax_m where TAX_DESC='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull( vo.getTaxName()))+"' and  ifnull(ACTIVITY_CODE,'')='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull( vo.getLbxActivityCode()))+"' and ifnull(STATE_ID,'') ='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull( vo.getTxtStateCode().trim())) + "'";
	...
	public ArrayList<TaxMasterVO> searchTaxMasterData(TaxMasterVO vo) {
		...
		try {
			...
			bufInsSql.append(" IF(A.REC_STATUS='A','Active','Inactive')as REC_STATUS FROM PAY_TAX_M A join pay_activity_m B  on B.ACTIVITY_CODE=A.ACTIVITY_CODE where 'a'='a' ");
	...
	public ArrayList<BPMasterVO> openEditBp(BPMasterVO vo) {
		...
		try {
			...
			bufInsSql.append("IF(a.REC_STATUS='A','Active','Inactive') STATUS from pay_bp_m a ");
	...
	public ArrayList<BPMasterVO> searchBpMasterData(BPMasterVO vo) {
		...
		try {
			...
			bufInsSql.append("IF(a.REC_STATUS='A','Active','Inactive') STATUS from pay_bp_m a join pay_bp_activity_map b");
			...
			bufInsSqlTempCount.append(" IF(a.REC_STATUS='A','Active','Inactive') STATUS from pay_bp_m a join pay_bp_activity_map b");
	 ...
	 try {
		 ...
		 String query = "Select  SCHEME_NAME from pay_scheme_m where SCHEME_NAME='"+CommonFunction.checkNull(vo.getSchemeName())+"'";
			...
			if (vo.getTdsApp()!= null && vo.getTdsApp().equals("on")) {
				...
				bufInsSql.append("DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//EFFECTIVE_DATE
	...
	public ArrayList<SchemeVO> searchScemeMasterData(SchemeVO vo) {
		...
		try {
			...
			bufInsSql.append("Select SCHEME_NAME,NARRATION,DATE_FORMAT(EFFECTIVE_DATE,'"+dateFormat+"'),");
			...
			bufInsSql.append("if(REC_STATUS='A','Active','Inactive')as REC_STATUS from pay_scheme_m  where 'a'='a'");
	...
	public ArrayList<SchemeVO> openEditSchemeM(SchemeVO vo) {
		...
		try {
			...
			bufInsSql.append("Select SCHEME_NAME,NARRATION,DATE_FORMAT(EFFECTIVE_DATE,'%d-%m-%Y'),SCHEME_PARAMETER,");
			...
			bufInsSql.append(" if(A.REC_STATUS='A','Active','Inactive')as REC_STATUS,SLAB_ON from pay_scheme_m a  where SCHEME_NAME ='"+vo.getSchemeName()+"'");	
	...
	public String updateSchemeMaster(SchemeVO vo) {
	 ...
	 try {
			...
			if (vo.getTdsApp()!= null && vo.getTdsApp().equals("on")) {
				...
				bufInsSql.append("Update pay_scheme_m SET NARRATION=?,EFFECTIVE_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),SCHEME_PARAMETER=?,SLAB_ON=?,");				
				bufInsSql.append(" SERVICE_TAX_APPLICABLE=?,TDS_APPLICABLE=?,TAT=?,CITY_ID=?,REC_STATUS=?,");
				bufInsSql.append(" MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)");//SCHEME_NAME
	...
	public ArrayList<SchemeBpMapVO> searchScemeBpMapData(SchemeBpMapVO vo) {
		...
		try {
			...
			bufInsSql.append(" Select a.SCHEME_BP_MAP_ID,b.BP_NAME,a.SCHEME_NAME,d.NARRATION,c.ACTIVITY_DESC,if(a.REC_STATUS='A','Active','Inactive')as REC_STATUS");
	...
	public ArrayList<SchemeBpMapVO> openEditSchemeBPMap(SchemeBpMapVO vo) {
		...
		try {
			...
			bufInsSql.append(" A.TARGET,A.TARGET_COMMISION,A.PRIORITY,if(a.REC_STATUS='A','Active','Inactive')as REC_STATUS,SPECIFIC_TARGET,SPECIFIC_TARGET_ON ");
			bufInsSql.append(" from pay_scheme_bp_map a join pay_bp_m b on a.BP_ID=b.BP_ID ");
	 ...
	 try {
		...
		if (!st) {
			...
			if (vo.getSpecificTar()!= null && vo.getSpecificTar().equals("on")) {
				...
				bufInsSql.append(" AUTHOR_ID=?,");
				bufInsSql.append(" AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)");
	...
	public ArrayList<PaymentReceiptVO> searchPaymentReceiptMaker(
		...
		try {
			...
			bufInsSqlTempCount.append("SELECT COUNT(1) FROM (");
			bufInsSqlTempCount.append("Select a.PAYMENT_ID,b.BP_NAME,a.PAYMENT_TYPE,a.PAYMENT_MODE,DATE_FORMAT(a.PAYMENT_DATE,'"+dateFormat+"'),a.AMOUNT,a.remark");
	...
	public String savePaymentReceiptMaker(PaymentReceiptVO vo) {
	 ...
	 try {
				...
				bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//PAYMENT_DATE 
	...
	public ArrayList<PaymentReceiptVO> openEditPaymentReceiptMaker(
		...
		try {
			...
			bufInsSql.append("Select a.PAYMENT_ID,a.BP_ID,b.BP_NAME,a.PAYMENT_TYPE,a.PAYMENT_MODE,DATE_FORMAT(a.PAYMENT_DATE,'"+dateFormat+"'),a.AMOUNT,a.INSTRUMENT_NO,DATE_FORMAT(a.INSTRUMENT_DATE,'"+dateFormat+"'),a.REMARK");
	...
	public String updatePaymentReceiptMaker(PaymentReceiptVO vo,String recStatus) {
	 ...
	 try {
				...
				bufInsSql.append("update  pay_receipt_payment_dtl set BP_ID=?,PAYMENT_TYPE=?,PAYMENT_MODE=?,INSTRUMENT_NO=?,INSTRUMENT_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),PAYMENT_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),AMOUNT=?,REMARK=?,REC_STATUS=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) ");				
	...
	public  ArrayList<PaymentReceiptVO> allocatePaymentReceiptBehindList(PaymentReceiptVO vo){
		...
		try {
			...
			bufInsSql.append("  ifnull(ORG_TOTAL_AMOUNT,0) ORG_TOTAL_AMOUNT,ifnull(ORG_TOTAL_AMOUNT,0)-ifnull(BALANCE_AMOUNT,0) BALANCE_AMOUNT ,"); 
			...
			bufInsSql.append(" where BP_ID='"+vo.getLbxBpId()+"' and  (ifnull(ORG_TOTAL_AMOUNT,0) -  ifnull(TOTAL_RECIVED_AMOUNT,0)) > 0 and");
			...
			bufInsSql.append(" (Select ACTIVITY_DESC FROM PAY_ACTIVITY_M C WHERE A.ACTIVITY_CODE=C.ACTIVITY_CODE) ACTIVITY_DESC,");
			bufInsSql.append(" IFNULL( GROSS_INSENTIVE_AMOUNT,0),IFNULL( GROSS_INSENTIVE_AMOUNT,0)-IFNULL( TOTAL_RECIVED_AMOUNT,0) BALANCE_AMOUNT,"); 
			...
			bufInsSql.append(" where BP_ID='"+vo.getLbxBpId()+"' and  (ifnull(GROSS_INSENTIVE_AMOUNT,0) -  ifnull(TOTAL_RECIVED_AMOUNT,0)) > 0");
	 ...
	 try {
		   ...
		   logger.info("In saveAllocatePaymentReceipt.....................................Dao Impl" + query);
		    ...
		    int size=txnCaseId.size();
		     // bufInsSqlTxn.append("update pay_transaction_case_dtl  set TOTAL_RECIVED_AMOUNT=ifnull(TOTAL_RECIVED_AMOUNT,0)+?,BALANCE_AMOUNT=ORG_TOTAL_AMOUNT-TOTAL_RECIVED_AMOUNT where ");
...
//		String stateId="";
		...
		try {
			...
			bufInsSql.append("SELECT pam.ACTIVITY_DESC,pbsm.START_DAY,pbsm.END_DAY,if(pbsm.REC_STATUS='A','Active','Inactive')as REC_STATUS ,pbsm.BATCH_SCH_ID,pbsm.START_MONTH,pbsm.END_MONTH FROM pay_batch_sch_m pbsm inner join pay_activity_m pam on 'a'='a' and pam.ACTIVITY_CODE=pbsm.ACTIVITY_CODE");
...
//			 if (!st) {
	...
	public ArrayList<ScheduleMasterVO> openEditScheduleMaster(
		...
		try {
			...
			bufInsSql.append("SELECT pam.ACTIVITY_DESC,pbsm.START_DAY,pbsm.END_DAY,IF(pbsm.REC_STATUS='A','Active','Inactive')as REC_STATUS ,pbsm.BATCH_SCH_ID,pbsm.ACTIVITY_CODE ,pbsm.START_MONTH,pbsm.END_MONTH from pay_batch_sch_m pbsm inner join pay_activity_m pam");
