...
public class SchemeMasterBehindAction extends Action{
	private static final Logger logger = Logger.getLogger(SchemeMasterBehindAction.class.getName());
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		logger.info("In SchemeMasterBehindAction................");
		HttpSession session = request.getSession();
		boolean flag=false;
		UserObject userobj=(UserObject)session.getAttribute("userobject");
		if(userobj==null){
			logger.info("here execute method of SchemeMasterBehindAction action the session is out----------------");
		...
		ArrayList<SchemeVO> list=new ArrayList<SchemeVO>();
		  ...
		  logger.info("current page link .......... "+request.getParameter("d-49520-p"));
			...
			schemeVO.setCurrentPageLink(currentPageLink);
			list= payMaster.searchScemeMasterData(schemeVO);
