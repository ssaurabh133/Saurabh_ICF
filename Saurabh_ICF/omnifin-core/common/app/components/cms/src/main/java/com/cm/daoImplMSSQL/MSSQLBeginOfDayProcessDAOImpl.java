...
public class MSSQLBeginOfDayProcessDAOImpl implements BeginOfDayProcessDAO{

	private static final Logger logger = Logger.getLogger(MSSQLBeginOfDayProcessDAOImpl.class.getName());
	...
	int no=Integer.parseInt(resource.getString("msg.pageSizeForMaster"));
	 ...
	 public ArrayList showBodData(String busnissDate)
	 {
		...
		try
		    ...
		    query.append(" LEFT OUTER JOIN EOD_BOD_PROCESS_LOG_DTL B ON A.PROCESS_NAME=B.PROCESS_NAME AND  " );
		    query.append("B.EOD_BOD_PROCESS_ID IN  (SELECT MAX(EOD_BOD_PROCESS_ID) FROM EOD_BOD_PROCESS_LOG_DTL " );
		    query.append("where ");
		    query.append(dbo);
		    query.append("DATE_FORMAT(PROCESS_DATE,'"+dateFormat+"')='"+busnissDate+"' GROUP BY PROCESS_NAME) " );
		    query.append("where a.REC_STATUS='A' And A.PROCESS_TYPE='B'");
	 ...
	 public String getBodProcessStatus()
		{
		 logger.info("In getBodProcessStatus() method of  BeginOfDayProcessDAOImpl");
			...
			logger.info("In....getBodProcessStatus"+query);
			status=ConnectionDAO.singleReturn(query);
