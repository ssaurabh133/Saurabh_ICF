...
public class ChangeRateDAOImpl implements ChangeRateDAO{
	private static final Logger logger = Logger.getLogger(ChangeRateDAOImpl.class.getName());
		...
		public ArrayList<ChangeRateVo> nonEmiBasedMakerGrid(Object ob, String type) 
	{
			...
			try {
				...
				bufInsSql.append(" NEW_LOAN_FINAL_RATE FROM cr_non_emi_based_loan_dtl WHERE REC_STATUS='"+type+"'   ");
		...
		public ArrayList<ChangeRateVo> getValueForChangeRate(String lbxLoanID) {
			...
			try{
		        ...
		        bufInsSql.append(" SELECT C.LOAN_LOAN_AMOUNT,C.LOAN_BALANCE_PRINCIPAL,IFNULL(C.LOAN_FINAL_RATE,0)," +
			...
			logger.info("In updateMakerData......................");
			 ArrayList qryList = new ArrayList();
				...
				try{					
					...
					UpdatedQry.append(" UPDATE cr_non_emi_based_loan_dtl SET LOAN_FINAL_RATE=?,NEW_LOAN_FINAL_RATE=?,MAKER_ID=?,MAKER_DATE=STR_TO_DATE(?,'"+dateFormat+"'),REC_STATUS='P' ");
...
//							updatePrepStmtObject.addString((NoEmiBasedVo.getMakerDate()).trim());
		...
		public boolean saveAuthorDecision(Object ob) 
		{
			...
			query1.append("update cr_non_emi_based_loan_dtl set REC_STATUS='"+vo.getDecision()+"',AUTHOR_ID='"+vo.getMakerId()+"',AUTHOR_DATE=STR_TO_DATE('"+vo.getMakerDate()+"','"+dateFormatWithTime+"') ,AUTHOR_COMMENTS='"+vo.getComments()+"' where LOAN_ID='"+CommonFunction.checkNull(vo.getLbxLoanNo())+"' ");
			logger.info("query1:::::::::::::::::::::::::: "+query1.toString());
			qryList.add(query1);
	        try {
	        	fwdStatus = ConnectionDAO.sqlInsUpdDelete(qryList);
				...
				" cr_non_emi_based_loan_dtl d WHERE d.LOAN_ID='"+CommonFunction.checkNull(vo.getLbxLoanNo())+"')-IFNULL(LOAN_BASE_RATE ,0)," +
		...
		public String getMakerDate(String loanId) 
		{
			...
			query.append("select date_format(MAKER_DATE,'%d-%m-%Y') from cr_non_emi_based_loan_dtl where LOAN_ID="+loanId.trim());
