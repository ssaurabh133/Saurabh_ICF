...
public class DeleteInstrumentDAOImpl implements DeleteInstrumentDAO {
	private static final Logger logger = Logger.getLogger(DeleteInstrumentDAOImpl.class.getName());
...
//	                  		" where a.LOAN_REPAYMENT_TYPE='I' AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"'");
	                  
	                  //SIDHARTH change in query user_id from search removed as  query taking time
	                  bufInsSql.append("select DISTINCT c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc," +
	                  		"	d.SCHEME_DESC,DATE_FORMAT(a.LOAN_APPROVAL_DATE,'%d-%m-%Y'),f.CUSTOMER_NAME from  cr_loan_dtl a " +
	                  		"  join cr_pdc_instrument_dtl C ON(a.LOAN_ID=c.PDC_LOAN_ID AND C.PDC_STATUS in ('A','H','L'))" +
	                  		" LEFT JOIN cr_pdc_hold_reason_dtl HL ON(C.pdc_instrument_id=HL.pdc_instrument_id )  " +
	                  		" AND IFNULL(HL.main_status,'A')<>'F'  " +
	                  		"join cr_product_m b on(a.LOAN_PRODUCT = b.PRODUCT_ID) " +
	                  		"join cr_scheme_m d on(a.LOAN_SCHEME = d.SCHEME_ID) " +
	                  		" join com_branch_m e on(e.branch_id=a.loan_branch ) " +
	                  		"join gcd_customer_m f  on(f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID) " +
	                  		" where 1=1"); 
	                 
	                  //SIDHARTH change in query bcs prev query taking time ends
  
	                  
//	                  sbAppendToSQLCount.append("select distinct count(1) from(select c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc,d.SCHEME_DESC,  DATE_FORMAT(a.LOAN_APPROVAL_DATE,'%d-%m-%Y'),f.CUSTOMER_NAME ," +
//		                  		"(SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=c.MAKER_ID) USERID from  cr_loan_dtl a " +
...
//				  sbAppendToSQLCount.append(" SELECT COUNT(DISTINCT C.PDC_LOAN_ID) FROM  CR_LOAN_DTL A   JOIN CR_PDC_INSTRUMENT_DTL C ON(A.LOAN_ID=C.PDC_LOAN_ID) ");  
//				  sbAppendToSQLCount.append(" JOIN( SELECT MAX(PDC_INSTRUMENT_ID)ID,PDC_LOAN_ID,MAKER_ID FROM CR_PDC_INSTRUMENT_DTL X WHERE PDC_STATUS IN ('A','H','L') "); 
//				  sbAppendToSQLCount.append(" AND  NOT EXISTS (SELECT 1 FROM CR_PDC_HOLD_REASON_DTL Y WHERE X.PDC_INSTRUMENT_ID= Y.PDC_INSTRUMENT_ID AND Y.MAIN_STATUS IN ('F')) GROUP BY PDC_LOAN_ID ) D  ");
//				  sbAppendToSQLCount.append(" ON(D.PDC_LOAN_ID=C.PDC_LOAN_ID)   ");
	                  
	                  sbAppendToSQLCount.append(" SELECT COUNT(DISTINCT C.PDC_LOAN_ID) FROM  CR_LOAN_DTL A   JOIN CR_PDC_INSTRUMENT_DTL C ON(A.LOAN_ID=C.PDC_LOAN_ID AND C.PDC_STATUS IN ('A','H','L')) ");  
				  ...
				  sbAppendToSQLCount.append(" WHERE A.LOAN_REPAYMENT_TYPE='I' AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"' ");
				  sbAppendToSQLCount.append(" AND  NOT EXISTS (SELECT 1 FROM CR_PDC_HOLD_REASON_DTL Y WHERE C.PDC_INSTRUMENT_ID= Y.PDC_INSTRUMENT_ID AND Y.MAIN_STATUS ='F') ");
...
//		                  sbAppendToSQLCount.append(")as b");
			    			...
			    			logger.info("In searchDeleteMaker() of DeleteInstrumentDAOImpl count query : "+sbAppendToSQLCount.toString());
...
//		    			}
		    			logger.info("In searchDeleteMaker() of DeleteInstrumentDAOImpl main query : "+bufInsSql.toString());
...
//		}
	        ...
	        try{
	               ...
	               bufInsSql.append( " select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc,d.SCHEME_DESC, "+ 
	                          ...
	                          " and c.PDC_INSTRUMENT_ID in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F') and hold_reason_status in ('X')  AND MAKER_ID!='"+instructionCapMakerVO.getUserID()+"') ");
...
//			 hese values cant be null
				...
				try{
						...
						if((instructionCapMakerVO.getDecision()).equalsIgnoreCase("X"))
						{
							...
							for(int i=0;i<instrumentID.length;i++)
							{
								...
								query.append("update cr_pdc_instrument_dtl set pdc_status=(select to_status from cr_pdc_hold_reason_dtl where main_status='F' and  pdc_instrument_id='"+instrumentID[i]+"' limit 1 )," +
								" pdc_remarks='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getComments()).trim())+"',author_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getMakerID()).trim())+"',author_date=DATE_ADD(STR_TO_DATE('"+instructionCapMakerVO.getMakerDate()+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)  where pdc_instrument_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instrumentID[i]).trim())+"' ");
				...
				}else
				{
					for(int i=0;i<instrumentID.length;i++){
						...
						query.append("update cr_pdc_instrument_dtl set pdc_status=(select from_status from cr_pdc_hold_reason_dtl where main_status = 'F' and pdc_instrument_id='"+instrumentID[i]+"' limit 1 )," +
