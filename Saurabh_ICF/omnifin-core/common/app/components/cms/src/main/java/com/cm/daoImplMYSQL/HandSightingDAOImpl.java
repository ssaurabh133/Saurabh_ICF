...
public class HandSightingDAOImpl implements HandSightingDAO {

	private static final Logger logger = Logger.getLogger(HandSightingDAOImpl.class.getName());
		...
		try {
			...
			if(status){
				...
				PrepStmtObject updatePrepStmtObject = new PrepStmtObject();
					queryUpdate.append("update cr_loan_dtl set HAND_SIGHTING_RECEIVED_DATE=STR_TO_DATE(?,'"+dateFormat+ "'),HAND_SIGHTING_STATUS=?  where LOAN_ID=? ");
	...
	public boolean updateFileTracking(Object ob) {
		...
		try {
			...
			queryUpdate
					.append("update CR_HAND_SIGHTING_DTL set RECEIVED_DATE=STR_TO_DATE(?,'"+dateFormat+ "'),HAND_SIGHTING_STATUS=?,REMARKS=?  where LOAN_ID=? ");
			...
			if(status){
				...
				PrepStmtObject updatePrepStmtObject = new PrepStmtObject();
				queryUpdateLoan.append("update cr_loan_dtl set HAND_SIGHTING_RECEIVED_DATE=STR_TO_DATE(?,'"+dateFormat+ "'),HAND_SIGHTING_STATUS=?  where LOAN_ID=? ");
	...
	public ArrayList getFileTrackingData(Object ob) {
		...
		try {
			...
			bufInsSql.append("select loan_no,loan_id,DATE_FORMAT(received_date,'"+dateFormat+"'),HAND_SIGHTING_STATUS,remarks,DEAL_ID,s.USER_NAME  from CR_HAND_SIGHTING_DTL t join sec_user_m s on t.maker_id=s.USER_ID where t.loan_id='"
	...
	public ArrayList getFileTrackingDataForView(Object ob) {
		...
		try {
			...
			bufInsSql
					.append("SELECT TRACKING_ID,LOAN_ID,LOAN_NO,DATE_FORMAT(RECEIVED_DATE,'"+dateFormat+"'),HAND_SIGHTING_STATUS,REMARKS,DEAL_ID,s.USER_NAME FROM CR_HAND_SIGHTING_DTL t join sec_user_m s on t.maker_id=s.USER_ID where loan_id='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(fileTrackVo.getLbxLoanNoHIDSearch()).trim())+ "' ");
			bufInsSqlTempCount
