...
//	String userName="";
	...
	query.append("select (sum(ifnull(b.pmnt_amount,0))) as activePayment from cr_txnadvice_dtl a, cr_pmnt_dtl b,");
	...
	public String getMakerDate(String loanId) 
	{
		String makeDate="";
		StringBuilder query = new StringBuilder();
		query.append("select date_format(MAKER_DATE,'%d-%m-%Y') from cr_termination_dtl where LOAN_ID="+loanId.trim());
...
//		 in.add(CommonFunction.checkNull(vo.getMakerDate()).trim());//
		 ...
		 String date1=CommonFunction.changeFormat(CommonFunction.checkNull(vo.getMakerDate()).trim());
      	    if(date1 != null)
      	    	in.add(date1);
      	in.add(CommonFunction.checkNull("L").trim());
      	out.add(s3);
		out.add(s1);
 	    out.add(s2);
...
//		con = null;
	...
	try {
		...
		query.append(" termination_date=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");
		query.append(" rec_status=?, maker_id=?, maker_date=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)");
...
//		String query1 = "";
		...
		try {
			...
			query1.append("SELECT period_diff(date_format(loan_maturity_date,'%Y%m'),date_format(STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(businessDate).trim())+"','"+dateFormat+"'),'%Y%m')) as remaining_tenure from cr_loan_dtl where loan_id='"
