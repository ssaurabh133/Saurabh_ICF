...
import com.cm.vo.LoanDetailUploadVo;
import com.connect.ConnectionDAO;
import com.connect.DaoImplInstanceFactory;
import com.login.dao.UserSessionCheck;
import com.login.roleManager.UserObject;

public class LoanDetailUploadDispatchAction extends DispatchAction {
	...
	public ActionForward uploadData(ActionMapping mapping,ActionForm form,
		...
		logger.info(" in uploadData method of LoanDetailUploadBehindAction:::::::::::::::::::::::");

		
		HttpSession session = request.getSession();	
		UserObject userobj=(UserObject)session.getAttribute("userobject");
		if(userobj==null){
			logger.info("in  uploadData method of  LoanDetailUploadBehindAction action the session is out----------------");
		...
		LoanDetailUploadVo  loanDetailUploadVo=new LoanDetailUploadVo();
	    DynaValidatorForm manualDeviationDynaValidatorForm=(DynaValidatorForm)form;	    
	    org.apache.commons.beanutils.BeanUtils.copyProperties(loanDetailUploadVo, manualDeviationDynaValidatorForm);
		...
		boolean uploadStatus=false;	
			...
			uploadStatus=service.whereToUpload(request,loanDetailUploadVo.getDocFile());			
			loanDetailUploadVo.setFileName(request.getAttribute("fileName").toString());
			loanDetailUploadVo.setDocPath(request.getAttribute("filePath").toString());
			...
			if(uploadStatus)
			{
				int count=service.countLine(loanDetailUploadVo.getDocFile().toString());
				logger.info("In uploadData() : count:==>>"+count+"loanDetailUploadVo.getDocFile().toString():::::::"+loanDetailUploadVo.getDocFile().toString());
				if(count>1000)
				{
				...
				session.setAttribute("Processfile",loanDetailUploadVo.getDocFile().toString());
	...
	public ActionForward startProcess(ActionMapping mapping,ActionForm form,
			HttpServletRequest request,HttpServletResponse response) throws Exception {
		logger.info(" in startProcess method of LoanDetailUploadBehindAction:::::::::::::::::::::::");
		...
		LoanDetailUploadVo  loanDetailUploadVo=new LoanDetailUploadVo();
	    DynaValidatorForm loanDetailDynaValidatorForm=(DynaValidatorForm)form;	    
	    org.apache.commons.beanutils.BeanUtils.copyProperties(loanDetailUploadVo, loanDetailDynaValidatorForm);
		...
		logger.info("startProcess() : file:==>>"+file);
			...
			boolean status=service.readExcelforLoanDetailUpload(request, response,file);
			if(status)
			{		
			...
			HttpServletRequest request, HttpServletResponse response) throws Exception 
	{
			...
			if(userobj!=null)
			{
				...
				logger.info("in  errorLogDownload method of LoanDetailUploadBehindAction action the session is out----------------");
				return mapping.findForward("sessionOut");
			}
			Object sessionId = session.getAttribute("sessionID");
			//for check User session start
			...
			String reportName="rpt_loan_detail_upload";
