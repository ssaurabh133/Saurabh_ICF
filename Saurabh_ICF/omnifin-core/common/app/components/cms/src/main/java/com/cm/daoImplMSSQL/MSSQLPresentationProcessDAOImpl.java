...
public class MSSQLPresentationProcessDAOImpl implements PresentationProcessDAO {
	private static final Logger logger = Logger.getLogger(MSSQLPresentationProcessDAOImpl.class.getName());
...
//		        
	...
	{
		String batch_id="0";
		String query="select max(BATCH_ID) from presentation_process_upload_temp  WITH (ROWLOCK)  where MAKER_ID='"+maker_id.trim()+"'";
		logger.info("Query for gettiing max(BAtch_id)  :  "+query);
		try
		{
			batch_id=ConnectionUploadDAO.singleReturn(query);
