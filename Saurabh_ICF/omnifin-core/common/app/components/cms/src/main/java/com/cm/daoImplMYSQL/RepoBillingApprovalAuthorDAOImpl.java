...
public class RepoBillingApprovalAuthorDAOImpl implements RepoBillingApprovalAuthorDAO {

	private static final Logger logger = Logger.getLogger(RepoBillingApprovalAuthorDAOImpl.class.getName());
	...
	public boolean saveRepoBillingApprovalChecker(Object ob) {
		...
		try {
			...
			if (CommonFunction.checkNull(st).equalsIgnoreCase("A")) {
				...
				StringBuilder bufInsSql = new StringBuilder();
				bufInsSql.append("UPDATE CR_LOAN_REPO_APPROVAL_DTL SET REC_STATUS=?,AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) WHERE LOAN_ID='"+ vo.getLbxDealNo()+"' ");
