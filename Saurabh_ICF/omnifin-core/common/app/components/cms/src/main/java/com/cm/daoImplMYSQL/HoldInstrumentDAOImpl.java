...
public class HoldInstrumentDAOImpl implements HoldInstrumentDAO {
	private static final Logger logger = Logger.getLogger(HoldInstrumentDAOImpl.class.getName());
...
//	        String userName="";
     ...
     // bufInsSql.append( " select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc,d.SCHEME_DESC,  DATE_FORMAT(a.LOAN_APPROVAL_DATE,'%d-%m-%Y'),f.CUSTOMER_NAME, us.USER_NAME, ifnull(tab.USER_NAME,'')from cr_loan_dtl a join cr_pdc_instrument_dtl c on(a.LOAN_ID=c.PDC_LOAN_ID )join cr_product_m b  on(a.LOAN_PRODUCT = b.PRODUCT_ID )join cr_scheme_m d on(a.LOAN_SCHEME = d.SCHEME_ID )join com_branch_m e  on(e.branch_id=a.loan_branch )join gcd_customer_m f on(f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID )left outer join  (select main.pdc_loan_id,user.USER_NAME from (select max(b.pdc_instrument_id)id,a.pdc_loan_id from cr_pdc_instrument_dtl a join  cr_pdc_hold_reason_dtl b on(a.pdc_instrument_id=b.pdc_instrument_id and b.to_status='H' and b.hold_reason_status='H' and b.main_status='F')group by a.pdc_loan_id )main join  cr_pdc_hold_reason_dtl  holdInst on(main.id=holdInst.pdc_instrument_id and holdInst.to_status='H' and holdInst.hold_reason_status='H' and holdInst.main_status='F')join sec_user_m user on(user.USER_ID=holdInst.maker_id)) tab  on(tab.pdc_loan_id=a.LOAN_ID)left outer join (select m.pdc_loan_id,u.user_name from (select max(pdc_instrument_id)id,pdc_loan_id from cr_pdc_instrument_dtl group by pdc_loan_id order by pdc_loan_id)m join cr_pdc_instrument_dtl c on(m.id=c.pdc_instrument_id )join SEC_USER_M u on(u.USER_ID=c.MAKER_ID)) us on(us.pdc_loan_id=c.pdc_loan_id) where  c.PDC_STATUS in ('A','H')  and c.pdc_purpose='INSTALLMENT' AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"' ");
	    ...
	    bufInsSql.append( " select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc,d.SCHEME_DESC,DATE_FORMAT(a.LOAN_APPROVAL_DATE,'%d-%m-%Y'),f.CUSTOMER_NAME, ifnull(tab.USER_NAME,'')from cr_loan_dtl a join cr_pdc_instrument_dtl c on(a.LOAN_ID=c.PDC_LOAN_ID )join cr_product_m b  on(a.LOAN_PRODUCT = b.PRODUCT_ID )join cr_scheme_m d on(a.LOAN_SCHEME = d.SCHEME_ID )join com_branch_m e  on(e.branch_id=a.loan_branch )join gcd_customer_m f on(f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID )left outer join  (select main.pdc_loan_id,user.USER_NAME from (select max(b.pdc_instrument_id)id,a.pdc_loan_id from cr_pdc_instrument_dtl a 		join  cr_pdc_hold_reason_dtl b on(a.pdc_instrument_id=b.pdc_instrument_id and b.to_status='H' and b.hold_reason_status='H' 		and b.main_status='F')group by a.pdc_loan_id )main join  cr_pdc_hold_reason_dtl  holdInst on(main.id=holdInst.pdc_instrument_id  and holdInst.to_status='H' and holdInst.hold_reason_status='H' and holdInst.main_status='F') join sec_user_m user on(user.USER_ID=holdInst.maker_id)) tab  on(tab.pdc_loan_id=a.LOAN_ID) where  c.PDC_STATUS in ('A','H')  and c.pdc_purpose='INSTALLMENT' ");
//      sbAppendToSQLCount.append( "select distinct count(1) from(select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc,d.SCHEME_DESC,  DATE_FORMAT(a.LOAN_APPROVAL_DATE,'%d-%m-%Y'),f.CUSTOMER_NAME, us.USER_NAME, ifnull(tab.USER_NAME,'')from cr_loan_dtl a join cr_pdc_instrument_dtl c on(a.LOAN_ID=c.PDC_LOAN_ID )join cr_product_m b  on(a.LOAN_PRODUCT = b.PRODUCT_ID )join cr_scheme_m d on(a.LOAN_SCHEME = d.SCHEME_ID )join com_branch_m e  on(e.branch_id=a.loan_branch )join gcd_customer_m f on(f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID )left outer join  (select main.pdc_loan_id,user.USER_NAME from (select max(b.pdc_instrument_id)id,a.pdc_loan_id from cr_pdc_instrument_dtl a join  cr_pdc_hold_reason_dtl b on(a.pdc_instrument_id=b.pdc_instrument_id and b.to_status='H' and b.hold_reason_status='H' and b.main_status='F')group by a.pdc_loan_id )main join  cr_pdc_hold_reason_dtl  holdInst on(main.id=holdInst.pdc_instrument_id and holdInst.to_status='H' and holdInst.hold_reason_status='H' and holdInst.main_status='F')join sec_user_m user on(user.USER_ID=holdInst.maker_id)) tab  on(tab.pdc_loan_id=a.LOAN_ID)left outer join (select m.pdc_loan_id,u.user_name from (select max(pdc_instrument_id)id,pdc_loan_id from cr_pdc_instrument_dtl group by pdc_loan_id order by pdc_loan_id)m join cr_pdc_instrument_dtl c on(m.id=c.pdc_instrument_id )join SEC_USER_M u on(u.USER_ID=c.MAKER_ID)) us on(us.pdc_loan_id=c.pdc_loan_id) where a.LOAN_REPAYMENT_TYPE='I' and c.PDC_STATUS in ('A','H')  and c.pdc_purpose='INSTALLMENT' AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"' ");

		  sbAppendToSQLCount.append(" SELECT COUNT(DISTINCT C.PDC_LOAN_ID) FROM CR_LOAN_DTL A "); 
		  sbAppendToSQLCount.append(" JOIN CR_PDC_INSTRUMENT_DTL C ON(A.LOAN_ID=C.PDC_LOAN_ID AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"')");
		  if((!((customerName.toString()).equalsIgnoreCase("")))) {
			  sbAppendToSQLCount.append(" JOIN GCD_CUSTOMER_M F ON(F.CUSTOMER_ID = A.LOAN_CUSTOMER_ID) ");
		  }
		  sbAppendToSQLCount.append(" LEFT OUTER JOIN  ( SELECT MAIN.PDC_LOAN_ID,USER.USER_NAME  ");
		  sbAppendToSQLCount.append(" FROM (SELECT MAX(B.PDC_INSTRUMENT_ID)ID,A.PDC_LOAN_ID ,B.MAKER_ID FROM CR_PDC_INSTRUMENT_DTL A "); 
		  sbAppendToSQLCount.append(" JOIN  CR_PDC_HOLD_REASON_DTL B ON(A.PDC_INSTRUMENT_ID=B.PDC_INSTRUMENT_ID AND B.TO_STATUS='H' AND B.HOLD_REASON_STATUS='H' AND B.MAIN_STATUS='F') GROUP BY A.PDC_LOAN_ID )MAIN "); 
		  sbAppendToSQLCount.append(" JOIN SEC_USER_M USER ON(USER.USER_ID=MAIN.MAKER_ID)) TAB  ON(TAB.PDC_LOAN_ID=A.LOAN_ID) ");
		  //sbAppendToSQLCount.append(" WHERE A.LOAN_REPAYMENT_TYPE='I'    ");
		  sbAppendToSQLCount.append(" WHERE C.PDC_STATUS IN ('A','H')   ");
		  sbAppendToSQLCount.append(" AND C.PDC_PURPOSE='INSTALLMENT' ");
...
//	                  sbAppendToSQLCount.append(" )as b"); 
		    			...
		    			logger.info("in searchHoldInstrumentMaker() of HoldInstrumentDAOImpl COUNT query :::: "+sbAppendToSQLCount.toString());
...
//		    			}
		    			logger.info("in searchHoldInstrumentMaker() of HoldInstrumentDAOImpl MAIN query :::: "+bufInsSql.toString());
...
//			}
		        ...
		        try{
		               ...
		               bufInsSql.append("select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,d.SCHEME_DESC,e.branch_desc," +
								...
								" and c.pdc_purpose='INSTALLMENT' and c.PDC_INSTRUMENT_ID in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F') and hold_reason_status in ('H')  AND MAKER_ID!='"+instructionCapMakerVO.getUserID()+"')  ");

		               			sbAppendToSQLCount.append("select distinct count(1) from(select c.pdc_loan_id from cr_loan_dtl a, cr_pdc_instrument_dtl c, "+
								" cr_product_m b,cr_scheme_m d,com_branch_m e,gcd_customer_m f where a.LOAN_ID=c.PDC_LOAN_ID and f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID and "+
								" e.branch_id=a.loan_branch  and a.LOAN_PRODUCT = b.PRODUCT_ID and a.LOAN_SCHEME = d.SCHEME_ID "+
								" and c.pdc_purpose='INSTALLMENT' and c.PDC_INSTRUMENT_ID in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F') and hold_reason_status in ('H')  AND MAKER_ID!='"+instructionCapMakerVO.getUserID()+"') ");
...
//		               
		        		                  ...
		        		                  bufInsSql.append("  order by a.LOAN_APPROVAL_DATE"); 
		        		                  sbAppendToSQLCount.append(" group by c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,d.SCHEME_DESC,e.branch_desc,a.LOAN_APPROVAL_DATE,f.CUSTOMER_NAME, c.MAKER_ID ) as b"); 
	    				...
	    				" a.ECUSTOMER_ACCOUNT_TYPE,a.SPONSOR_BANKBRANCH_CODE,a.UTILITY_NUMBER,a.HOLD_REMARKS" +
	    		  		...
	    		  		" and a.pdc_instrument_id not in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F'))");
	           	              ...
	           	              if((!((date.toString()).equalsIgnoreCase("")))) {
	           	                   bufInsSql.append("  AND a.PDC_INSTRUMENT_DATE=STR_TO_DATE('"+date+"','%d-%m-%Y')");
...
//					     "+instrumentidList[i]+"','"+newStatusList[i]+"','"+checkedStatusList[i]+"','"+holdRE+"','H','F') " ;
		...
		public ArrayList<InstructionCapMakerVO> getValuesforHoldInstrumentAuthor(int id) {
			...
			try{
			  ...
			  ArrayList mainlist=new ArrayList();
			  ArrayList subList=new ArrayList();
			  query.append("select a.PDC_LOAN_ID,a.PDC_INSTRUMENT_MODE,a.PDC_INSTRUMENT_NO,b.bank_name,a.PDC_ISSUEING_BANK_ID, a.PDC_ISSUEING_BRANCH_ID,c.bank_branch_name,a.PDC_ISSUING_MICR_CODE,a.PDC_LOCATION,a.PDC_BIN_NO, a.PDC_INSTRUMENT_AMOUNT,DATE_FORMAT(a.PDC_INSTRUMENT_DATE,'%d-%m-%Y'),a.pdc_status,a.pdc_instrument_id, e.hold_reason,case when e.from_status='A' then 'Active' when e.from_status='H' then 'Hold' end as frStatus,case when e.to_status='A' then 'Active' when e.to_status='H' then 'Hold' else 'Cancelled'end as toStatus,a.PDC_INSTL_NO, a.PDC_ISSUING_IFSC_CODE,a.PDC_ISSUEING_BANK_ACCOUNT,a.PDC_PURPOSE,a.ECS_TRANSACTION_CODE, a.ECUSTOMER_ACCOUNT_TYPE,a.SPONSOR_BANKBRANCH_CODE,a.UTILITY_NUMBER,a.HOLD_REMARKS,DATE_FORMAT(CR.INSTL_DATE,'%d-%m-%Y') from cr_pdc_instrument_dtl a LEFT OUTER JOIN com_bank_m b ON(a.PDC_ISSUEING_BANK_ID = b.BANK_ID )LEFT OUTER JOIN com_bankbranch_m c ON(a.PDC_ISSUEING_BRANCH_ID = c.bank_BRANCH_ID )LEFT OUTER JOIN cr_loan_dtl d ON(a.pdc_loan_id = d.loan_id )LEFT OUTER JOIN cr_repaysch_dtl cr ON(A.PDC_LOAN_ID=CR.LOAN_ID AND A.PDC_INSTL_NO=CR.INSTL_NO )left outer join cr_pdc_hold_reason_dtl e on(a.pdc_instrument_id = e.pdc_instrument_id )where a.pdc_purpose='INSTALLMENT' and e.main_status='F' and e.hold_reason_status='H' and a.pdc_loan_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(id).trim())+"'  and a.pdc_status in ('A','H') order by a.PDC_INSTRUMENT_DATE ");
