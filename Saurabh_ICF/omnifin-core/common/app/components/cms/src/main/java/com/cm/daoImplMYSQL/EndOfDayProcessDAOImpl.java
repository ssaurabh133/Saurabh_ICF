...
public class EndOfDayProcessDAOImpl implements EndOfDayProcessDAO{
	private static final Logger logger = Logger.getLogger(EndOfDayProcessDAOImpl.class.getName());
	...
	try
	    ...
	    query.append("SELECT A.PROCESS_NAME,B.START_TIME,B.END_TIME,B.PROCESS_STATUS " +
	    		...
	    		" where DATE_FORMAT(PROCESS_DATE,'"+dateFormat+"')='"+businessDate+"' GROUP BY PROCESS_NAME)" +
...
{		
	...
	ArrayList<String> list=new ArrayList<String>();
		try{
			...
			query1.append("select  DATE_FORMAT(REPORT_DATE,'"+dateFormat+"')as REPORT_DATE from eod_reports_dtl WHERE USER_ID=(SELECT USER_ID FROM SEC_USER_M WHERE USER_EMAIL='"+mailTo+"') AND DAILY='N' ");
			...
			if(reportDate.equalsIgnoreCase(businessDate))
			{
				...
				query2.append("select concat((select PARAMETER_VALUE from parameter_mst where PARAMETER_KEY='EOD_REPORT_PATH'),'/', REPORT_NAME,'.pdf') REPORT_PATH from eod_reports_dtl WHERE USER_ID=(SELECT USER_ID FROM SEC_USER_M WHERE USER_EMAIL='"+mailTo+"') AND DAILY='N' and DATE_FORMAT(REPORT_DATE,'"+dateFormat+"')='"+businessDate+"'");
