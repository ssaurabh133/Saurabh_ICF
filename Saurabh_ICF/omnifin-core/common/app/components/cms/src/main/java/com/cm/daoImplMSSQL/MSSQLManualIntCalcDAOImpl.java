...
public class MSSQLManualIntCalcDAOImpl implements ManualIntCalcDAO{
	
	
	private static final Logger logger = Logger.getLogger(MSSQLManualIntCalcDAOImpl.class.getName());
	...
	public ArrayList<ManualIntCalcVO> getMICValues(int lbxLoanNoHID,String businessDate) {
		...
		try {
			 query.append(" select a.loan_no,");
			 query.append(dbo);
			 query.append("DATE_FORMAT(a.loan_approval_date,'"+dateFormat+"'), b.customer_name,c.product_desc, d.scheme_desc,");
			 query.append(" a.loan_id,");
			 query.append(dbo);
			 query.append("DATE_FORMAT(a.LOAN_REPAY_EFF_DATE,'"+dateFormat+"'),");
			 query.append(dbo);
			 query.append("DATE_FORMAT(a.LAST_ACCRUAL_DATE,'"+dateFormat+"'),format(a.LOAN_EFF_RATE,2),");
			 query.append(dbo);
			 query.append("DATE_FORMAT(a.LAST_LPP_DATE,'"+dateFormat+"'),a.LOAN_BALANCE_PRINCIPAL ");			
