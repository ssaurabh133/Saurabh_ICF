...
public class MSSQLDisbCancellationDAOImpl implements DisbCancellationDAO{
	
	private static final Logger logger = Logger.getLogger(MSSQLDisbCancellationDAOImpl.class.getName());
...
//		String userName="";
		...
		try {
			...
			bufInsSql.append("select distinct a.loan_no, b.customer_name, c.product_desc, "
						+ "d.scheme_desc,a.loan_loan_amount, "+dbo+"DATE_FORMAT(a.loan_approval_date,'"+dateFormat+"'),a.loan_id from cr_loan_dtl a,gcd_customer_m b,"
...
//			String query1 = "";
			...
			try {
				...
				query.append(" "+dbo+"DATE_FORMAT(a.loan_approval_date,'"+dateFormat+"'),a.loan_loan_amount,c.product_desc, ");
...
//			String query1 = "";
			...
			try {
				...
				query.append("select  A.LOAN_ID,a.disbursal_no,"+dbo+"DATE_FORMAT(a.disbursal_date,'"+dateFormat+"'),a.disbursal_amount,");
				query.append("(select DESCRIPTION from generic_master where generic_key='BPTYPE' and value=DISBURSAL_TO)");
				query.append(" ,b.customer_name,CANCELLATION_REMARKS,case when ISNULL(CANCELLATION_FLAG,'')='P' then '1' else '0' end,"+dbo+"DATE_FORMAT(a.CANCELLATION_DATE,'"+dateFormat+"') ");	
				query.append(" ,a.LOAN_DISBURSAL_ID  from cr_loan_disbursal_dtl a,gcd_customer_m b");			
				query.append(" where a.loan_id='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(loanId).trim()) + "'");
				query.append(" AND A.REC_STATUS='A' and b.CUSTOMER_ID=disbursal_to_id "); 
...
//			String query1 = "";
			...
			try {
				...
				query.append("select  A.LOAN_ID,a.disbursal_no,"+dbo+"DATE_FORMAT(a.disbursal_date,'"+dateFormat+"'),a.disbursal_amount,");
				query.append("(select DESCRIPTION from generic_master where generic_key='BPTYPE' and value=DISBURSAL_TO)");
				query.append(" ,b.customer_name,CANCELLATION_REMARKS,ISNULL(CANCELLATION_FLAG,'')='F',"+dbo+"DATE_FORMAT(a.CANCELLATION_DATE,'"+dateFormat+"') ");	
				query.append(" ,a.LOAN_DISBURSAL_ID from cr_loan_disbursal_dtl a,gcd_customer_m b");			
				query.append(" where a.loan_id='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(loanId).trim()) + "'");
				query.append(" AND A.REC_STATUS='A' and b.CUSTOMER_ID=disbursal_to_id and a.CANCELLATION_FLAG='F' "); 
				query.append(" order BY a.disbursal_no desc ");
				logger.info("In getLoanDetails : query" + query);
				mainlist = ConnectionDAO.sqlSelect(query.toString());
		...
		try {
			for(int i=0;i<chk.length;i++){
			    ...
			    query.append("update cr_loan_disbursal_dtl set CANCELLATION_FLAG='P' ,CANCELLATION_MAKER_ID=?,");			   
			    query.append("CANCELLATION_MAKER_DATE=");
			    query.append(dbo);
			    query.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
			    query.append(",CANCELLATION_DATE=");
			    query.append(dbo);
			    query.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
		...
		public boolean forwardCancellationData(DisbCancellationVO vo){
			...
			try {
				for(int i=0;i<chk.length;i++){
					 ...
					 query=new StringBuilder();
					 chkAllVal=chkAllVal+"'"+disbId[i]+"',";
				query.append("update cr_loan_disbursal_dtl set CANCELLATION_FLAG='F' ,CANCELLATION_MAKER_ID=?,CANCELLATION_MAKER_DATE=");
				query.append(dbo);
				query.append(" STR_TO_DATE(?,'"+dateFormatWithTime+"')+ ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
				query.append(" where loan_id=? and LOAN_DISBURSAL_ID=?");
...
//					tempresult=temp+inParameter1;	
				...
				query.append("select case when sum(TXN_ADJUSTED_AMOUNT)>0 then 'Y' else 'N' end,case when sum(AMOUNT_IN_PROCESS)>0 then 'Y' else 'N' end");
...
//			String query1 = "";		
			try {
				...
				query.append("select ISNULL(BILL_FLAG,'N'),ISNULL(REC_TYPE,'')  from cr_repaysch_dtl");
...
//					tempresult=temp+inParameter1;	
					...
					String queryCheck="Select count(1) from CR_INSTRUMENT_DTL  where TXNID='"+vo.getTaLoanID()+"' and INSTRUMENT_NO='"+tempInstNo+"' and isnull(TXN_TYPE,'')='LIM' AND ISNULL(REC_STATUS,'A')<>'X' ";
					logger.info("queryCheck:------------------"+queryCheck);
				    String queryCheckResult=ConnectionDAO.singleReturn(queryCheck);
				    if(!CommonFunction.checkNull(queryCheckResult).equalsIgnoreCase("0"))
				    {
