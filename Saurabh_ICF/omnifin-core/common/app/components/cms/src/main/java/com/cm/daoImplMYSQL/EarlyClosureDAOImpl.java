...
public class EarlyClosureDAOImpl implements EarlyClosureDAO{
	
	
	private static final Logger logger = Logger.getLogger(EarlyClosureDAOImpl.class.getName());
	...
	public ArrayList<ClosureSearchVO> searchClosureData(Object ob, String status, String type) {
		...
		try {
			...
			bufInsSql.append("select distinct a.loan_no, b.customer_name, c.product_desc, "
						+ "d.scheme_desc,a.loan_loan_amount, DATE_FORMAT(a.loan_approval_date,'"+dateFormat+"'),a.loan_id, e.termination_id, (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=e.MAKER_ID) MAKER_ID from cr_loan_dtl a,gcd_customer_m b,"
...
//		con.setAutoCommit(false);
	...
	//	cst.setString(4, CommonFunction.checkNull(vo.getEffectiveDate().trim()));//
		...
		if((CommonFunction.checkNull(vo.getBalancePrincipal().trim()).equalsIgnoreCase("")))
			...
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getBalancePrincipal().trim())).doubleValue());
		
		if((CommonFunction.checkNull(vo.getSecureDeposit().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getSecureDeposit()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getOverdueInstallments().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getOverdueInstallments()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getSecureDepositInterest().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getSecureDepositInterest()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getInterestTillDate().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getInterestTillDate()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getSecureDepositTDS().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getSecureDepositTDS()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getUnBilledInstallments().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getUnBilledInstallments()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getGapSDInterest().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getGapSDInterest()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getForeClosurePenalty().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getForeClosurePenalty()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getInterstTillLP().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getInterstTillLP()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getLppAmount().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getLppAmount()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getGapSDTDS().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getGapSDTDS()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getOtherDues().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getOtherDues()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getOtherRefunds().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getOtherRefunds()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getNetReceivablePayable().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getNetReceivablePayable()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getOverduePrincipal().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getOverduePrincipal()).trim()).doubleValue());
		
		if((CommonFunction.checkNull(vo.getWaiveOffAmount().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getWaiveOffAmount()).trim()).doubleValue());
		//added by  arun for AdvanceEmiRefunds  Starts 
		if((CommonFunction.checkNull(vo.getAdvanceEmiRefunds().trim()).equalsIgnoreCase("")))
			in.add(0.00);
		else
			in.add(myFormatter.parse(CommonFunction.checkNull(vo.getAdvanceEmiRefunds()).trim()).doubleValue());
		...
		in.add(CommonFunction.checkNull(vo.getvClosureType().trim()));//Virender
...
//        s2 = cst.getString(27);
	...
	public String getMakerDate(String loanId) 
	{
		...
		query.append("select date_format(MAKER_DATE,'%d-%m-%Y') from cr_termination_dtl where LOAN_ID="+loanId.trim()+" and rec_status !='X'");
...
//		Amit Changes Starts
		query.append(",e.rec_status,e.termination_type,e.CLOSURE_TYPE,(select Description from generic_master where generic_key='closure' and value=e.CLOSURE_TYPE) as CLOSURE_TYPE_DESC ");//Virender
//		Amit Changes Ends
...
//		Amit Changes Ends
		...
		realize.append("select * from cr_instrument_dtl where TXNID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(loanId).trim())+"' and ifnull(TXN_TYPE,'')='LIM' and rec_status ='D'");
		 realize.append(" AND IF (IFNULL((select PARAMETER_VALUE from parameter_mst where PARAMETER_KEY='REALIZE_FLAG'),'N')='Y',rec_status in('P','F','D'),rec_status in('P','F'))");
...
//				Amit Changes Ends
				vo.setvClosureType((CommonFunction.checkNull(subList.get(35)).trim()));//Virender
				vo.setvClosureTypeDesc((CommonFunction.checkNull(subList.get(36)).trim()));//Virender
				/*Advance Emi Refund*/
		...
		query1.append("SELECT period_diff(date_format(a.loan_maturity_date,'%Y%m'),date_format(b.termination_date,'%Y%m')) as remaining_tenure ");
	...
	String status="";
	String query = "select sum(ifnull(AMOUNT_IN_PROCESS,0)) from cr_txnadvice_dtl where loan_id='"+lbxLoanNoHID+"' and rec_STATUS='A'";
	logger.info("query checkClosureAmtInProces: "+query);
	String amtInProcStr = "";
	try
	{
		amtInProcStr = CommonFunction.checkNull(ConnectionDAO.singleReturn(query));
	...
	try {
		...
		query.append("update cr_termination_dtl set request_no=?,MAKER_REMARKS=?, termination_date=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");
		...
		query.append(" net_rec_pay=?, rec_principal_overdue=?, waiveoff_amount=?, ADVANCE_EMI_REFUNDS=?,rec_status=?, maker_id=?, maker_date=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)");
	...
	query.append(" a.DISBURSAL_STATUS, (select sum(ifnull(disbursal_amount,0)) from cr_loan_disbursal_dtl where");
	...
	query1.append("SELECT period_diff(date_format(loan_maturity_date,'%Y%m'),date_format(b.termination_date,'%Y%m')) "); 
	...
	try{
		...
		String query="SELECT DATE_FORMAT(ADVICE_DATE,'"+dateFormat+"'),(Select CHARGE_DESC From com_charge_code_m "+
	...
	try {
		...
		query.append("select INSTL_NO,DATE_FORMAT(INSTL_DATE,'"+dateFormat+"'),INSTL_AMOUNT,PRIN_COMP,INT_COMP,EXCESS_INT," +
				" if(ADV_FLAG='Y','YES','NO'),PRIN_OS,INSTL_AMOUNT_RECD,if(BILL_FLAG='Y','YES','NO') as bill,OTHER_CHARGES,SEQ_NO,IF(BILL_FLAG='Y' AND INSTL_AMOUNT<>INSTL_AMOUNT_RECD and REC_TYPE<>'D', DATEDIFF(now(), INSTL_DATE),'0'), CASE WHEN BILL_FLAG='Y' AND INSTL_AMOUNT<>INSTL_AMOUNT_RECD THEN  DATEDIFF(now(), INSTL_DATE) ELSE '0' END as 'OD_DAYS',vat_amount, service_amount from cr_repaysch_dtl where LOAN_ID="+CommonFunction.checkNull(loanId)+" and  ifnull(REC_STATUS,'A') <>'X' order by INSTL_DATE,SEQ_NO ");
		...
		for (int i = 0; i <size; i++) {
			...
			if (subList.size() > 0) {
				...
				if(!CommonFunction.checkNull(subList.get(10)).trim().equalsIgnoreCase(""))
	    		{
				 ...
				 repvo.setOverDueDay(CommonFunction.checkNull(subList.get(12)));
				 //add by saorabh
				 if(!CommonFunction.checkNull(subList.get(13)).trim().equalsIgnoreCase(""))
		    		{
				 ...
				 if(!CommonFunction.checkNull(subList.get(14)).trim().equalsIgnoreCase(""))
		    		{
	    	    		Number reconNum =myFormatter.parse((CommonFunction.checkNull(subList.get(14))).trim());  
...
//			}			
	...
	public ArrayList<ClosureVO> getClosureValues(int lbxLoanNoHID,String businessDate) {
		...
		ClosureVO loanvo1 = null;
		try {
			 query.append(" select distinct a.loan_no,DATE_FORMAT(a.loan_approval_date,'"+dateFormat+"'), b.customer_name,c.product_desc, d.scheme_desc,");
			 query.append(" a.loan_repayment_freq, a.loan_tenure, DATE_FORMAT(a.loan_maturity_date,'"+dateFormat+"'),a.loan_id,");
			...
			query1 = "SELECT period_diff(date_format(loan_maturity_date,'%Y%m'),date_format(STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(businessDate).trim())+"','"+dateFormat+"'),'%Y%m')) as remaining_tenure from cr_loan_dtl where loan_id='"
					+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(lbxLoanNoHID).trim()) + "'";
			logger.info("In getClosureValues : " + query1);
			mainlist = ConnectionDAO.sqlSelect(query1);
			size = mainlist.size();
			logger.info("In getClosureValues.....mainlist size: "+ size);
			for (int i = 0; i < size; i++) {
	...
	public ArrayList getWaiveOffList(ArrayList<String> inList) 
	{
		...
		else
		{
			...
			ArrayList data= null;
			ClosureVO vo= null;
			bufInsSql.append(" SELECT CHARGE_CODE,MIN(CHARGES) CHARGES,SUM(IFNULL(ADVICE_AMOUNT,0))ADVICE_AMOUNT,SUM(IFNULL(ADJUSTED_AMOUNT,0))ADJUSTED_AMOUNT,SUM(IFNULL(AMOUNT_DUE,0))AMOUNT_DUE,SUM(IFNULL(WAIVE_OFF_AMOUNT,0))WAIVE_OFF_AMOUNT,SUM(IFNULL(BALANCE_AMOUNT,0))BALANCE_AMOUNT   " );
...
// method added by neeraj
	...
	//Virender Code for Early Closure Start
	public ArrayList<Object> getClosureTypeList() {	  	
		logger.info("in getClosureTypeList() method***********************");    	
		ArrayList<Object>   list=new ArrayList<Object>();
		try{
			String query=("select VALUE,DESCRIPTION  from GENERIC_MASTER WHERE GENERIC_KEY='CLOSURE' and REC_STATUS ='A' ");
			logger.info("query for getClosureTypeList:::::::::"+query);
			ArrayList doneBy = ConnectionDAO.sqlSelect(query);
			AssetForCMVO vo = null;
			logger.info("sizew of getClosureTypeList list:::::::"+doneBy.size());
