...
public  class MSSQLRepayScheduleDAOImpl implements RepayScheduleDAO 
{
	private static final Logger logger = Logger.getLogger(MSSQLRepayScheduleDAOImpl.class.getName());
	...
	public ArrayList<UpdateRepayscheduleSearchVO> searchRepaySchedule(UpdateRepayscheduleSearchVO vo,String type)
	{
		...
		try
		{
	        ...
	        bufInsSql.append(" and d.scheme_id = a.loan_scheme and e.rec_status='"+type+"' and e.resch_type='U' and e.loan_id=a.loan_id AND A.LOAN_BRANCH='"+vo.getBranchId()+"' ");
...
//					}
	 ...
	 tempCount.append(ConnectionDAO.singleReturn(queryCheck.toString()));
		...
		if(count==0)
		{
			try{
				  ...
				  query.append(" (select ");
				  query.append("ISNULL(sum(disbursal_amount),0) from cr_loan_disbursal_dtl where loan_id='"+lbxLoanNoHID+"' and rec_status='A') as disbursedAmount," );
				  ...
				  query.append(" (select count(loan_id) from cr_resch_dtl where loan_id='"+lbxLoanNoHID+"' and RESCH_TYPE='U' and rec_status='A') as loanIdcountIncr_resch_dtl," );
				  query.append(" a.loan_repayment_type," );
				  query.append(" a.LOAN_EMI_AMOUNT,a.loan_due_day,");
				  query.append(dbo);
				  query.append("DATE_FORMAT((select ");
				  query.append(" TOP 1 ");
				  query.append(" max(instl_date) from cr_repaysch_dtl where loan_id='"+lbxLoanNoHID+"' and bill_flag='Y' and instl_date> ");
				  query.append(" a.loan_repay_eff_date),'"+dateFormat+"'),A.LOAN_REPAYMENT_FREQ" );
	...
	public ArrayList getCycleDateList() {
		...
		try{
			StringBuilder query=new StringBuilder();
		 query.append("SELECT CAST(VALUE AS INTEGER) id,CAST(VALUE AS INTEGER) id1 FROM generic_master WHERE GENERIC_KEY = 'CYCLE_DATE' and rec_status='A' ORDER BY id");
	...
	public String updateRepayScheduleData(UpdateRepayscheduleSearchVO ob) {
		...
		logger.info("In updateRepayScheduleData method of ");
...
//	    String s3="";
	...
	/*        String reschDate=CommonFunction.changeFormat(CommonFunction.checkNull(vo.getreschDate()).trim());
	        ...
	        if(CommonFunction.checkNull(vo.getreschDate()).equalsIgnoreCase("")){
				in.add("");
			}
			else
			{
				in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getreschDate()).trim()));
			}
	        ...
	        /*String date1=CommonFunction.changeFormat(CommonFunction.checkNull(vo.getCurentDueDate()).trim());
	 	    ...
	 	    if(date1 != null)
	 	    	in.add(date1);*/
	        if(CommonFunction.checkNull(vo.getCurentDueDate()).equalsIgnoreCase("")){
				in.add("");
			}
			else
			{
				in.add(CommonFunction.changeFormat(vo.getCurentDueDate()));
			}
			...
			{
				in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getNextDueDate()).trim()));
			}
...
//	        in.add((CommonFunction.checkNull(vo.getMakerDate()).trim()));
			...
			{
				in.add(CommonFunction.changeFormat((CommonFunction.checkNull(vo.getmakerDate()).trim())));
			}
	...
	public ArrayList<UpdateRepayscheduleSearchVO> getRepayScheduleData(String reschId,String lbxLoanNoHID){
		...
		UpdateRepayscheduleSearchVO vo;
		 ...
		 StringBuilder query=new StringBuilder();
		  ...
		  query.append(" a.loan_balance_principal, " );
		  query.append(dbo);
		  query.append("DATE_FORMAT(e.OLD_DUE_DATE,'"+dateFormat+"') as next_due_date," );
		  query.append(" (select count(loan_id) from cr_resch_dtl where loan_id='"+lbxLoanNoHID+"' and RESCH_TYPE='U' and rec_status='A') as loanIdcountIncr_resch_dtl," );
		  query.append(" a.loan_repayment_type," );
		  query.append(" a.LOAN_EMI_AMOUNT,a.loan_due_day,e.request_no,e.resch_id,");
		  query.append(dbo);
		  query.append("date_format(e.new_due_date,'"+dateFormat+"') as new_due_date,e.remarks,e.resch_charges,e.DEFERAL_FROM_INSTL,");
		  query.append(dbo);
		  query.append("DATE_FORMAT((select ");
		  query.append(" TOP 1 ");
		  ...
		  query.append(" where b.customer_id = a.loan_customer_id and c.product_id = a.loan_product");
		  query.append(" and d.scheme_id = a.loan_scheme and dl.DEAL_ID=a.LOAN_DEAL_ID and a.loan_id='"+lbxLoanNoHID+"' and a.loan_id=e.loan_id and e.resch_id='"+reschId+"'");
...
public boolean updateAndFarwordRepayScheduleData(UpdateRepayscheduleSearchVO ob){
		...
		PrepStmtObject insertPrepStmtObject = new PrepStmtObject();
		 try{
			   ...
			   bufInsSql.append(" UPDATE cr_resch_dtl set REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
			   bufInsSql.append(dbo);
			   bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
			   bufInsSql.append(" where RESCH_ID=?");
...
{
      logger.info("In getNewRepayScheduleDueDate DAOImpl Due_Date_Change");
      String status="";
...
//    {
            try
            {
              ...
              cst=con.prepareCall("{call Due_Date_Change(?,?,?,?,?,?,?,?,?,?,?,?)}");
              ...
              if(s1.equalsIgnoreCase("S"))
              {
                  status=s1;
                        ...
                        query.append(" case when R_ADV_FLAG='Y' then 'YES' else 'NO' end,R_Prin_OS from Repay_Temp where R_LOAN_ID="+loanId+" and maker_id='"+CommonFunction.checkNull(vo.getAuthorId()).trim()+"' order by R_Seq_No");
                        logger.info("Query in getNewRepayScheduleDueDate-----" + query.toString());
                        int j=1;
...
public String saveDueDateAuthor(LoanInitAuthorVo vo) {
	...
	logger.info("In Due_Date_Change ........ DAOImpl");
...
//		cst.registerOutParameter(8, Types.CHAR);
//		cst.executeUpdate();
	    outMessages=(ArrayList) ConnectionDAO.callSP("Due_Date_Change",in,out);
	    s1.append(CommonFunction.checkNull(outMessages.get(0)));
	    s2.append(CommonFunction.checkNull(outMessages.get(1)));
	    s3.append(CommonFunction.checkNull(outMessages.get(2)));
...
public boolean deleteDueDateData(String loanId, String reschId) 
{
	...
	logger.info("In..deleteDueDateData"+query.toString());
