...
public class LinkLoanDAOImpl implements LinkLoanDAO {
	private static final Logger logger=Logger.getLogger(LinkLoanDAOImpl.class.getName());
...
public ArrayList<LinkLoanVo> getLoanDetails(LinkLoanVo linkLoanVo){
	...
	try {
		...
		query.append(" JOIN GCD_CUSTOMER_M GCD ON CLD.LOAN_CUSTOMER_ID=GCD.CUSTOMER_ID WHERE TRUE " );
		...
		queryTempCount.append(" JOIN GCD_CUSTOMER_M GCD ON CLD.LOAN_CUSTOMER_ID=GCD.CUSTOMER_ID WHERE TRUE " );
	  ...
	  logger.info("in getAfterSaveData method of LinkLoan DAO Iml:::::::::::::::: ");
		...
		try {
			 ...
			 logger.info("value of disbale :::::::::"+checkBoxFlag);
				...
				query.append(" JOIN  GCD_CUSTOMER_M GCD ON A.LOAN_CUSTOMER_ID=GCD.CUSTOMER_ID"); 
				query.append(" JOIN ( ");
				query.append("  SELECT PRIMARY_LOAN,LINK_LOAN FROM cr_post_loan_dtl_temp WHERE LINK_LOAN_ID='"+CommonFunction.checkNull(linkLoanvo.getLbxLoanNoHID())+"'");
				query.append(" )B ON A.LOAN_ID=B.LINK_LOAN ");
  ...
  public ArrayList<UpdateAssetVO> getLoanDetailsAuthor(UpdateAssetVO updateAssetVO) {
  	...
  	try {
		...
		query.append("  JOIN sec_user_branch_dtl S ON(S.USER_ID='"+CommonFunction.checkNull(updateAssetVO.getUserId())+"' AND S.REC_STATUS='A' AND CLD.LOAN_BRANCH=S.BRANCH_ID ) " );
  		...
  		queryTempCount.append("  JOIN sec_user_branch_dtl S ON(S.USER_ID='"+CommonFunction.checkNull(updateAssetVO.getUserId())+"' AND S.REC_STATUS='A' AND CLD.LOAN_BRANCH=S.BRANCH_ID )" );
  		queryTempCount.append("  JOIN cr_asset_collateral_m CACM ON (CACM.ASSET_ID = CLUAD.ASSET_ID )");
  		queryTempCount.append(" WHERE CLUAD.REC_STATUS = 'F' AND CLUAD.MAKER_ID<>'"+CommonFunction.checkNull(updateAssetVO.getUserId())+"' ");
...
//Manish Ends for Loan Details for Author
 //Method for get value after clicking in grid link
  public ArrayList<UpdateAssetVO> getAuthorScreenAssetVehical (UpdateAssetVO updateAssetVO) {  
		...
		/*bufInsSql.append(" select c.loan_id,a.asset_id,IF (A.ASSET_NEW_OLD='N','New','Old') as asset_new_old ,A.ASSET_COLLATERAL_DESC,A.VEHICLE_MAKE, ");
		bufInsSql.append(" A.VEHICLE_MODEL, cluad.VEHICLE_MANUFACTURING_YEAR, cluad.VEHICLE_REGISTRATION_NO, DATE_FORMAT(cluad.VEHICLE_REGISTRATION_DATE,'%d-%m-%Y'), ");
		bufInsSql.append(" cluad.VEHICLE_CHASIS_NUMBER, cluad.VEHICLE_Insurer, DATE_FORMAT(cluad.VEHICLE_INSURED_DATE,'%d-%m-%Y'), a.VEHICLE_COLLATERAL_COST,cluad.VEHICLE_OWNER , ");
		bufInsSql.append(" a.ASSET_SUPPLIER_DESC,a.ASSET_MANUFATURER_DESC,a.LOAN_AMOUNT,a.DEFAULT_LTV,cluad.ENGINE_NUMBER,a.VEHICLE_DISCOUNT,IFNULL(cluad.VEHICLE_IDV,'0.00') as VEHICLE_IDV, ");
		bufInsSql.append(" a.vehicle_grid_value,DATE_FORMAT(cluad.vechicle_invoice_date,'%d-%m-%Y'),cluad.invoice_no,a.vehicle_valuation_amount,");
		bufInsSql.append(" cluad.rc_received,DATE_FORMAT(cluad.rc_received_date,'%d-%m-%Y') ,a.VEHICLE_ASSET_USES_TYPE,a.COLLATERAL_SECURITY_MARGIN,s.state_desc, ");
		...
		bufInsSql.append(" DATE_FORMAT(cluad.PERMIT_RECEIVED_DATE,'%d-%m-%Y'),c.loan_no,gcd.customer_name,INVOICE_UPDATE_FLAG,RC_UPDATE_FLAG,INSURANCE_UPDATE_FLAG ,ifnull(cluad.REMARKS,'')as AUTHOR_REMARKS,DATE_FORMAT(cluad.endrs_date,'%d-%m-%Y'), DATE_FORMAT(cluad.renewal_date,'%d-%m-%Y'), cluad.prem_amt,DATE_FORMAT(cluad.prem_date,'%d-%m-%Y')");
		...
		*/
			...
			bufInsSql.append(" select c.loan_id,a.asset_id,IF (A.ASSET_NEW_OLD='N','New','Old') as asset_new_old ,A.ASSET_COLLATERAL_DESC,A.VEHICLE_MAKE, ");
			      ...
			      bufInsSql.append(" cluad.VEHICLE_CHASIS_NUMBER, cluad.VEHICLE_Insurer, DATE_FORMAT(cluad.VEHICLE_INSURED_DATE,'%d-%m-%Y'), a.VEHICLE_COLLATERAL_COST,cluad.VEHICLE_OWNER , ");
			       ...
			       bufInsSql.append(" cluad.rc_received,DATE_FORMAT(cluad.rc_received_date,'%d-%m-%Y') ,a.VEHICLE_ASSET_USES_TYPE,a.COLLATERAL_SECURITY_MARGIN,s.state_desc, ");
			       bufInsSql.append(" a.VEHICLE_VALUATION_AMOUNT,a.VEHICLE_VALUE,a.asset_standard,a.asset_collateral_value,g.DESCRIPTION,cluad.INVOICE_AMOUNT,cluad.PERMIT_RECEIVED, ");
			       bufInsSql.append(" DATE_FORMAT(cluad.PERMIT_RECEIVED_DATE,'%d-%m-%Y'),c.loan_no,gcd.customer_name,INVOICE_UPDATE_FLAG,RC_UPDATE_FLAG,INSURANCE_UPDATE_FLAG ,DATE_FORMAT(cluad.endrs_date,'%d-%m-%Y'), DATE_FORMAT(cluad.renewal_date,'%d-%m-%Y'), cluad.prem_amt,DATE_FORMAT(cluad.prem_date,'%d-%m-%Y')");
			       bufInsSql.append(" from cr_loan_update_asset_dtl cluad   ");
			       bufInsSql.append("  join cr_asset_collateral_m a on (cluad.ASSET_ID = a.ASSET_ID and a.ASSET_TYPE='ASSET' and a.asset_collateral_class='VEHICLE') ");
			       bufInsSql.append("  join cr_loan_collateral_m b on b.assetid=a.asset_id and b.loan_id=cluad.loan_id ");
			       bufInsSql.append("  join generic_master g on g.VALUE=a.SECURITY and g.GENERIC_KEY='SECURITY_TYPE'");
		...
		logger.info("IN getAuthorScreenAssetVehical() select data query ###>> "+ bufInsSql.toString());
		...
		bufInsSql=null;
			...
			for (int i = 0; i < searchlist.size(); i++) {
				...
				if (data.size() > 0) {
					...
					updateAssetVO.setInsuranceAgency(CommonFunction.checkNull(data.get(10)));
					...
					// Kumar Aman Changes Start
					updateAssetVO.setAuthorRemarks(CommonFunction.checkNull(data.get(43)));
					
					updateAssetVO.setEndorsementdate(CommonFunction.checkNull(data.get(44)));	
					updateAssetVO.setRenewaldate(CommonFunction.checkNull(data.get(45)));	
					updateAssetVO.setPremamt(CommonFunction.checkNull(data.get(46)));	
					updateAssetVO.setPremdate(CommonFunction.checkNull(data.get(47)));	*/
	// Kumar Aman Changes End
					 ...
					 updateAssetVO.setLbxLoanId(CommonFunction.checkNull(data.get(0)));
					           ...
					           updateAssetVO.setEndorsementdate(CommonFunction.checkNull(data.get(43)));
					           updateAssetVO.setRenewaldate(CommonFunction.checkNull(data.get(44)));
					           updateAssetVO.setPremamt(CommonFunction.checkNull(data.get(45)));
					           updateAssetVO.setPremdate(CommonFunction.checkNull(data.get(46)));
...
public ArrayList<UpdateAssetVO> openEditVehical(
	...
	ArrayList<UpdateAssetVO> list = new ArrayList<UpdateAssetVO>();
	logger.info("ratioid in openEditVehical***************************** = "+updateAssetVO.getLbxLoanId());
	...
	try {
		...
		logger.info("IN openEditVehical() select data query1 ### "+ bufInsSql.toString());
...
public boolean updateAsset(Object ob) {
	...
	try {
		...
		/*bufInsSql.append(" INVOICE_AMOUNT,PERMIT_RECEIVED,PERMIT_RECEIVED_DATE,REC_STATUS,MAKER_ID,MAKER_DATE,INVOICE_UPDATE_FLAG,RC_UPDATE_FLAG,INSURANCE_UPDATE_FLAG,REMARKS,endrs_date, renewal_date, prem_amt, prem_date)");*/
		bufInsSql.append(" INVOICE_AMOUNT,PERMIT_RECEIVED,PERMIT_RECEIVED_DATE,REC_STATUS,MAKER_ID,MAKER_DATE,INVOICE_UPDATE_FLAG,RC_UPDATE_FLAG,INSURANCE_UPDATE_FLAG,endrs_date, renewal_date, prem_amt, prem_date)");
			bufInsSql.append(" VALUES ( ");
			...
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//VEHICLE_REGISTRATION_DATE
			bufInsSql.append(" ?,");//VEHICLE_CHASIS_NUMBER
			bufInsSql.append(" ?,");//VEHICLE_INSURER
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//VEHICLE_INSURED_DATE
			bufInsSql.append(" ?,");//VEHICLE_OWNER
			bufInsSql.append(" ?,");//ENGINE_NUMBER
			bufInsSql.append(" ?,");//VEHICLE_IDV
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//VECHICLE_INVOICE_DATE
			bufInsSql.append(" ?,");//INVOICE_NO
			bufInsSql.append(" ?,");//RC_RECEIVED
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//RC_RECEIVED_DATE
			bufInsSql.append(" ?,");//INVOICE_AMOUNT
			bufInsSql.append(" ?,");//PERMIT_RECEIVED
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),");//PERMIT_RECEIVED_DATE
			bufInsSql.append(" ?,");//REC_STATUS
			bufInsSql.append(" ?,");//MAKER_ID
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND), ");//MAKER_DATE
...
*/			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND), ");//endorsement date 
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND), ");//renewal date
			bufInsSql.append(" ?,");  // premium amount
			bufInsSql.append(" DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)) "); // premium date
		//	bufInsSql.append(" ? ) ");//INSURANCE_UPDATE_FLAG
...
// Kumar Aman Change End				
		...
		if (CommonFunction.checkNull(vo.getInsuranceAgency()).equalsIgnoreCase(""))   //VEHICLE_INSURER
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getInsuranceAgency().toUpperCase().trim());
...
// Kumar Aman Change Start
		/*if (CommonFunction.checkNull(vo.getAuthorRemarks()).equalsIgnoreCase(""))   //REMARKS
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getAuthorRemarks());*/
		
		
		
		if (CommonFunction.checkNull(vo.getEndorsementdate()).equalsIgnoreCase(""))   //endorsement date
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getEndorsementdate().toUpperCase().trim());
		
		
		if (CommonFunction.checkNull(vo.getRenewaldate()).equalsIgnoreCase(""))   //renewal date
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getRenewaldate().toUpperCase().trim());
		
		
		if (CommonFunction.checkNull(vo.getPremamt()).equalsIgnoreCase(""))   //premium amount
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getPremamt().toUpperCase().trim());
		
		
		if (CommonFunction.checkNull(vo.getPremdate()).equalsIgnoreCase(""))   //premium date
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getPremdate().toUpperCase().trim());
// Kumar Aman Change End		
...
public boolean saveUpdateAsset(Object updateAssetVO) { 
	...
	try {
		...
		bufInsSql = new StringBuilder();
		insertPrepStmtObject = new PrepStmtObject();
		bufInsSql.append(" UPDATE  cr_loan_update_asset_dtl SET REC_STATUS=?,REMARKS=? ,AUTHOR_ID=?,AUTHOR_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) ");
		...
		insertPrepStmtObject = new PrepStmtObject();
				...
				bufInsSql.append(" UPDATE CR_ASSET_COLLATERAL_M CACM ");
				bufInsSql.append(" JOIN (SELECT ASSET_ID,VEHICLE_MANUFACTURING_YEAR,VEHICLE_REGISTRATION_NO,VEHICLE_REGISTRATION_DATE,");
				...
				bufInsSql.append(" FROM cr_loan_update_asset_dtl WHERE LOAN_ID='"+vo.getLbxLoanId()+"' AND ASSET_ID = '"+vo.getAssetId()+"') A ON (CACM.ASSET_ID = A.ASSET_ID AND CACM.asset_collateral_class = 'VEHICLE') "); 
...
@Override
//public ArrayList<UpdateAssetVO> getUpdateAssetVehical(String lbxLoanId,String lbxAssetId) {
public ArrayList<UpdateAssetVO> getUpdateAssetVehical(Object obj) {
	...
	try{
        ...
        bufInsSql.append(" select  CLD.LOAN_ID,IF (CACM.ASSET_NEW_OLD='N','New','Old') as ASSET_NEW_OLD,CACM.ASSET_COLLATERAL_DESC, ");
        ...
        bufInsSql.append(" CACM.VEHICLE_MANUFACTURING_YEAR,CACM.VEHICLE_REGISTRATION_NO,DATE_FORMAT(CACM.VEHICLE_REGISTRATION_DATE,'"+dateFormat+"'),CACM.VEHICLE_CHASIS_NUMBER,CACM.VEHICLE_INSURER," );
        bufInsSql.append(" DATE_FORMAT(CACM.VEHICLE_INSURED_DATE,'"+dateFormat+"'),CACM.VEHICLE_OWNER,CACM.ENGINE_NUMBER,CACM.VEHICLE_IDV,DATE_FORMAT(CACM.VECHICLE_INVOICE_DATE,'"+dateFormat+"'),CACM.INVOICE_NO,CACM.RC_RECEIVED," );
        bufInsSql.append(" DATE_FORMAT(CACM.RC_RECEIVED_DATE,'"+dateFormat+"'),CACM.INVOICE_AMOUNT,CACM.PERMIT_RECEIVED,DATE_FORMAT(CACM.PERMIT_RECEIVED_DATE,'"+dateFormat+"'),t.INVOICE_UPDATE_FLAG,t.RC_UPDATE_FLAG,t.INSURANCE_UPDATE_FLAG " );
...
public ArrayList<UpdateAssetVO> fetchPDDUpdateData(Object obj) {
	...
	try{
        ...
        StringBuffer bufInsSql =    new StringBuffer();
        bufInsSql.append(" SELECT INVOICE_NO,DATE_FORMAT(VECHICLE_INVOICE_DATE,'%d-%m-%Y') AS VECHICLE_INVOICE_DATE,INVOICE_AMOUNT,VEHICLE_CHASIS_NUMBER,ENGINE_NUMBER, ");
        bufInsSql.append(" RC_RECEIVED,DATE_FORMAT(RC_RECEIVED_DATE,'%d-%m-%Y') AS RC_RECEIVED_DATE,VEHICLE_REGISTRATION_NO,DATE_FORMAT(VEHICLE_REGISTRATION_DATE,'%d-%m-%Y') AS VEHICLE_REGISTRATION_DATE,VEHICLE_MANUFACTURING_YEAR,VEHICLE_OWNER,PERMIT_RECEIVED,DATE_FORMAT(PERMIT_RECEIVED_DATE,'%d-%m-%Y') AS PERMIT_RECEIVED_DATE, ");
        bufInsSql.append(" VEHICLE_INSURER,DATE_FORMAT(VEHICLE_INSURED_DATE,'%d-%m-%Y') AS VEHICLE_INSURED_DATE,VEHICLE_IDV ,endrs_date,renewal_date,prem_amt,prem_date");
       ...
       //	UpdateAssetVO updateAssetVO=new UpdateAssetVO();
       	for(int i=0;i<mainlist.size();i++){
			...
			if(subList.size()>0){
				...
				//vo.setUpdateFlag(CommonFunction.checkNull(vo.getUpdateVal()));				
				
// Kumar Aman Changes start
				vo.setEndorsementdate(CommonFunction.checkNull(subList.get(16))); 		//Endorsment Date
				vo.setRenewaldate(CommonFunction.checkNull(subList.get(17)));			//Renewal Date
				vo.setPremamt(CommonFunction.checkNull(subList.get(18)));				//Premium Amount
				vo.setPremdate(CommonFunction.checkNull(subList.get(19)));				//Premium Date
// Kumar aman Changes End				
...
//Nishant space ends
public ArrayList<UpdateAssetVO> getUpdatedVehical(String LoanId)
   {
    ArrayList searchlist = new ArrayList();
    ArrayList assetlist = new ArrayList();
 
    UpdateAssetVO updateAssetVO = new UpdateAssetVO();
    logger.info("In fetch getUpdatedVeichal()--->>>: ");
 
    boolean status = false;
     try {
      logger.info("In getUpdatedVeichal().....................................Dao Impl");
 
      StringBuffer bufInsSql = new StringBuffer();
      bufInsSql.append(new StringBuilder().append(" SELECT CLD.LOAN_NO,CLD.LOAN_ID,ASSET_ID,(select ASSET_COLLATERAL_DESC from cr_asset_collateral_m where asset_id=ass.ASSET_ID) as ASSET_COLLATERAL_DESC , VEHICLE_CHASIS_NUMBER, ENGINE_NUMBER, VEHICLE_REGISTRATION_NO from cr_loan_update_asset_dtl ASS JOIN CR_LOAN_DTL CLD ON CLD.LOAN_ID=ass.LOAN_ID where cld.loan_id='").append(LoanId).append("'").toString());
 
      logger.info(new StringBuilder().append("SELECT Query----->>").append(bufInsSql.toString()).toString());
      searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());
 
      logger.info(new StringBuilder().append("SIZE Query----->>").append(searchlist.size()).toString());
      ...
      for (int i = 0; i < searchlist.size(); i++) {
        ...
        if (data.size() > 0) {
          ...
          updateAssetVO.setLoanNo(new StringBuilder().append("<a href=updateAssetEdit.do?method=openEditVehical&loanId=").append(CommonFunction.checkNull(data.get(1)).toString()).append("&assetId=").append(CommonFunction.checkNull(data.get(2)).toString()).append(">").append(CommonFunction.checkNull(data.get(0)).toString()).append("</a>").toString());
 
          updateAssetVO.setPddLbxLoanId(CommonFunction.checkNull(data.get(1)));
 
          updateAssetVO.setPddassetId(CommonFunction.checkNull(data.get(2)));
 
          updateAssetVO.setPddassetsCollateralDesc(CommonFunction.checkNull(data.get(3)));
 
          updateAssetVO.setPddvehicleChesisNo(CommonFunction.checkNull(data.get(4)));
 
          updateAssetVO.setPddengineNumber(CommonFunction.checkNull(data.get(5)));
 
          updateAssetVO.setPddvehicleRegNo(CommonFunction.checkNull(data.get(6)));
 
          assetlist.add(updateAssetVO);
          updateAssetVO = null;
          data.clear();
          data = null;
         }
