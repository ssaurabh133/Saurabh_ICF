...
public class PdcViewerDAOImpl implements PdcViewerDao{
	
	
	private static final Logger logger = Logger.getLogger(PdcViewerDAOImpl.class.getName());
	...
	public ArrayList<PdcViewerVo> searchPDCViewerData(PdcViewerVo vo) 
	{
		...
		try {
			...
			bufInsSql.append(",(IFNULL(cr.INSTL_AMOUNT,0)+IFNULL(cr.OTHER_CHARGES,0)) as TOTAL_EMI,MAKER_REMARKS,DATE_FORMAT(cr_pdc_instrument_dtl.MAKER_DATE,'%d-%m-%Y'),DATE_FORMAT(cr_pdc_instrument_dtl.AUTHOR_DATE,'%d-%m-%Y') ");
			...
			{
				bufInsSql.append(" AND PDC_STATUS!='X'");
				bufInsSqlTempCount.append(" AND PDC_STATUS!='X'");
			}
