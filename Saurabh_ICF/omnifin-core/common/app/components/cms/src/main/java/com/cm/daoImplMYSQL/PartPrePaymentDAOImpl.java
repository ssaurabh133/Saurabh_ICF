...
public class PartPrePaymentDAOImpl implements PartPrePaymentDAO 
{
	private static final Logger logger = Logger.getLogger(PartPrePaymentDAOImpl.class.getName());
	...
	public ArrayList<PartPrePaymentMakerVO> selectPartPrePaymentData(String loanId,String reschId, String recStatus) 
	{
		...
		try{
		 ...
		 query.append("select a.loan_id,a.loan_no, b.customer_name, c.product_desc, d.scheme_desc," +
			  	 ...
			  	 " DATE_FORMAT((select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id='"+loanId+"' and RESCH_TYPE='P' and rec_status='A'),'"+dateFormat+"')," +
			  	 " a.loan_repayment_type, e.REQUEST_NO, DATE_FORMAT(e.RESCH_EFF_DATE,'"+dateFormat+"'), e.PREPAYMENT_AMOUNT, e.RESCH_CHARGES, "+
			  	 " e.PRE_PAYMENT_PARAMETER, e.RESCH_REASON, e.remarks, e.DEFERAL_FROM_INSTL,(Select ifnull((sum(ifnull(txn.ADVICE_AMOUNT,0))-(sum(ifnull(txn.TXN_ADJUSTED_AMOUNT,0))+sum(ifnull(txn.AMOUNT_IN_PROCESS,0)))),0) d from cr_txnadvice_dtl txn where LOAN_ID='"+loanId+"' and ADVICE_TYPE='P')amoumt "+
				 " , a.LOAN_INSTALLMENT_TYPE, " +
			  	 "IFNULL(DATE_FORMAT((select max(instl_date) from cr_repaysch_dtl where loan_id='"+loanId+"' and bill_flag='Y' ),'"+dateFormat+"'),A.LOAN_REPAY_EFF_DATE)"+
			  	// " DATE_FORMAT((select max(instl_date) from cr_repaysch_dtl where loan_id='"+loanId+"' and bill_flag='Y' ),'"+dateFormat+"')" +
			...
			if(mainlist.size()!=0)
			{
			 ...
			 for(int i=0;i<mainlist.size();i++){
				...
				if(subList.size()>0){
					...
					vo.setInstType((CommonFunction.checkNull(subList.get(20))));
					vo.setPartPrePaymentCal(cal);
					vo.setRealize(real);
					vo.setMaxInstallmentDate((CommonFunction.checkNull(subList.get(21))));
	...
	public ArrayList<PaymentMakerForCMVO> viewReceivableForPartPrePayment(int loanId)
	{
		logger.info("In viewReceivableForPartPrePayment method of PartPrePaymentDAOImpl");
 		...
 		try{
 			...
 			StringBuilder query=new StringBuilder();
 		 query.append("SELECT DATE_FORMAT(ADVICE_DATE,'"+dateFormat+"'),(Select CHARGE_DESC From com_charge_code_m "+
		...
		if(count==0)
		{
			try{
				  query.append("select a. loan_id,a.loan_no, b.customer_name, c.product_desc,  d.scheme_desc, " +
				  	 ...
				  	 " ifnull(DATE_FORMAT((select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id='"+lbxLoanNoHID+"' and RESCH_TYPE='P' and rec_status='A'),'"+dateFormat+"'),'')," +
					 ...
					 " (Select ifnull((sum(ifnull(txn.ADVICE_AMOUNT,0))-(sum(ifnull(txn.TXN_ADJUSTED_AMOUNT,0))+sum(ifnull(txn.AMOUNT_IN_PROCESS,0)))),0) d from cr_txnadvice_dtl txn where LOAN_ID='"+lbxLoanNoHID+"' and ADVICE_TYPE='P' )amoumt "
					 + ",a.LOAN_INSTALLMENT_TYPE "
				 ...
				 StringBuilder cal=new StringBuilder();
				  cal.append(ConnectionDAO.singleReturn("Select ((sum(txn.ADVICE_AMOUNT)-sum(txn.TXN_ADJUSTED_AMOUNT)-sum(txn.AMOUNT_IN_PROCESS))-(Select (sum(txn.ADVICE_AMOUNT)-sum(txn.TXN_ADJUSTED_AMOUNT)-sum(txn.AMOUNT_IN_PROCESS)) d from  cr_txnadvice_dtl txn where LOAN_ID='"+lbxLoanNoHID+"' and ADVICE_TYPE='R' ) )d from  cr_txnadvice_dtl txn where LOAN_ID='"+lbxLoanNoHID+"' and ADVICE_TYPE='P'  "));
...
//	    }
	...
	// add by saorabh
		public String getRecStatusForDisbursal(String lbxLoanNoHID) {
			...
			String query = "select count(1) from cr_loan_disbursal_dtl where loan_id = '"+lbxLoanNoHID+"' and rec_status in( 'F','P')";
			String str = "";
