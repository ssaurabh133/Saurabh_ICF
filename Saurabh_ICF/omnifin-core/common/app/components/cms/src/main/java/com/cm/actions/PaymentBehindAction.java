...
public class PaymentBehindAction extends Action {
	private static final Logger logger = Logger.getLogger(PaymentBehindAction.class.getName());
	
	public ActionForward execute(ActionMapping mapping,ActionForm form,
			HttpServletRequest request,HttpServletResponse response)
	throws Exception {
	
		logger.info("In PaymentBehindAction  ");
		
		HttpSession session = request.getSession();
	
		UserObject userobj=(UserObject)session.getAttribute("userobject");
		if(userobj==null){
			logger.info("here in execute method of PaymentBehindAction action the session is out----------------");
