...
public class ReleaseInstrumentDAOImpl implements ReleaseInstrumentDAO {
	private static final Logger logger = Logger.getLogger(ReleaseInstrumentDAOImpl.class.getName());
...
//		        
    ...
    bufInsSql.append( " select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc,d.SCHEME_DESC, "+ 
         ...
         " e.branch_id=a.loan_branch  and a.LOAN_PRODUCT = b.PRODUCT_ID and a.LOAN_SCHEME = d.SCHEME_ID and c.PDC_STATUS in ('A','H')  "+
         "  and c.pdc_instrument_id not in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F')) ");
...
//            " DATE_FORMAT(a.LOAN_APPROVAL_DATE,'"+dateFormat+"'),f.CUSTOMER_NAME, (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=c.MAKER_ID) USERID from cr_loan_dtl a, cr_pdc_instrument_dtl c, "+
//            "  cr_product_m b,cr_scheme_m d,com_branch_m e,gcd_customer_m f where a.LOAN_ID=c.PDC_LOAN_ID and f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID and "+
//            " e.branch_id=a.loan_branch and a.LOAN_REPAYMENT_TYPE='I' and a.LOAN_PRODUCT = b.PRODUCT_ID and a.LOAN_SCHEME = d.SCHEME_ID and c.PDC_STATUS in ('A','H')  "+
//            "  and c.pdc_instrument_id not in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F')) AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"' ");
		...
		sbAppendToSQLCount.append(" JOIN CR_PDC_INSTRUMENT_DTL C ON A.LOAN_ID=C.PDC_LOAN_ID AND C.PDC_STATUS IN ('A','H') AND A.LOAN_REPAYMENT_TYPE='I' "); 
		...
		sbAppendToSQLCount.append(" LEFT JOIN cr_pdc_hold_reason_dtl D ON C.PDC_INSTRUMENT_ID=D.PDC_INSTRUMENT_ID AND D.MAIN_STATUS='F' WHERE D.pdc_hold_reason_id IS NULL  ");
//		
//		sbAppendToSQLCount.append(" WHERE NOT EXISTS (SELECT 1 FROM CR_PDC_HOLD_REASON_DTL D WHERE C.PDC_INSTRUMENT_ID=D.PDC_INSTRUMENT_ID AND D.MAIN_STATUS='F') ");
//		sbAppendToSQLCount.append(" AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"' ");
...
//		                  sbAppendToSQLCount.append(" )as b");
			    			logger.info("In searchReleaseInstrumentMaker() of ReleaseInstrumentDAOImpl  COUNT query : "+sbAppendToSQLCount.toString());
...
//			    			}
			    			logger.info("In searchReleaseInstrumentMaker() of ReleaseInstrumentDAOImpl MAIN query : "+bufInsSql.toString());
...
//			}
		        ...
		        try{
		               ...
		               bufInsSql.append("select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,d.SCHEME_DESC,e.branch_desc," +
								...
								" and c.PDC_INSTRUMENT_ID in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F') and hold_reason_status in ('L') AND MAKER_ID!='"+instructionCapMakerVO.getUserID()+"') ");
...
//	        String date="";
			...
			try{
			  		...
			  		" a.pdc_loan_id='"+id+"' and a.pdc_loan_id = d.loan_id and a.pdc_status in ('A','H')" +
			  		" and a.pdc_instrument_id not in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F'))");
	           	              ...
	           	              if((!((date.toString()).equalsIgnoreCase("")))) {
	           	                   bufInsSql.append("  AND a.PDC_INSTRUMENT_DATE=STR_TO_DATE('"+date+"','"+dateFormatWithTime+"')");
...
//			  		" a.ECUSTOMER_ACCOUNT_TYPE,a.SPONSOR_BANKBRANCH_CODE,a.UTILITY_NUMBER" +
//			  		
//			  		" from cr_pdc_instrument_dtl a,com_bank_m b,com_bankbranch_m c,cr_loan_dtl d,cr_pdc_hold_reason_dtl e where" +
//			  		" a.PDC_ISSUEING_BANK_ID = b.BANK_ID and a.PDC_ISSUEING_BRANCH_ID = c.bank_BRANCH_ID " +
//			  		" and a.pdc_instrument_id = e.pdc_instrument_id and a.pdc_loan_id = d.loan_id " +
//			  		" and e.main_status='F' and e.hold_reason_status='L'  and" +
//			  		" a.pdc_loan_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(id).trim())+"'  and a.pdc_status in ('A','H')" +
//			  		"   order by a.PDC_INSTRUMENT_DATE ";

			 logger.info("In getValuesforHoldInstrument "+query);
			 mainlist=ConnectionDAO.sqlSelect(query.toString());
				...
				if((instructionCapMakerVO.getDecision()).equalsIgnoreCase("L")){
									...
									for(int i=0;i<instrumentID.length;i++){
										...
										query.append("update cr_pdc_instrument_dtl set pdc_status=(select to_status from cr_pdc_hold_reason_dtl where main_status = 'F' and pdc_instrument_id='"+instrumentID[i]+"')," +
												" pdc_remarks='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getComments()).trim())+"',author_id='"+instructionCapMakerVO.getMakerID()+"',author_date=DATE_ADD(STR_TO_DATE('"+instructionCapMakerVO.getMakerDate()+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)  where pdc_instrument_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instrumentID[i]).trim())+"' ");
