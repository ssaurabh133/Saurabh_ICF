...
//import com.cm.daoImplMYSQL.CodeDescVo;
...
public class MSSQLAdditionalDisbursalDAOImpl implements AdditionalDisbursalDAO
{

	private static final Logger logger = Logger.getLogger(MSSQLAdditionalDisbursalDAOImpl.class.getName());
...
//		
		...
		catch(Exception e)
		{
	       ...
	       try{
	              ...
	              bufInsSql.append(" cr_product_m c, cr_scheme_m d, cr_resch_dtl e where b.customer_id = a.loan_customer_id and c.product_id = a.loan_product");
	              bufInsSql.append(" and d.scheme_id = a.loan_scheme and e.rec_status='"+type+"' and e.resch_type='A' and e.loan_id=a.loan_id AND A.LOAN_BRANCH='"+vo.getBranchId()+"' ");
...
//		
	...
	public ArrayList<AdditionalDisbursalProcessVO> retrieveAdditionDisbValues(String lbxLoanNoHID)
	{
		...
		logger.info("In retrieveAdditionDisbValues queryCheck....."+queryCheck.toString());
		...
		if(count==0)
		{
			try{
				  ...
				  query.append(" (select ISNULL(sum(disbursal_amount),0) from cr_loan_disbursal_dtl where loan_id='"+lbxLoanNoHID+"' and rec_status='A') as disbursedAmount," );
				  query.append(" a.loan_balance_principal, ");
				  query.append(dbo);
				  query.append("DATE_FORMAT((select min(instl_date) from cr_repaysch_dtl where loan_id='"+lbxLoanNoHID+"' and");
				  query.append(" bill_flag='N' and instl_date>(select parameter_value from parameter_mst where parameter_key='BUSINESS_DATE')),'"+dateFormat+"') as next_due_date,");
				  query.append(" (select count(loan_id) from cr_resch_dtl where loan_id='"+lbxLoanNoHID+"' and RESCH_TYPE='A' and rec_status='A') as loanIdCount_cr_resch_dtl," );
				  query.append(" a.loan_repayment_type," );
				  query.append(" (select min(instl_no) from cr_repaysch_dtl where loan_id='"+lbxLoanNoHID+"' and bill_flag='N' ");
				  query.append(" and instl_date>(select parameter_value from parameter_mst where parameter_key='BUSINESS_DATE')) as instl_no");
				  query.append(" from cr_loan_dtl a,gcd_customer_m b,cr_product_m c, cr_scheme_m d,cr_deal_loan_dtl dl");
				  query.append(" where b.customer_id = a.loan_customer_id and c.product_id = a.loan_product");
				  query.append(" and d.scheme_id = a.loan_scheme and dl.DEAL_ID=a.LOAN_DEAL_ID and a.loan_id='"+lbxLoanNoHID+"'");
...
//	    String s3="";
		...
		try
		{
	        ...
	        /*String date1=CommonFunction.changeFormat(CommonFunction.checkNull(vo.getNextDueDate()).trim());
			...
			{
				in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getNextDueDate()).trim()));//48
			}
	        
	        /*String date2=CommonFunction.changeFormat(CommonFunction.checkNull(vo.getDisbursalDate()).trim());

	 	    if(date2 != null)
	 	    	in.add(date2);*/
	        if(CommonFunction.checkNull(vo.getDisbursalDate().trim()).equalsIgnoreCase("")){
				in.add("");
			}
			else
			{
				in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getDisbursalDate()).trim()));
			}
...
//	        in.add((CommonFunction.checkNull(vo.getMakerDate()).trim()));
			...
			{
				in.add(CommonFunction.changeFormat((CommonFunction.checkNull(vo.getMakerDate()).trim())));//48
			}
	...
	public ArrayList<AdditionalDisbursalProcessVO> selectAdditionDisbursalData(String loanId,String reschId, String recStatus) 
		{
			...
			try{
			 ...
			 query.append("select dl.DEAL_SANCTION_AMOUNT,dl.DEAL_UTILIZED_AMOUNT,a.loan_id,a.loan_no, b.customer_name, c.product_desc, d.scheme_desc," );
			 query.append(" (select ISNULL(sum(disbursal_amount),0) from cr_loan_disbursal_dtl where loan_id='"+loanId+"' and rec_status='A') as disbursedAmount,");
			 query.append(" a.loan_balance_principal,");
			 query.append(dbo);
			 query.append("DATE_FORMAT((select min(instl_date) from cr_repaysch_dtl where loan_id='"+loanId+"'" );
			 query.append(" and bill_flag='N' and instl_date>(select parameter_value from parameter_mst where parameter_key='BUSINESS_DATE')),'"+dateFormat+"') as Next_Due_Date," );
			 query.append(" (select count(loan_id) from cr_resch_dtl where loan_id='"+loanId+"' and RESCH_TYPE='A' and rec_status='A') as loanIdcount_cr_resch_dtl," );
			 query.append(dbo);
			 query.append("DATE_FORMAT((select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id='"+loanId+"' and RESCH_TYPE='A' and rec_status='A'),'"+dateFormat+"') as effectiveDate," );
			 query.append(" a.loan_repayment_type, e.REQUEST_NO, ");
			 query.append(dbo);
			 query.append("DATE_FORMAT(e.RESCH_EFF_DATE,'"+dateFormat+"'), e.PREPAYMENT_AMOUNT, e.RESCH_CHARGES, ");
			 query.append(" e.PRE_PAYMENT_PARAMETER, e.RESCH_REASON, e.remarks, e.DEFERAL_FROM_INSTL ");
			 query.append(" from cr_loan_dtl a,gcd_customer_m b,cr_product_m c, cr_scheme_m d,cr_deal_loan_dtl dl, cr_resch_dtl e" );
			 query.append(" where b.customer_id = a.loan_customer_id and c.product_id = a.loan_product");
			 query.append(" and d.scheme_id = a.loan_scheme and dl.DEAL_ID=a.LOAN_DEAL_ID and a.loan_id=e.loan_id and e.loan_id="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(loanId).trim())+"");
			 query.append(" and e.resch_id="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(reschId).trim())+" ");
...
//				cst.executeUpdate();
			    ...
			    outMessages=(ArrayList) ConnectionDAO.callSP("Additional_Disbursal_Author",in,out);
			    s1.append(CommonFunction.checkNull(outMessages.get(0)));
			    s2.append(CommonFunction.checkNull(outMessages.get(1)));
			    s3.append(CommonFunction.checkNull(outMessages.get(2)));
...
//				s2 = cst.getString(8);
	...
	public ArrayList<RepayScheduleVo> getNewRepayScheduleAdditionalDisbursal(
		...
		try
		{
			...
			query.append(" case when R_ADV_FLAG='Y' then 'YES' else 'NO' end as R_ADV_FLAG,R_Prin_OS from Repay_Temp where R_LOAN_ID="+loanId+" order by R_Seq_No");
...
//	    int chkStatus = Integer.parseInt(chkStatusStr);
	    logger.info("Input Parameter(Additional_Disbursal_Author): I_COMPANY_ID: "+vo.getCompanyId()+" I_RESCH_ID: "+reschId+" I_CURR_DATE: "+vo.getAuthorDate()+" I_USER_ID: "+vo.getAuthorId()+" I_STATUS: "+"T"+" I_REMARKS: ");
//	    if(chkStatus>0)
//	    {
	            try
	            {
	              con.setAutoCommit(false);
	              cst=con.prepareCall("{call Additional_Disbursal_Author(?,?,?,?,?,?,?,?,?,?,?,?)}");
	              ...
	              if(s1.equalsIgnoreCase("S"))
	              {
	                  status=s1;
	                        ...
	                        query.append(dbo);
	                        query.append("DATE_FORMAT(R_Due_Date,'"+dateFormat+"'),R_Instl_Amount,R_Prin_Comp,R_Int_Comp,R_EXCESS_Int_Comp,");
	                        query.append(" case when R_ADV_FLAG='Y' then 'YES' else 'NO' end as R_ADV_FLAG,R_Prin_OS from Repay_Temp where R_LOAN_ID="+loanId+" and maker_id='"+userId+"' order by R_Seq_No");
	                        logger.info("Query in getRepayScheduleAddDisbursal-----" + query.toString());
	                        int j=1;
...
//	    }
	...
	@Override
	public ArrayList getGenericMasterInfo(String string) {
