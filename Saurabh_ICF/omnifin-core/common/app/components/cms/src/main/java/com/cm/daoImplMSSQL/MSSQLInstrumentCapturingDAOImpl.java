...
public class MSSQLInstrumentCapturingDAOImpl implements InstrumentCapturingDAO{
	private static final Logger logger = Logger.getLogger(MSSQLInstrumentCapturingDAOImpl.class.getName());
...
//	                  		" d.SCHEME_DESC,DATE_FORMAT(a.LOAN_APPROVAL_DATE,'"+dateFormat+"'),f.CUSTOMER_NAME from cr_loan_dtl a, cr_pdc_instrument_dtl c," +
//	                  		" cr_product_m b,cr_scheme_m d,com_branch_m e,gcd_customer_m f where c.pdc_status in ('P','A','H') and a.LOAN_ID=c.PDC_LOAN_ID and f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID and " +
//	                  		" e.branch_id=a.loan_branch and a.LOAN_PRODUCT = b.PRODUCT_ID and a.LOAN_REPAYMENT_TYPE='I' and a.LOAN_SCHEME = d.SCHEME_ID AND A.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"' AND C.MAKER_ID='"+instructionCapMakerVO.getUserID()+"'");
...
//	                		  "DATE_FORMAT(CLD.LOAN_APPROVAL_DATE,'"+dateFormat+"'),GCM.CUSTOMER_NAME from cr_loan_dtl CLD, cr_pdc_instrument_dtl CPI, cr_product_m CPM,cr_scheme_m CSM,com_branch_m CBM,"+
//	                		  "gcd_customer_m GCM where  CLD.LOAN_CUSTOMER_ID=GCM.CUSTOMER_ID and CLD.LOAN_REPAYMENT_TYPE='I' AND NOT EXISTS (SELECT 1 FROM cr_pdc_instrument_dtl a "+
//	                		  "WHERE a.pdc_loan_id = CLD.loan_id AND pdc_status ='F')AND EXISTS (SELECT 1 FROM cr_pdc_instrument_dtl a WHERE a.pdc_loan_id = CLD.loan_id AND a.pdc_status ='P' AND a.MAKER_ID='"+instructionCapMakerVO.getUserID()+"')"+
//	                		  "AND CLD.LOAN_BRANCH='"+instructionCapMakerVO.getBranchId()+"'");
			    	  ...
			    	  {
			    		  bufInsSql.append(" FROM CR_LOAN_DTL CLD INNER JOIN CR_PDC_INSTRUMENT_DTL CPI ON CLD.LOAN_ID = CPI.PDC_LOAN_ID AND ((CPI.PDC_STATUS ='P' AND CPI.MAKER_ID ='"+instructionCapMakerVO.getReportingToUserId()+"') OR CPI.PDC_STATUS NOT IN ('P','F')) ");
			    	  }
					 ...
					 // bufInsSql.append(" INNER JOIN (SELECT PDC_LOAN_ID , MAX(PDC_INSTRUMENT_ID) AS PID FROM CR_PDC_INSTRUMENT_DTL GROUP BY PDC_LOAN_ID) ABC ON CPI.PDC_LOAN_ID = ABC.PDC_LOAN_ID AND CPI.PDC_INSTRUMENT_ID = ABC.PID ");
					  ...
					  sbAppendToSQLCount.append(" SELECT COUNT(DISTINCT CPI.PDC_LOAN_ID) "); 
					  if(CommonFunction.checkNull(lbxLoanNoHID.toString()).trim().equalsIgnoreCase(""))
			    	  {
						  sbAppendToSQLCount.append(" FROM CR_LOAN_DTL CLD INNER JOIN CR_PDC_INSTRUMENT_DTL CPI ON CLD.LOAN_ID = CPI.PDC_LOAN_ID AND (CPI.PDC_STATUS ='P' AND CPI.MAKER_ID ='"+instructionCapMakerVO.getReportingToUserId()+"') ");
			    	  }else
			    	  {
			    		  sbAppendToSQLCount.append(" FROM CR_LOAN_DTL CLD INNER JOIN CR_PDC_INSTRUMENT_DTL CPI ON CLD.LOAN_ID = CPI.PDC_LOAN_ID AND ((CPI.PDC_STATUS ='P' AND CPI.MAKER_ID ='"+instructionCapMakerVO.getReportingToUserId()+"') OR CPI.PDC_STATUS NOT IN ('P','F')) ");
			    	  }
	                 ...
	                 /* if((!((customerName.toString()).equalsIgnoreCase("")))) {
	                  ...
	                  bufInsSql.append(" group by  CPI.pdc_loan_id,CLD.loan_no,CPM.PRODUCT_DESC,CLD.LOAN_LOAN_AMOUNT,CBM.branch_desc, CSM.SCHEME_DESC,CLD.LOAN_APPROVAL_DATE,GCM.CUSTOMER_NAME,cpi.MAKER_ID  ");
	               	                  
	                  //sbAppendToSQLCount.append(") as f");
	                 // sbAppendToSQLCount.append(" group by  CPI.pdc_loan_id ,CLD.loan_no,CPM.PRODUCT_DESC,CLD.LOAN_LOAN_AMOUNT,CBM.branch_desc, CSM.SCHEME_DESC,CLD.LOAN_APPROVAL_DATE,GCM.CUSTOMER_NAME,cpi.MAKER_ID )AS B");
	    			...
	    			bufInsSql.append(" ORDER BY CPI.pdc_loan_id OFFSET ");
	...
	public ArrayList<InstructionCapMakerVO> searchInstrumentAuthor(
		    ...
		    int endRecordIndex = no;
		 	   ...
		 	   try{
		              ...
		              // customerName.append(StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getCustomerName()).trim()));
		               ...
		               bufInsSql.append(" select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc," +
		                  		" d.SCHEME_DESC,");
		               bufInsSql.append(dbo);
		               bufInsSql.append("DATE_FORMAT(a.LOAN_APPROVAL_DATE,'"+dateFormat+"')LOAN_APPROVAL_DATE,f.CUSTOMER_NAME , (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=C.MAKER_ID AND C.PDC_STATUS='F') MAKER_ID ");
		               bufInsSql.append("from cr_loan_dtl a, cr_pdc_instrument_temp_dtl c," +
		     ...
		     /*          sbAppendToSQLCount.append(" SELECT COUNT(1) FROM CR_PDC_INSTRUMENT_DTL C "); 
		               ...
		               sbAppendToSQLCount.append(" select distinct count(1) from ( ");
		               sbAppendToSQLCount.append(" select distinct c.pdc_loan_id,a.loan_no,b.PRODUCT_DESC,a.LOAN_LOAN_AMOUNT,e.branch_desc," +
		                  		" d.SCHEME_DESC,");
		               sbAppendToSQLCount.append(dbo);
		               sbAppendToSQLCount.append("DATE_FORMAT(a.LOAN_APPROVAL_DATE,'"+dateFormat+"')LOAN_APPROVAL_DATE,f.CUSTOMER_NAME , (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=C.MAKER_ID AND C.PDC_STATUS='F') MAKER_ID ");
		               sbAppendToSQLCount.append("from cr_loan_dtl a, cr_pdc_instrument_temp_dtl c," +
		                  		" cr_product_m b,cr_scheme_m d,com_branch_m e,gcd_customer_m f where a.LOAN_ID=c.PDC_LOAN_ID and f.CUSTOMER_ID = a.LOAN_CUSTOMER_ID and " +
	...
	public ArrayList instrumentPurposeList() {
		...
		for(int i=0;i<arrList.size();i++){
			...
			logger.info("In instrumentPurposeList..."+subList.size());
	...
	public ArrayList<InstructionCapMakerVO> getValuesforUpdate(int loanID) {
		...
		StringBuilder query = new StringBuilder(); 
			try{
			  ...
			  query.append("SELECT ");
			  query.append(dbo);
			  query.append("DATE_FORMAT(CP.PDC_INSTRUMENT_DATE,'%d-%m-%Y'),CP.PDC_INSTRUMENT_NO,CR.INSTL_AMOUNT,CP.PDC_INSTRUMENT_AMOUNT,c.loan_id,c.loan_no, "+
	            " CP.pdc_instrument_id,CR.INSTL_No,CP.pdc_status,");
			  query.append(dbo);
			  query.append("DATE_FORMAT(CR.INSTL_DATE,'%d-%m-%Y'), B.BANK_NAME, d.BANK_BRANCH_NAME,"+ 
	            " CP.PDC_ISSUING_MICR_CODE, CP.PDC_ISSUING_IFSC_CODE, CP.PDC_ISSUEING_BANK_ACCOUNT, CP.PDC_PURPOSE,cp.pdc_instrument_mode," +
	            "  cp.ECS_TRANSACTION_CODE,cp.ECUSTOMER_ACCOUNT_TYPE,cp.SPONSOR_BANKBRANCH_CODE,cp.UTILITY_NUMBER,");
			  query.append(dbo);
			  query.append("DATE_FORMAT(cp.MAKER_DATE,'%d-%m-%Y'),cp.MAKER_ID,CP.SUBMIT_BY,CP.NAME_OF_PDC_SUBMIT_BY,CP.PDC_LOCATION,Z.DESCRIPTION as PDC_CLEARING_TYPE,CP.MAKER_REMARKS,(ISNULL(cr.INSTL_AMOUNT,0)+ISNULL(cr.OTHER_CHARGES,0)) as TOTAL_EMI "+
...
//			query.append("select LOAN_INSTALLMENT_MODE,LOAN_ADVANCE_INSTL,LOAN_INSTALLMENT_TYPE,SUM(R.OTHER_CHARGES) TOTAL_CHARGE,(");
//			query.append("ISNULL(SUM(R.OTHER_CHARGES),0)+R.INSTL_AMOUNT) TOTAL_CHR_INST  FROM cr_loan_dtl L, cr_repaysch_dtl R WHERE R.LOAN_ID=L.LOAN_ID AND L.loan_id="+lbxLoanNoHID);
			query.append("select LOAN_INSTALLMENT_MODE,LOAN_ADVANCE_INSTL,LOAN_INSTALLMENT_TYPE,r.TOTAL_CHARGE,(r.TOTAL_CHARGE+i.INSTL_AMOUNT)TOTAL_CHR_INST,LOAN_REPAYMENT_TYPE FROM cr_loan_dtl L ");
			query.append(" left join(select loan_id,SUM(isnull(OTHER_CHARGES,0)) TOTAL_CHARGE from cr_repaysch_dtl 	where loan_id="+lbxLoanNoHID+"	group by loan_id)r on(r.loan_id=l.LOAN_ID) ");
			query.append(" left join(select loan_id,INSTL_AMOUNT from cr_repaysch_dtl 	where instl_no=1 and loan_id="+lbxLoanNoHID+")i on(i.loan_id=l.LOAN_ID) ");
			query.append(" where l.loan_id="+lbxLoanNoHID);
			arrList = ConnectionDAO.sqlSelect(query.toString());
			...
			for(int i=0;i<arrList.size();i++){
				...
				logger.info("In insNonIns..."+subList.size());
					...
					if(subList.size()>0){
						    ...
						    poolIdMakerVO1.setLoanAdvanceInstallment((CommonFunction.checkNull(subList.get(1)).trim()));
						    
						    }
						   
						    if(CommonFunction.checkNull(subList.get(2)).trim().equalsIgnoreCase("E"))
						    {
						    	poolIdMakerVO1.setInstallmentType("Eq. INSTALLMENT");
						    }
						    else if(CommonFunction.checkNull(subList.get(2)).trim().equalsIgnoreCase("G"))
						    {
						    	poolIdMakerVO1.setInstallmentType("Gr. INSTALLMENT");
						    }
						    else if(CommonFunction.checkNull(subList.get(2)).trim().equalsIgnoreCase("P"))
						    {
						    	poolIdMakerVO1.setInstallmentType("Eq. PRINCIPAL");
						    }
						    else if(CommonFunction.checkNull(subList.get(2)).trim().equalsIgnoreCase("Q"))
						    {
						    	poolIdMakerVO1.setInstallmentType("Gr. PRINCIPAL1");
						    }
						    else if(CommonFunction.checkNull(subList.get(2)).trim().equalsIgnoreCase("R"))
						    {
						    ...
						    poolIdMakerVO1.setRepayment((CommonFunction.checkNull(subList.get(5)).trim()));				    
	...
	public ArrayList<InstructionCapMakerVO> getValuesforAuthorUpdate(int loanID) {
		...
		try{
			InstructionCapMakerVO instructionCapMakerVO=null;
			  ...
			  query.append("DATE_FORMAT(CR.INSTL_DATE,'%d-%m-%Y'), B.BANK_NAME, d.BANK_BRANCH_NAME,"+ 
	            " CP.PDC_ISSUING_MICR_CODE, CP.PDC_ISSUING_IFSC_CODE, CP.PDC_ISSUEING_BANK_ACCOUNT, CP.PDC_PURPOSE,cp.pdc_instrument_mode, "+
	            "  cp.ECS_TRANSACTION_CODE,cp.ECUSTOMER_ACCOUNT_TYPE,cp.SPONSOR_BANKBRANCH_CODE,cp.UTILITY_NUMBER,CP.SUBMIT_BY,CP.NAME_OF_PDC_SUBMIT_BY,CP.PDC_LOCATION,Z.DESCRIPTION as PDC_CLEARING_TYPE,CP.MAKER_REMARKS,(ISNULL(cr.INSTL_AMOUNT,0)+ISNULL(cr.OTHER_CHARGES,0)) as TOTAL_EMI  "+
	            " from cr_pdc_instrument_dtl CP left join cr_repaysch_dtl cr on CP.PDC_LOAN_ID=CR.LOAN_ID AND CP.PDC_INSTL_NO=CR.INSTL_NO"+
	            " left join cr_loan_dtl c on CP.PDC_LOAN_ID=c.LOAN_ID"+
	            " left join com_bank_m b on b.BANK_ID= cp.PDC_ISSUEING_BANK_ID "+
				" left join generic_master Z on Z.VALUE=CP.PDC_CLEARING_TYPE and GENERIC_KEY='CLEARING_TYPE' and Z.REC_STATUS='A' "+
...
//		  		" order by CP.PDC_INSTRUMENT_DATE";
	...
	public boolean updateFlag(int id,InstructionCapMakerVO vo, String[] checkedIDArr, String[] checkedDateArr) {
		   ...
		   StringBuilder query = new StringBuilder();
			try{
				logger.info("In updateFlag() checkedDateArr "+checkedDateArr.length);
		...
		{					
				...
				{	
					...
					{ 
						...
						queryy.append("update cr_pdc_instrument_dtl set PDC_INSTRUMENT_DATE=");
						queryy.append(dbo);
						queryy.append("STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(checkedDateArr[i]))+"','"+dateFormatWithTime+"') "+
					...
					query.append("update cr_pdc_instrument_dtl set pdc_status='F',maker_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getMakerID()).trim())+"'" +
							" ,maker_date=");
					//query.append("DATE_ADD(STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getMakerDate()).trim())+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)  ");
					//Saurabh space starts
					if(dbType.equalsIgnoreCase("MSSQL"))
					{
						query.append(" dbo.STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getMakerDate()).trim())+"','"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
					}
					else
					{
						query.append(" DATE_ADD(STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getMakerDate()).trim())+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)");
					...
					query.append(" where pdc_loan_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(id).trim())+"' and pdc_status='P'");
	  ...
	  public boolean updateIndiDeleteInstrument(String[] checkedholdList, String[] checkedStatusList, String[] instrumentidList, String[] newStatusList, InstructionCapMakerVO instructionCapMakerVO) {
			...
			try{
					...
					if((instructionCapMakerVO.getDecision()).equalsIgnoreCase("A")){
						...
						for(int i=0;i<instrumentID.length;i++){
							...
							if(dbType.equalsIgnoreCase("MSSQL"))
							{
								query.append(" dbo.STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getMakerDate()).trim())+"','"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
							}
							else
							{
								query.append(" DATE_ADD(STR_TO_DATE('"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getMakerDate()).trim())+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)");
		...
		public boolean updateCommentNDecision(
		    ...
		    try{
					...
					query.append(",pdc_remarks='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getComments()).trim())+"' where pdc_loan_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(loanID).trim())+"' and pdc_status ='F'");
			...
			try
			{
				...
				else
				{
					...
					query.append("DATE_FORMAT(CR.INSTL_DATE,'%d-%m-%Y'), B.BANK_NAME, d.BANK_BRANCH_NAME,"+ 
					            ...
					            " cp.ECUSTOMER_ACCOUNT_TYPE,cp.SPONSOR_BANKBRANCH_CODE,cp.UTILITY_NUMBER,cp.pdc_instrument_mode,");
					query.append(dbo);
					query.append("DATE_FORMAT(cp.MAKER_DATE,'%d-%m-%Y'),cp.MAKER_ID ,"+				           
					            " CP.SUBMIT_BY,CP.NAME_OF_PDC_SUBMIT_BY,CP.PDC_LOCATION,Z.DESCRIPTION as PDC_CLEARING_TYPE,CP.MAKER_REMARKS,(ISNULL(cr.INSTL_AMOUNT,0)+ISNULL(cr.OTHER_CHARGES,0)) as TOTAL_EMI  from cr_pdc_instrument_dtl CP left join cr_repaysch_dtl cr on CP.PDC_LOAN_ID=CR.LOAN_ID AND CP.PDC_INSTL_NO=CR.INSTL_NO"+
					            " left join cr_loan_dtl c on CP.PDC_LOAN_ID=c.LOAN_ID"+
					            " left join com_bank_m b on b.BANK_ID= cp.PDC_ISSUEING_BANK_ID "+
 								" left join generic_master Z on Z.VALUE=CP.PDC_CLEARING_TYPE and GENERIC_KEY='CLEARING_TYPE' and Z.REC_STATUS='A' "+
					            " left join com_bankbranch_m d  on d.BANK_BRANCH_ID=cp.PDC_ISSUEING_BRANCH_ID"+
		...
		public ArrayList<InstructionCapMakerVO> getValuesforInstrumentforloanViewer(int id) 
		{
			...
			try
			{
				...
				query.append(dbo);
				query.append("DATE_FORMAT(CR.INSTL_DATE,'%d-%m-%Y') ");
				...
				query.append(" ,(ISNULL(cr.INSTL_AMOUNT,0)+ISNULL(cr.OTHER_CHARGES,0)) as TOTAL_EMI,a.MAKER_REMARKS,su.USER_NAME as MAKER_NAME,dbo.DATE_FORMAT(a.MAKER_DATE,'%d-%m-%Y'),a.PDC_REMARKS AS AUTHOR_REMARKS,sua.USER_NAME as AUTHOR_NAME,dbo.DATE_FORMAT(a.AUTHOR_DATE,'%d-%m-%Y') ");
				query.append(" from cr_pdc_instrument_dtl a left join CR_INSTRUMENT_UPDATE_DTL ciud on (ciud.PDC_INSTRUMENT_ID=a.PDC_INSTRUMENT_ID) left outer join com_bank_m b on (a.PDC_ISSUEING_BANK_ID = b.BANK_ID)left outer join com_bankbranch_m c on(a.PDC_ISSUEING_BRANCH_ID = c.bank_BRANCH_ID)left outer join cr_loan_dtl d on(a.pdc_loan_id = d.loan_id)LEFT OUTER JOIN cr_repaysch_dtl cr ON(A.PDC_LOAN_ID=CR.LOAN_ID AND A.PDC_INSTL_NO=CR.INSTL_NO ) left join generic_master Z on Z.VALUE=a.PDC_CLEARING_TYPE and GENERIC_KEY='CLEARING_TYPE' and Z.REC_STATUS='A' left join sec_user_m su on su.USER_ID=a.MAKER_ID left join sec_user_m sua on sua.USER_ID=a.AUTHOR_ID where a.pdc_loan_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(id).trim())+"' and a.pdc_instrument_id not in (select pdc_instrument_id from cr_pdc_hold_reason_dtl where main_status in ('F'))   order by a.PDC_INSTRUMENT_DATE");
...
//		    		}
		           ...
		           try{
		                  ...
		                  bufInsSql.append("select distinct CPI.pdc_loan_id,CLD.loan_no,CPM.PRODUCT_DESC,CLD.LOAN_LOAN_AMOUNT,CBM.branch_desc, CSM.SCHEME_DESC,");
		                  bufInsSql.append(dbo);
		                  bufInsSql.append("DATE_FORMAT(CLD.LOAN_APPROVAL_DATE,'"+dateFormat+"')LOAN_APPROVAL_DATE,GCM.CUSTOMER_NAME, (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=cpi.MAKER_ID) MAKER_ID ");
		                  bufInsSql.append(" from cr_loan_dtl CLD inner join cr_pdc_instrument_temp_dtl CPI on cld.loan_id = cpi.PDC_LOAN_ID "+
				                 ...
				                 sbAppendToSQLCount.append(dbo);
				                 sbAppendToSQLCount.append("DATE_FORMAT(CLD.LOAN_APPROVAL_DATE,'"+dateFormat+"')LOAN_APPOVAL_DATE,GCM.CUSTOMER_NAME, (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=cpi.MAKER_ID) MAKER_ID from cr_loan_dtl CLD inner join cr_pdc_instrument_temp_dtl CPI on cld.loan_id = cpi.PDC_LOAN_ID "+
									"inner join cr_product_m CPM on cpm.PRODUCT_ID = cld.LOAN_PRODUCT inner join cr_scheme_m CSM on csm.SCHEME_ID = cld.LOAN_SCHEME "+
									"inner join com_branch_m CBM on cbm.BRANCH_ID = cld.LOAN_BRANCH inner join gcd_customer_m GCM on gcm.customer_id = cld.LOAN_CUSTOMER_ID INNER JOIN (SELECT PDC_LOAN_ID , MAX(PDC_INSTRUMENT_ID) AS pid  FROM cr_pdc_instrument_temp_dtl where MAKER_ID='"+instructionCapMakerVO.getReportingToUserId()+"'  GROUP BY PDC_LOAN_ID) ABC ON CPI.PDC_LOAN_ID = ABC.PDC_LOAN_ID AND CPI.PDC_INSTRUMENT_ID = ABC.pid "+
									"where  CLD.LOAN_REPAYMENT_TYPE='I' AND pdc_status ='P' AND CPI.MAKER_ID='"+instructionCapMakerVO.getReportingToUserId()+"' "+
									" AND CLD.LOAN_BRANCH in (SELECT BRANCH_ID FROM sec_user_branch_dtl WHERE USER_ID='"+instructionCapMakerVO.getReportingToUserId()+"' AND REC_STATUS='A')");
			...
			//StringBuilder query = new StringBuilder(); 
			  StringBuffer bufInsSql = new StringBuffer();
				try{
                  ...
                  bufInsSql.append(dbo);
                  bufInsSql.append("DATE_FORMAT(CP.PDC_INSTRUMENT_DATE,'%d-%m-%Y'),CP.PDC_INSTRUMENT_NO,CR.INSTL_AMOUNT,CP.PDC_INSTRUMENT_AMOUNT,c.loan_id,c.loan_no, "+
		            " CP.pdc_instrument_id,CR.INSTL_No,CP.pdc_status,");
                  bufInsSql.append(dbo);
                  bufInsSql.append("DATE_FORMAT(CR.INSTL_DATE,'%d-%m-%Y'), B.BANK_NAME, d.BANK_BRANCH_NAME,"+ 
		            " CP.PDC_ISSUING_MICR_CODE, CP.PDC_ISSUING_IFSC_CODE, CP.PDC_ISSUEING_BANK_ACCOUNT, CP.PDC_PURPOSE,cp.pdc_instrument_mode," +
		            "  cp.ECS_TRANSACTION_CODE,cp.ECUSTOMER_ACCOUNT_TYPE,cp.SPONSOR_BANKBRANCH_CODE,cp.UTILITY_NUMBER,");
                  bufInsSql.append(dbo);
                  bufInsSql.append("DATE_FORMAT(cp.MAKER_DATE,'%d-%m-%Y'),cp.MAKER_ID "+
...
//	                  } 
				...
				else
				{
				    ...
				    if((!((CommonFunction.checkNull(instructionCapMakerVO.getRemarks()).trim()).equalsIgnoreCase("")))) {
	                      bufInsSql.append("  MAKER_REMARKS='"+StringEscapeUtils.escapeSql(StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getRemarks()).trim()))+"',");
	                      appendSQL=true;
	                  }
...
//					
						...
						if(InstrumentCheck.size()<= 0)
						{
					      ...
					      logger.info(insertPrepStmtObject.printQuery());
			              status=ConnectionDAO.sqlInsUpdDeletePrepStmt(qryList);
			              logger.info("In insertInstrumentIntoTempForUpdate......................"+status);
					  ...
					  if(checkedDateArr.length>0)
						{
						    qryList=new ArrayList();
						     ...
						     {	
						    	...
						    	StringBuilder updateQuery =new StringBuilder();
						    	updateQuery.append("update cr_pdc_instrument_temp_dtl set PDC_INSTRUMENT_DATE=dbo.STR_TO_DATE(?,'"+dateFormat+"') WHERE PDC_INSTRUMENT_ID=? ");
		...
		/*		    if(!purpose.equalsIgnoreCase("") && purpose.equalsIgnoreCase("PRE EMI"))	
				    ...
				    if((!((CommonFunction.checkNull(instructionCapMakerVO.getMaker()).trim()).equalsIgnoreCase("")))) {
	                      bufInsSql.append("  MAKER_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(instructionCapMakerVO.getMaker()).trim())+"',");
	                      appendSQL=true;
	                  }
		...
		public boolean forwardInstruments(int id,InstructionCapMakerVO vo) {
			   ...
			   StringBuilder query = new StringBuilder();
				try{
						...
						query.append(" where pdc_loan_id='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(id).trim())+"' and pdc_status!='X'");
...
//				}
			        ...
			        try{
			               ...
			               sbAppendToSQLCount.append(" select distinct count(1) from (select c.pdc_loan_id from cr_loan_dtl a, cr_pdc_instrument_temp_dtl c," +
			                 ...
			                 //bufInsSql.append(" order by a.LOAN_APPROVAL_DATE");		//asesh
		...
		public ArrayList<InstructionCapMakerVO> getValueForUpdateAuthor(int loanID) {
			...
			try{
				InstructionCapMakerVO instructionCapMakerVO=null;
				  ...
				  query.append("DATE_FORMAT(CR.INSTL_DATE,'%d-%m-%Y'), B.BANK_NAME, d.BANK_BRANCH_NAME,"+ 
		            " CP.PDC_ISSUING_MICR_CODE, CP.PDC_ISSUING_IFSC_CODE, CP.PDC_ISSUEING_BANK_ACCOUNT, CP.PDC_PURPOSE,cp.pdc_instrument_mode, "+
		            "  cp.ECS_TRANSACTION_CODE,cp.ECUSTOMER_ACCOUNT_TYPE,cp.SPONSOR_BANKBRANCH_CODE,cp.UTILITY_NUMBER,CP.PDC_LOCATION,Z.DESCRIPTION as PDC_CLEARING_TYPE,CP.MAKER_REMARKS,(ISNULL(cr.INSTL_AMOUNT,0)+ISNULL(cr.OTHER_CHARGES,0)) as TOTAL_EMI "+
		            " from cr_pdc_instrument_temp_dtl CP left join cr_repaysch_dtl cr on CP.PDC_LOAN_ID=CR.LOAN_ID AND CP.PDC_INSTL_NO=CR.INSTL_NO"+
		            " left join cr_loan_dtl c on CP.PDC_LOAN_ID=c.LOAN_ID"+
		            " left join com_bank_m b on b.BANK_ID= cp.PDC_ISSUEING_BANK_ID "+
		            " left join com_bankbranch_m d  on d.BANK_BRANCH_ID=cp.PDC_ISSUEING_BRANCH_ID"+
						...
						query1.append("select PDC_INSTRUMENT_ID,PDC_LOAN_ID,PDC_INSTRUMENT_MODE,PDC_INSTRUMENT_TYPE," +
							...
							int i=0;
								...
								else
								{
									...
									{
										status =ConnectionDAO.sqlInsUpdDeletePrepStmt(qryList);
										logger.info("In authorUpdateInstruments.........update status else: "+status);
										if(status)
										{
											logger.info("instructionCapMakerVO.getDecision() inside else" + instructionCapMakerVO.getDecision());
											if((instructionCapMakerVO.getDecision()).equalsIgnoreCase("A"))
											{
												logger.info("before delete");
												boolean st = ConnectionDAO.sqlInsUpdDelete(delList);
