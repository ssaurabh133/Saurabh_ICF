...
public class KnockOffCancellationMakerBehindAction extends Action {
	private static final Logger logger = Logger.getLogger(KnockOffCancellationMakerBehindAction.class.getName());
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)throws Exception 
	{
		logger.info(" in execute() of  KnockOffCancellationMakerBehindAction ");
		HttpSession session = request.getSession();
		boolean flag=false;
		UserObject userobj=(UserObject)session.getAttribute("userobject");
		if(userobj==null){
			logger.info(" in execute() of  KnockOffCancellationMakerBehindAction action the session is out----------------");
