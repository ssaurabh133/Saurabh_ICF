...
//Date_add, date_format, str_to_date, ifnull, date, sysdate, datediff
...
public class MSSQLWaiveOffDAOImpl implements WaiveOffDAO {
	
	private static final Logger logger = Logger.getLogger(MSSQLWaiveOffDAOImpl.class.getName());
...
//			}
		...
		try {
			...
			bufInsSql.append( " from cr_loan_dtl a,gcd_customer_m b,business_partner_view c, generic_master g,cr_waiveoff_dtl e");
			bufInsSql.append( " where b.CUSTOMER_ID=a.LOAN_CUSTOMER_ID");
			bufInsSql.append( " AND e.LOAN_ID=a.LOAN_ID");
			bufInsSql.append(" and c.LOAN_ID=a.loan_id");
			bufInsSql.append( " and e.BP_ID = c.BP_ID");
			bufInsSql.append( " AND e.REC_STATUS='P' AND e.BP_TYPE = c.BP_TYPE AND g.VALUE=c.BP_TYPE AND A.LOAN_BRANCH='"+Vo.getBranchId()+"' AND E.MAKER_ID='"+Vo.getReportingToUserId()+"'");
			...
			if (appendSQL) {
				...
				{*/
					  ...
					  if(Vo.getCurrentPageLink()>1)
					  {
						...
						logger.info("Search searchwaiveOff query for SQL SERVER : " + bufInsSql.toString());
...
//			}
		...
		try {
			...
			bufInsSql.append( " where b.CUSTOMER_ID=a.LOAN_CUSTOMER_ID");
			bufInsSql.append( " AND e.LOAN_ID=a.LOAN_ID");
			bufInsSql.append( " AND c.BP_ID = E.BP_ID AND C.BP_TYPE= E.BP_TYPE AND g.VALUE=c.BP_TYPE");
			bufInsSql.append( " AND e.REC_STATUS='F' AND A.LOAN_BRANCH='"+Vo.getBranchId()+"' and E.MAKER_ID!='"+Vo.getUserId()+"'");
			...
			if (appendSQL) {
				...
				{*/
					  ...
					  if(Vo.getCurrentPageLink()>1)
					  {
						...
						bufInsSql.append(endRecordIndex);
						bufInsSql.append(" ROWS ONLY ");
						logger.info("Search searchWaiveOffAuthor query for SQL SERVER : " + bufInsSql.toString());
	...
	public ArrayList selectWaiveOffAuthorData(String loanId, String bpid,String waveOffId)
	{
		...
		try {
			...
			query.append("select distinct a.LOAN_NO ,b.CUSTOMER_NAME,c.BP_TYPE,c.BP_NAME,d.CHARGE_CODE,d.CHARGE_DESC,f.ADVICE_AMOUNT," +
					...
					"e.WAIVE_OFF_TAX_AMOUNT1,e.WAIVE_OFF_TAX_AMOUNT2,e.MAKER_REMARKS,g.DESCRIPTION,f.amount_in_process,a.REC_STATUS,");
			query.append(dbo);
			query.append("date_format(e.WAIVEOFF_DATE,'"+dateFormat+"') as WAIVEOFF_DATE "
	...
	public ArrayList saveWaiveOffMaker(Object ob, HttpServletRequest request)
	{
		...
		else if (count <= 0)
		{
			...
			try {
				...
				// String maxId="";
				 query3.append("Select max(WAIVEOFF_ID) from cr_waiveoff_dtl  WITH (ROWLOCK) ");
...
//				} else {
					...
					query1.append("update cr_waiveoff_dtl set REC_STATUS='P', " +
							...
							"MAKER_DATE=");
					query1.append(dbo);
						query1.append("STR_TO_DATE('"+CommonFunction.checkNull(vo.getMakerDate()).trim()+"','"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
					
					
					query1.append(" ,APPROVED_BY='"+CommonFunction.checkNull(vo.getLbxapprovedBy()).trim()+"' where WAIVEOFF_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getWaveOffId())).trim()+"'");
		...
		try {
	      ...
	      //String query = "select a.LOAN_NO ,b.CUSTOMER_NAME,c.BP_TYPE,c.BP_NAME,d.CHARGE_CODE,d.CHARGE_DESC,e.ADVICE_AMOUNT,e.TXN_ADJUSTED_AMOUNT,e.WAIVE_OFF_AMOUNT,e.REMARKS"
			query.append("select a.LOAN_NO ,b.CUSTOMER_NAME,c.BP_TYPE,c.BP_NAME,d.CHARGE_CODE,d.CHARGE_DESC,f.ADVICE_AMOUNT," +
					...
					"f.TXN_ADJUSTED_AMOUNT,e.WAIVE_OFF_TAX_AMOUNT1,e.WAIVE_OFF_TAX_AMOUNT2,c.BP_ID,E.MAKER_REMARKS,a.LOAN_ID,e.TXN_ADVICE_ID,e.TAX_RATE1,e.TAX_RATE2,g.DESCRIPTION,f.amount_in_process,a.REC_STATUS,");
			query.append(dbo);
			query.append("date_format(e.WAIVEOFF_DATE,'"+dateFormat+"') as WAIVEOFF_DATE, "
	...
	public boolean saveAndForwordWaiveOffMaker(Object ob)
	{
		...
		PrepStmtObject insertPrepStmtObject = new PrepStmtObject();
			...
			try {
				...
				logger.info("In saveAndForwordWaiveOffMaker.......AMOUNTINPROCESS.....after adjusted.........."	+ amtinProcess);
				
				query.append("update cr_waiveoff_dtl  set REC_STATUS='F',WAIVE_OFF_AMOUNT="
						+myFormatter.parse(StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getWaivOffAmount().trim())).toString())
						+",MAKER_REMARKS='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getRemarks().trim()))+ "'"
						+" where  BP_TYPE='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getBusinessPartnerType().trim()))	+ "' and  CHARGE_CODE_ID="
								+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getLbxChargeCodeHID().trim()))+" and WAIVEOFF_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getWaveOffId().trim()))+"'");
				
				query1.append("update cr_txnAdvice_dtl set AMOUNT_IN_PROCESS="+amtinProcess+" where BP_TYPE='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getBusinessPartnerType().trim()))+"' and TXNADVICE_ID='"+vo.getTxnAdviceId()+"' " +
						"and  CHARGE_CODE_ID="+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getLbxChargeCodeHID().trim()))+"");
...
//						+" and WAIVEOFF_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(vo.getWaveOffId().trim()))+"' and t.TXNADVICE_ID='"+vo.getTxnAdviceId()+"'");
				...
				queryList.add(query1.toString());
				logger.info("In saveAndForwordWaiveOffMaker ........ update query: "+ query.toString());
				logger.info("In saveAndForwordWaiveOffMaker ........ update query 1: "+ query1.toString());
				status = ConnectionDAO.sqlInsUpdDelete(queryList);
				logger.info("In saveAndForwordWaiveOffMaker.........update status: "+ status);
			} catch (Exception e) {
				logger.info(e.getMessage());
		}
			finally
