...
public class MSSQLassetVerificationDAOImpl implements assetVerificationDAO {
	
	private static final Logger logger = Logger.getLogger(MSSQLassetVerificationDAOImpl.class.getName());
						...
						if(qryList.size()>0){
						    status1=ConnectionDAO.sqlInsUpdDeletePrepStmt(qryList);
				              ...
				              if(status1)
				  			{
				            	  ...
				            	  query3.append("Select distinct max(ASSET_VERIFICATION_ID) from cr_asset_verification_dtl  WITH (ROWLOCK) ");
...
public boolean modifyForwaAssetVerification(AssetVerificationVO assetVerificationVO,String appraiserId,String loanId,String[] assetIdlist, String appraiser,String appr) {
	...
	try{
		...
		logger.info("appraiserId---------->"+appraiserId);
		 ...
		 bufInsSql.append(" APPRAISER_NAME=?,MAKER_ID=?,MAKER_DATE=");
		 bufInsSql.append(dbo);
		 bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9),");
		 bufInsSql.append("REC_STATUS='I' " );
		 bufInsSql.append("where ASSET_ID = '"+assetIdlist[i]+"' and LOAN_ID = '"+loanId+"' and APPRAISER_TYPE='"+appraiser+"' and APPRAISER_ID='"+appraiserId+"' and REC_STATUS='P' ");
		...
		if(val.equalsIgnoreCase("")){
		 ...
		 insertPrepStmtObject = new PrepStmtObject();
			 ...
			 bufInsSql.append("insert into cr_asset_verification_dtl(ASSET_ID,LOAN_ID,INITIATION_DATE,APPRAISER_TYPE,APPRAISER_ID,APPRAISER_NAME," + 
				 ...
				 logger.info("In.....saveAssetVerification"+bufInsSql.toString());
				 logger.info("assetIDList--------->"+assetIDList[i]);
				 if(CommonFunction.checkNull(assetIDList[i]).equalsIgnoreCase(""))
						insertPrepStmtObject.addNull();
				else
		...
		for(int i=0;i<loanIDList.length;i++)
		{
		 ...
		 insertPrepStmtObject = new PrepStmtObject();
			 ...
			 bufInsSql.append("insert into cr_asset_verification_dtl(ASSET_ID,LOAN_ID,INITIATION_DATE,APPRAISER_TYPE,APPRAISER_ID,APPRAISER_NAME," + 
				 ...
				 logger.info("In.....forwardAssetVerification"+bufInsSql.toString());
				 if(CommonFunction.checkNull(assetIDList[i]).equalsIgnoreCase(""))
						insertPrepStmtObject.addNull();
				else
					insertPrepStmtObject.addString((CommonFunction.checkNull(assetIDList[i]).trim()));
...
//       	         		"left outer join cr_scheme_m on(B.LOAN_SCHEME=cr_scheme_m.SCHEME_ID)where B.LOAN_REPAYMENT_TYPE='I' AND B.LOAN_BRANCH='"+assetVerificationVO.getBranchId()+"'");
       	            ...
       	            bufInsSql.append("case when C.APPRAISER_TYPE='EA' then 'External' else 'Internal' end as APPRAISER_TYPE, ");     	          
       	            bufInsSql.append("ISNULL((select AGENCY_NAME from com_agency_m " );
       	            bufInsSql.append("where AGENCY_CODE=C.APPRAISER_ID),C.APPRAISER_NAME) AS APPRAISER_NAME,(SELECT USER_NAME FROM SEC_USER_M WHERE " );
       	            bufInsSql.append("USER_ID=C.MAKER_ID) MAKER_ID,B.LOAN_ID,C.APPRAISER_ID ");
       	            bufInsSql.append(" from cr_loan_dtl B left outer join gcd_customer_m A on(B.LOAN_CUSTOMER_ID=A.CUSTOMER_ID) " );
       	            bufInsSql.append(" join cr_asset_verification_dtl C on(C.LOAN_ID=B.LOAN_ID and c.REC_STATUS='P')left outer join  cr_product_m " );
       	            bufInsSql.append("on(B.LOAN_PRODUCT=cr_product_m.PRODUCT_ID)left outer join cr_scheme_m on(B.LOAN_SCHEME=cr_scheme_m.SCHEME_ID)where" );
       	            bufInsSql.append(" B.LOAN_REPAYMENT_TYPE='I' AND B.LOAN_BRANCH='"+assetVerificationVO.getBranchId()+"'"); 
...
/*       	               bufInsSqlTempCount.append("select count(1) from(select distinct B.LOAN_NO,A.CUSTOMER_NAME,cr_product_m.PRODUCT_DESC," +
       	         ...
       	         */
       	            ...
       	            bufInsSqlTempCount.append("case when C.APPRAISER_TYPE='EA' then 'External' else 'Internal' end as APPRAISER_TYPE, ");
       	            bufInsSqlTempCount.append("ISNULL((select AGENCY_NAME from com_agency_m " );
       	            bufInsSqlTempCount.append("where AGENCY_CODE=C.APPRAISER_ID),C.APPRAISER_NAME) AS APPRAISER_NAME,(SELECT USER_NAME FROM SEC_USER_M WHERE " );
       	            bufInsSqlTempCount.append("USER_ID=C.MAKER_ID) MAKER_ID,B.LOAN_ID,C.APPRAISER_ID from cr_loan_dtl B left outer join gcd_customer_m A on(B.LOAN_CUSTOMER_ID=A.CUSTOMER_ID) " );
       	            bufInsSqlTempCount.append(" join cr_asset_verification_dtl C on(C.LOAN_ID=B.LOAN_ID and c.REC_STATUS='P')left outer join  cr_product_m " );
       	            bufInsSqlTempCount.append("on(B.LOAN_PRODUCT=cr_product_m.PRODUCT_ID)left outer join cr_scheme_m on(B.LOAN_SCHEME=cr_scheme_m.SCHEME_ID)where" );
       	            bufInsSqlTempCount.append(" B.LOAN_REPAYMENT_TYPE='I' AND B.LOAN_BRANCH='"+assetVerificationVO.getBranchId()+"'");
...
//    			{
    			...
    			bufInsSql.append(" ORDER BY B.LOAN_NO OFFSET ");
	...
	try{
		...
		logger.info("In searchAssetVerRepCap().....................................Dao Impl");
       	               ...
       	               StringBuffer bufInsSql =    new StringBuffer();
       	            	...
       	            	bufInsSql.append(" from cr_asset_collateral_m c,cr_loan_dtl b,gcd_customer_m d,cr_asset_verification_dtl a" );
       	            	bufInsSql.append(" left join com_agency_m ag on a.APPRAISER_ID=ag.AGENCY_CODE " );
       	            	bufInsSql.append(" left join com_agency_user_mapping agm on ag.AGENCY_CODE=agm.AGENCY_CODE " );  
       	            	bufInsSql.append(" where b.LOAN_customer_id=d.CUSTOMER_ID and a.loan_id = b.loan_id and a.asset_id = c.asset_id AND B.LOAN_REPAYMENT_TYPE='I' AND A.REC_STATUS='I' " );
       	            	bufInsSql.append(" AND B.LOAN_BRANCH='"+assetVerificationVO.getBranchId()+"' and (a.APPRAISER_ID = '"+assetVerificationVO.getReportingToUserId()+"' or agm.USER_ID = '"+assetVerificationVO.getReportingToUserId()+"') ");
...
//					{
					...
					bufInsSql.append(" ORDER BY a.asset_verification_id OFFSET ");
					bufInsSql.append(startRecordIndex);
					bufInsSql.append(" ROWS FETCH next ");
					bufInsSql.append(endRecordIndex);
					bufInsSql.append(" ROWS ONLY ");
...
public ArrayList repCapAsset(AssetVerificationVO assetVerificationVO,
	...
	try{
		logger.info("In AssetVerification...Dao Impl.");
                ...
                bufInsSql.append( "select a.asset_verification_id,a.loan_id,b.loan_no,c.customer_name,f.PRODUCT_DESC,e.SCHEME_DESC,b.LOAN_LOAN_AMOUNT,");
                bufInsSql.append(dbo);
                bufInsSql.append("DATE_FORMAT(b.LOAN_APPROVAL_DATE,'"+dateFormat+"'),d.ASSET_COLLATERAL_DESC," );
                bufInsSql.append(" a.APPRAISER_TYPE,ISNULL((select AGENCY_NAME from com_agency_m where AGENCY_CODE=a.APPRAISER_ID),A.APPRAISER_NAME) AS APPRAISER_NAME,");
                bufInsSql.append(" a.ASSET_LOCATION,");
                bufInsSql.append(dbo);
                bufInsSql.append("DATE_FORMAT(a.VISIT_DATE,'"+dateFormat+"'),a.ASSET_CONDITION,a.INVOICE_COLLECTED,a.INVOICE_NO,a.VISIT_REMARKS,a.MAKER_REMARKS,a.APPROVAL_REMARKS " );
                bufInsSql.append(" from cr_asset_verification_dtl a,cr_loan_dtl b,gcd_customer_m c,cr_asset_collateral_m d,cr_scheme_m e,cr_product_m f " );
                bufInsSql.append(" where a.LOAN_ID = b.loan_id and b.LOAN_CUSTOMER_ID = c.CUSTOMER_ID and a.ASSET_ID = d.asset_id and a.rec_status = 'I' and e.SCHEME_ID=b.LOAN_SCHEME and f.PRODUCT_ID=b.LOAN_PRODUCT");
...
public boolean saveAssetRepCap(
	...
	try{
		 ...
		 insertPrepStmtObject = new PrepStmtObject();
			 ...
			 bufInsSql.append("update cr_asset_verification_dtl set ASSET_LOCATION=? ,VISIT_DATE=");
			 bufInsSql.append(dbo);
			 bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') , ASSET_CONDITION=?," );
			 bufInsSql.append(" INVOICE_COLLECTED=?,INVOICE_NO=?,VISIT_REMARKS=?,REC_STATUS='I',MAKER_REMARKS=?,MAKER_ID=?,MAKER_DATE=");
			 bufInsSql.append(dbo);
			 bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
			 bufInsSql.append("where ASSET_VERIFICATION_ID = ?");
			 
			 logger.info(" IN saveAssetRepCap.............."+bufInsSql.toString());
	...
	try{
		...
		customerName.append(StringEscapeUtils.escapeSql(CommonFunction.checkNull(assetVerificationVO.getCustomerName()).trim()));
                ...
                logger.info("In searchAssetVerRepComplete().....................................Dao Impl");
       	         ...
       	         bufInsSql.append(" from cr_asset_verification_dtl a,cr_asset_collateral_m c,cr_loan_dtl b,gcd_customer_m d " );
       	         bufInsSql.append(" where b.LOAN_customer_id=d.CUSTOMER_ID and a.loan_id = b.loan_id and a.asset_id = c.asset_id " );
       	         bufInsSql.append("AND A.REC_STATUS='V' AND B.LOAN_REPAYMENT_TYPE='I' AND B.LOAN_BRANCH='"+assetVerificationVO.getBranchId()+"'AND A.MAKER_ID!='"+assetVerificationVO.getUserId()+"'");
...
public boolean forwardAssetRepCap(
	...
	try{
		 ...
		 insertPrepStmtObject = new PrepStmtObject();
			 ...
			 bufInsSql.append(dbo);
			 bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') , ASSET_CONDITION=?," );
			 bufInsSql.append(" INVOICE_COLLECTED=?,INVOICE_NO=?,VISIT_REMARKS=?,MAKER_ID=?,MAKER_DATE="); 
			 bufInsSql.append(dbo);
			 bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9),");	
			 bufInsSql.append("REC_STATUS='V' where ASSET_VERIFICATION_ID = ?");
			 
			 logger.info("IN forwardAssetRepCap............"+bufInsSql.toString());
...
public ArrayList repCapAssetComplete(AssetVerificationVO assetVerificationVO,
	...
	try{
                ...
                bufInsSql.append("DATE_FORMAT(b.LOAN_APPROVAL_DATE,'"+dateFormat+"'),d.ASSET_COLLATERAL_DESC," );
                bufInsSql.append(" a.APPRAISER_TYPE,ISNULL((select AGENCY_NAME from com_agency_m where AGENCY_CODE=a.APPRAISER_ID),A.APPRAISER_NAME) AS APPRAISER_NAME," );
                bufInsSql.append(" a.ASSET_LOCATION,a.ASSET_CONDITION,a.INVOICE_COLLECTED,a.INVOICE_NO,");
                bufInsSql.append(dbo);
                bufInsSql.append("DATE_FORMAT(a.VISIT_DATE,'"+dateFormat+"'),a.VISIT_REMARKS,a.MAKER_REMARKS,a.APPROVAL_REMARKS " );
                bufInsSql.append(" from cr_asset_verification_dtl a,cr_loan_dtl b,gcd_customer_m c,cr_asset_collateral_m d,cr_scheme_m e,cr_product_m f" );
                bufInsSql.append(" where a.LOAN_ID = b.loan_id and b.LOAN_CUSTOMER_ID = c.CUSTOMER_ID and a.ASSET_ID = d.asset_id and a.rec_status = 'V' and e.SCHEME_ID=b.LOAN_SCHEME and f.PRODUCT_ID=b.LOAN_PRODUCT");
...
public boolean saveAssetCapRepAuthor(
	...
	try{
		 ...
		 insertPrepStmtObject = new PrepStmtObject();
			 ...
			 bufInsSql.append("update cr_asset_verification_dtl set APPROVAL_REMARKS=?,REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
			 bufInsSql.append(dbo);
			 bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");		
			 bufInsSql.append("where ASSET_VERIFICATION_ID = ?");
			 
			 logger.info("IN.........saveAssetCapRepAuthor"+bufInsSql.toString());
...
//	 String assetVarId="";
	...
	try{
		...
		assetVarId.append(StringEscapeUtils.escapeSql(CommonFunction.checkNull(assetVerificationVO.getAssetVerId()).trim()));
       	 ...
       	 bufInsSql.append("SELECT DISTINCT A.LOAN_ID,D.LOAN_NO,B.CUSTOMER_NAME,case when A.APPRAISER_TYPE='I' then 'Internal' else 'External' end as Appraiser,A.APPRAISER_NAME, ");
/*       	   bufInsSql.append("(SELECT C.AGENCY_NAME FROM com_agency_m C WHERE A.APPRAISER_ID=C.AGENCY_CODE)AS EXTRNAL_APPRAISER, ");*/
