...
public class MaturityClosureDAOImpl implements MaturityClosureDAO{
	
	private static final Logger logger = Logger.getLogger(MaturityClosureDAOImpl.class.getName());
	...
	try {
		...
		query.append(" e.rec_principal_overdue, e.waiveoff_amount,e.ADVANCE_EMI_REFUNDS,e.closure_type,(select Description from generic_master where generic_key='closure' and value=e.closure_type) as ClosureTypeDesc");//Virender
		...
		for (int i = 0; i < size; i++) {
			...
			if (subList.size() > 0) {
				...
				vo.setvClosureType((CommonFunction.checkNull(subList.get(33)).trim()));//Virender
				vo.setvClosureTypeDesc((CommonFunction.checkNull(subList.get(34)).trim()));//Virender
				closureData.add(vo);
			}
		}
