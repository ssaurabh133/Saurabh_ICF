...
public class InstrumentCapBehindAction extends Action {
	private static final Logger logger = Logger.getLogger(CommonPageBehindAction.class.getName());
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception{
			logger.info(" In the InstrumentCapBehindAction----------");
			HttpSession session = request.getSession();
			boolean flag=false;
			UserObject userobj=(UserObject)session.getAttribute("userobject");
			if(userobj==null){
				logger.info("here in execute method of InstrumentCapBehindAction action the session is out----------------");
			...
			request.setAttribute("maker", "present");
