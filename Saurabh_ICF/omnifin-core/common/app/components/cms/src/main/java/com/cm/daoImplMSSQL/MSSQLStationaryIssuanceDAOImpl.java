...
public class MSSQLStationaryIssuanceDAOImpl implements StationaryIssuanceDAO
{
			private static final Logger logger=Logger.getLogger(MSSQLStationaryIssuanceDAOImpl.class.getName());
    ...
    public boolean getsaveIssue(StationaryMasterForm stationaryMasterForm) 
		...
		{
			logger.info("stationaryMasterForm.getAllBranch(): "+stationaryMasterForm.getAllBranch());
    ...
    public List<StationaryMasterForm> getSearch(StationaryMasterForm stationaryMasterForm) {
		  ...
		  ArrayList<StationaryMasterForm> list = new ArrayList<StationaryMasterForm>();
		  	...
		  	try {		
		  			...
		  			query.append("select csa.BOOK_NO,CASE csa.STATIONARY_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END STATIONARY_TYPE,BRANCH_DESC,USER_NAME,");
		  			query.append(dbo);
		  			query.append("DATE_FORMAT(ISSUING_DATE,'%d-%m-%Y') ISSUING_DATE,CASE csa.REC_STATUS WHEN 'A' THEN 'ACTIVE' ELSE 'INACTIVE' END REC_STATUS ,csd.INSTRUMENT_FROM,csd.INSTRUMENT_TO from cr_stationary_allocation_dtl csa INNER JOIN cr_stationary_dtl CSD ON CSD.Book_No=csa.Book_No LEFT join com_branch_m cb on csa.ISSUING_BRANCH=cb.BRANCH_ID LEFT JOIN sec_user_m sm ON csa.ISSUING_USER=sm.USER_ID WHERE csa.HO_REC_STATUS='A' AND csa.ISSUING_BRANCH='"+CommonFunction.checkNull(stationaryMasterForm.getBranchid())+"' AND CSD.BRANCH_ACKNOWLEDGEMENT_DATE<>'' AND ISNULL(CSA.REC_STATUS,'')<>'A' ");
			...
			{     
				  ...
				  formStationary.setBookIssue("<a href=StationaryIssuanceDispatchAction.do?method=openEdit&bookNo="+ CommonFunction.checkNull(rs.getString(1))+">"+ CommonFunction.checkNull(rs.getString(1)).toString()+"</a>");
    ...
    public List<StationaryMasterForm> getSrchData(StationaryMasterForm stationaryMasterForm) {
		  ...
		  logger.info("St"+St);
		  	...
		  	try {		
	  		    ...
	  		    searchQuery.append("SELECT csa.BOOK_NO,CASE csa.STATIONARY_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END STATIONARY_TYPE,BRANCH_DESC,USER_NAME," );
	  		  searchQuery.append(dbo);
	  		    searchQuery.append("date_format(ISSUING_DATE,'"+dateFormat+"') ISSUING_DATE,CASE csa.REC_STATUS WHEN 'A' THEN 'ACTIVE' ELSE 'INACTIVE' END REC_STATUS ,csd.INSTRUMENT_FROM,csd.INSTRUMENT_TO from cr_stationary_allocation_dtl csa INNER JOIN cr_stationary_dtl CSD ON CSD.Book_No=csa.Book_No LEFT join com_branch_m cb on csa.ISSUING_BRANCH=cb.BRANCH_ID LEFT JOIN sec_user_m sm ON csa.ISSUING_USER=sm.USER_ID WHERE csa.ISSUING_BRANCH= '"+CommonFunction.checkNull(stationaryMasterForm.getBranchid())+"'");
    ...
    public List<StationaryMasterForm> getEditData(StationaryMasterForm formStationary) {
	    ...
	    try{
			 ...
			 logger.info("bookNo ********* : "+bookNo);
				...
				bufInsSql.append("select csa.BOOK_NO,CASE csa.STATIONARY_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END STATIONARY_TYPE,CSA.ISSUING_BRANCH,csa.issuing_user,csa.REC_STATUS,"+dbo+"date_format(ISSUING_DATE,'%d-%m-%Y') ISSUING_DATE,BRANCH_DESC,USER_NAME ,csd.INSTRUMENT_FROM,csd.INSTRUMENT_TO,csa.ALL_BRANCH,"+dbo+"date_format(csa.HO_ISSUING_DATE,'%d-%m-%Y') HO_ISSUING_DATE, "+dbo+"DATE_FORMAT( csd.return_to_ho_date,'%d-%m-%Y') return_to_ho_date from cr_stationary_allocation_dtl csa INNER JOIN cr_stationary_dtl CSD ON CSD.Book_No=csa.Book_No LEFT join com_branch_m cb on csa.ISSUING_BRANCH=cb.BRANCH_ID LEFT JOIN sec_user_m sm ON csa.ISSUING_USER=sm.USER_ID");
			...
			bufInsSql.append(" ORDER BY csa.BOOK_NO");
    ...
    public List<StationaryMasterForm> getHoAllocationSearch(
		...
		logger.info("getHoAllocationSearch --> ");
		  ...
		  logger.info("St"+St);
		  	...
		  	try {		
		  		StringBuilder query= new StringBuilder();
	  			query.append("SELECT csa.BOOK_NO,CASE csa.STATIONARY_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END STATIONARY_TYPE,BRANCH_DESC,''," );
	  			query.append(dbo);
	  			query.append("DATE_FORMAT(HO_ISSUING_DATE,'"+dateFormat+"') HO_ISSUING_DATE,CASE csa.HO_REC_STATUS WHEN 'A' THEN 'ACTIVE' ELSE 'INACTIVE' END HO_REC_STATUS,csd.INSTRUMENT_FROM,csd.INSTRUMENT_TO FROM cr_stationary_allocation_dtl csa INNER JOIN cr_stationary_dtl CSD ON CSD.Book_No=csa.Book_No LEFT join com_branch_m cb on csa.HO_ISSUING_BRANCH=cb.BRANCH_ID WHERE  ISNULL(CSA.HO_REC_STATUS,'')='' ");
			...
			{     
				  StationaryMasterForm formStationary=new StationaryMasterForm();	 
				  formStationary.setBookIssue("<a href=hoAllocationDispatchAction.do?method=openEdit&bookNo="+ CommonFunction.checkNull(rs.getString(1))+">"+ CommonFunction.checkNull(rs.getString(1)).toString()+"</a>");
				  formStationary.setDesc(rs.getString(2));
				  formStationary.setBranchid(rs.getString(3));
				  formStationary.setUserIssue(rs.getString(4));
				  formStationary.setIssuedate(rs.getString(5));
...
public List<StationaryMasterForm> getHoAllocationEdit(StationaryMasterForm formStationary) {
    ...
    try{
		 ...
		 logger.info("bookNo ********* : "+bookNo);
			...
			bufInsSql.append("select csa.BOOK_NO,CASE csa.STATIONARY_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END STATIONARY_TYPE,CSA.HO_ISSUING_BRANCH,csa.HO_REC_STATUS," );
			bufInsSql.append(dbo);
			bufInsSql.append("DATE_FORMAT(HO_ISSUING_DATE,'%d-%m-%Y') HO_ISSUING_DATE,BRANCH_DESC,HO_REMARKS ,csd.INSTRUMENT_FROM,csd.INSTRUMENT_TO," );
			bufInsSql.append(dbo);
			bufInsSql.append("DATE_FORMAT(csd.addition_date,'%d-%m-%Y') addition_date  FROM cr_stationary_allocation_dtl csa INNER JOIN cr_stationary_dtl CSD ON CSD.Book_No=csa.Book_No LEFT join com_branch_m cb on csa.HO_ISSUING_BRANCH=cb.BRANCH_ID ");			
...
public boolean saveForwardCancelReceipt(StationaryMasterForm stationaryMasterForm) {
	  ...
	  try
	  {
		  logger.info("stationaryMasterForm.getStatus(): "+stationaryMasterForm.getStatus());
		  logger.info("stationaryMasterForm.getMakerId(): "+stationaryMasterForm.getMakerId());
		  logger.info("stationaryMasterForm.getMakerDate(): "+stationaryMasterForm.getMakerDate());
	
	   con = ConnectionDAO.getConnection();
	   String sql3= " update cr_stationary_status_dtl set STATUS=?,REMARKS=?,USED_BY =?,USED_DATE="+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"') where STATUS='A' AND RECEPT_CHEQUE_NO='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getReceiptNo()))+"'";
...
//	  pstm.setString(5,CommonFunction.checkNull(stationaryMasterForm.getMakerId()));
	  ...
	  logger.info("saveForwardCancelReceipt status "+ message);
...
public String checkReceiptNoUsedCanceled(StationaryMasterForm stationaryMasterForm) {
	...
	try
	{        
//		String bookQuery ="select book_no from cr_stationary_status_dtl where  RECEPT_CHEQUE_NO='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getReceiptNo()))+"' AND STATUS='A' " ;
//	          logger.info("Get book no corresponding Receipt No book Query: "+ bookQuery);
//	          String bookNo=ConnectionDAO.singleReturn(bookQuery);
	
			String query ="select ISNULL(STATUS,'') from cr_stationary_status_dtl where  RECEPT_CHEQUE_NO='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getReceiptNo()))+"'" ;
...
public String prevCheckReceiptNoUsedCanceled(
	...
	{       int previousReceiptNo=1;
		    ...
		    {
		    	String sqlQuery ="select ISNULL(STATUS,'')  from cr_stationary_status_dtl where BOOK_NO='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(bookNo))+"' AND RECEPT_CHEQUE_NO='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(previousReceiptNo))+"'" ;
...
public boolean saveForwardUsedReceipt(StationaryMasterForm stationaryMasterForm) {
     ...
     int message=0;
	  ...
	  try
	  {
		   ...
		   String sql3=" update cr_stationary_status_dtl set STATUS=?,REMARKS=?,USED_BY =?,USED_DATE="+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"') where STATUS='A' AND RECEPT_CHEQUE_NO ='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getReceiptNo()))+"' AND BOOK_NO='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(bookNo))+"'";		
	...
	try
	{     
		String sqlQuery ="select count(1) from cr_stationary_status_dtl where ISNULL(STATUS,'')<>'' AND RECEPT_CHEQUE_NO='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getReceiptNo()))+"' " +
    ...
    public List<StationaryMasterForm> getAcknowledgement(StationaryMasterForm stationaryMasterForm) {
		...
		try {		
				query.append("select csa.BOOK_NO,CASE csa.STATIONARY_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END STATIONARY_TYPE,BRANCH_DESC,USER_NAME,dbo.date_format(ISSUING_DATE,'"+dateFormat+"') ISSUING_DATE,CASE csa.REC_STATUS WHEN 'A' THEN 'ACTIVE' ELSE 'INACTIVE' END REC_STATUS,csd.INSTRUMENT_FROM,csd.INSTRUMENT_TO from cr_stationary_allocation_dtl csa INNER JOIN cr_stationary_dtl CSD ON CSD.Book_No=csa.Book_No  LEFT join com_branch_m cb on csa.ISSUING_BRANCH=cb.BRANCH_ID LEFT JOIN sec_user_m sm ON csa.ISSUING_USER=sm.USER_ID WHERE csa.HO_REC_STATUS='A' AND csa.ISSUING_BRANCH= '"+CommonFunction.checkNull(stationaryMasterForm.getBranchid())+"' AND ISNULL(CSD.BRANCH_ACKNOWLEDGEMENT_DATE,'')='' ");
			...
			{    
				  ...
				  formStationary.setBookIssue("<a href=StationaryIssuanceDispatchAction.do?method=openEditAcknowledgement&bookNo="+ CommonFunction.checkNull(rs.getString(1))+">"+ CommonFunction.checkNull(rs.getString(1)).toString()+"</a>");
				  formStationary.setDesc(rs.getString(2));
				  formStationary.setBranchid(rs.getString(3));
				  formStationary.setUserIssue(rs.getString(4));
				  formStationary.setIssuedate(rs.getString(5));
				  ...
				  list.add(formStationary);
						...
						//logger.info("in -->>>>>>>> getAcknowledgement inside query...."+ list);
			 }	
		   catch(SQLException e) {
		  	 	e.printStackTrace();
			}finally {
    ...
    public boolean getSaveAcknow(StationaryMasterForm stationaryMasterForm) {
		...
		try {						
			sql3.append(" update cr_stationary_dtl set BRANCH_ACKNOWLEDGEMENT_DATE=dbo.STR_TO_DATE(?, '"+dateFormatWithTime+"')," +
		...
		logger.info("in -->getSrchAcknow ");
		  ...
		  ArrayList<StationaryMasterForm> list = new ArrayList<StationaryMasterForm>();
			...
			{     
				  ...
				  formStationary.setBookIssue("<a href=StationaryIssuanceDispatchAction.do?method=openEditAcknowledgement&bookNo="+
				  ...
				  list.add(formStationary);				  
						...
						//logger.info("in -->>>>>>>> getSrchAcknow inside query...."+ list);
			 }	
		   catch(SQLException e) {
		  	 	e.printStackTrace();
			}finally {
    ...
    public List<StationaryMasterForm> getSrchHo(StationaryMasterForm stationaryMasterForm) {
		logger.info("in -->getSrchHo ");
		  ...
		  StationaryMasterForm formStationary=null;
		  	...
		  	try {
	  		    searchQuery.append(" SELECT csa.BOOK_NO,CASE csa.STATIONARY_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END STATIONARY_TYPE,BRANCH_DESC,USER_NAME,dbo.date_format(ISSUING_DATE,'"+dateFormat+"') ISSUING_DATE,CASE csa.REC_STATUS WHEN 'A' THEN 'ACTIVE' ELSE 'INACTIVE' END REC_STATUS ,csd.INSTRUMENT_FROM,csd.INSTRUMENT_TO from cr_stationary_allocation_dtl csa INNER JOIN cr_stationary_dtl CSD ON CSD.Book_No=csa.Book_No LEFT join com_branch_m cb on csa.ISSUING_BRANCH=cb.BRANCH_ID LEFT JOIN sec_user_m sm ON csa.ISSUING_USER=sm.USER_ID WHERE csa.ISSUING_BRANCH= '"+CommonFunction.checkNull(stationaryMasterForm.getBranchid())+"' AND CSA.REC_STATUS='A' AND ISNULL(CSD.RETURN_TO_HO_FLAG,'')<>'R' ");
	  		    if(!CommonFunction.checkNull(St).equalsIgnoreCase(""))
	  		    {
	  		    	searchQuery.append(" AND csa.STATIONARY_TYPE='"+CommonFunction.checkNull(St)+"'");
	  		    }
			...
			{     
				  ...
				  formStationary.setBookIssue("<a href=StationaryIssuanceDispatchAction.do?method=openEditHo&bookNo="+
						  CommonFunction.checkNull(rs.getString(1))+">"+ CommonFunction.checkNull(rs.getString(1)).toString()+"</a>");
				  formStationary.setDesc(rs.getString(2));
				  formStationary.setBranchid(rs.getString(3));
				  formStationary.setUserIssue(rs.getString(4));
	...
	public boolean getSaveAcknowHo(StationaryMasterForm stationaryMasterForm) {
		...
		try {
			sql3.append(" update cr_stationary_dtl set RETURN_TO_HO_FLAG='R',RETURN_TO_HO_DATE=dbo.STR_TO_DATE(?, '"+dateFormatWithTime+"')," +
		  ...
		  StationaryMasterForm formStationary=null;
			...
			{     
				  ...
				  list.add(formStationary);				  
						...
						logger.info("in -->>>>>>>> getSrchAcknowHo inside query...."+ list);
			 }	
		   catch(SQLException e) {
		  	 	e.printStackTrace();
			}finally {
		...
		logger.info("in -->getSearchHoStationaryAcknow ");
		  ...
		  ArrayList<StationaryMasterForm> list = new ArrayList<StationaryMasterForm>();
			...
			{     
				  ...
				  formStationary.setBookIssue("<a href=StationaryIssuanceDispatchAction.do?method=HoStationaryAcknow&bookNo="+
						  CommonFunction.checkNull(rs.getString(1))+">"+ CommonFunction.checkNull(rs.getString(1)).toString()+"</a>");
				  formStationary.setDesc(rs.getString(2));
				  formStationary.setBranchid(rs.getString(3));
				  formStationary.setUserIssue(rs.getString(4));
...
public List<StationaryMasterForm> SrchAcknowStationaryHo(StationaryMasterForm stationaryMasterForm) {
		  ...
		  ArrayList<StationaryMasterForm> list = new ArrayList<StationaryMasterForm>();
			...
			{     
				formStationary=new StationaryMasterForm();
				  ...
				  formStationary.setBookIssue("<a href=StationaryIssuanceDispatchAction.do?method=stationaryHoAcknowledge&bookNo="+
						  CommonFunction.checkNull(rs.getString(1))+">"+ CommonFunction.checkNull(rs.getString(1)).toString()+"</a>");
				  formStationary.setDesc(rs.getString(2));
				  formStationary.setBranchid(rs.getString(3));
				  formStationary.setUserIssue(rs.getString(4));
	...
	public boolean SaveStationaryAcknowHo(StationaryMasterForm stationaryMasterForm) {
		...
		try {						
				 sql3.append(" update cr_stationary_dtl set HO_STATIONARY_DATE=dbo.STR_TO_DATE(?, '"+dateFormatWithTime+"')," +
						"HO_STATIONARY_REMARKS=? where BOOK_NO ='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getBookIssue()))+"'");
			
				  pstm = con.prepareStatement(sql3.toString());
				  pstm.setString(1,CommonFunction.checkNull(stationaryMasterForm.getAdditionDate()));
				  pstm.setString(2,CommonFunction.checkNull(stationaryMasterForm.getRemarks()));
				  					  																									
				  logger.info("update query SaveStationaryAcknowHo:---   mradul "+ sql3);
				  message = pstm.executeUpdate();
				  pstm=null;
				    if(message>0)
				    {
...
public List<StationaryMasterForm> getStationaryBranchChangeSearch(
		...
		logger.info("getStationaryBranchChangeSearch --> ");
		  ...
		  StationaryMasterForm formStationary=null;
			...
			{     
				  formStationary=new StationaryMasterForm();	 
				  formStationary.setBookIssue("<a href=StationaryBranchgChangeDispatchAction.do?method=openEditBranchChange&bookNo="+ CommonFunction.checkNull(rs.getString(1))+">"+ CommonFunction.checkNull(rs.getString(1)).toString()+"</a>");
				  formStationary.setDesc(rs.getString(2));
				  formStationary.setBranchid(rs.getString(3));
				  formStationary.setUserIssue(rs.getString(4));
				  formStationary.setIssuedate(rs.getString(5));
	...
	public boolean saveStationarybranchChange(StationaryMasterForm stationaryMasterForm) {
		 logger.info("saveForwardCancelReceipt ");
		  ...
		  try
		  {
			...
			sql3.append(" update cr_stationary_allocation_dtl set ISSUING_BRANCH=?,HO_ISSUING_BRANCH=?,REMARKS=? where BOOK_NO ='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getBookIssue()))+"'" );
			sql4.append(" update cr_stationary_status_dtl set issuing_branch= ? where book_no='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(stationaryMasterForm.getBookIssue()))+"'");
             ...
             pstm1 = con.prepareStatement(sql4.toString());
			  ...
			  pstm.setString(2,CommonFunction.checkNull(stationaryMasterForm.getLbxBranchId()));
			  pstm.setString(3,CommonFunction.checkNull(stationaryMasterForm.getHoRemarks()));
