...
{
  private static final Logger logger = Logger.getLogger(ReportsDAOImpl.class.getName());
  ...
  public ArrayList getReportFormat()
  {
    ...
    try {
      ...
      String query = "SELECT VALUE,DESCRIPTION FROM generic_master WHERE GENERIC_KEY='REPORT_FORMAT' and rec_status ='A' order by if(value=IFNULL((select PARAMETER_VALUE from parameter_mst where PARAMETER_KEY = 'DEFAULT_REPORT_FORMAT'),'H'),0,1), VALUE desc ";
  ...
  public ArrayList getReportFormat_NEW()
  {
    ...
    try {
      ...
      String query = "SELECT VALUE,DESCRIPTION FROM generic_master WHERE GENERIC_KEY='REPORT_FORMAT_NEW' and rec_status ='A' order by if(value=IFNULL((select PARAMETER_VALUE from parameter_mst where PARAMETER_KEY = 'DEFAULT_REPORT_FORMAT'),'H'),0,1), VALUE desc ";
  ...
  public ArrayList getCompanyAddress(int company_ID)
  {
    ...
    try {
      ...
      String query = new StringBuilder().append("select COMPANY_MSG,concat(ifnull(ADDRESS_LINE1,''),' ',ifnull(ADDRESS_LINE2,''),' ',ifnull(ADDRESS_LINE3,''))addressLine1,if(CITY is null,' ',concat(CITY,' ')),if(PINCODE is null,' ',concat('-',PINCODE)) as PINCODE,if(PHONE_NO is null,' ',concat('Phone No-:',PHONE_NO))as PHONE_NO,if(FAX is null,' ',concat('Fax: ',FAX))as FAX,if(EMAIL_ID is null,' ',concat('Email :',EMAIL_ID))as EMAIL_ID,if(trim(ifnull(WEBSITE,''))='',' ',concat('Website:',WEBSITE))as WEBSITE from com_company_m where company_id='").append(company_ID).append("'").toString();
  ...
  public boolean checkNoOfLoan(String fromDateDB, String toDateDB)
  {
    ...
    query.append(new StringBuilder().append("SELECT COUNT(DISTINCT a.LOAN_ID) FROM cr_loan_dtl a JOIN gcd_customer_m c on(c.CUSTOMER_ID=a.LOAN_CUSTOMER_ID)JOIN cr_deal_loan_dtl e ON(e.DEAL_ID=a.LOAN_DEAL_ID)WHERE date(a.LOAN_APPROVAL_DATE) >='").append(fromDateDB).append("' ").append("AND date(a.LOAN_APPROVAL_DATE) <='").append(toDateDB).append("' and a.REC_STATUS in('A','C') and a.LOAN_ID not in(select LOAN_ID from closed_loans)ORDER BY a.LOAN_ID").toString());
  ...
  public String getStartingDate(String businessDate)
  {
    String stDate = "";
    StringBuffer bufInsSql = new StringBuffer();
    bufInsSql.append(new StringBuilder().append(" SELECT DATE_FORMAT(DATE_ADD(LAST_DAY(DATE_ADD('").append(businessDate.trim()).append("',INTERVAL -1 MONTH)),INTERVAL 1 DAY),'%d-%m-%Y')").toString());
  ...
  public String getEndDate() {
    String edDate = "";
    StringBuffer bufInsSql = new StringBuffer();
    bufInsSql.append(" SELECT LAST_DAY((select PARAMETER_VALUE from parameter_mst where PARAMETER_KEY='BUSINESS_DATE'))");
  ...
  public String getLastMonthDate(String date)
  {
    String edDate = "";
    StringBuffer bufInsSql = new StringBuffer();
    bufInsSql.append(new StringBuilder().append("select date_format(DATE_ADD(str_to_date('").append(date).append("','%d-%m-%Y'), INTERVAL -1 MONTH),'%d-%m-%Y')").toString());
    logger.info(new StringBuilder().append("query to getEndDate  :  ").append(bufInsSql.toString()).toString());
    try
    {
      edDate = ConnectionReportDumpsDAO.singleReturn(bufInsSql.toString());
  ...
  public ArrayList<ReportsForm> getfinanceYear()
  {
    ...
    try
    {
      String query = "select DATE_FORMAT(start_date,'%d-%m-%Y')year from gl_accounting_year_m order by start_date";
      logger.info(new StringBuilder().append("query To get finantialYear   :  ").append(query).toString());
      ArrayList formatlist = ConnectionReportDumpsDAO.sqlSelect(query.toString());
      for (int i = 0; i < formatlist.size(); i++)
      {
        ArrayList data = (ArrayList)formatlist.get(i);
  ...
  public String getMaxDefaultDate()
  {
    ...
    try
    {
      String query = "select DATE_FORMAT(min(end_date),'%d-%m-%Y')year from gl_accounting_year_m";
      logger.info(new StringBuilder().append("query To get default max Finantial Date   :  ").append(query).toString());
      date = ConnectionReportDumpsDAO.singleReturn(query);
    }
    catch (Exception e) {
      e.printStackTrace();
    }return date;
  ...
  public String getcutOffDate()
  {
    ...
    try
    {
      String query = "select DATE_FORMAT(max(MVMT_DATE),'%d-%m-%Y')date from cr_loan_dtl_eod";
      logger.info(new StringBuilder().append("Query for find cutOffDate   :  ").append(query).toString());
      date = ConnectionReportDumpsDAO.singleReturn(query);
    }
    catch (Exception e) {
      e.printStackTrace();
  ...
  public ArrayList getDONo(String pLoanId, String userId, String nhbDate)
  {
    ...
    try
    {
      String query = new StringBuilder().append("select count(1) from cr_do_dtl where loan_id='").append(CommonFunction.checkNull(pLoanId).trim()).append("'").toString();
      ...
      if (count == 0)
      {
        ...
        query = new StringBuilder().append("insert into cr_do_dtl(LOAN_ID,USER_ID,REPORT_DATE) values(?,?,DATE_ADD(STR_TO_DATE(?, '").append(this.dateFormatWithTime).append("'),INTERVAL CURTIME() HOUR_SECOND))").toString();
        ...
        record = "Duplicate";
      }query = new StringBuilder().append("select DO_SEQUENCE from cr_do_dtl where loan_id='").append(CommonFunction.checkNull(pLoanId).trim()).append("'").toString();
      logger.info(new StringBuilder().append("In getDONo() of ReportDAOImpl.   query    :   ").append(query).toString());
  ...
  public String getcamTemplatePath()
  {
    String path = "";
    StringBuffer bufInsSql = new StringBuffer();
    bufInsSql.append(" select parameter_value from parameter_mst where parameter_key = 'Mobile_Download_Path'");
    logger.info(new StringBuilder().append("query to camTemplatePath  :  ").append(bufInsSql.toString()).toString());
    try
    {
      path = ConnectionReportDumpsDAO.singleReturn(bufInsSql.toString());
      logger.info(new StringBuilder().append("Application Server Path : ").append(path).toString());
  ...
  public String getCutoffDateForIncipient()
  {
    ...
    bufInsSql.append(" select max(mvmt_date)mvmt_date from cr_loan_dtl_eod ");
    ...
    try
    {
      ...
      if (s1.equalsIgnoreCase("S")) {
        if (CommonFunction.checkNull(source).trim().equalsIgnoreCase("NR"))
        {
          ...
          query = new StringBuilder().append("select (select sum(DISBURSAL_AMOUNT)DISBURSAL_AMOUNT from cr_loan_disbursal_dtl where rec_status='A' and loan_id=").append(pLoanId).append(")DISBURSED_AMOUNT,").append("(select DEAL_SANCTION_AMOUNT from cr_deal_loan_dtl cdld left join cr_loan_dtl cld on cld.LOAN_DEAL_LOAN_ID=cdld.deal_loan_id where loan_id=").append(pLoanId).append(")DEAL_SANCTION_AMOUNT,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(address_dtl.customer_name,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')customer_name,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(address_dtl.addressLine1,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine1,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(address_dtl.addressLine2,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine2,").append(" phone, concat(com_branch_m.branch_desc,' ')as branch_desc,cr_loan_dtl.LOAN_NO,cr_loan_dtl.LOAN_id,concat(cr_product_m.product_desc,' ')as product_desc,").append(" concat(cr_scheme_m.scheme_desc,' ')as scheme_desc,IF(cr_loan_dtl.loan_approval_date='0000-00-00 00:00:00',NULL,cr_loan_dtl.loan_approval_date) as loan_start_date,").append(" if(st_date.instl_date='0000-00-00',NULL,st_date.instl_date) as install_start_date, IF(ed_date.instl_date='0000-00-00',NULL,ed_date.instl_date) as install_end_date,").append(" cr_loan_dtl.loan_tenure as total_tenure,CASE lOAN_REPAYMENT_FREQ WHEN 'M' THEN 'Monthly ' WHEN 'B' THEN 'Bimonthly ' WHEN 'Q' THEN 'Quarterly ' WHEN 'H' THEN 'Half yearly  'else 'Yearly ' end as freqency,").append(" cr_loan_dtl.LOAN_NO_OF_INSTALLMENT as no_of_instalments,if (cr_loan_dtl.REC_STATUS='C', ( select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)+ifnull(cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=18 and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ) , cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL) as LOAN_RECEIVED_PRINCIPAL,").append(" (SELECT sum(INSTL_AMOUNT_RECD/INSTL_AMOUNT) FROM CR_REPAYSCH_DTL WHERE BILL_FLAG='Y' AND INSTL_AMOUNT_RECD>0 AND REC_TYPE='I' AND LOAN_ID='").append(pLoanId).append("' group by loan_id) AS EMI_RECD,").append(" (IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_INSTL_AMOUNT,0)-IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_PRINCIPAL,0))+ ifnull((select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=9 and txn_type in ('ECA','MCA') and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ),0) AS RECD_INTEREST,").append(" (select count(loan_id) from cr_statement_tmp where loan_id='").append(pLoanId).append("' and next_status='B' and USER_ID='").append(userID).append("') as no_bounce,").append(" cr_loan_dtl.loan_loan_amount,cr_loan_dtl.loan_emi_amount as emi_amount, cr_loan_dtl.loan_balance_principal as principal_os,").append(" ifnull(instlDue.instl_due,0) over_due_amount,ifnull(B.other_charge_due,0)as other_charge_due,cr_loan_dtl.REC_STATUS, ").append(" IF(cr_loan_dtl.REC_STATUS='C', 'CLOSED ','ACTIVE')  as loan_status,").append(" ifnull(cm.collateral_amount,0)collateral_amount,cr_loan_dtl.loan_no, IF(L.TXN_DATE='0000-00-00',NULL,L.TXN_DATE)TXN_DATE,").append(" concat(L.PARTICULARS,' ')as PARTICULARS,L.DEBIT,L.CREDIT,CASE cr_loan_dtl.LOAN_RATE_TYPE WHEN 'F' THEN 'FLAT ' WHEN 'P' THEN 'PTPM ' WHEN 'E' THEN 'EFFECTIVE ' END AS LOAN_RATE_TYPE,").append(" cr_loan_dtl.LOAN_FINAL_RATE,0.00 as Opening_bls,cr_loan_dtl.LOAN_MATURITY_DATE ,IFNULL(TERM.TERMINATION_DATE,'')  AS TERMINATION_DATE,").append(" concat(if(ifnull(LEGAL_FLAG,'NA')='A','LEGAL|',''),if(ifnull(REPO_FLAG,'NA')='A','REPO|',''),IFNULL((select GROUP_CONCAT(DESCRIPTION SEPARATOR '|')from generic_master where generic_key='CASE_MARKING_FLAG' AND INSTR(CONCAT( '|',CASE_MARKING_FLAG,'|'),CONCAT('|',VALUE,'|') ) > 0 ),''))legalRepoFlag ").append(" ,concat(Vehicle.VEHICLE_ASSET_DETAILS,' ')as VEHICLE_ASSET_DETAILS,IFNULL(EA.EXCESS,0) as EXCESS,concat(Vehicles.VEHICLE_NO,'')as VEHICLE_No ").append(" ,concat(Vehicles.VEHICLE_MAKE,' ')as  VEHICLE_MAKE,concat(Vehicles.VEHICLE_MODEL,' ')as  VEHICLE_MODEL,concat(Vehicles.ENGINE_NUMBER,' ')as  ENGINE_NUMBER,concat(Vehicles.VEHICLE_CHASIS_NUMBER,' ')as  VEHICLE_CHASIS_NUMBER ,ifnull(cr_loan_dtl.loan_dpd,'')  as loan_dpd").append(" from  cr_loan_dtl  ").append("  LEFT JOIN CR_TERMINATION_DTL TERM ON cr_loan_dtl.LOAN_ID=TERM.LOAN_ID AND TERM.REC_STATUS='A'  ").append(" left outer join ").append(" (").append(" \t\tselect cr_loan_dtl.LOAN_ID,concat(gcd_customer_m.customer_name,' ')as customer_name, ").append(" \t\tif(ADDRESS_LINE1 is null,if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,' ',concat(ADDRESS_LINE3,' ')),if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE2,' '),concat(concat(ADDRESS_LINE2,','),concat(ADDRESS_LINE3,' ')))),if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE1,' '),concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE3,' '))),if(ADDRESS_LINE3 is null,concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,' ')),concat(concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,',')),concat(ADDRESS_LINE3,' ')))))as addressLine1,").append(" \t\tconcat(if(DISTRICT_DESC is null,if(STATE_DESC is null,if(COUNTRY_DESC is null,' ',concat('(',COUNTRY_DESC,') ')),if(COUNTRY_DESC is null,concat(STATE_DESC,' '),concat(concat(STATE_DESC,','),concat('(',COUNTRY_DESC,') ')))),if(STATE_DESC is null,if(COUNTRY_DESC is null,concat(DISTRICT_DESC,' '),concat(concat(DISTRICT_DESC,','),concat('(',COUNTRY_DESC,') '))),if(COUNTRY_DESC is null,concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,' ')),concat(concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,',')),concat('(',COUNTRY_DESC,') '))))),if(PINCODE is null,' ',concat('- ',PINCODE)))as addressLine2,").append(" \t\tif(PRIMARY_PHONE is null,if(ALTERNATE_PHONE is null,' ',concat('Phone No.-',concat(ALTERNATE_PHONE,' '))),if(ALTERNATE_PHONE is null,concat('Phone No.-',concat(PRIMARY_PHONE,' ')),concat('Phone No.-',concat(concat(PRIMARY_PHONE,','),concat(ALTERNATE_PHONE,' '))))) as phone").append("  \t\tfrom  cr_loan_dtl  ").append(" \t\tjoin gcd_customer_m on(cr_loan_dtl.loan_customer_id=gcd_customer_m.customer_id    and cr_loan_dtl.LOAN_ID='").append(pLoanId).append("') ").append(" \t\tleft outer join com_address_m on((com_address_m.BPID=gcd_customer_m.CUSTOMER_ID) and(com_address_m.COMMUNICATION_ADDRESS='Y'))").append(" \t\tleft outer join com_country_m on(com_country_m.COUNTRY_ID=com_address_m.COUNTRY)").append(" \t\tleft outer join com_state_m on((com_state_m.COUNTRY_ID=com_address_m.COUNTRY)and(com_state_m.STATE_ID=com_address_m.STATE))").append("  \t\tleft outer join com_district_m on(com_district_m.STATE_ID=com_address_m.STATE)and(com_district_m.DISTRICT_ID=com_address_m.DISTRICT) limit 1 ").append(" )address_dtl on cr_loan_dtl.loan_id=address_dtl.loan_id   ").append(" join com_branch_m on(com_branch_m.branch_id=cr_loan_dtl.loan_branch)").append(" join cr_product_m on(cr_product_m.product_id=cr_loan_dtl.loan_product) ").append(" join cr_scheme_m  on(cr_scheme_m.scheme_id=cr_loan_dtl.loan_scheme)  ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,min(instl_date)'instl_date' ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere  ").append("   LOAN_ID='").append(pLoanId).append("' and  IFNULL(REC_TYPE,'I')='I' group by loan_id ").append(" ) as st_date on(st_date.loan_id=cr_loan_dtl.loan_id) ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,instl_date,instl_no ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere instl_no=(select max(instl_no) from cr_repaysch_dtl where LOAN_ID='").append(pLoanId).append("') ").append(" \t\tand LOAN_ID='").append(pLoanId).append("'").append(" ) as ed_date on(ed_date.loan_id=cr_loan_dtl.loan_id) ").append(" left outer join ").append(" (").append(" \t\tselect  sum(cr_sd_dtl.SD_AMOUNT) as collateral_amount,cr_sd_dtl.LOAN_ID ").append(" \t\tfrom cr_sd_dtl ").append(" \t\twhere rec_status not in('X','L','P','F') and  cr_sd_dtl.LOAN_ID='").append(pLoanId).append("'").append(" ) as cm on(cr_loan_dtl.LOAN_ID=cm.LOAN_ID) ").append(" left outer join ").append(" (").append(" \t\tselect  sum(ifnull(cr_txnadvice_dtl.advice_amount,0))- sum(ifnull(txn_adjusted_amount,0)) as other_charge_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl where advice_type = 'R' and CHARGE_CODE_ID <> 7 and CHARGE_CODE_ID <> 27 and loan_id='").append(pLoanId).append("' ").append(" \t\tgroup by loan_id").append(" ) as B on(B.loan_id=cr_loan_dtl.loan_id)  ").append(" left outer join ").append(" (").append(" \t\tselect  sum(ifnull(cr_txnadvice_dtl.advice_amount,0))- sum(ifnull(txn_adjusted_amount,0)) as instl_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl where advice_type = 'R' and CHARGE_CODE_ID IN(7,27) and loan_id='").append(pLoanId).append("' ").append(" \t\tgroup by loan_id").append(" ) as instlDue on(instlDue.loan_id=cr_loan_dtl.loan_id)  ").append(" LEFT OUTER JOIN ").append(" (").append(" \t\tselect TXN_DATE,LOAN_ID, PARTICULARS , DEBIT , CREDIT ").append(" \t\tFROM CR_STATEMENT_TMP where USER_ID='").append(userID).append("' ").append(" ) AS L ON(L.LOAN_ID=cr_loan_dtl.LOAN_ID)").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(c.ASSET_COLLATERAL_CLASS='VEHICLE',concat(c.VEHICLE_MAKE,' | ',c.VEHICLE_MODEL),c.ASSET_COLLATERAL_DESC))  as VEHICLE_ASSET_DETAILS ").append("\tfrom  cr_loan_collateral_m b ").append("\tleft join cr_asset_collateral_m c on c.ASSET_ID=b.ASSETID ").append("\twhere b.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicle on (Vehicle.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(t.ASSET_COLLATERAL_CLASS='VEHICLE',t.vehicle_registration_no,''))  as VEHICLE_NO ").append("   ,GROUP_CONCAT(t.VEHICLE_MAKE SEPARATOR '|')  as VEHICLE_MAKE,GROUP_CONCAT(t.VEHICLE_MODEL SEPARATOR '|')  as VEHICLE_MODEL,GROUP_CONCAT(t.ENGINE_NUMBER SEPARATOR '|')  as ENGINE_NUMBER,GROUP_CONCAT(t.VEHICLE_CHASIS_NUMBER SEPARATOR '|')  as VEHICLE_CHASIS_NUMBER ").append("\tfrom  cr_loan_collateral_m s ").append("\tleft join cr_asset_collateral_m t on t.ASSET_ID=s.ASSETID  and t.ASSET_COLLATERAL_CLASS='VEHICLE' ").append("\twhere s.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicles on (Vehicles.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tselect LOAN_ID,SUM(IFNULL(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0),0.00)) AS EXCESS ").append("\tfrom cr_txnadvice_dtl where ADVICE_TYPE='P' and CHARGE_CODE_ID=10 ").append("\tand LOAN_ID='").append(pLoanId).append("' ").append("\tgroup by LOAN_ID ").append(" ) as EA on (EA.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" where cr_loan_dtl.LOAN_ID='").append(pLoanId).append("' and ifnull(cr_loan_dtl.REC_STATUS,'P')<>'X' order by L.TXN_DATE").toString();
        }
        else
        {
          query = new StringBuilder().append("  select (select sum(DISBURSAL_AMOUNT)DISBURSAL_AMOUNT from cr_loan_disbursal_dtl where rec_status='A' and loan_id=").append(pLoanId).append(")DISBURSED_AMOUNT,").append("(select DEAL_SANCTION_AMOUNT from cr_deal_loan_dtl cdld left join cr_loan_dtl cld on cld.LOAN_DEAL_LOAN_ID=cdld.deal_loan_id where loan_id=").append(pLoanId).append(")DEAL_SANCTION_AMOUNT,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(add_dtl.customer_name,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')customer_name,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(add_dtl.addressLine1,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine1,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(add_dtl.addressLine2,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine2,").append(" phone,concat(com_branch_m.branch_desc,' ')as branch_desc,cr_loan_dtl.LOAN_NO,cr_loan_dtl.LOAN_id,concat(cr_product_m.product_desc,' ')as product_desc,").append(" concat(cr_scheme_m.scheme_desc,' ')as scheme_desc,IF(cr_loan_dtl.loan_approval_date='0000-00-00 00:00:00',NULL,cr_loan_dtl.loan_approval_date) as loan_start_date,").append(" if(st_date.instl_date='0000-00-00',NULL,st_date.instl_date) as install_start_date, IF(ed_date.instl_date='0000-00-00',NULL,ed_date.instl_date) as install_end_date,").append(" cr_loan_dtl.loan_tenure as total_tenure,CASE lOAN_REPAYMENT_FREQ  WHEN 'M' THEN 'Monthly ' WHEN 'B' THEN 'Bimonthly ' WHEN 'Q' THEN 'Quarterly ' WHEN 'H' THEN 'Half yearly  'else 'Yearly ' end as freqency,").append(" cr_loan_dtl.LOAN_NO_OF_INSTALLMENT as no_of_instalments,if (cr_loan_dtl.REC_STATUS='C', ( select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)+ifnull(cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=18 and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ) , cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL) as LOAN_RECEIVED_PRINCIPAL, ").append(" (SELECT sum(INSTL_AMOUNT_RECD/INSTL_AMOUNT) FROM CR_REPAYSCH_DTL WHERE BILL_FLAG='Y' AND INSTL_AMOUNT_RECD>0 AND REC_TYPE='I' AND LOAN_ID='").append(pLoanId).append("' group by loan_id) AS EMI_RECD,").append(" (IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_INSTL_AMOUNT,0)-IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_PRINCIPAL,0))+ ifnull((select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=9 and txn_type in ('ECA','MCA') and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ),0)AS RECD_INTEREST,").append(" (select count(loan_id) from cr_statement_tmp where loan_id='").append(pLoanId).append("' and next_status='B' and USER_ID='").append(userID).append("') as no_bounce,").append(" cr_loan_dtl.loan_loan_amount,cr_loan_dtl.loan_emi_amount as emi_amount,cr_loan_dtl.loan_balance_principal as principal_os,").append(" ifnull(instlDue.instl_due,0) over_due_amount,ifnull(B.other_charge_due,0)as other_charge_due,cr_loan_dtl.REC_STATUS,").append(" IF(cr_loan_dtl.REC_STATUS='C', 'CLOSED ','ACTIVE')  as loan_status,").append(" ifnull(cm.collateral_amount,0)collateral_amount,cr_loan_dtl.loan_no,IF(L.TXN_DATE='0000-00-00',NULL,L.TXN_DATE)TXN_DATE,").append(" concat(L.PARTICULARS,' ')as PARTICULARS,L.DEBIT,L.CREDIT,CASE cr_loan_dtl.LOAN_RATE_TYPE WHEN 'F' THEN 'FLAT ' WHEN 'P' THEN 'PTPM ' WHEN 'E' THEN 'EFFECTIVE ' END AS LOAN_RATE_TYPE,").append(" cr_loan_dtl.LOAN_FINAL_RATE,ifnull(opBls.balance,0) as Opening_bls,cr_loan_dtl.LOAN_MATURITY_DATE ,IFNULL(TERM.TERMINATION_DATE,'') AS TERMINATION_DATE ").append(" ,concat(if(ifnull(LEGAL_FLAG,'NA')='A','LEGAL|',''),if(ifnull(REPO_FLAG,'NA')='A','REPO|',''),IFNULL((select GROUP_CONCAT(DESCRIPTION SEPARATOR '|')from generic_master where generic_key='CASE_MARKING_FLAG' AND INSTR(CONCAT( '|',CASE_MARKING_FLAG,'|'),CONCAT('|',VALUE,'|') ) > 0 ),''))legalRepoFlag ").append(" ,concat(Vehicle.VEHICLE_ASSET_DETAILS,' ')as VEHICLE_ASSET_DETAILS,IFNULL(EA.EXCESS,0) as EXCESS,concat(Vehicles.VEHICLE_NO,' ')as VEHICLE_No ").append(" ,concat(Vehicles.VEHICLE_MAKE,' ')as  VEHICLE_MAKE,concat(Vehicles.VEHICLE_MODEL,' ')as  VEHICLE_MODEL,concat(Vehicles.ENGINE_NUMBER,' ')as  ENGINE_NUMBER,concat(Vehicles.VEHICLE_CHASIS_NUMBER,' ')as  VEHICLE_CHASIS_NUMBER ,ifnull(cr_loan_dtl.loan_dpd,'')  as loan_dpd").append(" from  cr_loan_dtl  ").append("  LEFT JOIN CR_TERMINATION_DTL TERM ON cr_loan_dtl.LOAN_ID=TERM.LOAN_ID AND TERM.REC_STATUS='A'  ").append(" left outer join").append(" (").append(" \t\tselect cr_loan_dtl.LOAN_ID,concat(gcd_customer_m.customer_name,' ')as customer_name, if(ADDRESS_LINE1 is null,if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,' ',concat(ADDRESS_LINE3,' ')),if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE2,' '),concat(concat(ADDRESS_LINE2,','),concat(ADDRESS_LINE3,' ')))),if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE1,' '),concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE3,' '))),if(ADDRESS_LINE3 is null,concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,' ')),concat(concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,',')),concat(ADDRESS_LINE3,' ')))))as addressLine1,").append(" \t\tconcat(if(DISTRICT_DESC is null,if(STATE_DESC is null,if(COUNTRY_DESC is null,' ',concat('(',COUNTRY_DESC,') ')),if(COUNTRY_DESC is null,concat(STATE_DESC,' '),concat(concat(STATE_DESC,','),concat('(',COUNTRY_DESC,') ')))),if(STATE_DESC is null,if(COUNTRY_DESC is null,concat(DISTRICT_DESC,' '),concat(concat(DISTRICT_DESC,','),concat('(',COUNTRY_DESC,') '))),if(COUNTRY_DESC is null,concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,' ')),concat(concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,',')),concat('(',COUNTRY_DESC,') '))))),if(PINCODE is null,' ',concat('- ',PINCODE)))as addressLine2,").append(" \t\tif(PRIMARY_PHONE is null,if(ALTERNATE_PHONE is null,' ',concat('Phone No.-',concat(ALTERNATE_PHONE,' '))),if(ALTERNATE_PHONE is null,concat('Phone No.-',concat(PRIMARY_PHONE,' ')),concat('Phone No.-',concat(concat(PRIMARY_PHONE,','),concat(ALTERNATE_PHONE,' '))))) as phone  ").append(" \t\tfrom  cr_loan_dtl  ").append(" \t\tjoin gcd_customer_m on(cr_loan_dtl.loan_customer_id=gcd_customer_m.customer_id  and cr_loan_dtl.LOAN_ID='").append(pLoanId).append("') ").append(" \t\tleft outer join com_address_m on((com_address_m.BPID=gcd_customer_m.CUSTOMER_ID) and(com_address_m.COMMUNICATION_ADDRESS='Y'))  ").append(" \t\tleft outer join com_country_m on(com_country_m.COUNTRY_ID=com_address_m.COUNTRY) ").append(" \t\tleft outer join com_state_m on((com_state_m.COUNTRY_ID=com_address_m.COUNTRY)and(com_state_m.STATE_ID=com_address_m.STATE))   ").append(" \t\tleft outer join com_district_m on(com_district_m.STATE_ID=com_address_m.STATE)and(com_district_m.DISTRICT_ID=com_address_m.DISTRICT) limit 1 ").append(" )add_dtl on cr_loan_dtl.loan_id=add_dtl.loan_id  ").append(" left outer join ").append(" (").append(" \t\tselect (a.dr-a.cr)as balance ").append(" \t\tfrom ").append(" \t\t(").append("\t\t\tselect sum(debit)dr,sum(credit)cr ").append(" \t\t\tfrom CR_STATEMENT_TMP ").append(" \t\t\twhere user_id='").append(userID).append("' and txn_date < '").append(fromDate).append("' ").append(" \t\t\torder by txn_date").append(" \t\t)a").append(" )opBls on(true)  ").append(" join com_branch_m on(com_branch_m.branch_id=cr_loan_dtl.loan_branch) ").append(" join cr_product_m on(cr_product_m.product_id=cr_loan_dtl.loan_product)  ").append(" join cr_scheme_m  on(cr_scheme_m.scheme_id=cr_loan_dtl.loan_scheme)   ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,min(instl_date)'instl_date' ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere  ").append("   LOAN_ID='").append(pLoanId).append("' and  IFNULL(REC_TYPE,'I')='I' group by loan_id ").append(" ) as st_date on(st_date.loan_id=cr_loan_dtl.loan_id) ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,instl_date,instl_no ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere instl_no=(select max(instl_no) from cr_repaysch_dtl where LOAN_ID='").append(pLoanId).append("') ").append(" \t\tand LOAN_ID='").append(pLoanId).append("'").append(" ) as ed_date on(ed_date.loan_id=cr_loan_dtl.loan_id)  ").append(" left outer join ").append(" (").append(" \t\tselect  sum(cr_sd_dtl.SD_AMOUNT) as collateral_amount,cr_sd_dtl.LOAN_ID ").append(" \t\tfrom cr_sd_dtl ").append(" \t\twhere rec_status not in('X','L','P','F') and  cr_sd_dtl.LOAN_ID='").append(pLoanId).append("'").append(" ) as cm on(cr_loan_dtl.LOAN_ID=cm.LOAN_ID) ").append(" left outer join ").append(" (").append(" \t\tselect advice_date,sum(ifnull(cr_txnadvice_dtl.advice_amount,0)- ifnull(txn_adjusted_amount,0)) as other_charge_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl ").append(" \t\twhere advice_type = 'R' and CHARGE_CODE_ID <> 7 and CHARGE_CODE_ID <> 27  and loan_id='").append(pLoanId).append("' ").append(" \t\tand advice_date>='").append(fromDate).append("' and advice_date<='").append(toDate).append("' group by loan_id").append(" ) as B on(B.loan_id=cr_loan_dtl.loan_id)  ").append(" left outer join ").append(" (").append(" \t\tselect advice_date,sum(ifnull(cr_txnadvice_dtl.advice_amount,0)- ifnull(txn_adjusted_amount,0)) as instl_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl ").append(" \t\twhere advice_type = 'R' and CHARGE_CODE_ID in(7,27)  and loan_id='").append(pLoanId).append("' ").append(" \t\tand advice_date>='").append(fromDate).append("' and advice_date<='").append(toDate).append("' group by loan_id").append(" ) as instlDue on(instlDue.loan_id=cr_loan_dtl.loan_id)  ").append(" LEFT OUTER JOIN ").append(" (").append(" \t\tselect '").append(fromDate).append("' TXN_DATE,").append(pLoanId).append(" LOAN_ID,'Openinig Balance' PARTICULARS,0 DEBIT,0 CREDIT ").append(" \t\tunion all ").append(" \t\tselect TXN_DATE,LOAN_ID, PARTICULARS ,DEBIT,CREDIT ").append(" \t\tFROM CR_STATEMENT_TMP  ").append(" \t\twhere USER_ID='").append(userID).append("' and TXN_DATE >='").append(fromDate).append("' and TXN_DATE <='").append(toDate).append("' ").append(" ) AS L ON(L.LOAN_ID=cr_loan_dtl.LOAN_ID)   ").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(c.ASSET_COLLATERAL_CLASS='VEHICLE',concat(c.VEHICLE_MAKE,' | ',c.VEHICLE_MODEL),c.ASSET_COLLATERAL_DESC))  as VEHICLE_ASSET_DETAILS ").append("\tfrom  cr_loan_collateral_m b ").append("\tleft join cr_asset_collateral_m c on c.ASSET_ID=b.ASSETID ").append("\twhere b.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicle on (Vehicle.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(t.ASSET_COLLATERAL_CLASS='VEHICLE',t.vehicle_registration_no,''))  as VEHICLE_NO ").append("   ,GROUP_CONCAT(t.VEHICLE_MAKE SEPARATOR '|')  as VEHICLE_MAKE,GROUP_CONCAT(t.VEHICLE_MODEL SEPARATOR '|')  as VEHICLE_MODEL,GROUP_CONCAT(t.ENGINE_NUMBER SEPARATOR '|')  as ENGINE_NUMBER,GROUP_CONCAT(t.VEHICLE_CHASIS_NUMBER SEPARATOR '|')  as VEHICLE_CHASIS_NUMBER ").append("\tfrom  cr_loan_collateral_m s ").append("\tleft join cr_asset_collateral_m t on t.ASSET_ID=s.ASSETID  and t.ASSET_COLLATERAL_CLASS='VEHICLE' ").append("\twhere s.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicles on (Vehicles.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tselect LOAN_ID,SUM(IFNULL(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0),0.00)) AS EXCESS ").append("\tfrom cr_txnadvice_dtl where ADVICE_TYPE='P' and CHARGE_CODE_ID=10 ").append("\tand LOAN_ID='").append(pLoanId).append("' ").append("\tgroup by LOAN_ID ").append(" ) as EA on (EA.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" where cr_loan_dtl.LOAN_ID='").append(pLoanId).append("' and ifnull(cr_loan_dtl.REC_STATUS,'P')<>'X' order by L.TXN_DATE  ").toString();
        }
  ...
  {
    logger.info("in getDateRangeLimitSpecial() of ReportsDAOImpl ");
    StringBuilder bufInsSql = new StringBuilder();
    ...
    String getDateRangeLimitSpecial = "";
    try
    {
      getDateRangeLimitSpecial = ConnectionReportDumpsDAO.singleReturn(bufInsSql.toString());
    }
    catch (Exception e) {
      e.printStackTrace();
    }logger.info(new StringBuilder().append("dateRengeLimit   :   ").append(getDateRangeLimitSpecial).toString());
    return getDateRangeLimitSpecial;
  }
  ...
  public String checkLinkLoanFlag()
  {
    ...
    try {
      ...
      logger.info(new StringBuilder().append("Flag  value is::::::::").append(linkLoanFlag).toString());
      query = null;
    }
    catch (Exception e)
    {
  ...
  public ArrayList getNocCount(String pLoanId, String userId, String nhbDate)
  {
    ...
    try
    {
      ...
      if (count == 0)
      {
        record = "Original";
        insertPrepStmtObject = new PrepStmtObject();
        query = new StringBuilder().append("insert into cr_NOC_dtl(LOAN_ID,USER_ID,REPORT_DATE) values(?,?,DATE_ADD(STR_TO_DATE(?, '").append(this.dateFormatWithTime).append("'),INTERVAL CURTIME() HOUR_SECOND))").toString();
        if (CommonFunction.checkNull(pLoanId).equalsIgnoreCase(""))
          insertPrepStmtObject.addNull();
        else
          insertPrepStmtObject.addString(pLoanId.trim());
  ...
  public boolean getEligibilityRecord(String pDealId, String pBusinessDate, String userId)
  {
    ...
    try
    {
      ...
      if (CommonFunction.checkNull(s1).trim().equalsIgnoreCase("S"))
      {
        ...
        out.add(s3);
        out.add(s4);
        logger.info(new StringBuilder().append("SCORECARD_CREATION(").append(in.toString()).append(",").append(out.toString()).toString());
        outMessages = (ArrayList)ConnectionReportDumpsDAO.callSP("SCORECARD_CREATION", in, out);
        s3 = CommonFunction.checkNull(outMessages.get(0));
        s4 = CommonFunction.checkNull(outMessages.get(1));
        logger.info(new StringBuilder().append("SCORECARD_CREATION   s1  : ").append(s3).toString());
        logger.info(new StringBuilder().append("SCORECARD_CREATION   s2  : ").append(s4).toString());
        in.clear();
        in = null;
        out.clear();
        out = null;
        outMessages.clear();
        outMessages = null;
        if (CommonFunction.checkNull(s3).trim().equalsIgnoreCase("S")) {
          status = true;
        }
      }
    }
  ...
  {
    logger.info("In getReportData(int currentPageLink,ChequeAllocationVo chequeAllocationVo1).....");
    ...
    try
    {
      bufInsSql.append(new StringBuilder().append("SELECT DISTINCT INSTRUMENT_ID,(CASE CID.TXN_TYPE WHEN 'DC' THEN CDD.DEAL_NO WHEN 'LIM' THEN CLD.LOAN_NO end) AS LOAN_NO,(CASE CID.TXN_TYPE WHEN 'DC' THEN cdcm.CUSTOMER_NAME WHEN 'LIM' THEN  gcm.CUSTOMER_NAME end) AS CUSTOMER_NAME,cbm.BRANCH_DESC,RECIPT_NO,INSTRUMENT_NO, CASE INSTRUMENT_TYPE WHEN 'K' THEN 'KNOCKOFF' WHEN 'P' THEN 'PAYABLE' WHEN 'R' THEN 'RECEIVABLE' END AS INSTRUMENT_TYPE ,INSTRUMENT_AMOUNT, CASE  CID.REC_STATUS WHEN 'A' THEN 'APPROVED' WHEN 'C' THEN 'SEND TO CUSTOMER' WHEN 'D' THEN 'DEPOSITED'WHEN 'R' THEN 'REALIZED' WHEN 'P' THEN 'PENDING' WHEN 'X' THEN 'CANCELLED' WHEN 'B' THEN 'BOUNCED' WHEN 'F' THEN 'FORWARDED' END AS REC_STATUS,TDS_AMOUNT,DATE_FORMAT(INSTRUMENT_DATE,'%d-%m-%Y'),BM.BANK_NAME,BRM.BANK_BRANCH_NAME,CID.DEPOSIT_BANK_ACCOUNT,CB.BRANCH_DESC from CR_INSTRUMENT_DTL CID left JOIN CR_LOAN_DTL CLD on CLD.LOAN_ID=CID.TXNID and CID.txn_type='LIM' left JOIN CR_DEAL_DTL CDD ON (CDD.DEAL_ID = CID.txnid  and CID.txn_type='DC') left join gcd_customer_m gcm on gcm.customer_id= CLD.LOAN_CUSTOMER_ID  and CID.txn_type='LIM' left join cr_deal_customer_m cdcm on CDCM.GCD_CUSTOMER_ID= CDD.DEAL_CUSTOMER_ID  and CID.txn_type='DC' inner join com_branch_m cbm on cbm.BRANCH_ID= if(CID.TXN_TYPE = 'DC',CDD.deal_branch, CLD.LOAN_BRANCH ) LEFT JOIN COM_BANK_M BM ON BM.BANK_ID=CID.DEPOSIT_BANK_ID LEFT JOIN COM_BANKBRANCH_M BRM ON BRM.BANK_BRANCH_ID=CID.DEPOSIT_BRANCH_ID LEFT JOIN COM_BRANCH_M CB ON CB.BRANCH_ID=CID.DEFAULT_BRANCH  WHERE INSTRUMENT_TYPE='").append(CommonFunction.checkNull(chequeAllocationVo1.getInstrumentTypesearch())).append("' ").toString());
      bufInsSqlCount.append(new StringBuilder().append("SELECT COUNT(1) from CR_INSTRUMENT_DTL CID left JOIN CR_LOAN_DTL CLD on CLD.LOAN_ID=CID.TXNID and CID.txn_type='LIM' left JOIN CR_DEAL_DTL CDD ON (CDD.DEAL_ID = CID.txnid  and CID.txn_type='DC') left join gcd_customer_m gcm on gcm.customer_id= CLD.LOAN_CUSTOMER_ID  and CID.txn_type='LIM' left join cr_deal_customer_m cdcm on CDCM.GCD_CUSTOMER_ID= CDD.DEAL_CUSTOMER_ID  and CID.txn_type='DC' inner join com_branch_m cbm on cbm.BRANCH_ID= if(CID.TXN_TYPE = 'DC',CDD.deal_branch, CLD.LOAN_BRANCH ) LEFT JOIN COM_BANK_M BM ON BM.BANK_ID=CID.DEPOSIT_BANK_ID LEFT JOIN COM_BANKBRANCH_M BRM ON BRM.BANK_BRANCH_ID=CID.DEPOSIT_BRANCH_ID LEFT JOIN COM_BRANCH_M CB ON CB.BRANCH_ID=CID.DEFAULT_BRANCH  WHERE INSTRUMENT_TYPE='").append(CommonFunction.checkNull(chequeAllocationVo1.getInstrumentTypesearch())).append("' ").toString());
  ...
  public boolean getLoanDetails(String noc_loan_id, String nhb_date)
  {
    ...
    try
    {
      sb.append(new StringBuilder().append("UPDATE CR_LOAN_DTL SET NOC_FLAG='Y',NOC_DATE=STR_TO_DATE(?,'").append(this.dateFormat).append("') WHERE LOAN_ID=? ").toString());
  ...
  public String CibilIndividualFileGenerator(String fromDate, String userId)
  {
    ...
    try
    {
      String dateFormat = new StringBuilder().append("select STR_TO_DATE('").append(fromDate).append("','%d-%m-%Y')").toString();
      String Date = CommonFunction.checkNull(ConnectionDAO.singleReturn(dateFormat));
      in.add(Date);
      in.add(Date);
      in.add(CommonFunction.checkNull(userId).trim());
      out.add(s1);
      out.add(s2);
      logger.info(new StringBuilder().append("CIBIL_TUDF_INDIVIDUAL_REPORT_GENERATOR (").append(in.toString()).append(",").append(out.toString()).append(")").toString());
      outMessages = (ArrayList)ConnectionReportDumpsDAO.callSP("CIBIL_TUDF_INDIVIDUAL_REPORT_GENERATOR", in, out);
      ...
      if (s1.equalsIgnoreCase("S"))
      {
        ...
        out1.add(s4);
        out1.add(s5);
        logger.info(new StringBuilder().append("CIBIL_TUDF_INDIVIDUAL_FILE_GENERATOR (").append(in1.toString()).append(",").append(out1.toString()).append(")").toString());
        outMessages1 = (ArrayList)ConnectionReportDumpsDAO.callSP("CIBIL_TUDF_INDIVIDUAL_FILE_GENERATOR", in1, out1);
        s3 = CommonFunction.checkNull(outMessages1.get(0));
        s4 = CommonFunction.checkNull(outMessages1.get(1));
        s5 = CommonFunction.checkNull(outMessages1.get(2));
        if (s1.equalsIgnoreCase("S"))
        {
          result = s5;
          logger.info(new StringBuilder().append("result: ").append(result.length()).toString());
        }
      }
    }
    catch (Exception e)
  ...
  public String CibilCorporateFileGenerator(String fromDate, String userId) {
    ...
    try
    {
      ...
      out.add(s1);
      out.add(s2);
      logger.info(new StringBuilder().append("CIBIL_TUDF_CORPORATE_REPORT_GENERATOR (").append(in.toString()).append(",").append(out.toString()).append(")").toString());
      outMessages = (ArrayList)ConnectionReportDumpsDAO.callSP("CIBIL_TUDF_CORPORATE_REPORT_GENERATOR", in, out);
      ...
      if (s1.equalsIgnoreCase("S"))
      {
        ...
        out1.add(s4);
        out1.add(s5);
        logger.info(new StringBuilder().append("CIBIL_TUDF_CORPORATE_FILE_GENERATOR (").append(in1.toString()).append(",").append(out1.toString()).append(")").toString());
        outMessages1 = (ArrayList)ConnectionReportDumpsDAO.callSP("CIBIL_TUDF_CORPORATE_FILE_GENERATOR", in1, out1);
        s3 = CommonFunction.checkNull(outMessages1.get(0));
        s4 = CommonFunction.checkNull(outMessages1.get(1));
        s5 = CommonFunction.checkNull(outMessages1.get(2));
        if (s1.equalsIgnoreCase("S"))
  ...
  public String CibilCorporateLCFileGenerator(String fromDate, String userId)
  {
    logger.info("In getCibilReport()");
    ArrayList in = new ArrayList();
    ArrayList out = new ArrayList();
    ...
    try
    {
      ...
      out.add(s1);
      out.add(s2);
      logger.info(new StringBuilder().append(in.toString()).append(",").append(out.toString()).toString());
      logger.info(new StringBuilder().append("CIBIL_TUDF_CORPORATE_LC_REPORT_GENERATOR (").append(in.toString()).append(",").append(out.toString()).append(")").toString());
      outMessages = (ArrayList)ConnectionDAO.callSP("CIBIL_TUDF_CORPORATE_LC_REPORT_GENERATOR", in, out);
      ...
      if (s1.equalsIgnoreCase("S"))
      {
        ...
        out1.add(s4);
        out1.add(s5);
        logger.info(new StringBuilder().append("CIBIL_TUDF_CORPORATE_LC_FILE_GENERATOR (").append(in1.toString()).append(",").append(out1.toString()).append(")").toString());
        outMessages1 = (ArrayList)ConnectionDAO.callSP("CIBIL_TUDF_CORPORATE_LC_FILE_GENERATOR", in1, out1);
        s3 = CommonFunction.checkNull(outMessages1.get(0));
        s4 = CommonFunction.checkNull(outMessages1.get(1));
        s5 = CommonFunction.checkNull(outMessages1.get(2));
        if (s1.equalsIgnoreCase("S"))
        {
          result = s5;
          logger.info(new StringBuilder().append("result: ").append(result).toString());
        }
      }
    }
    catch (Exception e)
  ...
  public String CibilFileNameGenerator(String fromDate)
  {
    ...
    try
    {
      String query = new StringBuilder().append("select concat(Parameter_value,'-',date_Format(str_to_date('").append(fromDate).append("','%d-%m-%Y'),'%d%m%Y'),'-',TIME_FORMAT(NOW(), '%H%i%s'),'.txt') from parameter_mst where parameter_key='MEMBER_ID' ").toString();
      logger.info(new StringBuilder().append("query for fetching MemberId::::").append(query).toString());
      String memberId = CommonFunction.checkNull(ConnectionDAO.singleReturn(query));
      logger.info(new StringBuilder().append("MemberId::::").append(memberId).toString());
      FilePath = memberId;
    }
    catch (Exception e)
    {
...
//Shashank Agnihotri Starts For Nesl Report
 public ArrayList NeslFileGenerator(String fromDate, String toDate)
 {

	 
	    logger.info("In NeslFileGenerator()");
	    ArrayList in = new ArrayList();
	    ArrayList out = new ArrayList();
		...
		//String dateFormat1 = new StringBuilder().append("select STR_TO_DATE('").append(toDate).append("','%d-%m-%Y')").toString();
		//String Date1 = CommonFunction.checkNull(ConnectionDAO.singleReturn(dateFormat1));
		    
		  in.add(Date);
		  //in.add(Date1);
	      out.add(s1);
	      out.add(s2);
	      logger.info(new StringBuilder().append("Nesl_REPORT_GENERATOR (").append(in.toString()).append(",").append(out.toString()).append(")").toString());
	      outMessages = (ArrayList)ConnectionReportDumpsDAO.callSP("Nesl_REPORT_GENERATOR", in, out);
	  	...
	  	String fd="SELECT * from cr_NeSL_record_dtl;";
		
		logger.info("Nesl_REPORT_GENERATOR query for going to execute  :  "+fd);
 ...
 public ArrayList NeslLCFileGenerator(String fromDate, String toDate)
 {


	 
	    logger.info("In NeslLCFileGenerator()");
	    ArrayList in = new ArrayList();
	    ArrayList out = new ArrayList();
	    ...
	    try
	    {
	      ...
	      out.add(s1);
	      out.add(s2);
	      logger.info(new StringBuilder().append("Nesl_LC_REPORT_GENERATOR (").append(in.toString()).append(",").append(out.toString()).append(")").toString());
	      outMessages = (ArrayList)ConnectionReportDumpsDAO.callSP("Nesl_LC_REPORT_GENERATOR", in, out);
	  	...
	  	String fd="SELECT * from cr_NeSL_LC_record_dtl;";
		
		logger.info("Nesl_LC_REPORT_GENERATOR query for going to execute  :  "+fd);
		resultList = ConnectionReportDumpsDAO.sqlSelect(fd);
 ...
 // Shashank Agnihotri Ends For Nesl Report

  public ArrayList generateSOAForCancelLoan(int companyId, String pLoanId, String pBusinessDate, String userID, String source, String fromDate, String toDate, String soaFor)
  {
    ArrayList result = new ArrayList();
    ArrayList in = new ArrayList();
    ArrayList out = new ArrayList();
    ...
    try
    {
      ...
      if (s1.equalsIgnoreCase("S")) {
        if (CommonFunction.checkNull(source).trim().equalsIgnoreCase("NR"))
        {
          logger.info(new StringBuilder().append("p_loan_id  : ").append(pLoanId).toString());
          logger.info(new StringBuilder().append("userId     : ").append(userID).toString());
          query = new StringBuilder().append("select (select sum(DISBURSAL_AMOUNT)DISBURSAL_AMOUNT from cr_loan_disbursal_dtl where rec_status='A' and loan_id=").append(pLoanId).append(")DISBURSED_AMOUNT,").append("(select DEAL_SANCTION_AMOUNT from cr_deal_loan_dtl cdld left join cr_loan_dtl cld on cld.LOAN_DEAL_LOAN_ID=cdld.deal_loan_id where loan_id=").append(pLoanId).append(")DEAL_SANCTION_AMOUNT,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(address_dtl.customer_name,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')customer_name,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(address_dtl.addressLine1,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine1,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(address_dtl.addressLine2,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine2,").append(" phone, concat(com_branch_m.branch_desc,' ')as branch_desc,cr_loan_dtl.LOAN_NO,cr_loan_dtl.LOAN_id,concat(cr_product_m.product_desc,' ')as product_desc,").append(" concat(cr_scheme_m.scheme_desc,' ')as scheme_desc,IF(cr_loan_dtl.loan_approval_date='0000-00-00 00:00:00',NULL,cr_loan_dtl.loan_approval_date) as loan_start_date,").append(" if(st_date.instl_date='0000-00-00',NULL,st_date.instl_date) as install_start_date, IF(ed_date.instl_date='0000-00-00',NULL,ed_date.instl_date) as install_end_date,").append(" cr_loan_dtl.loan_tenure as total_tenure,CASE lOAN_REPAYMENT_FREQ WHEN 'M' THEN 'Monthly ' WHEN 'B' THEN 'Bimonthly ' WHEN 'Q' THEN 'Quarterly ' WHEN 'H' THEN 'Half yearly  'else 'Yearly ' end as freqency,").append(" cr_loan_dtl.LOAN_NO_OF_INSTALLMENT as no_of_instalments,if (cr_loan_dtl.REC_STATUS='C', ( select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)+ifnull(cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=18 and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ) , cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL) as LOAN_RECEIVED_PRINCIPAL,").append(" (SELECT sum(INSTL_AMOUNT_RECD/INSTL_AMOUNT) FROM CR_REPAYSCH_DTL WHERE BILL_FLAG='Y' AND INSTL_AMOUNT_RECD>0 AND REC_TYPE='I' AND LOAN_ID='").append(pLoanId).append("' group by loan_id) AS EMI_RECD,").append(" (IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_INSTL_AMOUNT,0)-IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_PRINCIPAL,0))+ ifnull((select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=9 and txn_type in ('ECA','MCA') and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ),0) AS RECD_INTEREST,").append(" (select count(loan_id) from cr_statement_tmp where loan_id='").append(pLoanId).append("' and next_status='B' and USER_ID='").append(userID).append("') as no_bounce,").append(" cr_loan_dtl.loan_loan_amount,cr_loan_dtl.loan_emi_amount as emi_amount, cr_loan_dtl.loan_balance_principal as principal_os,").append(" ifnull(instlDue.instl_due,0) over_due_amount,ifnull(B.other_charge_due,0)as other_charge_due,cr_loan_dtl.REC_STATUS, ").append(" CASE cr_loan_dtl.REC_STATUS WHEN 'L' THEN 'CANCELED' WHEN 'C' THEN 'CLOSED' WHEN 'A' THEN 'APPROVED' WHEN 'X' THEN 'REJECTED' WHEN 'P' THEN 'PENDING' WHEN 'F' THEN 'FORWARED' END AS loan_status,").append(" ifnull(cm.collateral_amount,0)collateral_amount,cr_loan_dtl.loan_no, IF(L.TXN_DATE='0000-00-00',NULL,L.TXN_DATE)TXN_DATE,").append(" concat(L.PARTICULARS,' ')as PARTICULARS,L.DEBIT,L.CREDIT,CASE cr_loan_dtl.LOAN_RATE_TYPE WHEN 'F' THEN 'FLAT ' WHEN 'P' THEN 'PTPM ' WHEN 'E' THEN 'EFFECTIVE ' END AS LOAN_RATE_TYPE,").append(" cr_loan_dtl.LOAN_FINAL_RATE,0.00 as Opening_bls,cr_loan_dtl.LOAN_MATURITY_DATE ,IFNULL(TERM.TERMINATION_DATE,'')  AS TERMINATION_DATE,").append(" concat(if(ifnull(LEGAL_FLAG,'NA')='A','LEGAL|',''),if(ifnull(REPO_FLAG,'NA')='A','REPO|',''),IFNULL((select GROUP_CONCAT(DESCRIPTION SEPARATOR '|')from generic_master where generic_key='CASE_MARKING_FLAG' AND INSTR(CONCAT( '|',CASE_MARKING_FLAG,'|'),CONCAT('|',VALUE,'|') ) > 0 ),''))legalRepoFlag ").append(" ,concat(Vehicle.VEHICLE_ASSET_DETAILS,' ')as VEHICLE_ASSET_DETAILS,IFNULL(EA.EXCESS,0) as EXCESS,concat(Vehicles.VEHICLE_NO,'')as VEHICLE_No ").append(" ,concat(Vehicles.VEHICLE_MAKE,' ')as  VEHICLE_MAKE,concat(Vehicles.VEHICLE_MODEL,' ')as  VEHICLE_MODEL,concat(Vehicles.ENGINE_NUMBER,' ')as  ENGINE_NUMBER,concat(Vehicles.VEHICLE_CHASIS_NUMBER,' ')as  VEHICLE_CHASIS_NUMBER ,ifnull(cr_loan_dtl.loan_dpd,'')  as loan_dpd").append(" ,(select  date_format(MIN(DISBURSAL_DATE),'%d-%b-%Y')as AUTHOR_DATE from cr_loan_disbursal_dtl where loan_id='").append(pLoanId).append("')as AUTHOR_DATE, ").append(" PENAL.PDUE_CHARGE,PENAL.PRECEIVED,PENAL.PPENDING,PENAL.PWAIVE_OFF_AMOUNT, ").append(" BOUNCE.BDUE_CHARGE,BOUNCE.BRECEIVED,BOUNCE.BPENDING,BOUNCE.BWAIVE_OFF_AMOUNT, ").append(" RETRIEVAL.RDUE_CHARGE,RETRIEVAL.RRECEIVED,RETRIEVAL.RPENDING,RETRIEVAL.RWAIVE_OFF_AMOUNT, ").append(" PDCNACH.NDUE_CHARGE,PDCNACH.NRECEIVED,PDCNACH.NPENDING,PDCNACH.NWAIVE_OFF_AMOUNT ").append(" from  cr_loan_dtl  ").append("  LEFT JOIN CR_TERMINATION_DTL TERM ON cr_loan_dtl.LOAN_ID=TERM.LOAN_ID AND TERM.REC_STATUS='A'  ").append(" left outer join ").append(" (").append(" \t\tselect cr_loan_dtl.LOAN_ID,concat(gcd_customer_m.customer_name,' ')as customer_name, ").append(" \t\tif(ADDRESS_LINE1 is null,if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,' ',concat(ADDRESS_LINE3,' ')),if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE2,' '),concat(concat(ADDRESS_LINE2,','),concat(ADDRESS_LINE3,' ')))),if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE1,' '),concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE3,' '))),if(ADDRESS_LINE3 is null,concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,' ')),concat(concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,',')),concat(ADDRESS_LINE3,' ')))))as addressLine1,").append(" \t\tconcat(if(DISTRICT_DESC is null,if(STATE_DESC is null,if(COUNTRY_DESC is null,' ',concat('(',COUNTRY_DESC,') ')),if(COUNTRY_DESC is null,concat(STATE_DESC,' '),concat(concat(STATE_DESC,','),concat('(',COUNTRY_DESC,') ')))),if(STATE_DESC is null,if(COUNTRY_DESC is null,concat(DISTRICT_DESC,' '),concat(concat(DISTRICT_DESC,','),concat('(',COUNTRY_DESC,') '))),if(COUNTRY_DESC is null,concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,' ')),concat(concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,',')),concat('(',COUNTRY_DESC,') '))))),if(PINCODE is null,' ',concat('- ',PINCODE)))as addressLine2,").append(" \t\tif(PRIMARY_PHONE is null,if(ALTERNATE_PHONE is null,' ',concat('Phone No.-',concat(ALTERNATE_PHONE,' '))),if(ALTERNATE_PHONE is null,concat('Phone No.-',concat(PRIMARY_PHONE,' ')),concat('Phone No.-',concat(concat(PRIMARY_PHONE,','),concat(ALTERNATE_PHONE,' '))))) as phone").append("  \t\tfrom  cr_loan_dtl  ").append(" \t\tjoin gcd_customer_m on(cr_loan_dtl.loan_customer_id=gcd_customer_m.customer_id    and cr_loan_dtl.LOAN_ID='").append(pLoanId).append("') ").append(" \t\tleft outer join com_address_m on((com_address_m.BPID=gcd_customer_m.CUSTOMER_ID) and(com_address_m.COMMUNICATION_ADDRESS='Y'))").append(" \t\tleft outer join com_country_m on(com_country_m.COUNTRY_ID=com_address_m.COUNTRY)").append(" \t\tleft outer join com_state_m on((com_state_m.COUNTRY_ID=com_address_m.COUNTRY)and(com_state_m.STATE_ID=com_address_m.STATE))").append("  \t\tleft outer join com_district_m on(com_district_m.STATE_ID=com_address_m.STATE)and(com_district_m.DISTRICT_ID=com_address_m.DISTRICT) limit 1 ").append(" )address_dtl on cr_loan_dtl.loan_id=address_dtl.loan_id   ").append(" join com_branch_m on(com_branch_m.branch_id=cr_loan_dtl.loan_branch)").append(" join cr_product_m on(cr_product_m.product_id=cr_loan_dtl.loan_product) ").append(" join cr_scheme_m  on(cr_scheme_m.scheme_id=cr_loan_dtl.loan_scheme)  ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,min(instl_date)'instl_date' ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere  ").append("   LOAN_ID='").append(pLoanId).append("' and  IFNULL(REC_TYPE,'I')='I' group by loan_id ").append(" ) as st_date on(st_date.loan_id=cr_loan_dtl.loan_id) ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,instl_date,instl_no ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere instl_no=(select max(instl_no) from cr_repaysch_dtl where LOAN_ID='").append(pLoanId).append("') ").append(" \t\tand LOAN_ID='").append(pLoanId).append("'").append(" ) as ed_date on(ed_date.loan_id=cr_loan_dtl.loan_id) ").append(" left outer join ").append(" (").append(" \t\tselect  sum(cr_sd_dtl.SD_AMOUNT) as collateral_amount,cr_sd_dtl.LOAN_ID ").append(" \t\tfrom cr_sd_dtl ").append(" \t\twhere rec_status not in('X','L','P','F') and  cr_sd_dtl.LOAN_ID='").append(pLoanId).append("'").append(" ) as cm on(cr_loan_dtl.LOAN_ID=cm.LOAN_ID) ").append(" left outer join ").append(" (").append(" \t\tselect  sum(ifnull(cr_txnadvice_dtl.advice_amount,0))- sum(ifnull(txn_adjusted_amount,0)) as other_charge_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl where advice_type = 'R' and CHARGE_CODE_ID <> 7 and CHARGE_CODE_ID <> 27 and loan_id='").append(pLoanId).append("' ").append(" \t\tgroup by loan_id").append(" ) as B on(B.loan_id=cr_loan_dtl.loan_id)  ").append(" left outer join ").append(" (").append(" \t\tselect  sum(ifnull(cr_txnadvice_dtl.advice_amount,0))- sum(ifnull(txn_adjusted_amount,0)) as instl_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl where advice_type = 'R' and CHARGE_CODE_ID IN(7,27) and loan_id='").append(pLoanId).append("' ").append(" \t\tgroup by loan_id").append(" ) as instlDue on(instlDue.loan_id=cr_loan_dtl.loan_id)  ").append(" LEFT OUTER JOIN ").append(" (").append(" \t\tselect TXN_DATE,LOAN_ID, PARTICULARS , DEBIT , CREDIT ").append(" \t\tFROM CR_STATEMENT_TMP where USER_ID='").append(userID).append("' ").append(" ) AS L ON(L.LOAN_ID=cr_loan_dtl.LOAN_ID)").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(c.ASSET_COLLATERAL_CLASS='VEHICLE',concat(c.VEHICLE_MAKE,' | ',c.VEHICLE_MODEL),c.ASSET_COLLATERAL_DESC))  as VEHICLE_ASSET_DETAILS ").append("\tfrom  cr_loan_collateral_m b ").append("\tleft join cr_asset_collateral_m c on c.ASSET_ID=b.ASSETID ").append("\twhere b.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicle on (Vehicle.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(t.ASSET_COLLATERAL_CLASS='VEHICLE',t.vehicle_registration_no,''))  as VEHICLE_NO ").append("   ,GROUP_CONCAT(t.VEHICLE_MAKE SEPARATOR '|')  as VEHICLE_MAKE,GROUP_CONCAT(t.VEHICLE_MODEL SEPARATOR '|')  as VEHICLE_MODEL,GROUP_CONCAT(t.ENGINE_NUMBER SEPARATOR '|')  as ENGINE_NUMBER,GROUP_CONCAT(t.VEHICLE_CHASIS_NUMBER SEPARATOR '|')  as VEHICLE_CHASIS_NUMBER ").append("\tfrom  cr_loan_collateral_m s ").append("\tleft join cr_asset_collateral_m t on t.ASSET_ID=s.ASSETID  and t.ASSET_COLLATERAL_CLASS='VEHICLE' ").append("\twhere s.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicles on (Vehicles.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tselect LOAN_ID,SUM(IFNULL(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0),0.00)) AS EXCESS ").append("\tfrom cr_txnadvice_dtl where ADVICE_TYPE='P' and CHARGE_CODE_ID=10 ").append("\tand LOAN_ID='").append(pLoanId).append("' ").append("\tgroup by LOAN_ID ").append(" ) as EA on (EA.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as PDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as PRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as PPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as PWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='109' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC ) PENAL ON PENAL.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append("\tLEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as BDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as BRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as BPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as BWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='110' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC  ) BOUNCE ON BOUNCE.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append("\tLEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as RDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as RRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as RPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as RWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='336' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC  ) RETRIEVAL ON RETRIEVAL.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append("\tLEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as NDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as NRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as NPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as NWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='335' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC  ) PDCNACH ON PDCNACH.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append(" where cr_loan_dtl.LOAN_ID='").append(pLoanId).append("' and ifnull(cr_loan_dtl.REC_STATUS,'P')<>'X' order by L.TXN_DATE").toString();
        }
        else
        {
          query = new StringBuilder().append("  select (select sum(DISBURSAL_AMOUNT)DISBURSAL_AMOUNT from cr_loan_disbursal_dtl where rec_status='A' and loan_id=").append(pLoanId).append(")DISBURSED_AMOUNT,").append("(select DEAL_SANCTION_AMOUNT from cr_deal_loan_dtl cdld left join cr_loan_dtl cld on cld.LOAN_DEAL_LOAN_ID=cdld.deal_loan_id where loan_id=").append(pLoanId).append(")DEAL_SANCTION_AMOUNT,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(add_dtl.customer_name,''),' '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')customer_name,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(add_dtl.addressLine1,''),'   '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine1,").append(" replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(concat(ifnull(add_dtl.addressLine2,''),'   '),'!','! '),'%','% '),'(','( '),')',') '),'-','- '),'{','{ '),'[','[ '),'}','} '),']','] '),':',': '),';','; '),'\"','\" '),'/','/ '),'?','? '),'.','. '),',',', ')addressLine2,").append(" phone,concat(com_branch_m.branch_desc,' ')as branch_desc,cr_loan_dtl.LOAN_NO,cr_loan_dtl.LOAN_id,concat(cr_product_m.product_desc,'    ')as product_desc,").append(" concat(cr_scheme_m.scheme_desc,' ')as scheme_desc,IF(cr_loan_dtl.loan_approval_date='0000-00-00 00:00:00',NULL,cr_loan_dtl.loan_approval_date) as loan_start_date,").append(" if(st_date.instl_date='0000-00-00',NULL,st_date.instl_date) as install_start_date, IF(ed_date.instl_date='0000-00-00',NULL,ed_date.instl_date) as install_end_date,").append(" cr_loan_dtl.loan_tenure as total_tenure,CASE lOAN_REPAYMENT_FREQ  WHEN 'M' THEN 'Monthly ' WHEN 'B' THEN 'Bimonthly ' WHEN 'Q' THEN 'Quarterly ' WHEN 'H' THEN 'Half yearly  'else 'Yearly ' end as freqency,").append(" cr_loan_dtl.LOAN_NO_OF_INSTALLMENT as no_of_instalments,if (cr_loan_dtl.REC_STATUS='C', ( select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)+ifnull(cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=18 and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ) , cr_loan_dtl.LOAN_RECEIVED_PRINCIPAL) as LOAN_RECEIVED_PRINCIPAL, ").append(" (SELECT sum(INSTL_AMOUNT_RECD/INSTL_AMOUNT) FROM CR_REPAYSCH_DTL WHERE BILL_FLAG='Y' AND INSTL_AMOUNT_RECD>0 AND REC_TYPE='I' AND LOAN_ID='").append(pLoanId).append("' group by loan_id) AS EMI_RECD,").append(" (IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_INSTL_AMOUNT,0)-IFNULL(CR_LOAN_DTL.LOAN_RECEIVED_PRINCIPAL,0))+ ifnull((select  sum(ifnull(cr_txnadvice_dtl.txn_adjusted_amount,0)) as aa from cr_txnadvice_dtl join cr_loan_dtl on cr_loan_dtl.loan_id=cr_txnadvice_dtl.loan_id where cr_txnadvice_dtl.advice_type = 'R' and cr_txnadvice_dtl.CHARGE_CODE_ID=9 and txn_type in ('ECA','MCA') and cr_loan_dtl.loan_id='").append(pLoanId).append("' group by cr_loan_dtl.loan_id ),0)AS RECD_INTEREST,").append(" (select count(loan_id) from cr_statement_tmp where loan_id='").append(pLoanId).append("' and next_status='B' and USER_ID='").append(userID).append("') as no_bounce,").append(" cr_loan_dtl.loan_loan_amount,cr_loan_dtl.loan_emi_amount as emi_amount,cr_loan_dtl.loan_balance_principal as principal_os,").append(" ifnull(instlDue.instl_due,0) over_due_amount,ifnull(B.other_charge_due,0)as other_charge_due,cr_loan_dtl.REC_STATUS,").append(" CASE cr_loan_dtl.REC_STATUS WHEN 'L' THEN 'CANCELED' WHEN 'C' THEN 'CLOSED' WHEN 'A' THEN 'APPROVED' WHEN 'X' THEN 'REJECTED' WHEN 'P' THEN 'PENDING' WHEN 'F' THEN 'FORWARED' END AS loan_status,").append(" ifnull(cm.collateral_amount,0)collateral_amount,cr_loan_dtl.loan_no,IF(L.TXN_DATE='0000-00-00',NULL,L.TXN_DATE)TXN_DATE,").append(" concat(L.PARTICULARS,' ')as PARTICULARS,L.DEBIT,L.CREDIT,CASE cr_loan_dtl.LOAN_RATE_TYPE WHEN 'F' THEN 'FLAT ' WHEN 'P' THEN 'PTPM ' WHEN 'E' THEN 'EFFECTIVE ' END AS LOAN_RATE_TYPE,").append(" cr_loan_dtl.LOAN_FINAL_RATE,ifnull(opBls.balance,0) as Opening_bls,cr_loan_dtl.LOAN_MATURITY_DATE ,IFNULL(TERM.TERMINATION_DATE,'') AS TERMINATION_DATE ").append(" ,concat(if(ifnull(LEGAL_FLAG,'NA')='A','LEGAL|',''),if(ifnull(REPO_FLAG,'NA')='A','REPO|',''),IFNULL((select GROUP_CONCAT(DESCRIPTION SEPARATOR '|')from generic_master where generic_key='CASE_MARKING_FLAG' AND INSTR(CONCAT( '|',CASE_MARKING_FLAG,'|'),CONCAT('|',VALUE,'|') ) > 0 ),''))legalRepoFlag ").append(" ,concat(Vehicle.VEHICLE_ASSET_DETAILS,' ')as VEHICLE_ASSET_DETAILS,IFNULL(EA.EXCESS,0) as EXCESS,concat(Vehicles.VEHICLE_NO,' ')as VEHICLE_No ").append(" ,concat(Vehicles.VEHICLE_MAKE,' ')as  VEHICLE_MAKE,concat(Vehicles.VEHICLE_MODEL,' ')as  VEHICLE_MODEL,concat(Vehicles.ENGINE_NUMBER,' ')as  ENGINE_NUMBER,concat(Vehicles.VEHICLE_CHASIS_NUMBER,' ')as  VEHICLE_CHASIS_NUMBER ,ifnull(cr_loan_dtl.loan_dpd,'')  as loan_dpd ").append(" ,(select  date_format(MIN(DISBURSAL_DATE),'%d-%b-%Y')as AUTHOR_DATE from cr_loan_disbursal_dtl where loan_id='").append(pLoanId).append("')as AUTHOR_DATE, ").append(" PENAL.PDUE_CHARGE,PENAL.PRECEIVED,PENAL.PPENDING,PENAL.PWAIVE_OFF_AMOUNT, ").append(" BOUNCE.BDUE_CHARGE,BOUNCE.BRECEIVED,BOUNCE.BPENDING,BOUNCE.BWAIVE_OFF_AMOUNT, ").append(" RETRIEVAL.RDUE_CHARGE,RETRIEVAL.RRECEIVED,RETRIEVAL.RPENDING,RETRIEVAL.RWAIVE_OFF_AMOUNT, ").append(" PDCNACH.NDUE_CHARGE,PDCNACH.NRECEIVED,PDCNACH.NPENDING,PDCNACH.NWAIVE_OFF_AMOUNT ").append(" from  cr_loan_dtl  ").append("  LEFT JOIN CR_TERMINATION_DTL TERM ON cr_loan_dtl.LOAN_ID=TERM.LOAN_ID AND TERM.REC_STATUS='A'  ").append(" left outer join").append(" (").append(" \t\tselect cr_loan_dtl.LOAN_ID,concat(gcd_customer_m.customer_name,' ')as customer_name, if(ADDRESS_LINE1 is null,if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,' ',concat(ADDRESS_LINE3,' ')),if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE2,' '),concat(concat(ADDRESS_LINE2,','),concat(ADDRESS_LINE3,' ')))),if(ADDRESS_LINE2 is null,if(ADDRESS_LINE3 is null,concat(ADDRESS_LINE1,' '),concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE3,' '))),if(ADDRESS_LINE3 is null,concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,' ')),concat(concat(concat(ADDRESS_LINE1,','),concat(ADDRESS_LINE2,',')),concat(ADDRESS_LINE3,' ')))))as addressLine1,").append(" \t\tconcat(if(DISTRICT_DESC is null,if(STATE_DESC is null,if(COUNTRY_DESC is null,' ',concat('(',COUNTRY_DESC,') ')),if(COUNTRY_DESC is null,concat(STATE_DESC,' '),concat(concat(STATE_DESC,','),concat('(',COUNTRY_DESC,') ')))),if(STATE_DESC is null,if(COUNTRY_DESC is null,concat(DISTRICT_DESC,' '),concat(concat(DISTRICT_DESC,','),concat('(',COUNTRY_DESC,') '))),if(COUNTRY_DESC is null,concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,' ')),concat(concat(concat(DISTRICT_DESC,','),concat(STATE_DESC,',')),concat('(',COUNTRY_DESC,') '))))),if(PINCODE is null,' ',concat('- ',PINCODE)))as addressLine2,").append(" \t\tif(PRIMARY_PHONE is null,if(ALTERNATE_PHONE is null,' ',concat('Phone No.-',concat(ALTERNATE_PHONE,' '))),if(ALTERNATE_PHONE is null,concat('Phone No.-',concat(PRIMARY_PHONE,' ')),concat('Phone No.-',concat(concat(PRIMARY_PHONE,','),concat(ALTERNATE_PHONE,' '))))) as phone  ").append(" \t\tfrom  cr_loan_dtl  ").append(" \t\tjoin gcd_customer_m on(cr_loan_dtl.loan_customer_id=gcd_customer_m.customer_id  and cr_loan_dtl.LOAN_ID='").append(pLoanId).append("') ").append(" \t\tleft outer join com_address_m on((com_address_m.BPID=gcd_customer_m.CUSTOMER_ID) and(com_address_m.COMMUNICATION_ADDRESS='Y'))  ").append(" \t\tleft outer join com_country_m on(com_country_m.COUNTRY_ID=com_address_m.COUNTRY) ").append(" \t\tleft outer join com_state_m on((com_state_m.COUNTRY_ID=com_address_m.COUNTRY)and(com_state_m.STATE_ID=com_address_m.STATE))   ").append(" \t\tleft outer join com_district_m on(com_district_m.STATE_ID=com_address_m.STATE)and(com_district_m.DISTRICT_ID=com_address_m.DISTRICT) limit 1 ").append(" )add_dtl on cr_loan_dtl.loan_id=add_dtl.loan_id  ").append(" left outer join ").append(" (").append(" \t\tselect (a.dr-a.cr)as balance ").append(" \t\tfrom ").append(" \t\t(").append("\t\t\tselect sum(debit)dr,sum(credit)cr ").append(" \t\t\tfrom CR_STATEMENT_TMP ").append(" \t\t\twhere user_id='").append(userID).append("' and txn_date < '").append(fromDate).append("' ").append(" \t\t\torder by txn_date").append(" \t\t)a").append(" )opBls on(true)  ").append(" join com_branch_m on(com_branch_m.branch_id=cr_loan_dtl.loan_branch) ").append(" join cr_product_m on(cr_product_m.product_id=cr_loan_dtl.loan_product)  ").append(" join cr_scheme_m  on(cr_scheme_m.scheme_id=cr_loan_dtl.loan_scheme)   ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,min(instl_date)'instl_date' ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere  ").append("   LOAN_ID='").append(pLoanId).append("' and  IFNULL(REC_TYPE,'I')='I' group by loan_id ").append(" ) as st_date on(st_date.loan_id=cr_loan_dtl.loan_id) ").append(" left outer join ").append(" (").append(" \t\tselect  loan_id,instl_date,instl_no ").append(" \t\tfrom cr_repaysch_dtl ").append(" \t\twhere instl_no=(select max(instl_no) from cr_repaysch_dtl where LOAN_ID='").append(pLoanId).append("') ").append(" \t\tand LOAN_ID='").append(pLoanId).append("'").append(" ) as ed_date on(ed_date.loan_id=cr_loan_dtl.loan_id)  ").append(" left outer join ").append(" (").append(" \t\tselect  sum(cr_sd_dtl.SD_AMOUNT) as collateral_amount,cr_sd_dtl.LOAN_ID ").append(" \t\tfrom cr_sd_dtl ").append(" \t\twhere rec_status not in('X','L','P','F') and  cr_sd_dtl.LOAN_ID='").append(pLoanId).append("'").append(" ) as cm on(cr_loan_dtl.LOAN_ID=cm.LOAN_ID) ").append(" left outer join ").append(" (").append(" \t\tselect advice_date,sum(ifnull(cr_txnadvice_dtl.advice_amount,0)- ifnull(txn_adjusted_amount,0)) as other_charge_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl ").append(" \t\twhere advice_type = 'R' and CHARGE_CODE_ID <> 7 and CHARGE_CODE_ID <> 27  and loan_id='").append(pLoanId).append("' ").append(" \t\tand advice_date>='").append(fromDate).append("' and advice_date<='").append(toDate).append("' group by loan_id").append(" ) as B on(B.loan_id=cr_loan_dtl.loan_id)  ").append(" left outer join ").append(" (").append(" \t\tselect advice_date,sum(ifnull(cr_txnadvice_dtl.advice_amount,0)- ifnull(txn_adjusted_amount,0)) as instl_due,loan_id ").append(" \t\tfrom cr_txnadvice_dtl ").append(" \t\twhere advice_type = 'R' and CHARGE_CODE_ID in(7,27)  and loan_id='").append(pLoanId).append("' ").append(" \t\tand advice_date>='").append(fromDate).append("' and advice_date<='").append(toDate).append("' group by loan_id").append(" ) as instlDue on(instlDue.loan_id=cr_loan_dtl.loan_id)  ").append(" LEFT OUTER JOIN ").append(" (").append(" \t\tselect '").append(fromDate).append("' TXN_DATE,").append(pLoanId).append(" LOAN_ID,'Openinig Balance' PARTICULARS,0 DEBIT,0 CREDIT ").append(" \t\tunion all ").append(" \t\tselect TXN_DATE,LOAN_ID, PARTICULARS ,DEBIT,CREDIT ").append(" \t\tFROM CR_STATEMENT_TMP  ").append(" \t\twhere USER_ID='").append(userID).append("' and TXN_DATE >='").append(fromDate).append("' and TXN_DATE <='").append(toDate).append("' ").append(" ) AS L ON(L.LOAN_ID=cr_loan_dtl.LOAN_ID)   ").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(c.ASSET_COLLATERAL_CLASS='VEHICLE',concat(c.VEHICLE_MAKE,' | ',c.VEHICLE_MODEL),c.ASSET_COLLATERAL_DESC))  as VEHICLE_ASSET_DETAILS ").append("\tfrom  cr_loan_collateral_m b ").append("\tleft join cr_asset_collateral_m c on c.ASSET_ID=b.ASSETID ").append("\twhere b.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicle on (Vehicle.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tSELECT LOAN_ID, GROUP_CONCAT(if(t.ASSET_COLLATERAL_CLASS='VEHICLE',t.vehicle_registration_no,''))  as VEHICLE_NO ").append("   ,GROUP_CONCAT(t.VEHICLE_MAKE SEPARATOR '|')  as VEHICLE_MAKE,GROUP_CONCAT(t.VEHICLE_MODEL SEPARATOR '|')  as VEHICLE_MODEL,GROUP_CONCAT(t.ENGINE_NUMBER SEPARATOR '|')  as ENGINE_NUMBER,GROUP_CONCAT(t.VEHICLE_CHASIS_NUMBER SEPARATOR '|')  as VEHICLE_CHASIS_NUMBER ").append("\tfrom  cr_loan_collateral_m s ").append("\tleft join cr_asset_collateral_m t on t.ASSET_ID=s.ASSETID  and t.ASSET_COLLATERAL_CLASS='VEHICLE' ").append("\twhere s.LOAN_ID='").append(pLoanId).append("' ").append(" ) as Vehicles on (Vehicles.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT OUTER JOIN ").append(" (").append("\tselect LOAN_ID,SUM(IFNULL(IFNULL(ADVICE_AMOUNT,0)-IFNULL(TXN_ADJUSTED_AMOUNT,0)-IFNULL(AMOUNT_IN_PROCESS,0),0.00)) AS EXCESS ").append("\tfrom cr_txnadvice_dtl where ADVICE_TYPE='P' and CHARGE_CODE_ID=10 ").append("\tand LOAN_ID='").append(pLoanId).append("' ").append("\tgroup by LOAN_ID ").append(" ) as EA on (EA.LOAN_ID=cr_loan_dtl.LOAN_ID) ").append(" LEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as PDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as PRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as PPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as PWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='109' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC ) PENAL ON PENAL.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append("\tLEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as BDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as BRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as BPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as BWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='110' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC  ) BOUNCE ON BOUNCE.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append("\tLEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as RDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as RRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as RPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as RWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='336' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC  ) RETRIEVAL ON RETRIEVAL.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append("\tLEFT JOIN (select   b.loan_id AS LOAN_ID,CHARGE_DESC,A.CHARGE_CODE, ").append("    IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as NDUE_CHARGE, ").append("    ifnull(SUM(txn_adjusted_amount),0) as NRECEIVED, ").append(" \t IFNULL(sum(advice_amount+ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0)-IFNULL(sum((TXN_ADJUSTED_AMOUNT+WAIVE_OFF_AMOUNT+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0))),0) as NPENDING , ").append("    IFNULL(sum(ifnull(WAIVE_OFF_AMOUNT,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT1,0)+IFNULL(WAIVE_OFF_TAX_AMOUNT2,0)),0) as NWAIVE_OFF_AMOUNT ").append(" \t from com_charge_code_m a  ").append(" \t inner join cr_txnadvice_dtl b on a.CHARGE_CODE=b.CHARGE_CODE_ID AND B.BP_TYPE='CS' ").append(" \t and  b.loan_id='").append(pLoanId).append("' ").append(" \t WHERE     B.ADVICE_TYPE='R' AND B.charge_code_id ='335' ").append(" \t GROUP BY A.CHARGE_CODE,CHARGE_DESC  ) PDCNACH ON PDCNACH.LOAN_ID=CR_LOAN_DTL.LOAN_ID ").append(" where cr_loan_dtl.LOAN_ID='").append(pLoanId).append("' and ifnull(cr_loan_dtl.REC_STATUS,'P')<>'X' order by L.TXN_DATE  ").toString();
  ...
  public ArrayList fetchInstlDate(String fromDate, String toDate) {
    ArrayList list = new ArrayList();
    String query = "";
    try {
      query = new StringBuilder().append("SELECT distinct DATE_FORMAT(INSTL_DATE,'%M-%Y') as INSTL_DATE_1 FROM cr_repaysch_dtl R JOIN CR_LOAN_DTL l on r.loan_id = l.loan_id where INSTL_DATE>=str_to_date('").append(fromDate).append("','%d-%m-%Y')  and  INSTL_DATE<=str_to_date('").append(toDate).append("','%d-%m-%Y')  and INSTL_NO<>0 ").append("order by INSTL_DATE asc").toString();
  ...
  public ArrayList fetchLoanList(String fromDate, String toDate) {
    ArrayList list = new ArrayList();
    String query = "";
    try {
      query = new StringBuilder().append("SELECT distinct l.loan_id FROM cr_repaysch_dtl R JOIN CR_LOAN_DTL l on r.loan_id = l.loan_id where INSTL_DATE>=str_to_date('").append(fromDate).append("','%d-%m-%Y')  and  INSTL_DATE<=str_to_date('").append(toDate).append("','%d-%m-%Y')  and INSTL_NO<>0 ").append("order by l.loan_id asc").toString();
  ...
  public ArrayList fetchPrinAmtList(String loanId, ArrayList instlDateList) {
    ArrayList list = new ArrayList();
    StringBuilder query = new StringBuilder();
    int instlDateListSize = instlDateList.size();
    try {
      query.append(" select cld.LOAN_CUSTOMER_ID,cld.loan_no,case cld.REC_STATUS when 'A' then 'Approved' when 'C' then 'Closed'  when 'F' then 'Forwarded' when 'P' then 'Pending' when 'X' then 'Rejected' when 'L' then 'Canceled' end as status,  ifnull(case cld.SALE_OFF_FLAG when 'R' then 'RE-Finance' when 'S' then 'Securitized' end,'Non-Securitized') as SALE_OFF_FLAG,  ifnull(scz.POOL_ID,''),ifnull(sp.POOL_NAME,''),ifnull(cm.AGENCY_NAME,'') ");

      for (int i = 0; i < instlDateListSize; i++) {
        query.append(new StringBuilder().append(" ,ifnull((SELECT sum(PRIN_COMP) FROM CR_REPAYSCH_DTL  crd join cr_loan_dtl cld on cld.loan_id=crd.loan_id where DATE_FORMAT(INSTL_DATE,'%M-%Y')='").append(instlDateList.get(i).toString().replace('[', ' ').replace(']', ' ').trim()).append("'  and INSTL_NO<>0 and cld.loan_id='").append(loanId).append("'),0.00) ").toString());
        query.append(new StringBuilder().append(" ,ifnull((SELECT sum(PRIN_COMP_RECD) FROM CR_REPAYSCH_DTL  crd join cr_loan_dtl cld on cld.loan_id=crd.loan_id where DATE_FORMAT(INSTL_DATE,'%M-%Y')='").append(instlDateList.get(i).toString().replace('[', ' ').replace(']', ' ').trim()).append("'  and INSTL_NO<>0 and cld.loan_id='").append(loanId).append("'),0.00) ").toString());
      }
      query.append(new StringBuilder().append(" from cr_loan_dtl cld  left join SCZ_POOL_UPLOAD_DTL scz on scz.loan_id=cld.loan_id and scz.id=(select max(id) from SCZ_POOL_UPLOAD_DTL where loan_id=cld.loan_id and rec_status='A' )   left join cr_securitization_pool_m sp on sp.POOL_ID=scz.POOL_ID  left join com_agency_m cm on cm.AGENCY_CODE=sp.POOL_INSTITUTION_ID where cld.loan_id='").append(loanId).append("' ").toString());
  ...
  public ArrayList fetchIntAmtList(String loanId, ArrayList instlDateList) {
    ArrayList list = new ArrayList();
    StringBuilder query = new StringBuilder();
    int instlDateListSize = instlDateList.size();
    try {
      ...
      for (int i = 0; i < instlDateListSize; i++) {
        query.append(new StringBuilder().append(" ,ifnull((SELECT sum(INT_COMP) FROM CR_REPAYSCH_DTL  crd join cr_loan_dtl cld on cld.loan_id=crd.loan_id where DATE_FORMAT(INSTL_DATE,'%M-%Y')='").append(instlDateList.get(i).toString().replace('[', ' ').replace(']', ' ').trim()).append("'  and INSTL_NO<>0 and cld.loan_id='").append(loanId).append("'),0.00) ").toString());
        query.append(new StringBuilder().append(" ,ifnull((SELECT sum(INT_COMP_RECD) FROM CR_REPAYSCH_DTL  crd join cr_loan_dtl cld on cld.loan_id=crd.loan_id where DATE_FORMAT(INSTL_DATE,'%M-%Y')='").append(instlDateList.get(i).toString().replace('[', ' ').replace(']', ' ').trim()).append("'  and INSTL_NO<>0 and cld.loan_id='").append(loanId).append("'),0.00) ").toString());
      }
      query.append(new StringBuilder().append(" from cr_loan_dtl cld  left join SCZ_POOL_UPLOAD_DTL scz on scz.loan_id=cld.loan_id and scz.id=(select max(id) from SCZ_POOL_UPLOAD_DTL where loan_id=cld.loan_id and rec_status='A' )   left join cr_securitization_pool_m sp on sp.POOL_ID=scz.POOL_ID  left join com_agency_m cm on cm.AGENCY_CODE=sp.POOL_INSTITUTION_ID where cld.loan_id='").append(loanId).append("' ").toString());
  ...
  public ArrayList fetchAmtList(String loanId, String fromDate, String toDate) {
    ArrayList list = new ArrayList();
    ArrayList instlDateList = new ArrayList();
    instlDateList = instDateList(loanId, fromDate, toDate);
    StringBuilder query = new StringBuilder();
    int instDateListSize = instlDateList.size();
    try {
      query.append(" select cld.LOAN_CUSTOMER_ID,cld.loan_no,case cld.REC_STATUS when 'A' then 'Approved' when 'C' then 'Closed'  when 'F' then 'Forwarded' when 'P' then 'Pending' when 'X' then 'Rejected' when 'L' then 'Canceled' end as status,  ifnull(case cld.SALE_OFF_FLAG when 'R' then 'RE-Finance' when 'S' then 'Securitized' end,'Non-Securitized') as SALE_OFF_FLAG,  ifnull(scz.POOL_ID,''),ifnull(sp.POOL_NAME,''),ifnull(cm.AGENCY_NAME,'') ");

      for (int i = 0; i < instDateListSize; i++) {
        query.append(new StringBuilder().append(" ,ifnull((SELECT distinct DATE_FORMAT(INSTL_DATE,'%d-%m-%Y') FROM CR_REPAYSCH_DTL  crd join cr_loan_dtl cld on cld.loan_id=crd.loan_id where DATE_FORMAT(INSTL_DATE,'%d-%m-%Y')='").append(instlDateList.get(i).toString().replace('[', ' ').replace(']', ' ').trim()).append("'  and INSTL_NO<>0 and cld.loan_id='").append(loanId).append("'),'') ").toString());
        query.append(new StringBuilder().append(" ,ifnull((SELECT sum(PRIN_COMP) FROM CR_REPAYSCH_DTL  crd join cr_loan_dtl cld on cld.loan_id=crd.loan_id where DATE_FORMAT(INSTL_DATE,'%d-%m-%Y')='").append(instlDateList.get(i).toString().replace('[', ' ').replace(']', ' ').trim()).append("'  and INSTL_NO<>0 and cld.loan_id='").append(loanId).append("'),0.00) ").toString());
        query.append(new StringBuilder().append(" ,ifnull((SELECT sum(INT_COMP) FROM CR_REPAYSCH_DTL  crd join cr_loan_dtl cld on cld.loan_id=crd.loan_id where DATE_FORMAT(INSTL_DATE,'%d-%m-%Y')='").append(instlDateList.get(i).toString().replace('[', ' ').replace(']', ' ').trim()).append("'  and INSTL_NO<>0 and cld.loan_id='").append(loanId).append("'),0.00) ").toString());
      }
      query.append(new StringBuilder().append(" from cr_loan_dtl cld  left join SCZ_POOL_UPLOAD_DTL scz on scz.loan_id=cld.loan_id and scz.id=(select max(id) from SCZ_POOL_UPLOAD_DTL where loan_id=cld.loan_id and rec_status='A' )   left join cr_securitization_pool_m sp on sp.POOL_ID=scz.POOL_ID  left join com_agency_m cm on cm.AGENCY_CODE=sp.POOL_INSTITUTION_ID where cld.loan_id='").append(loanId).append("' ").toString());
  ...
  public ArrayList instDateList(String loanId, String fromDate, String toDate) {
    ArrayList list = new ArrayList();
    String query = "";
    try {
      query = new StringBuilder().append("SELECT distinct DATE_FORMAT(INSTL_DATE,'%d-%m-%Y') as INSTL_DATE_1 FROM cr_repaysch_dtl R JOIN CR_LOAN_DTL l on r.loan_id = l.loan_id where l.loan_Id='").append(loanId).append("' and INSTL_DATE>=str_to_date('").append(fromDate).append("','%d-%m-%Y')  and  INSTL_DATE<=str_to_date('").append(toDate).append("','%d-%m-%Y')  and INSTL_NO<>0 ").append("order by INSTL_DATE asc").toString();
