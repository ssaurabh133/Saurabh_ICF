...
public class MSSQLRepoBillingApprovalMakerDAOImpl implements RepoBillingApprovalMakerDAO {

	private static final Logger logger = Logger.getLogger(MSSQLRepoBillingApprovalMakerDAOImpl.class.getName());
		...
		try {
			...
			else
			{
				...
				if (vo.getBillingStopped() != null
					...
					logger.info("In insert Case Type master");
					StringBuilder bufInsSql = new StringBuilder();
					bufInsSql.append("UPDATE CR_LOAN_REPO_APPROVAL_DTL SET REPO_BILLING_FLAG=?,REPO_INTEREST_FLAG=?,SD_INTEREST_FLAG=?,REC_STATUS=?,MAKER_ID=?,MAKER_DATE=" );
					bufInsSql.append(dbo);
					bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
...
public boolean forwardRepoBillingApprovalMarkingDetails(Object ob) {
	...
	try {
		...
		if (vo.getBillingStopped() != null
			...
			logger.info("In insert Case Type master");
			StringBuilder bufInsSql = new StringBuilder();
			bufInsSql.append("UPDATE CR_LOAN_REPO_APPROVAL_DTL SET REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
			bufInsSql.append(dbo);
			bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9) ");
			bufInsSql.append("WHERE LOAN_ID='"+ vo.getLbxDealNo()+"' ");
