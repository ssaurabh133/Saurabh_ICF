...
public class MSSQLCaseMarkingAuthorDAOImpl implements CaseMarkingAuthorDAO {

	private static final Logger logger = Logger.getLogger(MSSQLCaseMarkingAuthorDAOImpl.class.getName());
	...
	public ArrayList<CaseMarkingAuthorVO> openEditCaseMarkingAuthor(
			CaseMarkingAuthorVO vo) {
		...
		try {
			StringBuilder bufInsSql = new StringBuilder();
			bufInsSql.append(" select a.case_id, a.loan_id,b.loan_no,c.customer_name,a.OTHER_DETAILS,a.REMARKS,a.CASE_MARKING_FLAG,dbo.DATE_FORMAT(a.CASE_MARKING_DATE,'"+ dateFormat +"'),a.AGENCY, d.description  FROM CR_CASE_MARKING_TMP_DTL a inner join cr_loan_dtl b on b.loan_id=a.loan_id inner join gcd_customer_m c on c.customer_id=a.loan_customer_id inner join generic_master d on d.value=a.case_status and d.GENERIC_KEY='CASE_MARKING_FLAG'");
	...
	@Override
	public ArrayList<Object> saveCaseMarkingCheckerDetails(
			CaseMarkingAuthorVO vo) {
