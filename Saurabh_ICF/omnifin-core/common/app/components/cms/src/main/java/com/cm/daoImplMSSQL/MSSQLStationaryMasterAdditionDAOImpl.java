...
public class MSSQLStationaryMasterAdditionDAOImpl implements StationaryMasterDAO
{
	private static final Logger logger=Logger.getLogger(MSSQLStationaryMasterAdditionDAOImpl.class.getName());
	   ...
	   try {
	    	  int stationaryID=0;
		    	  con = ConnectionDAO.getConnection();
	    	  	  ...
	    	  	  String sql = "insert into cr_stationary_m(COMPANY_ID,ADDITION_DATE, BOOK_TYPE, BANK_NAME)"
                                   + "values(?,"+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"'),?,?)";
		  ...
		  if(flag>0){
			        ...
			        logger.info("flag >0:--"+flag);
			  		rs=stmt.executeQuery("Select max(STATIONARY_ID) from cr_stationary_m  WITH (ROWLOCK)");
		 while(rs.next()){
		 ...
		 for(int i=0;i<bookNo.length;i++){
	     			  ...
	     			  String sql2 ="insert into cr_stationary_dtl ( BOOK_NO,STATIONARY_ID,ADDITION_DATE, INSTRUMENT_NO,INSTRUMENT_FROM,INSTRUMENT_TO,REC_STATUS,MAKER_ID,MAKER_DATE,ALLOCATED_TO,ALLOCATION_DATE  )"+ 
					  				"values(?,?,"+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"'),?,?,?,?,?,"+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"'),?,"+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"'))";
		...
		try {	
			 ...
			 String sql3= (" update cr_stationary_dtl set INSTRUMENT_NO=?,INSTRUMENT_FROM=?, INSTRUMENT_TO=?, REC_STATUS=?,MAKER_ID =?,MAKER_DATE="+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"'), ALLOCATED_TO=? ,ALLOCATION_DATE="+dbo+"STR_TO_DATE(?, '"+dateFormatWithTime+"') where BOOK_NO='"+bookNo2[i]+"'");
	...
	public List<StationaryMasterForm> getList(StationaryMasterForm stationaryMasterForm) {
		...
		{     
			  ...
			  if(CommonFunction.checkNull(rs.getString(9)).equalsIgnoreCase("A")){
				  formStationary.setBookNo1("<a href=StationaryAdditionMasterDispatchAction.do?method=openViewStationary&bookNo="+ CommonFunction.checkNull(rs.getString(1)).toString()+"&alloctoParameter="+CommonFunction.checkNull(stationaryMasterForm.getAllocto()).trim()+">"+ CommonFunction.checkNull(rs.getString(1)) + "</a>");
			  }else{
				  formStationary.setBookNo1("<a href=StationaryAdditionMasterDispatchAction.do?method=openEditStationary&bookNo="+ CommonFunction.checkNull(rs.getString(1)).toString()+"&alloctoParameter="+CommonFunction.checkNull(stationaryMasterForm.getAllocto()).trim()+">"+ CommonFunction.checkNull(rs.getString(1)) + "</a>"); 
			  }
	...
	public List<StationaryMasterForm> getdatalist(StationaryMasterForm formStationary) {
	 ...
	 try{
			...
			bufInsSql.append("select BOOK_NO,cb.BANK_NAME,CASE BOOK_TYPE WHEN 'R' THEN 'RECEIPT BOOK' WHEN 'C' THEN 'CHEQUE BOOK' END BOOK_TYPE,INSTRUMENT_NO,INSTRUMENT_FROM,INSTRUMENT_TO,CASE cd.REC_STATUS WHEN 'A' THEN 'ACTIVE' WHEN 'X' THEN 'INACTIVE' END AS REC_STATUS,");
			bufInsSql.append(dbo);
			bufInsSql.append("date_format(cd.ADDITION_DATE,'%d-%m-%Y') ADDITION_DATE,ALLOCATED_TO,");
			bufInsSql.append(dbo);
			bufInsSql.append("date_format(allocation_date,'%d-%m-%Y') ALLOCATION_DATE,cd.STATIONARY_ID,BOOK_TYPE,cb.bank_id FROM cr_stationary_dtl cd LEFT JOIN cr_stationary_m cm ON cd.STATIONARY_ID=cm.STATIONARY_ID LEFT JOIN COM_BANK_M cb ON cm.bank_name=cb.bank_id");			
		  ...
		  logger.info("St"+St);
			...
			{    		
				...
				if(CommonFunction.checkNull(rs.getString(8)).equalsIgnoreCase("A")){
					  formStationary.setBookNo1("<a href=StationaryAdditionMasterDispatchAction.do?method=openViewStationary&bookNo="+ CommonFunction.checkNull(rs.getString(1)).toString()+"&alloctoParameter="+CommonFunction.checkNull(stationaryMasterForm.getAllocto()).trim()+">"+ CommonFunction.checkNull(rs.getString(1)).toString() + "</a>");
				  }else{
					  formStationary.setBookNo1("<a href=StationaryAdditionMasterDispatchAction.do?method=openEditStationary&bookNo="+ CommonFunction.checkNull(rs.getString(1)).toString()+"&alloctoParameter="+CommonFunction.checkNull(stationaryMasterForm.getAllocto()).trim()+">"+ CommonFunction.checkNull(rs.getString(1)).toString() + "</a>"); 
				  }
...
public int checkDublicateReceiptNo(StationaryMasterForm stationaryMasterForm) {
		...
		int count=0;
		String paramQuery ="SELECT ISNULL(PARAMETER_VALUE,'N') FROM PARAMETER_MST WHERE PARAMETER_KEY='BOOK_NO_NON_MANDATORY'";
