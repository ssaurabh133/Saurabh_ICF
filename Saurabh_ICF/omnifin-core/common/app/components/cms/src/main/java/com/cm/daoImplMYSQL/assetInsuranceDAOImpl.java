...
public class assetInsuranceDAOImpl implements assetInsuranceDAO {
	
	private static final Logger logger = Logger.getLogger(assetInsuranceDAOImpl.class.getName());
	...
	int no=Integer.parseInt(resource.getString("msg.pageSizeForMaster"));
			...
			ArrayList<AssetForCMVO> getDataList=new ArrayList<AssetForCMVO>();
			 ...
			 ArrayList qryList = new ArrayList();
				...
				try{
		           ...
		           " RAISE_ADVICE_FLAG,REC_STATUS,MAKER_ID,MAKER_DATE,INITIATION_DATE,YEAR_NO,ENTITY_TYPE,ENTITY_ID,SURRENDER_VALUE,PREMIUM_FREQUENCY,NOMINEE,RELATIONSHIP_WITH_NOMINEE,ASSET_ID )");
					...
					if(CommonFunction.checkNull(assetMakervo.getEntityType()).equalsIgnoreCase("ASSET")||CommonFunction.checkNull(assetMakervo.getEntityType()).equalsIgnoreCase("COLLATRAL"))
					{
		...
		public String getAssetInsuranceId(){
			...
			logger.info("In AssetInsurance...Dao Impl.");
			try{
				assetInsuranceID.append("SELECT max(ASSET_INSURANCE_ID) from cr_asset_insurance_dtl for update");
				
				instId=ConnectionDAO.singleReturn(assetInsuranceID.toString());
		  		logger.info("IN paymentId ### "+assetInsuranceID);
		    }
		  	   catch(Exception e){
						e.printStackTrace();
					}
		...
		public ArrayList <AssetForCMVO>searchAssetData(AssetForCMVO assetMakervo)
		{
		    ...
		    logger.info("In searchAssetData...Dao Impl.");
		    ...
		    try{
							  ...
							  logger.info("assetMakervo.getEntityType()"+entityType);
			  					bufInsSql.append("Select distinct CAI.COVER_NOTE,CAI.POLICY_NO,CAI.INSURANCE_CO_ID,A.AGENCY_NAME,CAI.INSURANCE_BY,"+
					  			" CAI.SUM_ASSURED,CAI.PREMIUM_AMOUNT,date_format(CAI.START_DATE,'"+dateFormat+"'), " +
					  			" date_format(CAI.END_DATE,'"+dateFormat+"'),CAI.APPROVAL_REMARKS, CAI.LOAN_ID,CLD.LOAN_NO,GCM.CUSTOMER_NAME, "+
					  			...
					  			" CASE ASSET_NEW_OLD WHEN 'N' THEN 'NEW' WHEN 'O' THEN 'OLD' END AS ASSET_NEW_OLD,IFNULL(VEHICLE_MAKE,MACHINE_MAKE),IFNULL(VEHICLE_MODEL,VEHICLE_MODEL),  "+
					  			" DEALER_DESC,IFNULL(VEHICLE_ENGINE_NO,ENGINE_NUMBER),VEHICLE_CHASIS_NUMBER,VEHICLE_REGISTRATION_NO,  ");
			  					}
						  if(CommonFunction.checkNull(assetMakervo.getEntityType()).equals("ASSET") || CommonFunction.checkNull(assetMakervo.getEntityType()).equals("COLLATERAL"))
						  {
								bufInsSql.append(" m.ASSET_COLLATERAL_DESC ");
						  		...
						  		bufInsSql.append("  from cr_asset_insurance_dtl CAI "+
						  		" left JOIN cr_asset_collateral_m CAC  ON CAC.ASSET_ID=CAI.ENTITY_ID  "+
						  ...
						  if(CommonFunction.checkNull(assetMakervo.getEntityType()).equals("ASSET") || CommonFunction.checkNull(assetMakervo.getEntityType()).equals("COLLATERAL"))
						  {
				  			  bufInsSql.append(" JOIN (SELECT LOAN_ID, ASSET_COLLATERAL_DESC FROM cr_asset_insurance_dtl "+
								 ...
								 " WHERE cr_asset_insurance_dtl.ASSET_INSURANCE_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(assetMakervo.getAssetInsuranceId()).trim())+"' LIMIT 1) m "+
								 " on m.loan_id=CAI.LOAN_ID ");
			  			}else{
				  			bufInsSql.append(" JOIN (SELECT   gcd_customer_m.CUSTOMER_NAME,cr_asset_insurance_dtl.LOAN_ID from cr_asset_insurance_dtl "+
						  		...
						  		" ASSET_INSURANCE_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(assetMakervo.getAssetInsuranceId()).trim())+"' LIMIT 1 ) m "+
						  		" on m.loan_id=CAI.LOAN_ID  ");
						  	}		
						  	bufInsSql.append(" where ASSET_INSURANCE_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(assetMakervo.getAssetInsuranceId()).trim())+"'  limit 1 ");
		...
		public String saveForwardDataUpdate(AssetForCMVO assetMakervo){
			...
			logger.info("In saveForwardDataUpdate...Dao Impl.");
			ArrayList<AssetForCMVO> getDataList=new ArrayList<AssetForCMVO>();
			boolean status=false;
			String result="";
			...
			try{				
			    ...
			    {	
		         ...
		         sBUpdQry.append(" PREMIUM_AMOUNT= ?,START_DATE= STR_TO_DATE(?,'"+dateFormat+"'),END_DATE= STR_TO_DATE(?,'"+dateFormat+"'), ");
		         sBUpdQry.append(" REMARKS= ?,RAISE_ADVICE_FLAG=?,MAKER_ID=?,MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),REC_STATUS='"+recStatus+"',INITIATION_DATE=STR_TO_DATE(?,'"+dateFormat+"'),YEAR_NO=?,ENTITY_TYPE=?,ENTITY_ID=?,SURRENDER_VALUE=?,PREMIUM_FREQUENCY=?,NOMINEE=?,RELATIONSHIP_WITH_NOMINEE=? WHERE ASSET_INSURANCE_ID="+assetInsuranceId+"  ");
    	...
    	try{
    		...
    		PrepStmtObject updatePrepStmtObject = new PrepStmtObject();    
    		int statusProc=0;    	 
    		 logger.info(" In approveByAuthor BeforeProc assetInsuranceID: "+assetInsuranceID);
	   	...
	   	public ArrayList<AssetForCMVO> assetAuthorGrid(AssetForCMVO assetMakervo)
	       	{
	    	    ...
	    	    logger.info("In assetAuthorGrid...Dao Impl.");
	    		int count=0;
	    	    ...
	    	    ArrayList<AssetForCMVO> searchListGrid=new 	ArrayList<AssetForCMVO>();
	    	      ...
	    	      bufInsSql.append("Select distinct A.LOAN_ID,C.LOAN_NO,GCM.CUSTOMER_NAME,A.COVER_NOTE,A.POLICY_NO,A.INSURANCE_CO_ID,B.AGENCY_NAME" +
	    	    		   " ,date_format(A.START_DATE,'"+dateFormat+"'), date_format(A.END_DATE,'"+dateFormat+"'),(SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=A.MAKER_ID) MAKER_ID,ASSET_INSURANCE_ID,ENTITY_TYPE " +
	    	     ...
	    	     bufInsSqlTempCount.append("Select distinct Count(1) FROM cr_asset_insurance_dtl A,com_agency_m B,cr_loan_dtl C,gcd_customer_m GCM " +
	    		  ...
	    		  if(!((loanId.toString()).trim().equalsIgnoreCase("")) || !((loanNo.toString()).trim().equalsIgnoreCase(""))|| !((customerName.toString()).trim().equalsIgnoreCase(""))|| !((coverNoteNo.toString()).trim().equalsIgnoreCase("")) || !((policyNo.toString()).trim().equalsIgnoreCase("")) || !((insuranceAgencyId.toString()).trim().equalsIgnoreCase("")) || !((insuranceAgency.toString()).trim().equalsIgnoreCase("")) || !((startDate.toString()).trim().equalsIgnoreCase("")) || !((endDate.toString()).trim().equalsIgnoreCase(""))){
	    		    ...
	    		    if((!((loanNo.toString()).trim().equalsIgnoreCase("")))) {
	    	 	         bufInsSql.append(" AND C.LOAN_NO='"+loanNo+"' ");
	    	 	        bufInsSqlTempCount.append(" AND C.LOAN_NO='"+loanNo+"' ");
	    	 	    	 appendSQL=true;
	    		  ...
	    		  if((!((coverNoteNo.toString()).trim().equalsIgnoreCase("")))) {
	    		    	  bufInsSql.append(" AND A.COVER_NOTE='"+coverNoteNo+"' ");
	    		    	  bufInsSqlTempCount.append(" AND A.COVER_NOTE='"+coverNoteNo+"' ");
	    		    	  appendSQL=true;
	    		      }
	    				
	    				if((!((policyNo.toString()).trim().equalsIgnoreCase("")))) {
	    		    	  bufInsSql.append(" AND A.POLICY_NO='"+policyNo+"' ");
	    		    	  bufInsSqlTempCount.append(" AND A.POLICY_NO='"+policyNo+"' ");
	    		    	  appendSQL=true;
	    		      }
	      	        if((!((insuranceAgencyId.toString()).trim().equalsIgnoreCase("")))) {
	    	         bufInsSql.append(" AND A.INSURANCE_CO_ID='"+insuranceAgencyId+"' ");
	    	         bufInsSqlTempCount.append(" AND A.INSURANCE_CO_ID='"+insuranceAgencyId+"' ");
	    	    	 appendSQL=true;
	    	    	  
	    	      }
	      	      if((!((insuranceAgency.toString()).trim().equalsIgnoreCase("")))) {
	     	         bufInsSql.append(" AND B.AGENCY_NAME LIKE '%"+insuranceAgency+"%' ");
	     	        bufInsSqlTempCount.append(" AND B.AGENCY_NAME LIKE '%"+insuranceAgency+"%' ");
	     	    	 appendSQL=true;
	     	    	  
	     	      }
	      	    if((!((startDate.toString()).trim().equalsIgnoreCase("")))) {
	    	         bufInsSql.append(" AND A.START_DATE=STR_TO_DATE('"+startDate+"','"+dateFormatWithTime+"') ");
	    	         bufInsSqlTempCount.append(" AND A.START_DATE=STR_TO_DATE('"+startDate+"','"+dateFormatWithTime+"') ");
	    	        
	    	    	 appendSQL=true;
	    	    	  
	    	      }
	      	   if((!((endDate.toString()).trim().equalsIgnoreCase("")))) {
	    	         bufInsSql.append(" AND A.END_DATE=STR_TO_DATE('"+endDate+"','"+dateFormatWithTime+"') ");
	    	         bufInsSqlTempCount.append(" AND A.END_DATE=STR_TO_DATE('"+endDate+"','"+dateFormatWithTime+"') ");
	    	    	 appendSQL=true;
	    	    	  
	    	      }	 
	      	
	      	 if(!((CommonFunction.checkNull(assetMakervo.getLbxUserId()).equalsIgnoreCase("")))) {
	      		bufInsSql.append("  AND A.MAKER_ID='"+StringEscapeUtils.escapeSql(assetMakervo.getLbxUserId()).trim()+"' ");	
				bufInsSqlTempCount.append("  AND A.MAKER_ID='"+StringEscapeUtils.escapeSql(assetMakervo.getLbxUserId()).trim()+"' ");
				appendSQL = true;
		      }       				
	       			
	           count =Integer.parseInt(ConnectionDAO.singleReturn(bufInsSqlTempCount.toString()));
               logger.info("count Search Query---- "+bufInsSqlTempCount.toString());
			 ...
			 logger.info("main Search Query---- "+bufInsSql.toString());
			 searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString()); 
	    ...
	    public ArrayList <AssetForCMVO>savedDatatoApprove(AssetForCMVO assetMakervo)
	  	    ...
	  	    try{
				...
				bufInsSql.append("Select distinct CAI.COVER_NOTE,CAI.POLICY_NO,CAI.INSURANCE_CO_ID,A.AGENCY_NAME,CASE gms.value  WHEN 'B' THEN 'BORROWER' WHEN 'D' THEN 'DEALER' WHEN 'L' THEN 'IN HOUSE' END AS INSURANCE_BY,"+
	  			" CAI.SUM_ASSURED,CAI.PREMIUM_AMOUNT,date_format(CAI.START_DATE,'"+dateFormat+"'), " +
	  			" date_format(CAI.END_DATE,'"+dateFormat+"'),CAI.APPROVAL_REMARKS, CAI.LOAN_ID,CLD.LOAN_NO,GCM.CUSTOMER_NAME, "+
		  ...
		  if(CommonFunction.checkNull(assetMakervo.getEntityType()).equals("ASSET") || CommonFunction.checkNull(assetMakervo.getEntityType()).equals("COLLATERAL"))
		  {
				...
				bufInsSql.append(" m.customer_name ");
		  			...
		  			bufInsSql.append("  from cr_asset_insurance_dtl CAI "+
		  			" left join GENERIC_MASTER gms on ( gms.value=CAI.insurance_by and gms.GENERIC_KEY='INSURANC_DONE_BY')"+
		  			" left JOIN cr_asset_collateral_m CAC  ON CAC.ASSET_ID=CAI.ENTITY_ID AND CAI.REC_STATUS='F'  "+
	  			" JOIN cr_loan_dtl CLD on CAI.LOAN_ID=CLD.LOAN_ID "+
	 			" JOIN com_agency_m A on A.AGENCY_CODE=CAI.INSURANCE_CO_ID "+
	 			" LEFT JOIN CR_DSA_DEALER_M CDDM ON CAC.ASSET_SUPPLIER=CDDM.DEALER_ID "+
				" JOIN gcd_customer_m GCM  on CLD.LOAN_CUSTOMER_ID=GCM.CUSTOMER_ID ");
		  ...
		  if(CommonFunction.checkNull(assetMakervo.getEntityType()).equals("ASSET") || CommonFunction.checkNull(assetMakervo.getEntityType()).equals("COLLATERAL"))
		  {
			...
			bufInsSql.append(" JOIN (SELECT   gcd_customer_m.CUSTOMER_NAME,cr_asset_insurance_dtl.LOAN_ID from cr_asset_insurance_dtl "+
		  	...
		  	bufInsSql.append(" where ASSET_INSURANCE_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(assetMakervo.getAssetInsuranceId()).trim())+"'  limit 1 ");
	  			  ...
	  			  logger.info("In searchAssetData forward......... query..........."+bufInsSql.toString());
	  		     searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());
	  		    logger.info("searchAssetData size is...."+searchlist.size());
			...
			logger.info("here userid++++++++++++++++++++++++++++ "+assetMakervo.getReportingToUserId());
		      ...
		      bufInsSql.append("Select distinct A.LOAN_ID,C.LOAN_NO,GCM.CUSTOMER_NAME,A.COVER_NOTE,A.POLICY_NO,A.INSURANCE_CO_ID,B.AGENCY_NAME" +
		  	 ...
		  	 if (((StringEscapeUtils.escapeSql(CommonFunction.checkNull(assetMakervo.getLbxUserId())).trim().equalsIgnoreCase(""))))
				{
		  		 	bufInsSql.append(" AND A.MAKER_ID='"+StringEscapeUtils.escapeSql(assetMakervo.getUserId()).trim()+"' ");	
		  		 	bufInsSqlTempCount.append(" AND A.MAKER_ID='"+StringEscapeUtils.escapeSql(assetMakervo.getUserId()).trim()+"' ");
					appendSQL = true;			
				}
				else{
					bufInsSql.append(" AND A.MAKER_ID='"+StringEscapeUtils.escapeSql(assetMakervo.getLbxUserId()).trim()+"' ");	
		      ...
		      logger.info("assetMakerGrid Data size is...."+searchlist.size());
		     for(int i=0;i<searchlist.size();i++){
		        ArrayList data=(ArrayList)searchlist.get(i);
		...
		{
			    logger.info("in getAssetInsuranceViewer pop up() method DaoImpl****"+CommonFunction.checkNull(assetMakervo.getLoanId()));    	
		    	ArrayList searchlist=new ArrayList();
		    	ArrayList<AssetForCMVO> assetList=new ArrayList<AssetForCMVO>();
		    	
		    try{
		    	String query=("Select distinct CAI.POLICY_NO,A.AGENCY_NAME, "+
		    				" CASE gms.value  WHEN 'B' THEN 'BORROWER' WHEN 'D' THEN 'DEALER' WHEN 'L' THEN 'IN HOUSE' END AS INSURANCE_BY, "+
			    			...
			    			" CAI.PREMIUM_AMOUNT,date_format(CAI.START_DATE,'"+dateFormat+"'), "+
			    			" date_format(CAI.END_DATE,'"+dateFormat+"'),CAI.ASSET_INSURANCE_ID,CAI.LOAN_ID,ENTITY_TYPE "+
			    			" from cr_asset_insurance_dtl CAI "+
			    			" left join GENERIC_MASTER gms on ( gms.value=CAI.insurance_by and gms.GENERIC_KEY='INSURANC_DONE_BY')"+
			    			" left JOIN cr_asset_collateral_m CAC on  CAI.REC_STATUS='A' "+   
			    			" JOIN cr_loan_dtl CLD on CAI.LOAN_ID=CLD.LOAN_ID  "+
			    			" JOIN com_agency_m A on A.AGENCY_CODE=CAI.INSURANCE_CO_ID "+ 
			    			" JOIN gcd_customer_m GCM  on CLD.LOAN_CUSTOMER_ID=GCM.CUSTOMER_ID "+ 
			    			" where CAI.loan_id='"+CommonFunction.checkNull(assetMakervo.getLoanId())+"' and CAI.rec_status='A' ");
		...
		public ArrayList<AssetForCMVO> getAssetInsuranceViewerMaker(AssetForCMVO assetMakervo)
		{
		    ...
		    try{
		    	String query=("SELECT DISTINCT CAI.YEAR_NO,A.AGENCY_NAME,CASE CAI.ENTITY_TYPE  WHEN 'PRAPPL' THEN 'APPLICANT' WHEN 'COAPPL' THEN 'CO-APPLICANT' WHEN 'GUARANTOR' THEN 'GUARANTOR'  "+
		    				" WHEN 'ASSET' THEN 'ASSET' WHEN 'COLLATERAL' THEN 'COLLATERAL' END AS ENTITY_TYPE,GCM.CUSTOMER_NAME,DATE_FORMAT(CAI.START_DATE,'%D-%M-%Y'),  DATE_FORMAT(CAI.END_DATE,'%D-%M-%Y'), "+
		    				...
		    				+ " ,CASE CAI.REC_STATUS WHEN 'A' THEN 'Approve' WHEN 'P' THEN 'Pending' END AS REC_STATUS  "+
		    				" FROM CR_ASSET_INSURANCE_DTL CAI "+
		    				" left JOIN CR_ASSET_COLLATERAL_M CAC ON CAC.ASSET_ID=CAI.ENTITY_ID   "+
		    				" JOIN CR_LOAN_DTL CLD ON CAI.LOAN_ID=CLD.LOAN_ID  "+
		    				" JOIN COM_AGENCY_M A ON A.AGENCY_CODE=CAI.INSURANCE_CO_ID "+
			  		...
			  		if(data.size()>0){
			  			...
			  			if(CommonFunction.checkNull(data.get(17)).equalsIgnoreCase("Y")){
			  				
			  			  assetVO.setYearNo(CommonFunction.checkNull(data.get(0)));
			  			}
			  			else
			  			{
			  			  assetVO.setYearNo("<a href=assetProcessAction.do?method=showAssetData&loanId='"+CommonFunction.checkNull(data.get(13)).toString()+"'&entityType='"+CommonFunction.checkNull(data.get(14)).toString()+"'&assetInsuranceId='"+CommonFunction.checkNull(data.get(12)).toString()+"' >"+ CommonFunction.checkNull(data.get(0)).toString() +"</a>");
						  ...
						  if(!CommonFunction.checkNull(data.get(10)).equalsIgnoreCase(""))	
				  		 ...
				  		 assetVO.setRecStatus(CommonFunction.checkNull(data.get(19)).trim());
			  		      assetList.add(assetVO);				
		  	  		} 
		  	  	}
		    }
	...
	public ArrayList<AssetForCMVO> assetVehicleDetails(AssetForCMVO assetMakervo, String lbxEntity)
	{
	  	    ...
	  	    try{
				  ...
				  bufInsSql.append("SELECT DISTINCT ASSET_COLLATERAL_DESC,CASE ASSET_NEW_OLD WHEN 'N' THEN 'NEW' WHEN 'O' THEN 'OLD' END AS ASSET_NEW_OLD,IFNULL(VEHICLE_MAKE,MACHINE_MAKE),IFNULL(VEHICLE_MODEL,VEHICLE_MODEL), "+
							" DEALER_DESC,IFNULL(VEHICLE_ENGINE_NO,ENGINE_NUMBER),VEHICLE_CHASIS_NUMBER,VEHICLE_REGISTRATION_NO "+
					  ...
					  if(CommonFunction.checkNull(assetMakervo.getEntityType()).equals("ASSET") || CommonFunction.checkNull(assetMakervo.getEntityType()).equals("COLLATERAL"))
					  {
							bufInsSql.append(" m.ASSET_COLLATERAL_DESC ");
					  		...
					  		bufInsSql.append("  from cr_asset_insurance_dtl CAI "+
					 			...
					 			" left JOIN com_agency_m A on A.AGENCY_CODE=CAI.INSURANCE_CO_ID "+
			  					" JOIN gcd_customer_m GCM  on CLD.LOAN_CUSTOMER_ID=GCM.CUSTOMER_ID ");
    ...
    try{
    	String query=("select if(MAX(CAI.YEAR_NO) IS NOT NULL ,max(CAI.YEAR_NO),0)  FROM CR_ASSET_COLLATERAL_M CAC JOIN CR_ASSET_INSURANCE_DTL CAI ON CAC.ASSET_ID=CAI.ENTITY_ID AND  CAI.REC_STATUS='A'JOIN CR_LOAN_DTL CLD ON CAI.LOAN_ID=CLD.LOAN_ID  JOIN COM_AGENCY_M A ON A.AGENCY_CODE=CAI.INSURANCE_CO_ID  JOIN GCD_CUSTOMER_M GCM  ON CLD.LOAN_CUSTOMER_ID=GCM.CUSTOMER_ID WHERE CAI.LOAN_ID='"+loanId+"' ");
	...
	public boolean uploadCersaiReport(String vFileName, String UserId)  {
		...
		logger.info("Inside FinancialReportDaoImpl::fetchSystemOutputSheet()");
		...
		{			
			...
			String bday = CommonFunction.checkNull(ConnectionDAO.singleReturn(" select parameter_value from parameter_mst where parameter_key = 'business_date' "));
			String vPath=ConnectionDAO.singleReturn(vQuery1);
			vQuery1=null;

			String strPath=vPath+"/"+vFileName;
			logger.info("writeFilePath::"+strPath);
			objFile1 = new File(strPath);


			Workbook workbook;
			Sheet sheet;
			if(vFileName.contains("CERSAI_entry")){
				try {
					workbook = Workbook.getWorkbook(objFile1);
					...
					for(int j=2;j<maxRow;j++){
						...
						xlsCell=sheet.getCell(52,j);
						Queue_Ref_No=xlsCell.getContents();

						xlsCell=sheet.getCell(53,j);
						Cersai_Asset_ID =xlsCell.getContents();

						xlsCell=sheet.getCell(54,j);
						SI_ID =xlsCell.getContents();
						
						xlsCell=sheet.getCell(55,j);
						asset_id =xlsCell.getContents();

						insertPrepStmtObject = new PrepStmtObject();
						qryList = new ArrayList();
						String vvQry="update cr_asset_collateral_m set queue_ref_no='"+Queue_Ref_No+"',cersai_asset_id='"+Cersai_Asset_ID+"' ,si_id='"+SI_ID+"' "
								+ "where asset_id='"+asset_id+"'";
						insertPrepStmtObject.setSql(vvQry);
						qryList.add(insertPrepStmtObject);
						try {
							status = ConnectionDAO.sqlInsUpdDeletePrepStmt(qryList);
			...
			}else if(vFileName.contains("Risk_Category")){
				try {
					workbook = Workbook.getWorkbook(objFile1);
					sheet = workbook.getSheet(0);
					Cell xlsCell;
					...
					for(int j=1;j<maxRow;j++){
						...
						dealId=xlsCell.getContents();
						
						xlsCell=sheet.getCell(2,j);
						loanId=xlsCell.getContents();
						
						xlsCell=sheet.getCell(3,j);
						customerId=xlsCell.getContents();

						xlsCell=sheet.getCell(4,j);
						RiskCat =xlsCell.getContents();
						...
						ArrayList insertUpdate = new ArrayList();
						  ...
						  bufInsUpdSql.append(" INSERT INTO CR_RISK_CATEGORY_HST ");
				          bufInsUpdSql.append(" ( ");
				          bufInsUpdSql.append(" CUSTOMER_ID,RISK_CATEGORY,MAKER_ID,MAKER_DATE ) ");
				          bufInsUpdSql.append(" SELECT CUSTOMER_ID,RISK_CATEGORY,'"+UserId+"','"+bday+"' ");
				          bufInsUpdSql.append(" FROM GCD_CUSTOMER_M WHERE CUSTOMER_ID ='"+customerId+"' ");
						...
						insertPrepStmtObject = new PrepStmtObject();
						qryList = new ArrayList();
						String vvQry="update gcd_customer_m set RISK_CATEGORY='"+RiskCat+"'"
						...
						insertPrepStmtObject = new PrepStmtObject();
						qryList = new ArrayList();
						 vvQry="update cr_deal_customer_m set RISK_CATEGORY='"+RiskCat+"'"
								+ "where customer_id='"+customerId+"'";
						insertPrepStmtObject.setSql(vvQry);
						qryList.add(insertPrepStmtObject);
						try {
			...
			}else if(vFileName.contains("CGTMSE_Upload")){
				try {
					workbook = Workbook.getWorkbook(objFile1);
					sheet = workbook.getSheet(0);
					Cell xlsCell;
					...
					for(int j=1;j<maxRow;j++){
						...
						xlsCell=sheet.getCell(2,j);
						AlreadyClaimed =xlsCell.getContents();
						
						xlsCell=sheet.getCell(3,j);
						ClaimAmount =xlsCell.getContents();

						StringBuilder bufInsUpdSql=new StringBuilder();
						PrepStmtObject insertPrepStmtObject3 = new PrepStmtObject();
						ArrayList insertUpdate = new ArrayList();
						
						  bufInsUpdSql.append(" INSERT INTO CR_CGTMSE_RECORD_DTL ");
				          bufInsUpdSql.append(" ( ");
				          bufInsUpdSql.append(" LOAN_NO,ALREADY_CLAIMED,CLAIM_AMOUNT) ");
				          bufInsUpdSql.append(" SELECT '"+loanId+"','"+AlreadyClaimed+"','"+ClaimAmount+"' ");
				        
				         insertPrepStmtObject3.setSql(bufInsUpdSql.toString());
					     logger.info("insert history :"+insertPrepStmtObject3.printQuery());
					     insertUpdate.add(insertPrepStmtObject3);
					     try {
					    	 status=ConnectionDAO.sqlInsUpdDeletePrepStmt(insertUpdate);
							} catch (SQLException e1) {
								e1.printStackTrace();
							}
	...
	public boolean docUploadExcel(HttpServletRequest request,FormFile myFile) {
		boolean status = false;
		String fileName = "";
		String filePath = "";
		String filePath1 = "";
		...
		try 
			{
				String query = "select parameter_value from parameter_mst where parameter_key='CERSAI_UPLOAD_PATH'";
