...
public class DataUploadMakerBehindAction extends Action{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if(userobj!=null)
		{
			...
			logger.info("here in execute method of  DataUploadMakerBehindAction action the session is out----------------");
...
//		CreditManagementDAO service=new CreditManagementDAOImpl();
	   ...
	   DataUploadVO vo=new DataUploadVO();
	   vo.setMakerId(userId);
		logger.info("in DataUploadMakerBehindAction maker id is:"+userId);
		DataUploadAuthorDAO dao=new DataUploadAuthorDAOImpl();
		ArrayList<String> list=dao.searcProcessData(vo);
		if(list.size()>0){
