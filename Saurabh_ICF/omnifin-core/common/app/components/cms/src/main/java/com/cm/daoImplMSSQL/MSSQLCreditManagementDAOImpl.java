...
public class MSSQLCreditManagementDAOImpl implements CreditManagementDAO {
	
	private static final Logger logger = Logger.getLogger(MSSQLCreditManagementDAOImpl.class.getName());
	...
	{
		logger.info("in selectAsset() of MSSQLCreditManagementDAOImpl.");
		...
		query.append("SELECT c.ASSET_ID,c.ASSET_COLLATERAL_DESC,c.ASSET_COLLATERAL_VALUE,c.ASSET_TYPE, ASSET_COLLATERAL_CLASS " +
				" from "+assetTable+" c " +
...
public ArrayList<RepayScheduleVo> getNewRepaySchedulePartPayment(RepayScheduleVo vo, String loanId, String reschId) 
{
	...
	try
	{
		...
		query.append("DATE_FORMAT(R_Due_Date,'"+dateFormat+"'),R_Instl_Amount,R_Prin_Comp,R_Int_Comp,R_EXCESS_Int_Comp," +
				" CASE WHEN R_ADV_FLAG='Y' THEN 'YES' ELSE 'NO' END,R_Prin_OS from Repay_Temp where R_LOAN_ID="+loanId+" order by R_Seq_No");
...
//      cst=con.prepareCall("call Get_Resch_Charges(?,?,?,STR_TO_DATE(?,'"+dateFormat+"'),?,?,?,?,?)");
		...
		in.add(CommonFunction.checkNull(reschType).trim());
		 /*String date=CommonFunction.changeFormat(CommonFunction.checkNull(reschDate).trim());
  	    if(date != null)
  	    	in.add(date);*/
  	    if(CommonFunction.checkNull(reschDate).equalsIgnoreCase("")){
			in.add("");
		}
		else
		{
			in.add(CommonFunction.changeFormat(reschDate).trim());//48
		}
...
//			" where ld.loan_scheme=s.scheme_id and ld.loan_id="+loanId+" ");
	...
	query1.append("CASE WHEN ");
	//queryUpdate1.append(" dbo.STR_TO_DATE(?,'"+dateFormatWithTime+"') + ''+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
	query1.append(" ld.loan_disbursal_date + ''+ substring((cast(CONVERT(int,s.defr_lockin_period) as nvarchar(30))),0,9)");
	//query1.append("DATE_ADD(ld.loan_disbursal_date,INTERVAL s.defr_lockin_period MONTH)");
	query1.append(" >= ");
	query1.append(dbo);
	query1.append("STR_TO_DATE('"+StringEscapeUtils.escapeSql(effBusinessDate)+"','%d-%m-%Y') THEN 'yes' ELSE 'no' END as lockin_period_valid" +
			" from cr_scheme_m s, cr_loan_dtl ld left join cr_resch_dtl r on ld.loan_id=r.loan_id and r.resch_type='D'" +
			" where ld.loan_scheme=s.scheme_id and ld.loan_id="+loanId+" ");
	...
	query2.append("select CASE WHEN s.number_defr_allowed_total> (select count(loan_id) from cr_resch_dtl where" +
			" loan_id="+loanId+" and resch_type='D' and rec_status='A') THEN 'yes' ELSE 'no' END as" +
...
//			" where ld.loan_scheme=s.scheme_id and ld.loan_id="+loanId+" ");
	...
	query3.append("select case  when (select ISNULL(max(RESCH_EFF_DATE),0) from cr_resch_dtl where" +
			" loan_id="+loanId+" and RESCH_TYPE='D' and rec_status='A')=0 then 'yes'" +
			" when (select ISNULL(max(RESCH_EFF_DATE),0) from cr_resch_dtl where loan_id="+loanId+"" +
			" and RESCH_TYPE='D' and rec_status='A')<> 0 THEN ");
	
	query3.append(" case when");
	query3.append(" DATEADD(MONTH,s.minimum_gap_defr,(select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id="+loanId+" and RESCH_TYPE='D' and rec_status='A'))  <=");
	query3.append(dbo);
	query3.append("STR_TO_DATE('"+StringEscapeUtils.escapeSql(effBusinessDate)+"','%d-%m-%Y') then 'yes' else 'no' " +
			" end end as min_gap_succ_defr_valid from cr_scheme_m s, cr_loan_dtl ld" +
			" left join cr_resch_dtl r on ld.loan_id=r.loan_id and r.resch_type='D'" +
			" where ld.loan_scheme=s.scheme_id and ld.loan_id="+loanId+" ");
	...
	query4.append("select DATEPART(year,");
	query4.append(dbo);
	query4.append("str_to_date('"+StringEscapeUtils.escapeSql(effBusinessDate)+"','"+dateFormat+"'))");
	...
	query5.append("select DATEPART(year,resch_eff_date) from cr_resch_dtl where" +
	...
	if(count==0)
	{
		try{
			  ...
			  query.append("select a.loan_id,a.loan_no, b.customer_name, c.product_desc,  d.scheme_desc, " +
			  	 " (select ISNULL(sum(disbursal_amount),0) from cr_loan_disbursal_dtl where loan_id='"+lbxLoanNoHID+"' and rec_status='A')," +
			  ...
			  query.append(dbo);
			  query.append("DATE_FORMAT((select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id='"+lbxLoanNoHID+"' and RESCH_TYPE='D' and rec_status='A'),'"+dateFormat+"')," +
			  ...
			  query.append(dbo);
			  query.append("DATE_FORMAT((select min(instl_date) from cr_repaysch_dtl where loan_id='"+lbxLoanNoHID+"' and bill_flag='N'),'"+dateFormat+"')" +
...
public ArrayList<RepayScheduleVo> getNewRepayScheduleDeferral(RepayScheduleVo vo, String loanId, String reschId) 
{
	...
	try
	{
		...
		query.append(dbo);
		  query.append("DATE_FORMAT(R_Due_Date,'"+dateFormat+"'),R_Instl_Amount,R_Prin_Comp,R_Int_Comp,R_EXCESS_Int_Comp," +
			" case when R_ADV_FLAG='Y' then 'YES' else 'NO' end),R_Prin_OS from Repay_Temp where R_LOAN_ID="+loanId);
		logger.info("Query in getNewRepayScheduleDeferral-----" + query.toString());
		mainlist = ConnectionDAO.sqlSelect(query.toString());
		logger.info("mainlist size: "+mainlist.size());
		for (int i = 0; i < mainlist.size(); i++) 
...
//			+ " and  R.resch_id="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(reschId))+"";
		if(count>0)
		{
			query.append(" select distinct D.FROM_INSTL_NO,R.NEW_INSTL_NO,D.RECOVERY_PERCENT,R.NEW_INSTALLMENT_TYPE,"
		...
		{
			query.append(" select distinct R.DEFERAL_FROM_INSTL,R.NEW_INSTL_NO,D.RECOVERY_PERCENT,R.NEW_INSTALLMENT_TYPE,"
				+ " R.NEW_INSTL_NO,D.PRINCIPAL_AMOUNT,D.INSTALLMENT_AMOUNT,L.LOAN_RATE_TYPE,L.LOAN_BALANCE_PRINCIPAL,"
				+ " D.RECOVERY_TYPE "
				+ " from cr_loan_dtl L, cr_resch_dtl R left JOIN cr_resch_installment_plan D on R.LOAN_ID=D.LOAN_ID"
				+ " and R.RESCH_ID=D.RESCH_ID "
		...
		logger.info("viewNewInstallmentPlanRepricing Queryl: " + query.toString());
		mainList = ConnectionDAO.sqlSelect(query.toString());
		for (int i = 0; i < mainList.size(); i++)
		{
			subList = (ArrayList) mainList.get(i);
...
public boolean saveNewInstallPlan(InstallmentPlanForCMVO ipvo) {
	...
	try 
	{
		 ...
		 for(int k=0;k<FromInstallment.length;k++)  
		 {
			 ...
			 insertPrepStmtObject = new PrepStmtObject();
			 bufInsSql.append("insert into cr_resch_installment_plan (RESCH_ID,LOAN_ID,FROM_INSTL_NO,TO_INSTL_NO,RECOVERY_PERCENT,PRINCIPAL_AMOUNT,INSTALLMENT_AMOUNT,REC_STATUS,SEQ_NO,RECOVERY_TYPE,MAKER_ID,MAKER_DATE) values(?,?,?,?,?,?,?,?,?,?,?,");
...
//abhimanyu upload...
		public ArrayList getUploadSummary(String makerId, HttpServletRequest request)
		{
			...
			logger.info("In..getUploadSummary()...Query"+queryForTMP);
 ...
 //code by manisha for paymentMaker search grid data
		...
		public String getTxnAdviceId(){
			...
			try{
					txnId.append("SELECT max(TXNADVICE_ID) from cr_txnadvice_dtl  WITH (ROWLOCK) ");
		  ...
		  try{
		    ...
		    */
		    bufInsSql.append(" SELECT DISTINCT CLD.LOAN_ID,CLD.LOAN_NO,GCM.CUSTOMER_NAME,CID.BPTYPE, " +		  
			  		...
			  		"  CID.INSTRUMENT_MODE,");
		    bufInsSql.append(dbo);
		    bufInsSql.append("date_format(CID.RECEIVED_DATE,'"+dateFormat+"'),CID.INSTRUMENT_NO,");
		    bufInsSql.append(dbo);
		    bufInsSql.append("date_format(CID.INSTRUMENT_DATE,'"+dateFormat+"')," +
			  		...
			  		" where CID.TXNID=CLD.LOAN_ID and isnull(CID.TXN_TYPE,'')='LIM' " +
...
//		    logger.info("paymentMasearchAuthorDataker search List "+searchlist.get(i).toString());
		...
		//code by manisha for Receipt Maker search grid data
		 ...
		 public  int checkAllocation(ReceiptMakerVO receiptVO){
	  	...
	  	logger.info("IN checkAllocation  ### "+existCount);
	    ...
	    if(!(CommonFunction.checkNull(res)).equalsIgnoreCase("")){
	    	...
	    	result=Integer.parseInt(res.toString());
	  		...
	  		logger.info("IN checkAllocation 1  ### "+existCount);
	    }
	  	   catch(Exception e){
					e.printStackTrace();
				}
		...
		// ReceiptMakerVO vo= new ReceiptMakerVO();
         ...
         logger.info("In receiptAuthorGrid......Amount  "+Amount);
        	 ...
        	 query.append(" SELECT ISNULL(SUM(PMNT_AMOUNT),0) FROM cr_pmnt_dtl WHERE INSTRUMENT_ID='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(receiptVO.getInstrumentID()).trim())+"'");
       ...
       // manisha early clousre viewPayable
            			...
            			try {
            				 ...
            				 for(int k=0;k<FromInstallment.length;k++)  
            				{
            				  PrepStmtObject insertPrepStmtObject = new PrepStmtObject();
            				   ...
            				   bufInsSql.append("update cr_installment_plan set FROM_INSTL_NO=?,TO_INSTL_NO=?,RECOVERY_PERCENT=?,MAKER_ID=?,MAKER_DATE=");
            				   bufInsSql.append(dbo);
            				   bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ''+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
            				   bufInsSql.append(",PRINCIPAL_AMOUNT='0',INSTALLMENT_AMOUNT='0',REC_STATUS='P' where LOAN_ID=? ");
		...
		public boolean saveChargesInCm(ChargeVo ob) {
		    ...
		    PrepStmtObject insertPrepStmtObject = null;
	    		...
	    		// }
	    			StringBuilder query = new StringBuilder(); 
	    			query.append("update cr_txncharges_dtl set DEAL_CHARGE_FINAL_AMOUNT=?,DEAL_CHARGE_CALCULATED_AMOUNT=?,DEAL_CHARGE_TAX_AMOUNT1=?,DEAL_CHARGE_TAX_AMOUNT2=?,DEAL_CHARGE_TDS_AMOUNT=?,DEAL_CHARGE_NET_AMOUNT=?,REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
	    			query.append(dbo);
	    			query.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ''+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
	    			
	    		

	    			//DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) 
...
//		End Of abhi , Ravi code
		...
		public String checkDocMatrix(DocumentsVo vo)
		{
			...
			for(int k=0;k<vo.getDocumentId().length; k++)
			{
				...
				logger.info("docStatus: "+docStatus.toString());
...
//				}
		...
		public boolean saveDocumentPOD(Object ob) {
				...
				for(int k=0;k<size; k++)
				{
					...
					query.append("update cr_document_dtl_temp set doc_status_prev='"+docStatus+"',MAKER_ID='"+CommonFunction.checkNull(vo.getUserId().trim())+"',MAKER_DATE=");
					query.append(dbo);
					query.append("STR_TO_DATE('"+CommonFunction.checkNull(vo.getBussinessDate().trim())+"','"+dateFormatWithTime+"') + ''+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
					...
					catch(Exception e)
					{
						...
						bufInsSql.append(" DOC_REMARKS=?,REC_STATUS=?,MAKER_ID=?,MAKER_DATE=");
						bufInsSql.append(dbo);
						bufInsSql.append("STR_TO_DATE(?,'"+dateFormatWithTime+"') + ''+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
...
//				Changes By Amit Starts
				...
				if(chk.length>0)
				{
					...
					for(int i=0;i<sizeDoc;i++,maxId++)
					{
						...
						bufInsSql.append(" DOC_ORIGINAL, DOC_CHILD_FLAG, REC_STATUS, MAKER_ID,MAKER_DATE,TXN_DOC_ID)");
		...
		public ArrayList<DocumentsVo> getApplicationDocuments(String entityType,String commonId,String stage, String txnType,String recStatus,String source) {
			...
			query.append("DATE_FORMAT(DOC_EXPIRY_DATE,'"+dateFormat+"'), "+
						 "	DOC_MANDATORY,DOC_ORIGINAL,DOC_STATUS,DOC_STATUS_PREV,DOC_REMARKS,Doc_Expiry_Flag,DOC_CHILD_FLAG,DOC_CHILD_IDS,ENTITY_ID "+
						 "	from "+table+" d "+
		...
		try 
		{
			...
			if(!stage.equalsIgnoreCase("PRS"))
			{
				...
				query.append("DATE_FORMAT(DOC_EXPIRY_DATE,'"+ dateFormat+ "'),DOC_MANDATORY,DOC_ORIGINAL,DOC_STATUS,DOC_STATUS_PREV,DOC_REMARKS," +
					" Doc_Expiry_Flag,DOC_CHILD_FLAG,DOC_CHILD_IDS,ENTITY_ID from cr_document_dtl d, cr_loan_collateral_m lc," +
			...
			if(stage.equalsIgnoreCase("PRS"))
			{
				...
				query.append(dbo);
				query.append("DATE_FORMAT(DOC_EXPIRY_DATE,'"+ dateFormat+ "'),DOC_MANDATORY,DOC_ORIGINAL,DOC_STATUS,DOC_STATUS_PREV,DOC_REMARKS," +
					" Doc_Expiry_Flag,DOC_CHILD_FLAG,DOC_CHILD_IDS,ENTITY_ID from cr_document_dtl d, cr_deal_collateral_m dc," +
					" cr_asset_collateral_m ac" +
					" where DOC_TYPE='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(entityType)).trim()+"' " +
					" and TXNID="+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(commonId)).trim()+ " " +
					" and TXN_TYPE='"+ StringEscapeUtils.escapeSql(CommonFunction.checkNull(txnType)).trim()+ "'" +
...
// Function Added by Amit Starts
	...
	public ArrayList getAdditionalDocs(String entityType, String commonId, String stage, String txnType, String recStatus)
	{
		...
		try{
			...
			query.append(dbo);
			query.append("DATE_FORMAT(DOC_EXPIRY_DATE,'"+dateFormat+"'), "+ 
				 "	DOC_MANDATORY,DOC_ORIGINAL,DOC_STATUS,DOC_REMARKS"+ 
	...
	//Function Added by Amit Ends
		...
		public ArrayList getNoOfDisbInloan(String loanId) {
			...
			try{
				...
				query.append(dbo);
				query.append("DATE_FORMAT(LOAN_REPAY_EFF_DATE,'"+dateFormat+"') ,LOAN_NUMBER_DISBURSAL from  cr_loan_dtl WHERE LOAN_ID="+loanId);
			
			logger.info("getNoOfDisbInloan Queryl: "+query.toString());
					...
					if(uncheckedholdList[i].equalsIgnoreCase("test")){
						...
						StringBuilder q = new StringBuilder();
					     q.append("insert into cr_pdc_hold_reason_dtl(pdc_instrument_id,to_status,from_status,hold_reason,main_status,maker_id,maker_date) " +
					     	    ...
					     	    " '"+instructionCapMakerVO.getMakerID()+"',");
					     
						 q.append(dbo);
					     q.append("STR_TO_DATE('"+instructionCapMakerVO.getMakerDate()+"','"+dateFormatWithTime+"') + ''+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
					     
					  

					     //DATE_ADD(STR_TO_DATE('"+instructionCapMakerVO.getMakerDate()+"','"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND)  ) " );
			...
			public ArrayList<InstructionCapMakerVO> getbussinessPartnerList() {
				...
				StringBuilder query = new StringBuilder(); 
				  ...
				  try{
					  ...
					  logger.info("In getBPList:------"+query.toString());	
...
// 
				...
				inSeq.add((CommonFunction.checkNull(chequeStatusVO.getMakerID())));
        		String dateSeq=CommonFunction.changeFormat(CommonFunction.checkNull(chequeStatusVO.getMakerDate()).trim());
        		inSeq.add(dateSeq);
        		outSeq.add(sSeq1);
        		outSeq.add(sSeq2);
        		outSeq.add(SeqId);
...
//        		logger.info("After proc call...."); 
        		...
        		inSeq=null;
	       		...
	       		{
	       			logger.info("FUNCTION_SEQUENCE: "+e);
	       		}
	...
	try{
                ...
                customerName.append(StringEscapeUtils.escapeSql(CommonFunction.checkNull(chequeStatusVO.getCustomerName()).trim()));
       	            ...
       	            StringBuffer bufInsSqlTempCount = new StringBuffer();
       	               ...
       	               {
       	            	bufInsSql.append("select a.instrument_no,");
       	            	bufInsSql.append(dbo);
       	            	bufInsSql.append("DATE_FORMAT(a.INSTRUMENT_DATE,'%d-%m-%Y'),a.INSTRUMENT_AMOUNT,a.BPID,a.BPTYPE,b.bank_name,a.ISSUEING_BANK_ID, "+
       	            	...
       	            	bufInsSql.append(" a.DEPOSIT_BANK_ACCOUNT,u.REASON_ID,r.REASON_DESC,z.loan_no ");
       	            	bufInsSql.append(" from cr_instrument_dtl a "+
       	            			   ...
       	            			   " where    A.instrument_type='P' and isnull(a.TXN_TYPE,'')='LIM' and a.rec_status not in ('P','F') "+
               ...
               /* bufInsSql.append( "select a.instrument_no,DATE_FORMAT(a.INSTRUMENT_DATE,'"+dateFormat+"'),a.INSTRUMENT_AMOUNT,a.BPID,a.BPTYPE,b.bank_name,a.ISSUEING_BANK_ID," +
                ...
                bufInsSqlTempCount.append("select count(1) " +
                		" from cr_instrument_dtl a,com_bank_m b,com_bankbranch_m c,generic_master d,business_partner_view e  where" +
                		"  a.ISSUEING_BANK_ID = b.BANK_ID and a.ISSUEING_BRANCH_ID = c.bank_BRANCH_ID " +
                		" and instrument_type='P'  and a.TXNID = e.LOAN_ID and a.BPTYPE = d.value and a.bpid=e.bp_id and a.bptype=d.value and a.bptype=e.bp_type and isnull(a.TXN_TYPE,'')='LIM' and a.rec_status not in ('P','F') and a.INSTRUMENT_ID not in(select INSTRUMENT_ID from cr_instrument_update_dtl where REC_STATUS in ('P','F') )");
       
       	               }else{
       	            	   
       	            	bufInsSql.append("select a.instrument_no,");
       	            	bufInsSql.append(dbo);
       	            	bufInsSql.append("DATE_FORMAT(a.INSTRUMENT_DATE,'%d-%m-%Y')as INSTRUMENT_DATE,a.INSTRUMENT_AMOUNT,a.BPID,a.BPTYPE,b.bank_name,a.ISSUEING_BANK_ID, "+
       	            			   " c.bank_branch_name,a.ISSUEING_BRANCH_ID,a.ISSUEING_BANK_ACCOUNT,a.REC_STATUS,d.Description,e.bp_name,A.INSTRUMENT_ID,a.tds_amount,"+
       	            			   " a.pdc_instrument_id, a.instrument_type,a.DEPOSIT_BANK_ID,a.DEPOSIT_BRANCH_ID,a.DEPOSIT_MICR_CODE,a.DEPOSIT_IFSC_CODE, "+
       	            	...
       	            	bufInsSql.append(" from cr_instrument_dtl a "+
       	            			   ...
       	            			   " left join com_reason_m r on  r.REASON_ID=u.REASON_ID "+
       	            			   " left join cr_loan_dtl z on a.TXNID = z.loan_id "+
       	            			   " where    A.instrument_type='R' and isnull(a.TXN_TYPE,'')='LIM' and a.rec_status not in ('P','F') "+
       	            			   " and a.INSTRUMENT_ID not in(select INSTRUMENT_ID from cr_instrument_update_dtl where REC_STATUS in ('P','F') ) ");  
	            	...
	            	bufInsSqlTempCount.append("select count(1) " +
                		...
                		" and instrument_type='R' and a.BPTYPE = d.value  and a.TXNID = e.LOAN_ID and a.bpid=e.bp_id and a.bptype=d.value and a.bptype=e.bp_type and isnull(a.TXN_TYPE,'')='LIM' and a.rec_status not in ('P','F') and a.INSTRUMENT_ID not in(select INSTRUMENT_ID from cr_instrument_update_dtl where REC_STATUS in ('P','F') ) ");   
...
//   	               }
                ...
                int endRecordIndex=0;
      			...
      			bufInsSql.append(" ORDER BY a.INSTRUMENT_DATE OFFSET ");
...
//}
	...
	public boolean updateLoginStatus(String userId) {
		...
		try {
			...
			queryList.add(query.toString());
			logger.info("In updateLoginStatus" + query);
			status = ConnectionDAO.sqlInsUpdDelete(queryList);
...
public ArrayList<RepayScheduleVo> getOldRepayScheduleDisbursal(String loanId) 
	{
		...
		try 
		{
			...
			query.append("DATE_FORMAT(INSTL_DATE,'"+dateFormat+"'),INSTL_AMOUNT,PRIN_COMP,INT_COMP,EXCESS_INT, " +
						" case when ADV_FLAG='Y' then 'YES' else 'NO' end,PRIN_OS,OTHER_CHARGES from cr_repaysch_dtl_hst where LOAN_ID="+loanId+" and  ");
			query.append(" ISNULL(REC_STATUS,'A') <>'X'");
...
public ArrayList<InstructionCapMakerVO> getretriveCutInsValues(int lbxLoanNoHID) 
{
	...
	logger.info("In getretriveCutInsValues: "+lbxLoanNoHID);
	  ...
	  ArrayList mainlist=new ArrayList();
	  ArrayList subList=new ArrayList();
//	  query.append("SELECT distinct D.COUNT,C.INSTL_AMOUNT,B.CUSTOMER_NAME,A.LOAN_ID,A.LOAN_NO,e.pdc_instrument_mode,e.pdc_remarks,e.MAKER_REMARKS,A.LOAN_INSTALLMENT_TYPE,SUM(C.OTHER_CHARGES),");
//	  query.append("(ISNULL(SUM(C.OTHER_CHARGES),0)+C.INSTL_AMOUNT) FROM cr_pdc_instrument_dtl e,CR_LOAN_DTL A,GCD_CUSTOMER_M B,CR_REPAYSCH_DTL C," );
//	  query.append(" (SELECT LOAN_ID,count(1) COUNT FROM CR_REPAYSCH_DTL R where R.LOAN_ID='"+(CommonFunction.checkNull(lbxLoanNoHID).trim())+"' group by LOAN_ID) D " );
//	  query.append(" WHERE A.LOAN_CUSTOMER_ID=B.CUSTOMER_ID and a.loan_id = e.pdc_loan_id and A.LOAN_ID=C.LOAN_ID AND A.LOAN_ID=D.LOAN_ID and a.loan_id='"+(CommonFunction.checkNull(lbxLoanNoHID).trim())+"'");

	  query.append("SELECT distinct c.count,D.INSTL_AMOUNT,B.CUSTOMER_NAME,A.LOAN_ID,A.LOAN_NO,e.pdc_instrument_mode,e.pdc_remarks,");
	  query.append("e.MAKER_REMARKS,A.LOAN_INSTALLMENT_TYPE,c.OTHER_CHARGES,(c.OTHER_CHARGES+D.INSTL_AMOUNT) ");
	  query.append("FROM cr_pdc_instrument_dtl e  join CR_LOAN_DTL A on(a.loan_id = e.pdc_loan_id ) join GCD_CUSTOMER_M B on(A.LOAN_CUSTOMER_ID=B.CUSTOMER_ID ) ");
	  query.append("join (select pdc_loan_id,max(pdc_instrument_id)id from cr_pdc_instrument_dtl where pdc_loan_id='"+(CommonFunction.checkNull(lbxLoanNoHID).trim())+"' group by pdc_loan_id)u on(u.pdc_loan_id=e.pdc_loan_id and u.id=e.pdc_instrument_id) ");
	  query.append("left join (select loan_id,count(1) count,SUM(isnull(OTHER_CHARGES,0))OTHER_CHARGES from CR_REPAYSCH_DTL where loan_id='"+(CommonFunction.checkNull(lbxLoanNoHID).trim())+"' group by loan_id)C on(c.loan_id=a.loan_id) ");
	  query.append("left join(select loan_id,INSTL_AMOUNT from CR_REPAYSCH_DTL where instl_no=1 and loan_id='"+(CommonFunction.checkNull(lbxLoanNoHID).trim())+"' )D on (D.loan_id=a.loan_id) ");
	  query.append("where a.loan_id='"+(CommonFunction.checkNull(lbxLoanNoHID).trim())+"'");
...
{
	StringBuilder query=new StringBuilder();
	query.append("select a.DEAL_LOAN_AMOUNT from cr_loan_dtl L left join cr_deal_loan_dtl a on a.DEAL_ID=L.LOAN_DEAL_ID where L.LOAN_ID='"+loanId.trim()+"'");
	return  (String)ConnectionDAO.singleReturn(query.toString());
...
public ArrayList<DocumentsVo> getApplicationDocumentsForDeal(String entityType,String commonId,String txnType,String recStatus) {
	...
	query.append(dbo);
	query.append("DATE_FORMAT(DOC_EXPIRY_DATE,'"+dateFormat+"'), ");
	query.append("	DOC_MANDATORY,DOC_ORIGINAL,DOC_STATUS,DOC_STATUS_PREV,DOC_REMARKS,Doc_Expiry_Flag,DOC_CHILD_FLAG,DOC_CHILD_IDS,ENTITY_ID "+
				 "	from cr_document_dtl d "+
				 "	where DOC_TYPE='"+StringEscapeUtils.escapeSql(CommonFunction.checkNull(entityType)).trim()+"' " +
...
public ArrayList<DocumentsVo> getAssetCollateralDocumentsForTempAtDocCollection(
	...
	try 
	{
		if(!stage.equalsIgnoreCase("PRS"))
		{
			...
			query.append(dbo);
			query.append("DATE_FORMAT(DOC_EXPIRY_DATE,'"+ dateFormat+ "'),DOC_MANDATORY,DOC_ORIGINAL,DOC_STATUS,DOC_STATUS_PREV,DOC_REMARKS," +
				" Doc_Expiry_Flag,DOC_CHILD_FLAG,DOC_CHILD_IDS,ENTITY_ID from "+source+"" +
...
//MANISH
		...
		public boolean updateServiceBranch(LoanInitAuthorVo vo, String loanNo) {
			...
			if ((CommonFunction.checkNull(vo.getServiceBranch())).trim().equalsIgnoreCase(""))
				insertPrepStmtObject.addNull();
			else
				insertPrepStmtObject.addString((vo.getServiceBranch()).trim());
			...
			{
				query1.append("update cr_loan_dtl_edit set service_branch = ? where loanId = ?");
			...
			logger.info("In getretriveCutInsValues list,,,,,"+mainlist.size());
				...
				for(int i=0;i<mainlist.size();i++){
					...
					if(subList.size()>0){
						    ...
						    loanvo.setServiceBranch((CommonFunction.checkNull(subList.get(0)).trim()));
...
@Override
public boolean checkDocuments(String[] instrumentIDList,
...
public ArrayList<LoanDetailForCMVO> selectSblGblWaiverMakerSearch(String loanId,String disbursalNo) 
{
	...
	try{
	bufInsSql.append("select loan_no,disb_no,sblgbl_master_limit,sblgbl_capture_limit,differnce,remarks from cr_sblGbl_tmp where loan_no='"+loanId+"'");		
...
public boolean saveSblGblWaiver(LoanDetailForCMVO vo) 
{
	...
	boolean status=false;
	try{
			bufInsSql.append("insert into CR_SBL_M(loan_no,disb_no,sblgbl_master_limit,sblgbl_capture_limit,differnce,maker_id,MAKER_DATE,rec_status,remarks)");
		bufInsSql.append(" values ( ");
		bufInsSql.append(" ?,");
		bufInsSql.append(" ?,");
		bufInsSql.append(" ?,");
		...
		bufInsSql.append(" ,?,");
		bufInsSql.append("  ?)");
		if (CommonFunction.checkNull(vo.getLoanNo()).equalsIgnoreCase(""))
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getLoanNo().toUpperCase().trim());
		...
		if (CommonFunction.checkNull(vo.getSblGblMasterLimit()).equalsIgnoreCase(""))
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getSblGblMasterLimit().trim());

		if (vo.getSblGblCapturedLimit()==null || vo.getSblGblCapturedLimit().equalsIgnoreCase(""))
			insertPrepStmtObject.addNull();
		else
			insertPrepStmtObject.addString(vo.getSblGblCapturedLimit().trim());
...
public ArrayList<LoanDetailForCMVO> searchSblAuthor(String makerId) 
{
	...
	ArrayList detailList =new ArrayList();
	try{
	bufInsSql.append("select loan_no,disb_no,sblgbl_master_limit,sblgbl_capture_limit,differnce,remarks from cr_sblGbl_tmp where rec_status='F'");		
	logger.info("query : "+bufInsSql);
	searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());		
...
public boolean saveSblGblWaiverAuthor(LoanDetailForCMVO vo){
	...
	boolean status=false;
	try{
		if(vo.getDecision() !=null || vo.getDecision().equalsIgnoreCase("A") || vo.getDecision().equalsIgnoreCase("X")){
			bufInsSql.append("insert into cr_sblGbl_dtl(loan_no,disb_no,sblgbl_master_limit,sblgbl_capture_limit,differnce,maker_id,MAKER_DATE,rec_status,remarks,author_id,author_date,comments)");
		bufInsSql.append(" values ( ");
		bufInsSql.append(" ?,");
		bufInsSql.append(" ?,");
		bufInsSql.append(" ?,");
		...
		logger.info("In insertSblMaster......................"+ status);
			 ...
			 StringBuilder query1=new StringBuilder();
			 query1.append("update cr_sblGbl_tmp set rec_Status='"+vo.getDecision()+"',comments='"+vo.getComments()+"' where loan_no='"+vo.getLoanId()+"'");
			 insertPrepStmtObject.setSql(query1.toString());
...
public ArrayList<LoanDetailForCMVO> searchSblGblAuthorValues(String loanId,String disbursalId) 
{
	...
	ArrayList detailList =new ArrayList();
	try{
	bufInsSql.append("select loan_no,disb_no,sblgbl_master_limit,sblgbl_capture_limit,differnce,remarks from cr_sblGbl_tmp where loan_no='"+loanId+"' and disbursal_no='"+disbursalId+"'");		
	logger.info("query : "+bufInsSql);
	searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());		
	...
	for (int i = 0; i < searchlist.size(); i++) {
		...
		if (data.size() > 0) {
			LoanDetailForCMVO loanDetailForCMVO=new LoanDetailForCMVO();	
				...
				loanDetailForCMVO.setLoanNo("<a href=sblGblMakerSearch.do?method=openSblGblValuesAuthor&loanNo="+data.get(0).toString()+">"+(CommonFunction.checkNull(data.get(0)).trim()) +"</a>");
			loanDetailForCMVO.setDisbursalNo(CommonFunction.checkNull(data.get(1)).toString());
			loanDetailForCMVO.setSblGblMasterLimit(CommonFunction.checkNull(data.get(2)).toString());
			loanDetailForCMVO.setSblGblCapturedLimit(CommonFunction.checkNull(data.get(3)).toString());
			loanDetailForCMVO.setDiff(CommonFunction.checkNull(data.get(4)).toString());
...
public ArrayList<LoanDetailForCMVO> searchSblAuthorValues(String loanNo,String disbursalId) 
{
	...
	ArrayList detailList =new ArrayList();
	try{
	bufInsSql.append("select loan_no,disb_no,sblgbl_master_limit,sblgbl_capture_limit,differnce,remarks from cr_sblGbl_tmp  where loan_No='"+loanNo+"'");		
	logger.info("query : "+bufInsSql);
	searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());		
