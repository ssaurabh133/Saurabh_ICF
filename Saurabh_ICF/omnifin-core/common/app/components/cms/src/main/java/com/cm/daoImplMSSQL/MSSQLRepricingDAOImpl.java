...
public class MSSQLRepricingDAOImpl implements RepricingDAO 
{
	private static final Logger logger = Logger.getLogger(MSSQLRepricingDAOImpl.class.getName());
	...
	public String repricingMakerFeasibility(String lbxLoanNoHID, String effBusinessDate)
	{
		...
		query1.append("select case when ");
		
		query1.append("DATEADD(MONTH,s.resch_lockin_period,ld.loan_disbursal_date)");
		query1.append(" > ");
		query1.append(dbo);
		query1.append("STR_TO_DATE('"+StringEscapeUtils.escapeSql(effBusinessDate)+"','"+dateFormat+"') then  'yes' else 'no' end as lockin_period_valid" +
		...
		StringBuilder query2=new StringBuilder();
		 query2.append( "select case when s.number_resch_allowed_total> (select count(loan_id) from cr_resch_dtl where" +
				        " loan_id="+loanId+" and resch_type='R' and rec_status='A')then 'yes' else 'no' end as" +
		...
		query3.append( "select case  when (select max(RESCH_EFF_DATE) from cr_resch_dtl where" +
				...
				" and RESCH_TYPE='R' and rec_status='A')<> 0 then  ");
		query3.append("case when");
		/*query3.append("(DATE_ADD((select max(RESCH_EFF_DATE) from cr_resch_dtl" +
				" where loan_id="+loanId+" and RESCH_TYPE='R' and rec_status='A')");
		query3.append(",INTERVAL s.MINIMUM_GAP_RESCH MONTH)");*/
		query3.append("(select max(RESCH_EFF_DATE) from cr_resch_dtl" +
				" where loan_id="+loanId+" and RESCH_TYPE='R' and rec_status='A') +''+ substring((cast(CONVERT(int,s.MINIMUM_GAP_RESCH) as nvarchar(30))),0,9)");
		query3.append("<=");
		query3.append(dbo);
		query3.append("STR_TO_DATE('"+StringEscapeUtils.escapeSql(effBusinessDate)+"','"+dateFormat+"') then 'yes' else 'no' end " +
//				" STR_TO_DATE('"+StringEscapeUtils.escapeSql(effBusinessDate)+"','%d-%m-%Y'), 'yes', 'no')" +
//				" (select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id="+loanId+" and RESCH_TYPE='R' and rec_status='A') then 'yes' else 'no' end " +
		...
		query4.append("select DATEPART(year,");			//changed by asesh
		query4.append(dbo);
	 ...
	 query5.append("select DATEPART(year,resch_eff_date) from cr_resch_dtl where resch_type='R' and rec_status='A' and loan_id="+loanId);
...
//		retStr=null;
	 ...
	 tempCount.append(ConnectionDAO.singleReturn(queryCheck.toString()));
		...
		if(count==0)
		{
			try{
				  ...
				  query.append("ISNULL(sum(disbursal_amount),0) from cr_loan_disbursal_dtl where loan_id='"+lbxLoanNoHID+"' and rec_status='A')," +
				  ...
				  query.append(dbo);
				  query.append("DATE_FORMAT((select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id='"+lbxLoanNoHID+"' and RESCH_TYPE='R' and rec_status='A'),'"+dateFormat+"')," +
				  ...
				  query.append(dbo);
				  query.append("DATE_FORMAT((select min(instl_date) from cr_repaysch_dtl where loan_id='"+lbxLoanNoHID+"' and instl_date>(select parameter_value from parameter_mst where parameter_key='BUSINESS_DATE')),'"+dateFormat+"'),");
				  query.append(dbo);
				  query.append("date_format(a.LOAN_MATURITY_DATE,'"+dateFormat+"') as LOAN_MATURITY_DATE , a.LOAN_FLAT_RATE"+
	...
	public ArrayList<PartPrePaymentSearchVO> searchRepricing(PartPrePaymentSearchVO vo,String type)
	{
		...
		try
		{
	        ...
	        bufInsSqlTempCount.append( " from cr_loan_dtl a,gcd_customer_m b," +
	     			 " cr_product_m c, cr_scheme_m d, cr_resch_dtl e where b.customer_id = a.loan_customer_id and c.product_id = a.loan_product"+
	     			 " and d.scheme_id = a.loan_scheme and e.rec_status='"+type+"' and e.resch_type='R' and e.loan_id=a.loan_id AND A.LOAN_BRANCH='"+vo.getBranchId()+"' ");
					...
					{*/
						logger.info("current PAge Link no .................... "+vo.getCurrentPageLink());
						  if(vo.getCurrentPageLink()>1)
						  {
							...
							logger.info("Search searchRepricing query for SQL SERVER : " + bufInsSql.toString());
    ...
    public ArrayList<RepricingMakerVo> selectRericingData(String loanId,String reschId, String recStatus)
  {
	...
	try{
		  ...
		  ArrayList subList=new ArrayList();
		   ...
		   query.append(dbo);
		   query.append("DATE_FORMAT((select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id='"+loanId+"' and RESCH_TYPE='R' and rec_status='A'),'"+dateFormat+"'),e.deferal_from_instl,");
		   query.append(dbo);
		   query.append("DATE_FORMAT(e.RESCH_EFF_DATE,'"+dateFormat+"') as RESCH_EFF_DATE,e.PRE_PAYMENT_PARAMETER,e.REQUEST_NO,"+
		  	...
		  	query.append(dbo);
			query.append( "date_format(e.OLD_DUE_DATE,'"+dateFormat+"') as OLD_DUE_DATE,");
			query.append(dbo);
			query.append("date_format(e.OLD_MATURITY_DATE,'"+dateFormat+"') as OLD_MATURITY_DATE,");
			query.append(dbo);
			query.append("date_format(e.NEW_MATURITY_DATE,'"+dateFormat+"') as NEW_MATURITY_DATE,"+
		  	 " e.GAP_INTEREST,e.GAP_INTEREST_METHOD,e.RESCH_CHARGES,e.RESCH_REASON,e.remarks,a.LOAN_FLAT_RATE"+
			 ...
			 for(int i=0;i<mainlist.size();i++)
			 {
				...
				if(subList.size()>0)
				{
					...
					if((CommonFunction.checkNull(subList.get(28)).trim()).equalsIgnoreCase("S"))
						vo.setInstallmentTypeOld("Gr. Principal2");
	...
	try
	{
		StringBuilder query=new StringBuilder();
		 query.append("SELECT A.BASE_RATE_TYPE FROM cr_base_rate_m A WHERE A.REC_STATUS='A' and A.EFFECTIVE_FROM =" +
			...
			"A.BASE_RATE_TYPE= B.BASE_RATE_TYPE AND B.EFFECTIVE_FROM <= ");
		 query.append(dbo);
...
public ArrayList<RepayScheduleVo> getRepayScheduleDisbursal(String loanId) 
{
	...
	try {
		...
		query.append(dbo);
		query.append("DATE_FORMAT(INSTL_DATE,'"+dateFormat+"') as INSTL_DATE,INSTL_AMOUNT,PRIN_COMP,INT_COMP,EXCESS_INT," +
				" case when ADV_FLAG='Y' then 'YES' else 'NO' end as ADV_FLAG,PRIN_OS from cr_repaysch_dtl where LOAN_ID="+loanId);
...
//        cst.setString(4, CommonFunction.checkNull(vo.getMakerDate()).trim());
	   ...
	   if(CommonFunction.checkNull(vo.getNextDueDate()).trim().equalsIgnoreCase("")){
			in.add("");
		}
		else
		{
			in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getNextDueDate()).trim()));//28
		}
	    /*if(date2 != null)
	    	in.add(date2);//28
	    else
	    ...
	    //String date3=CommonFunction.checkNull(vo.getNextDueDate()).trim();
	    if(CommonFunction.checkNull(vo.getNextDueDate()).trim().equalsIgnoreCase("")){
			in.add("");
		}
		else
		{
			in.add(CommonFunction.checkNull(vo.getNextDueDate()).trim());//29
		}
	    /*if(date3 != null)
	    	in.add(date3);//29
	    else
	    ...
	    if(CommonFunction.checkNull(vo.getMaturityDateOld()).trim().equalsIgnoreCase("")){
			in.add("");
		}
		else
		{
			in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getMaturityDateOld()).trim()));//30
		}
//	    in.add(CommonFunction.checkNull(vo.getMaturityDateOld()).trim());
		...
		{
			in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getMaturityDate()).trim()));//31
		}
...
//       in.add( myFormatter.parse(CommonFunction.checkNull(vo.getInterestForGapPeriod()).trim())).doubleValue());
		...
		{
			in.add(CommonFunction.changeFormat(CommonFunction.checkNull(vo.getMakerDate()).trim()));//40
		}
	    out.add(s3);
	    out.add(s1);
	    out.add(s2);
...
public boolean updateRepricingData(RepricingMakerVo vo, String type)
{
	...
	try {
		...
		query.append("STR_TO_DATE(?,'"+dateFormat+"'), DEFERAL_FROM_INSTL=?," +
				...
				" NEW_INSTL_NO=?, NEW_MATURITY_DATE=");
		query.append(dbo);
		query.append("STR_TO_DATE(?,'"+dateFormat+"'), GAP_INTEREST=?," +
...
//    {
            ...
            {
                  con.setAutoCommit(false);
              cst=con.prepareCall("{call Repricing_Author(?,?,?,?,?,?,?,?,?,?,?,?)}");
              ...
              if(s1.equalsIgnoreCase("S"))
              {
                  status=s1;
                        ...
                        query.append(dbo);
                        query.append("DATE_FORMAT(R_Due_Date,'"+dateFormat+"') as R_Due_Date,R_Instl_Amount,R_Prin_Comp,R_Int_Comp,R_EXCESS_Int_Comp," +
                        " case when R_ADV_FLAG='Y' then 'YES' else 'NO' end as ADV_FLAG,R_Prin_OS from Repay_Temp where R_LOAN_ID="+loanId+" AND MAKER_ID='"+CommonFunction.checkNull(vo.getAuthorId()).trim()+"' order by R_Seq_No");
