...
//Date_add, date_format, str_to_date, ISNULL, date, sysdate, datediff,  if(
...
public class MSSQLSDLiquidationDAOImpl implements SDLiquidationDAO{
	
	private static final Logger logger = Logger.getLogger(MSSQLSDLiquidationDAOImpl.class.getName());
	...
	catch(Exception e)
	{
       ...
       try{
              ...
              bufInsSql.append("select distinct a.loan_no, b.customer_name, a.loan_loan_amount, c.product_desc, d.scheme_desc,"+
				 ...
				 "a.loan_id,e.sd_id,e.sd_liquidation_id, (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=E.MAKER_ID) MAKER_ID ");
			...
			/*  if((loanId==null && customerName==null) || (loanId.toString().equalsIgnoreCase("") && customerName.toString().equalsIgnoreCase("")) 
			  ...
			  {*/
				  ...
				  if(vo.getCurrentPageLink()>1)
				  {
					...
					logger.info("Search searchLiquidationData query for SQL SERVER : " + bufInsSql.toString());
...
//	String gapTds="";
	...
	try {
		...
		query.append("update cr_sd_liquidation_dtl set LIQUIDATION_AMOUNT=?, LIQUIDATION_INTEREST=?, LIQUIDATION_DATE=");
		query.append(dbo);
		...
		query.append(" where loan_id=? and sd_id=? and sd_liquidation_id=?");
	...
	public String getMakerDate(String loanId) 
	{
		...
		query.append("select ");
		query.append(dbo);
		query.append("date_format(MAKER_DATE,'"+dateFormat+"') as MAKER_DATE  from cr_sd_liquidation_dtl where LOAN_ID="+loanId.trim());
	...
	try{
		  query.append("select a.loan_id,a.loan_no, b.customer_name, a.loan_loan_amount, c.product_desc, " +
		  	 ...
		  	 " e.SD_COMPOUNDING_FREQ,");
		query.append(dbo);
		query.append("Date_format(e.SD_START_DATE,'"+dateFormat+"') as SD_START_DATE , ");
		query.append(dbo);
		query.append("Date_format(e.SD_MATURITY_DATE,'"+dateFormat+"') as SD_MATURITY_DATE, " +
		  	 " e.sd_total_interest, ");
		query.append(dbo);
		query.append("Date_format(e.SD_INTEREST_ACRRUED_DATE,'"+dateFormat+"') as SD_INTEREST_ACRRUED_DATE, e.TDS_RATE," +
		  	 " e.TDS_DEDUCTED, (select ");
		query.append("ISNULL(sum(liquidation_amount),0) from cr_sd_liquidation_dtl where rec_STATUS='A' and loan_id="+loanId+" and sd_id="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(sdId).trim())+") as liquidation_amount," +
		  	 " (select ");
		query.append("ISNULL(sum(liquidation_interest),0) from cr_sd_liquidation_dtl where rec_STATUS='A' and loan_id="+loanId+" and sd_id="+StringEscapeUtils.escapeSql(CommonFunction.checkNull(sdId).trim())+")as liquidation_interest,");
		query.append(dbo);
		query.append("Date_format(a.loan_approval_date,'"+dateFormat+"') as loan_approval_date, f.liquidation_amount,f.liquidation_interest," +
		  	 " f.gap_interest, f.gap_tds,f.liquidation_flag, f.remarks,f.MAKER_REMARKS,");
		query.append("(ISNULL(e.sd_total_interest,0)+");
		query.append("ISNULL(e.SD_INTEREST_ACCURED,0)) as finalInterest "+
...
return status;
	...
	try {
		...
		query.append(dbo);
		query.append("Date_format(a.loan_approval_date,'"+dateFormat+"') as loan_approval_date,a.rec_status,a.disbursal_status from cr_loan_dtl a,gcd_customer_m b,"
	...
	{
		try {	
			query.append("select distinct e.sd_amount, e.SD_INTEREST_TYPE, e.SD_INTEREST_RATE,e.SD_COMPOUNDING_FREQ,");
		query.append(dbo);
		query.append("Date_format(e.SD_START_DATE,'"+dateFormat+"') as SD_START_DATE, ");
		query.append(dbo);
		query.append("Date_format(e.SD_MATURITY_DATE,'"+dateFormat+"') as SD_MATURITY_DATE, " +
		...
		query.append("Date_format(e.SD_INTEREST_ACRRUED_DATE,'"+dateFormat+"') as SD_INTEREST_ACRRUED_DATE, e.TDS_RATE," +
				  	 " e.TDS_DEDUCTED, (select ");
		query.append("ISNULL(sum(liquidation_amount),0) from cr_sd_liquidation_dtl f where f.rec_STATUS='A' and f.loan_id="+lbxLoanNoHID+" and f.sd_id="+lbxSdNoHid+") as LIQUIDATION_AMOUNT," +
				  	 " (select ");
		query.append("ISNULL(sum(liquidation_interest),0) from cr_sd_liquidation_dtl f where f.rec_STATUS='A'and f.loan_id="+lbxLoanNoHID+" and f.sd_id="+lbxSdNoHid+") as LIQUIDATION_INTEREST ," );
					 
		query.append("(ISNULL(e.sd_total_interest,0)+");
		query.append("ISNULL(e.SD_INTEREST_ACCURED,0)) as finalInterest from cr_sd_dtl e" +
