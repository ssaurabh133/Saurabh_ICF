...
public class MSSQLManualnpaMovementDAOImpl implements ManualnpaMovementDAO{


	private static final Logger logger = Logger.getLogger(MSSQLManualnpaMovementDAOImpl.class.getName());
	...
	public ArrayList<ManualnpaMovementVO> searchManualNpa(ManualnpaMovementVO vo) {
		...
		try {
			...
			bufInsSql.append("select CLD.LOAN_NO,CMN.loan_id,GDM.CUSTOMER_NAME,(select USER_NAME from sec_user_m where USER_ID=CMN.maker_id) as username,CMN.last_npa_stage,CMN.current_npa_stage,CMN.manual_npa_id " +
					" ");
			bufInsSql.append("from cr_manual_npa_movement CMN,cr_loan_dtl CLD,gcd_customer_m GDM " +
					...
					||( vo.getCurrentPageLink()>1))
			{
				...
				bufInsSql.append(" ORDER BY CMN.loan_id OFFSET ");
...
//Ritu
			...
			query2.append("NPA_STAGE FROM CR_NPA_M WHERE SEQ_NO=(SELECT SEQ_NO FROM CR_NPA_M WHERE NPA_STAGE='"+NpaStage+"') AND NPA_STAGE!='SALEOFF' AND REC_STATUS='A' ORDER BY SEQ_NO ASC ");
			if(!dbType.equalsIgnoreCase("MSSQL") && dbo.equalsIgnoreCase(""))
			{
				query2.append("LIMIT 1");
			}
			logger.info("query2:::::::::::::::::::::::::::::::"+query2.toString());
			String npaNextStage=ConnectionDAO.singleReturn(query2.toString());
		...
		if(status)
		{
			String query="select max(manual_npa_id) id from cr_manual_npa_movement  WITH (ROWLOCK) ";
	...
	try {
		...
		"(select s.SCHEME_DESC from cr_scheme_m s  where s.SCHEME_ID=b.LOAN_SCHEME) as scheme,");
		 bufInsSql.append(dbo);
		 bufInsSql.append("DATE_FORMAT(a.approval_date,'"+dateFormat+"') approvalDate,");
		 bufInsSql.append(dbo);
		 bufInsSql.append("DATE_FORMAT(a.disbursal_date,'"+dateFormat+"') disbursalDate,a.disbursal_amt,a.inst_overdue_amt,comments from cr_manual_npa_movement a,cr_loan_dtl b,gcd_customer_m g " +
	...
	try {
		 ...
		 bufInsSql.append("select a.loan_id,a.LOAN_LOAN_AMOUNT,a.LOAN_BALANCE_PRINCIPAL,a.LOAN_RECEIVED_PRINCIPAL,a.LOAN_OVERDUE_PRINCIPAL,");
		 bufInsSql.append("ISNULL(a.LOAN_OVERDUE_AMOUNT + a.LOAN_BALANCE_PRINCIPAL,0)as totaloverdue," +
		 		"a.LOAN_EMI_AMOUNT,a.LOAN_DPD,a.LOAN_DPD_STRING,a.NPA_FLAG,a.LOAN_NO,");
		 bufInsSql.append(dbo);
		 bufInsSql.append("DATE_FORMAT(a.LOAN_APPROVAL_DATE,'"+dateFormat+"') approvalDate," +
		 		"(select SUM(b.DISBURSAL_AMOUNT) from cr_loan_disbursal_dtl b where b.LOAN_ID=a.loan_id) as DISBURSAL_AMOUNT,(select MAX( ");
		 bufInsSql.append(dbo);
		 bufInsSql.append("DATE_FORMAT(b.DISBURSAL_DATE,'"+dateFormat+"'))  from cr_loan_disbursal_dtl b where b.LOAN_ID=a.loan_id) as disbursalDate,a.LOAN_OVERDUE_AMOUNT"+
	...
	try {
		 ...
		 bufInsSql.append("update cr_manual_npa_movement set rec_status='F',maker_id=?,maker_date=");
		 if(dbType.equalsIgnoreCase("MSSQL"))
		 {
		 	bufInsSql.append(" dbo.STR_TO_DATE(?,'"+dateFormatWithTime+"') + ' '+ substring((cast(CONVERT(time,getdate()) as nvarchar(30))),0,9)");
		 }
		 ...
		 bufInsSql.append("where manual_npa_id=?");
	...
	try {
		...
		query2.append("SELECT TOP 1 NPA_STAGE FROM CR_NPA_M WHERE SEQ_NO=(SELECT SEQ_NO FROM CR_NPA_M WHERE NPA_STAGE='"+NpaStage+"') AND NPA_STAGE!='SALEOFF' AND REC_STATUS='A' ORDER BY SEQ_NO ASC ");
...
//		}
	...
	if(result.equalsIgnoreCase("save")){
		query.append("update cr_manual_npa_movement set current_npa_stage=?,maker_id=?,maker_date=");
		if(dbType.equalsIgnoreCase("MSSQL"))
		 {
		...
		query.append("where manual_npa_id='"+CommonFunction.checkNull(vo.getManualnpaId())+"'");
