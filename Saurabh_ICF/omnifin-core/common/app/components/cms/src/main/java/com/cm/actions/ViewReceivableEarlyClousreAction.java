...
public class ViewReceivableEarlyClousreAction extends Action {
	private static final Logger logger = Logger.getLogger(ViewReceivableEarlyClousreAction.class.getName());
	...
	throws Exception {
		...
		if(userobj==null){
			logger.info("here in execute method of ViewReceivableEarlyClousreAction action the session is out----------------");
		...
		ArrayList<PaymentMakerForCMVO> viewReceivabList= dao.viewReceivableForEarlyClousre(loanId);
		...
		return mapping.findForward("viewReceivableEarly");
