...
public class MSSQLFileTrackingDAOImpl implements FileTrackingDAO {

	private static final Logger logger = Logger.getLogger(MSSQLFileTrackingDAOImpl.class.getName());
		...
		try {
			...
			if(status){
				// Insert status into cr_file_track_status_dtl
				  ...
				  query3.append("Select distinct max(TRACKING_ID) from cr_file_track_dtl WITH (ROWLOCK)");
				...
				queryUpdate.append("update cr_loan_dtl set file_received_date=");
				queryUpdate.append(dbo);
				queryUpdate.append("STR_TO_DATE(?,'"+dateFormat+ "'),file_status=?  where LOAN_ID=? ");
	...
	public boolean updateFileTracking(Object ob) {
		...
		try {
			...
			queryUpdate.append("update cr_file_track_dtl set RECEIVED_DATE=" );
			queryUpdate.append(dbo);
			queryUpdate.append("STR_TO_DATE(?,'"+dateFormat+ "'),FILE_STATUS=?,REMARKS=? ,INDEX_REF_NO=? ,HOLD_REASON=?,MAKER_ID=?,MAKER_DATE=" );
			queryUpdate.append(dbo);
			queryUpdate.append("STR_TO_DATE(?,'"+dateFormat+ "') where LOAN_ID=? ");
			...
			if(status){
				...
				PrepStmtObject updatePrepStmtObject = new PrepStmtObject();
				queryUpdateLoan.append("update cr_loan_dtl set file_received_date=" );
				queryUpdateLoan.append(dbo);		
				queryUpdateLoan.append("STR_TO_DATE(?,'"+dateFormat+ "'),file_status=?  where LOAN_ID=? ");
	...
	public ArrayList searchfileTracking(Object ob) {
		...
		/*String loanNoSearch = (StringEscapeUtils.escapeSql(CommonFunction.checkNull(fileTrackVo.getLoanNoSearch())).trim());
		...
		try {
           ...
           if (!CommonFunction.checkNull(
			...
			logger.info("current PAge Link no .................... "+ fileTrackVo.getCurrentPageLink());
				...
				bufInsSql.append(" ORDER BY t.TRACKING_ID OFFSET ");
			...
			for (int i = 0; i <size ; i++) {
				...
				if (data.size() > 0) {
					...
					fileTrackingVO.setLoanNoSearch("<a href=fileTrackingDispatchAction.do?method=getFileTrackingData&LbxLoanNoHID="+ CommonFunction.checkNull(data.get(1)).toString()+ "&stageStep="+CommonFunction.checkNull(fileTrackVo.getStageStep())+"&viewMode="+ CommonFunction.checkNull(data.get(8)).toString()+"&trackingId="+ CommonFunction.checkNull(data.get(0)).toString()+">"
									+ CommonFunction.checkNull(data.get(2)).toString() + "</a>");
	...
	public ArrayList getFileTrackingDataForView(Object ob) {
		...
		try {
			...
			bufInsSql.append("SELECT t.TRACKING_ID,t.LOAN_ID,d.LOAN_NO," );
			bufInsSql.append(dbo);
			bufInsSql.append("DATE_FORMAT(RECEIVED_DATE,'"+dateFormat+"'),t.file_status,REMARKS,DEAL_ID,s.USER_NAME FROM cr_file_track_dtl t join cr_loan_dtl d on d.loan_id=t.loan_id join sec_user_m s on t.maker_id=s.USER_ID where 1=1  ");
	...
	public boolean getFileTrackingSequenceStatus(String trackingId,	String fileTrackStatus) {
        ...
        boolean status=false;	
	    String statusQuery="SELECT ISNULL(FILE_TRACKING_STATUS,'') FROM CR_FILE_TRACK_STATUS_DTL WHERE TRACKING_ID='"+CommonFunction.checkNull(trackingId)+"' AND  FILE_TRACKING_STATUS<>'SO' ";
        logger.info("check Status Query CR_FILE_TRACK_STATUS_DTL : "+statusQuery.toString());
        try
        {
	...
	{		
		...
		{		
			...
			bufInsSql.append(" JOIN GCD_CUSTOMER_M G ON CLD.LOAN_CUSTOMER_ID=G.CUSTOMER_ID AND ISNULL(CLD.FILE_STATUS,'')='' AND CLD.REC_STATUS<>'X' ");
			...
			if(!CommonFunction.checkNull(fileTrackVo.getStartDate()).equalsIgnoreCase(""))
				bufInsSql.append(" AND dbo.date(LOAN_APPROVAL_DATE) >='"+CommonFunction.changeFormat(CommonFunction.checkNull(fileTrackVo.getStartDate()))+"'");
			if(!CommonFunction.checkNull(fileTrackVo.getEndDate()).equalsIgnoreCase(""))
				bufInsSql.append(" AND dbo.date(LOAN_APPROVAL_DATE) <='"+CommonFunction.changeFormat(CommonFunction.checkNull(fileTrackVo.getEndDate()))+"'");
...
//Manish Space for Bulk file tracking
		...
		try {
			...
			//	{
				...
				if (CommonFunction.checkNull(trackId).equalsIgnoreCase(""))
					...
					insertPrepStmtObject.addString((trackId).trim());
...
// sachin balyan 
	public ArrayList searchfileTrackingOps(Object ob) 
	{
		...
		{	
		   ...
		   if(CommonFunction.checkNull(val).trim().equalsIgnoreCase(""))
			val="100";		
					bufInsSql.append(" select top "+CommonFunction.checkNull(val)+"   b.loan_id,b.loan_no,g.customer_name,s.branch_desc,b.file_status "  );
		    ...
		    if(!CommonFunction.checkNull(fileTrackVo.getStartDate()).equalsIgnoreCase(""))
				bufInsSql.append(" AND CAST(LOAN_APPROVAL_DATE AS DATE ) >='"+CommonFunction.changeFormat(CommonFunction.checkNull(fileTrackVo.getStartDate()))+"'");
			if(!CommonFunction.checkNull(fileTrackVo.getEndDate()).equalsIgnoreCase(""))
				bufInsSql.append(" AND CAST(LOAN_APPROVAL_DATE AS DATE ) <='"+CommonFunction.changeFormat(CommonFunction.checkNull(fileTrackVo.getEndDate()))+"'");
		     	val=null;
					logger.info("In searchfileTracking......... query..........."+ bufInsSql.toString());
					searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());
			int size=searchlist.size();
	...
	public boolean insertfileTrackingOps(Object ob,String[] loanIds,String[] receivedDates,String[] remarks) {
		...
		try {
			...
			for(int i=0; i<loanIds.length; i++)
			{
					...
					queryUpdate.append("update cr_file_track_dtl set RECEIVED_DATE=" );
					queryUpdate.append(dbo);
					queryUpdate.append("STR_TO_DATE(?,'"+dateFormat+ "'),FILE_STATUS='RO',REMARKS=? ,MAKER_ID=?,MAKER_DATE=" );
			        queryUpdate.append(dbo);
			        queryUpdate.append("STR_TO_DATE(?,'"+dateFormat+ "') where LOAN_ID=? ");
			...
			if(status){
					...
					for(int j=0;j<size; j++)
				{
					...
					bufInsSql.append(" ?,");//FILE_REMARKS
					bufInsSql.append(" ?,"); // MAKER_ID
					bufInsSql.append(" dbo.STR_TO_DATE(?,'"+dateFormat+ "'))"); // MAKER_DATE
				...
				if (CommonFunction.checkNull(fileTrackVo.getMakerDate()).equalsIgnoreCase(""))
					...
					queryUpdateLoan.append("update cr_loan_dtl set file_received_date=dbo.STR_TO_DATE(?,'"+dateFormat+ "'),file" +
							"_status='RO'  where LOAN_ID=? ");
	...
	{
		logger.info("In searchfileTrackingtostore...Dao Impl.");
		FileTrackingVO fileTrackVo = (FileTrackingVO) ob;
		FileTrackingVO fileTrackingVO = null;
		ArrayList<FileTrackingVO> fileTrackList = new ArrayList<FileTrackingVO>();		
		ArrayList searchlist = new ArrayList();
		...
		{	
		   ...
		   if(CommonFunction.checkNull(val).trim().equalsIgnoreCase(""))
			...
			if(!CommonFunction.checkNull(fileTrackVo.getEndDate()).equalsIgnoreCase(""))
				bufInsSql.append(" AND CAST(LOAN_APPROVAL_DATE AS DATE ) <='"+CommonFunction.changeFormat(CommonFunction.checkNull(fileTrackVo.getEndDate()))+"'");
		     	val=null;
					logger.info("In searchfileTrackingtostore......... query..........."+ bufInsSql.toString());
					searchlist = ConnectionDAO.sqlSelect(bufInsSql.toString());
			int size=searchlist.size();
			for (int i = 0; i < size; i++) 
			{				
	...
	public boolean insertfileTrackingtostore(Object ob,String[] loanIds,String[] receivedDates,String[] remarks) {
		...
		try {
			...
			for(int i=0; i<loanIds.length; i++)
			{
					...
					queryUpdate.append("update cr_file_track_dtl set RECEIVED_DATE=" );
					queryUpdate.append(dbo);
					queryUpdate.append("STR_TO_DATE(?,'"+dateFormat+ "'),FILE_STATUS='SS',REMARKS=? ,MAKER_ID=?,MAKER_DATE=" );
			        queryUpdate.append(dbo);
			        queryUpdate.append("STR_TO_DATE(?,'"+dateFormat+ "') where LOAN_ID=? ");
			...
			if ((CommonFunction.checkNull(loanIds[i])).trim().equalsIgnoreCase(""))
					...
					insertPrepStmtObject.setSql(queryUpdate.toString());
					qryList.add(insertPrepStmtObject);
					logger.info("IN  insertfileTrackingtostore ### "+ insertPrepStmtObject.printQuery());
					status = ConnectionDAO.sqlInsUpdDeletePrepStmt(qryList);
			if(status){
				...
				if (CommonFunction.checkNull(fileTrackVo.getMakerDate()).equalsIgnoreCase(""))
					...
					queryUpdateLoan=null;
					qryList1.add(updatePrepStmtObject);
					logger.info("IN  insertfileTrackingtostore ### "+ updatePrepStmtObject.printQuery());
					updatePrepStmtObject=null;
					statusUp = ConnectionDAO.sqlInsUpdDeletePrepStmt(qryList1);
				    }
				   }
	...
	public ArrayList getresultForOPS(String trackingId) {
		...
		try {
			...
			bufInsSql.append(" select  VALUE,DESCRIPTION  from generic_master a join cr_file_track_dtl b   on( (CHARINDEX(concat('|',a.VALUE,'|'),concat('|',HOLD_REASON,'|'))>0)) where generic_key='FILE_HOLD_REASON' and tracking_id ='"+ trackingId+ "' " );
