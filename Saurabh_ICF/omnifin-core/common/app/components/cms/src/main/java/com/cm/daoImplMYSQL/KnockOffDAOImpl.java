...
public class KnockOffDAOImpl implements KnockOffDAO {
	...
	private static final Logger logger = Logger.getLogger(KnockOffDAOImpl.class.getName());
...
//		}
	       try{
	              ...
	              bufInsSql.append("select a.loan_no, b.customer_name,e.bp_type,e.bp_name, f.loan_id, f.knockoff_id,f.KNOCKOFF_AMOUNT_RECEIVABLE, (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=F.MAKER_ID) MAKER_ID  " +
					 ...
					 "  where a.loan_id=f.loan_id and b.customer_id = a.loan_customer_id and e.BP_TYPE=f.BP_TYPE and e.BP_ID=f.BP_ID  and e.LOAN_ID=f.LOAN_ID and f.rec_status='"+type+"' AND A.LOAN_BRANCH='"+vo.getBranchId()+"' ");
	              bufInsSqlTempCount.append("select count(distinct a.loan_id) " +
	            	 " from cr_loan_dtl a , gcd_customer_m b, business_partner_view e , cr_knockoff_m f " +
					 " where a.loan_id=f.loan_id and b.customer_id = a.loan_customer_id and e.BP_TYPE=f.BP_TYPE and e.BP_ID=f.BP_ID  and e.LOAN_ID=f.LOAN_ID and f.rec_status='"+type+"' AND A.LOAN_BRANCH='"+vo.getBranchId()+"' ");
	...
	public ArrayList<KnockOffMakerVO> getKnockOffData(String knockOffId,String type) 
	{
	    ...
	    try{
			bufInsSql.append("select distinct a.loan_no, b.customer_name,e.bp_type,e.bp_name, f.bp_id,f.remarks,f.loan_id,F.MAKER_REMARKS,a.REC_STATUS,DATE_FORMAT(f.KNOCKOFF_DATE,'"+dateFormat+"')"+
...
public	boolean saveKnockOffCancelData(String knockOffId, String makerRemarks,String userId,String makerDate,String canStatus)
{
	...
	queryUpdate.append("update cr_knockoff_m set CANCELLATION_FLAG='"+canStatus.trim()+"',CAN_MAKER_REMARKS='"+makerRemarks.trim()+"',CAN_MAKER_DATE=DATE_ADD(STR_TO_DATE('"+makerDate.trim()+"', '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND),CAN_MAKER_ID='"+userId.trim()+"' where knockoff_id='"+knockOffId.trim()+"'");
		...
		try
	       {		    	  
			query.append("select ld.LOAN_ID,ld.LOAN_NO,cc.CHARGE_DESC,ko.TXN_ADVICE_ID,ko.ADVICE_AMOUNT,"+
						" ifnull((ta.ADVICE_AMOUNT-ta.TXN_ADJUSTED_AMOUNT-ta.AMOUNT_IN_PROCESS),0) as BALANCE_AMOUNT,ko.AMOUNT_IN_PROCESS,ko.KNOCKOFF_ID,ko.KNOCKOFF_AMOUNT,DATE_FORMAT(ta.ADVICE_DATE,'"+dateFormat+"') ADVICE_DATE"+
...
//		            int s3 = cst.getInt(13);
			...
			else if(!CommonFunction.checkNull(vo.getKnockOffId()).equalsIgnoreCase(""))
			{
				...
				try
				{
					...
					queryUpdate.append("update cr_knockoff_m set knockoff_date=STR_TO_DATE(?,'"+dateFormatWithTime+"'), knockoff_amount_receivable=?, knockoff_amount_payable=?, rec_status=?," +
							" MAKER_REMARKS=?, maker_id=?, maker_date=STR_TO_DATE(?,'"+dateFormatWithTime+"') where loan_id=? and knockoff_id=?");
					insertPrepStmtObject = new PrepStmtObject();
			...
			try 
			   {
				...
				// aditya start
				 ...
				 query3.append("select KNOCKOFF_ID from cr_knockoff_dtl where knockoff_id='"+knockOffId+"'");
				 boolean qry2Status = ConnectionDAO.checkStatus(query3.toString());
				 StringBuilder qry9 = new StringBuilder();
				 ArrayList list9 = new ArrayList();
				 logger.info("select KNOCKOFF_ID from cr_knockoff_dtl where knockoff_id="+knockOffId);
				 boolean status9 ;
				 logger.info("status ...."+qry2Status);
				 query3=null;
				 
				 if(qry2Status)
				   {
					qry9.append("delete from cr_knockoff_dtl where knockoff_id='"+knockOffId+"'"+"and ADVICE_TYPE='R'");
					logger.info(qry9.toString());
					list9.add(qry9);
					status9 =ConnectionDAO.sqlInsUpdDelete(list9);
					logger.info(status9);
					}
				 qry9=null;
				 list9=null;
				...
				/*	 else if(qry1Status)
					 {
						 ...
						 updateQuery.append("update cr_knockoff_dtl set knockoff_id=?, knockoff_amount=?, rec_status=?, maker_id=?, maker_date=STR_TO_DATE(?,'"+dateFormatWithTime+"') where txn_advice_id=? and seq_no=? and knockoff_id='"+knockOffId+"'");
						 insertPrepStmtObject = new PrepStmtObject();
				 ...
				 query8.append("select KNOCKOFF_ID from cr_knockoff_dtl where knockoff_id='"+knockOffId+"'");
				 boolean qry4Status = ConnectionDAO.checkStatus(query8.toString());
				 StringBuilder qry7 = new StringBuilder();
				 ArrayList list7 = new ArrayList();
				 boolean status7 ;
				 query8 = null;
				 if(qry4Status)
				   {
					qry7.append("delete from cr_knockoff_dtl where knockoff_id='"+knockOffId+"'"+"and ADVICE_TYPE='P'");
					logger.info(qry7.toString());
					list7.add(qry7);
					status7 =ConnectionDAO.sqlInsUpdDelete(list7);
					logger.info(status7);
					}
				 list7=null;
				 qry7=null;
...
//		        in.add((CommonFunction.checkNull(vo.getAuthorDate()).trim()));
	...
	public ArrayList<Object> getKnockOffDetailsDataNew(String loanNumber,String bpId, String adviceType,String bDate){
		...
		try
	       {
			query.append("Select l.LOAN_ID,date_format(c.ADVICE_DATE,'"+dateFormat+"'),d.CHARGE_DESC,c.TXNADVICE_ID,c.ADVICE_AMOUNT,ifnull((c.ADVICE_AMOUNT-c.TXN_ADJUSTED_AMOUNT-c.AMOUNT_IN_PROCESS),0) as BALANCE_AMOUNT,c.AMOUNT_IN_PROCESS"
				...
				+" where c.ADVICE_TYPE='"+adviceType+"' and l.LOAN_ID='"+loanNumber+"' and c.bp_id='"+bpId+"' and c.REC_STATUS='A' and ifnull((c.ADVICE_AMOUNT-c.TXN_ADJUSTED_AMOUNT-c.AMOUNT_IN_PROCESS),0)>0 AND c.ADVICE_DATE<=STR_TO_DATE('"+bDate+"', '"+dateFormat+"') order by c.ADVICE_DATE asc");
