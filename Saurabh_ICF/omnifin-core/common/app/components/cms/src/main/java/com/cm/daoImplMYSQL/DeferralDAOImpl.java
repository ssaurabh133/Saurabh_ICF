...
{
	private static final Logger logger = Logger.getLogger(DeferralDAOImpl.class.getName());
...
//		String userName="";
	...
	public ArrayList<DeferralMakerVo> selectDeferralData(String loanId,String reschId, String recStatus) 
	{
		...
		try{
			  ...
			  query.append("select a.loan_id,a.loan_no, b.customer_name, c.product_desc, d.scheme_desc," +
			  	 ...
			  	 " DATE_FORMAT((select max(RESCH_EFF_DATE) from cr_resch_dtl where loan_id='"+loanId+"' and RESCH_TYPE='D' and rec_status='A'),'"+dateFormat+"')," +
			  	 " e.instl_deferred, e.deferal_from_instl,DATE_FORMAT(e.RESCH_EFF_DATE,'"+dateFormat+"'),e.REQUEST_NO,"+
			  	 ...
			  	 " DATE_FORMAT((select min(instl_date) from cr_repaysch_dtl where loan_id='"+loanId+"' and bill_flag='N'),'"+dateFormat+"')," +
	...
	public String checkDeferralSaveFeasibility(DeferralMakerVo vo)
	{
		...
		StringBuilder query1=new StringBuilder();
		 ...
		 query1.append("select if(s.MAXIMUM_DEFR_MONTHS_ALLOWED< "+StringEscapeUtils.escapeSql(vo.getDeferredInstallmentNo())+", " +
		...
		StringBuilder query2=new StringBuilder();
		 query2.append("select if(s.MAXIMUM_DEFR_MONTHS_TOTAL< (select ifnull(sum(INSTL_DEFERRED),0) from" +
	...
	public boolean updateDeferralData(DeferralMakerVo vo, String type)
	{
		...
		try {
			...
			query.append("update cr_resch_dtl set RESCH_EFF_DATE=STR_TO_DATE(?,'"+dateFormat+"'), DEFERAL_FROM_INSTL=?," +
					...
					" RESCH_REASON=?, REC_STATUS=?, MAKER_ID=?, MAKER_DATE=DATE_ADD(STR_TO_DATE(?, '"+dateFormatWithTime+"'),INTERVAL CURTIME() HOUR_SECOND) where loan_id=? and RESCH_ID=? ");
...
//	        	con.rollback();
	...
	public ArrayList<RepayScheduleVo> getNewRepayScheduleDeferral(RepayScheduleVo vo, String loanId, String reschId) 
	{
	    ...
	    Connection con=ConnectionDAO.getConnection();
	      ...
	      try
	      {
	        logger.info("Input Parameter of Resch_Author: Company Id: "+vo.getCompanyId()+" reschId: "+reschId+" BDate: "+vo.getAuthorDate()+" User ID: "+vo.getAuthorId());
	        con.setAutoCommit(false);
	        cst=con.prepareCall("call Resch_Author(?,?,STR_TO_DATE(?,'"+dateFormat+"'),?,?,?,?,?,?,?,?,?)");
