...
import com.scz.vo.BankUploadFormatVO;
import com.scz.vo.BankUploadUploadVO;
import com.scz.vo.CrSecuritizationPoolDtlTempVO;
...
public class PoolIDDAOImpl implements PoolIDDAO {
	     ...
	     int endRecordIndex = no;	        
	 	 ...
	 	 try
	 	 {
	 		 ...
	 		 logger.info("In searchPoolIdMakerGrid......makerID  "+makerID);       
   	         ...
   	         bufInsSql.append("select P.pool_id,P.pool_name,DATE_FORMAT(P.pool_creation_date,'"+dateFormat+"'),DATE_FORMAT(P.POOL_CUTOFF_DATE,'"+dateFormat+"'),A.AGENCY_CODE,A.AGENCY_NAME,(SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=p.maker_id) USERID  "+
   	         ...
   	         if(!poolId.toString().trim().equalsIgnoreCase("0"))
   	         {
   	   ...
   	   logger.info("endRecordIndex .................... "+endRecordIndex);
   	         ...
   	         for(int i=0;i<searchlist.size();i++)
   	         {
   	        	 ...
   	        	 if(data.size()>0)
   	        	 {
   	        		 ...
   	        		 VO.setPoolID(Integer.parseInt((CommonFunction.checkNull(data.get(0)).trim())));
	...
	public String getPoolNo() 
	{
		 ...
		 query.append("select ifnull(max(pool_id)+1,1) from cr_securitization_pool_m");
		 String generateFirstId = ConnectionUploadDAO.singleReturn(query.toString());
		 query = null;
		 query=new StringBuilder();
		 query.append("select ifnull(max(pool_id)+1,1) from cr_securitization_pool_m_hst");
		 String generateSecondId = ConnectionUploadDAO.singleReturn(query.toString());
		 
		 if(Integer.parseInt(generateFirstId)>Integer.parseInt(generateSecondId))
			 return generateFirstId;
		 else
			 return generateSecondId;
		 
		/* 
		 logger.info("generateId---"+generateFirstId);
		query=null;
		 
		return generateFirstId;*/
	}
	
	public HSSFWorkbook getBankUploadFormat(PoolIdMakerVO poolIdMakerVO) { 
		...
		try{
	        ...
	        HSSFSheet firstSheet = workbook.createSheet("Bank_Upload_Format");
			...
			int i=1;
				  ...
				  String hql = "select poolID,instlNO,instlDate,instlAmount,prinComp,intComp,auPrinComp," +
					"auIntComp,distributionRatioBank,distributionRatioAu,prinOS from BankUploadFormatVO " +
					"where userID='"+poolIdMakerVO.getMakerID()+"' and poolID="+poolIdMakerVO.getPoolID()+"";
			...
			BankUploadFormatVO poolDtlVO = null;
			while(it.hasNext())
			{
				row= firstSheet.createRow(i);
			 Object ob = it.next();
			 Object[] objects = (Object[])ob;
			 	row.createCell(0).setCellValue(String.valueOf(objects[0]));						
			 	row.createCell(1).setCellValue(String.valueOf(objects[1])); 
		        row.createCell(2).setCellValue(DateFormator.ymdToDmy(String.valueOf(objects[2]))); 
		       	row.createCell(3).setCellValue(String.valueOf(objects[3]));
			 	row.createCell(4).setCellValue(String.valueOf(objects[4]));
			 	row.createCell(5).setCellValue(String.valueOf(objects[5]));
			 	
			 	row.createCell(6).setCellValue(String.valueOf(objects[6])); 
		        row.createCell(7).setCellValue(String.valueOf(objects[7])); 
		        row.createCell(8).setCellValue(String.valueOf(objects[8])); 
		       	row.createCell(9).setCellValue(String.valueOf(objects[9]));
		       	row.createCell(10).setCellValue(String.valueOf(objects[10]));
		      	i++;
				row=null;
			}
	...
	public ArrayList downLoadPoolErrorLog(HttpServletRequest request,HttpServletResponse response,String makerid,String poolId){
		...
		/*query.append("select LOAN_ID,LOAN_PRODUCT,LOAN_SCHEME,LOAN_CUSTOMER_ID,LOAN_CUSTOMER_TYPE,LOAN_CUSTOMER_CATEGORY," +
				...
				" WHERE MAKER_ID='"+makerid+"' and uploaded_flag='N'");*/ // comment by saorabh
			 
			 query.append("select LOAN_ID,POOL_ID,LOAN_NO,CUSTOMER_ID,REJECT_REASON " +
						" from SCZ_POOL_UPLOAD_TEMP" +
						" WHERE user_id='"+makerid+"' and uploaded_flag='N' and pool_id = "+poolId);
	...
	public ArrayList<PoolIdMakerVO> searchPoolIdAuthorGrid(PoolIdMakerVO poolIdMakerVO)
	{
	    ...
	    int endRecordIndex = no;
	 	...
	 	{ 	
	 		 ...
	 		 logger.info("temp data "+poolIdMakerVO.getTemp());
	 		 
	 		 type=poolIdMakerVO.getTemp();
	 		  if(type!=null){
	 			  if(type.equalsIgnoreCase("viewer")){
	 		  bufInsSql.append(" select P.pool_id,P.pool_name,P.pool_creation_date,P.POOL_CUTOFF_DATE,A.AGENCY_CODE,A.AGENCY_NAME "+
 		 		" , (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=P.MAKER_ID) MAKER_ID from cr_securitization_pool_m P,com_agency_m A where P.POOL_INSTITUTION_ID=A.AGENCY_CODE " +
	 	 		 		"  and P.rec_status='A'   ");
	 			   bufInsSqlTempCount.append("  select  count(1) from cr_securitization_pool_m P,com_agency_m A where P.POOL_INSTITUTION_ID=A.AGENCY_CODE and P.rec_status='A'   ");
	 			  }else{
	 				  bufInsSql.append(" select P.pool_id,P.pool_name,P.pool_creation_date,P.POOL_CUTOFF_DATE,A.AGENCY_CODE,A.AGENCY_NAME "+
	 		 		 		" , (SELECT USER_NAME FROM SEC_USER_M WHERE USER_ID=P.MAKER_ID) MAKER_ID from cr_securitization_pool_m P,com_agency_m A where P.POOL_INSTITUTION_ID=A.AGENCY_CODE " +
	         ...
	         for(int i=0;i<searchlist.size();i++)
	         {
                 ...
                 if(data.size()>0)
                 {
					 ...
					 VO.setPoolID(Integer.parseInt((CommonFunction.checkNull(data.get(0)).trim())));
					 VO.setPoolName((CommonFunction.checkNull(data.get(1)).trim()));
					 VO.setPoolCreationDate(String.valueOf(data.get(2)));
					 VO.setCutOffDate(String.valueOf(data.get(3)));                 	   
					 VO.setLbxinstituteID((CommonFunction.checkNull(data.get(4)).trim()));
					 VO.setInstituteID((CommonFunction.checkNull(data.get(5)).trim()));
					 VO.setUserName((CommonFunction.checkNull(data.get(6)).trim()));  
					 VO.setTotalRecordSize(count);  
	...
	public boolean forwardPoolIdMaker(int poolID) {
		   ...
		   ArrayList updateQuery =null;
			try{
					...
					updateQuery = new ArrayList();
					 ...
					 quer = session.createQuery("update CrSecuritizationPoolDtlTempVO set REC_STATUS='F' where POOL_ID= :poolID ");
					 quer.setParameter("poolID", StringEscapeUtils.escapeSql(CommonFunction.checkNull(poolID)));
					 int result = quer.executeUpdate();
					 transaction.commit();
					 status=true;
				 
				 if(status){
					 session.beginTransaction();
					 quer = session.createQuery("update PoolIdMakerVO set REC_STATUS='F' where POOL_ID= :poolID ");
					 quer.setParameter("poolID", StringEscapeUtils.escapeSql(CommonFunction.checkNull(poolID)));
					 result = quer.executeUpdate();
	...
	/* Rudra */
	 public boolean deletePoolID(int poolID){
		 ...
		 logger.info("In deletePoolID() method of PoolIDDAOImpl");
			...
			try{
				...
				Query query = session.createQuery("Select count(*) from PoolIdMakerVO where  POOL_ID = :poolID ");
				query.setParameter("poolID", CommonFunction.checkNull(poolID).trim());
				List list = query.list();
				logger.info("count query :-"+query.toString());
				logger.info("count length :-"+list);
				query=null;
				CallableStatement csmt = null;
				String flag=null;
				   String s2 =null;	
				if(!list.isEmpty()){
					csmt = session.connection().prepareCall("{CALL Delete_Pool(?,?,?)}");
			 		csmt.setString(1,CommonFunction.checkNull(poolID));
			 		...
			 		s2 = csmt.getString(3);
			 		logger.info("In deletePoolID() proc status flag:----"+flag);
			 		logger.info("In deletePoolID() proc message s2:----"+s2);
			 		status=true;
				 }
	 ...
	 public ArrayList getPoolData(int poolID,Object poolIdMakerVO) 
	 {
		 logger.info("In getPoolData() method of PoolIDDAOImpl");
			...
			try
			{
				...
				logger.info("Inside getPoolData() method... :");
				query.append("select POOL_ID,LOAN_ID, LOAN_PRODUCT,P.PRODUCT_DESC, LOAN_SCHEME,S.SCHEME_DESC,cm.CUSTOMER_NAME, LOAN_CUSTOMER_TYPE, LOAN_CUSTOMER_CATEGORY," +
					...
					" where pool_id ="+poolID+" and ifnull(A.rec_status,'P') = 'P' ");
				...
				for(int i=0;i<size;i++)
				{
					...
					if(subList.size()>0)
					{
						...
						poolIdMakerVO1.setPoolID(Integer.parseInt((CommonFunction.checkNull(subList.get(0)).trim())));
	 ...
	 public ArrayList<PoolIdMakerVO> savedPoolIdMakerData(PoolIdMakerVO poolIdMakerVO, int poolID) {
		    logger.info("In savedPoolIdMakerData() method of PoolIDDAOImpl");

		    ArrayList savedPoolListGrid = new ArrayList();
		    try
		    {
		      ArrayList mainList = new ArrayList();
		      ArrayList subList = new ArrayList();
		      ...
		      StringBuilder query = new StringBuilder();
		      query.append(new StringBuilder().append("select P.pool_id,P.pool_name,DATE_FORMAT(P.pool_creation_date,'%d-%m-%Y'),DATE_FORMAT(P.POOL_CUTOFF_DATE,'%d-%m-%Y'),P.POOL_TYPE,A.AGENCY_CODE,A.AGENCY_NAME,P.ASSIGNMENT_TYPE,DATE_FORMAT(P.ASSIGNMENT_DATE,'%d-%m-%Y'),P.DEAL_AMOUNT,P.INTEREST_RATE,P.CREDIT_ENHANCE_FLAG,P.MULTI_INVESTOR_FLAG,P.Assignment_Percentage from cr_securitization_pool_m P,com_agency_m A  where P.POOL_INSTITUTION_ID=A.AGENCY_CODE   AND POOL_ID='").append(StringEscapeUtils.escapeSql(CommonFunction.checkNull(Integer.valueOf(poolID)).trim())).append("'").append(" and P.rec_status='P' ").toString());

		      logger.info(new StringBuilder().append("In savedPoolIdMakerData from cr_securitization_pool_m:-").append(query).toString());
		      ...
		      for (int i = 0; i < mainList.size(); i++)
		      {
		        ...
		        {
		          PoolIdMakerVO VO = new PoolIdMakerVO();
		          VO.setHid(Integer.parseInt(CommonFunction.checkNull(subList.get(0)).trim()));
		          VO.setPoolID(Integer.parseInt(CommonFunction.checkNull(subList.get(0)).trim()));
		          ...
		          VO.setAssignType(CommonFunction.checkNull(subList.get(7)).trim());
		          VO.setAssignDate(CommonFunction.checkNull(subList.get(8)).trim());
		          VO.setDealAmount(new BigDecimal(String.valueOf(subList.get(9))));
		          VO.setInterestRate(new BigDecimal(String.valueOf(subList.get(10))));
		          VO.setCreditEnhancement(CommonFunction.checkNull(subList.get(11)).trim());
		          VO.setMultipleInvestor(CommonFunction.checkNull(subList.get(12)).trim());

		          VO.setAssignmentPercentage(CommonFunction.checkNull(subList.get(13)).trim());
		          logger.info(new StringBuilder().append("assignmentPersentage:::::").append(CommonFunction.checkNull(subList.get(13)).trim()).toString());
	 ...
	 public boolean insertLoanforPoolId(PoolIdMakerVO poolIdMakerVO, int poolID){
		 ...
		 logger.info("In insertLoanforPoolId() method of PoolIDDAOImpl");
			...
			try 
		    { 
				CrSecuritizationPoolDtlTempVO asc = new CrSecuritizationPoolDtlTempVO();
				/*asc.setPoolID(poolID);
				asc.setRec_status("P");
		...
		bufInsSql.append("LOAN_OVERDUE_AMOUNT,LOAN_BALANCE_INSTL_NUM,LOAN_BALANCE_INSTL_AMOUNT,REC_STATUS,MAKER_ID,MAKER_DATE)");
							 ...
							 if(CommonFunction.checkNull(poolID).trim().equalsIgnoreCase(""))
								...
								if(CommonFunction.checkNull(poolIdMakerVO.getLbxLoanNoHID()).trim().equalsIgnoreCase(""))
									asc.setLbxLoanNoHID(null);
								else
									asc.setLbxLoanNoHID(poolIdMakerVO.getLbxLoanNoHID().trim());
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanProduct()).trim().equalsIgnoreCase(""))
									asc.setLoanProduct(null);
								else
									asc.setLoanProduct(poolIdMakerVO.getLoanProduct().trim());
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanScheme()).trim().equalsIgnoreCase(""))
									asc.setLoanScheme(0);//Int
								else
									asc.setLoanScheme(Integer.parseInt(poolIdMakerVO.getLoanScheme().trim()));
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanCustomerID()).trim().equalsIgnoreCase(""))
									asc.setLoanCustomerID(0);//Int
								else
									asc.setLoanCustomerID(Integer.parseInt(poolIdMakerVO.getLoanCustomerID().trim()));
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanCustomerType()).trim().equalsIgnoreCase(""))
									asc.setLoanCustomerType(null);
								else
									asc.setLoanCustomerType(poolIdMakerVO.getLoanCustomerType().trim());
								
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanCustomerCtegory()).trim().equalsIgnoreCase(""))
									asc.setLoanCustomerCtegory(null);  //nulll
								else
									asc.setLoanCustomerCtegory(poolIdMakerVO.getLoanCustomerCtegory().trim());
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanCustomerConstituion()).trim().equalsIgnoreCase(""))
									asc.setLoanCustomerConstituion(null);
								else
									asc.setLoanCustomerConstituion(poolIdMakerVO.getLoanCustomerConstituion().trim());
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanCustomerBusinessSeg()).trim().equalsIgnoreCase(""))
									asc.setLoanCustomerBusinessSeg(null);         //nullll
								else
									asc.setLoanCustomerBusinessSeg(poolIdMakerVO.getLoanCustomerBusinessSeg().trim());
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanIndustry()).trim().equalsIgnoreCase(""))
									asc.setLoanIndustry(0);//Int        //nulll
								else
									asc.setLoanIndustry(Integer.parseInt(poolIdMakerVO.getLoanIndustry().trim()));
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanSubIndustry()).trim().equalsIgnoreCase(""))
									asc.setLoanSubIndustry(0);//Int  nulll
								else
									asc.setLoanSubIndustry(Integer.parseInt(poolIdMakerVO.getLoanSubIndustry().trim()));
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanDisbursalDate()).trim().equalsIgnoreCase(""))
									asc.setLoanDisbursalDate(null);
								else
									asc.setLoanDisbursalDate(poolIdMakerVO.getLoanDisbursalDate().trim()); //Date
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanMaturityDate()).trim().equalsIgnoreCase(""))
									asc.setLoanMaturityDate(null);
								else
									asc.setLoanMaturityDate(poolIdMakerVO.getLoanMaturityDate().trim()); //Date
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanTenure()).trim().equalsIgnoreCase(""))
									asc.setLoanTenure(0);//Int
								else
									asc.setLoanTenure(Integer.parseInt(poolIdMakerVO.getLoanTenure().trim()));
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanBalanceTenure()).trim().equalsIgnoreCase(""))
									asc.setLoanBalanceTenure(0);//Int
								else
									asc.setLoanBalanceTenure(Integer.parseInt(poolIdMakerVO.getLoanBalanceTenure().trim()));
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanInstallmentNum()).trim().equalsIgnoreCase(""))
									asc.setLoanInstallmentNum(0);//Int
								else
									asc.setLoanInstallmentNum(Integer.parseInt(poolIdMakerVO.getLoanInstallmentNum().trim()));
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanAdvEMINUm()).trim().equalsIgnoreCase(""))
									asc.setLoanAdvEMINUm(0);//Int
								else
									asc.setLoanAdvEMINUm(Integer.parseInt(poolIdMakerVO.getLoanAdvEMINUm().trim()));
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanInitRate()).trim().equalsIgnoreCase(""))
									asc.setLoanInitRate(null);
								else
									asc.setLoanInitRate(new BigDecimal(poolIdMakerVO.getLoanInitRate().trim())); //Big Decimal
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanDisbursalStatus()).trim().equalsIgnoreCase(""))
									asc.setLoanDisbursalStatus(null);
								else
									asc.setLoanDisbursalStatus(poolIdMakerVO.getLoanDisbursalStatus().trim());
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanNPAFlag()).trim().equalsIgnoreCase(""))
									asc.setLoanNPAFlag(null);
								else
									asc.setLoanNPAFlag(poolIdMakerVO.getLoanNPAFlag().trim());
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanDPD()).trim().equalsIgnoreCase(""))
									asc.setLoanDPD(0); //Int
								else
									asc.setLoanDPD(Integer.parseInt(poolIdMakerVO.getLoanDPD().trim()));
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanDPDString()).trim().equalsIgnoreCase(""))
									asc.setLoanDPDString(null);
								else
									asc.setLoanDPDString(poolIdMakerVO.getLoanDPDString().trim());
								
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanAssetCost()).trim().equalsIgnoreCase(""))
									asc.setLoanAssetCost(null);
								else
									asc.setLoanAssetCost(new BigDecimal(poolIdMakerVO.getLoanAssetCost().trim())); //BIg Decimal
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanAmount()).trim().equalsIgnoreCase(""))
									asc.setLaonAmount(null);
								else
									asc.setLaonAmount(new BigDecimal(poolIdMakerVO.getLoanAmount().trim())); 	//big Decimal
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanEMI()).trim().equalsIgnoreCase(""))
									asc.setLoanEMI(null);
								else
									asc.setLoanEMI(new BigDecimal(poolIdMakerVO.getLoanEMI().trim())); 		//big Decimal
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanAdvEMIAmount()).trim().equalsIgnoreCase(""))
									asc.setLoanAdvEMIAmount(null);  //nulll
								else
									asc.setLoanAdvEMIAmount(new BigDecimal(poolIdMakerVO.getLoanAdvEMIAmount().trim()));//big Decimal
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanBalPrincipal()).trim().equalsIgnoreCase(""))
									asc.setLoanBalPrincipal(null);
								else
									asc.setLoanBalPrincipal(new BigDecimal(poolIdMakerVO.getLoanBalPrincipal().trim())); //big Decimal
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanOverduePrincipal()).trim().equalsIgnoreCase(""))
									asc.setLoanOverduePrincipal(null);
								else
									asc.setLoanOverduePrincipal(new BigDecimal(poolIdMakerVO.getLoanOverduePrincipal().trim()));//big Decimal
								

								if(CommonFunction.checkNull(poolIdMakerVO.getLoanReceivedPrincipal()).trim().equalsIgnoreCase(""))
									asc.setLoanReceivedPrincipal(null);
								else
									asc.setLoanReceivedPrincipal(new BigDecimal(poolIdMakerVO.getLoanReceivedPrincipal().trim()));//big Decimal
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanOverdueInstNo()).trim().equalsIgnoreCase(""))
									asc.setLoanOverdueInstNo(null);
								else
									asc.setLoanOverdueInstNo(new BigDecimal(poolIdMakerVO.getLoanOverdueInstNo().trim()));//big Decimal
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanOverdueAmount()).trim().equalsIgnoreCase(""))
									asc.setLoanOverdueAmount(null);
								else
									asc.setLoanOverdueAmount(new BigDecimal(poolIdMakerVO.getLoanOverdueAmount().trim()));//big Decimal
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanBalnceInstNo()).trim().equalsIgnoreCase(""))
									asc.setLoanBalnceInstNo(null);
								else
									asc.setLoanBalnceInstNo(new BigDecimal(poolIdMakerVO.getLoanBalnceInstNo().trim()));//big Decimal
								
								if(CommonFunction.checkNull(poolIdMakerVO.getLoanBalInstlAmount()).trim().equalsIgnoreCase(""))
									asc.setLoanBalInstlAmount(null);
								else
									asc.setLoanBalInstlAmount(new BigDecimal(poolIdMakerVO.getLoanBalInstlAmount().trim()));//big Decimal
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getMakerID()).trim().equalsIgnoreCase(""))
									asc.setMakerID(null);
								else
									asc.setMakerID(poolIdMakerVO.getMakerID().trim());
								
								
								if(CommonFunction.checkNull(poolIdMakerVO.getMakerDate()).trim().equalsIgnoreCase(""))
									asc.setMakerDate(null);
								else
									asc.setMakerDate(poolIdMakerVO.getMakerDate().trim()); //Date
								*/
...
*/
	 public ArrayList<PoolIdMakerVO> savedPoolIdAuthorData(PoolIdMakerVO poolIdMakerVO, int poolID)
	  {
	    ...
	    try
	    {
	      ...
	      String type = poolIdMakerVO.getTemp();
	      if (type != null) {
	        if (type.equalsIgnoreCase("viewer")) {
	          query.append(new StringBuilder().append("select P.pool_id,P.pool_name,P.POOL_CREATION_DATE,P.POOL_CUTOFF_DATE,P.POOL_TYPE,A.AGENCY_CODE,A.AGENCY_NAME,P.ASSIGNMENT_TYPE,P.ASSIGNMENT_DATE,P.DEAL_AMOUNT,P.INTEREST_RATE,P.CREDIT_ENHANCE_FLAG,P.MULTI_INVESTOR_FLAG,P.Assignment_Percentage from cr_securitization_pool_m P,com_agency_m A where P.POOL_INSTITUTION_ID=A.AGENCY_CODE   AND POOL_ID='").append(StringEscapeUtils.escapeSql(CommonFunction.checkNull(Integer.valueOf(poolID)).trim())).append("'").append(" and P.rec_status='A'  ").toString());
	        }
	        else
	        {
	          query.append(new StringBuilder().append("select P.pool_id,P.pool_name,P.POOL_CREATION_DATE,P.POOL_CUTOFF_DATE,P.POOL_TYPE,A.AGENCY_CODE,A.AGENCY_NAME,P.ASSIGNMENT_TYPE,P.ASSIGNMENT_DATE,P.DEAL_AMOUNT,P.INTEREST_RATE,P.CREDIT_ENHANCE_FLAG,P.MULTI_INVESTOR_FLAG,P.Assignment_Percentage from cr_securitization_pool_m P,com_agency_m A where P.POOL_INSTITUTION_ID=A.AGENCY_CODE   AND POOL_ID='").append(StringEscapeUtils.escapeSql(CommonFunction.checkNull(Integer.valueOf(poolID)).trim())).append("'").append(" and P.rec_status='F' ").toString());
	        }
	      ...
	      for (int i = 0; i < mainList.size(); i++)
	      {
	        ...
	        if (subList.size() > 0) {
	          ...
	          VO.setPoolName(CommonFunction.checkNull(subList.get(1)).trim());
	          VO.setPoolCreationDate(CommonFunction.checkNull(subList.get(2)).trim());
	          logger.info(new StringBuilder().append(" savedPoolIdMakerData setPoolCreationDate:-------------").append(subList.get(2)).toString());
	          VO.setCutOffDate(CommonFunction.checkNull(subList.get(3)).trim());
	          VO.setPoolType(CommonFunction.checkNull(subList.get(4)).trim());
	          VO.setLbxinstituteID(CommonFunction.checkNull(subList.get(5)).trim());
	          logger.info(new StringBuilder().append(" savedPoolIdMakerData setLbxinstituteID:-------------").append(subList.get(5)).toString());
	          VO.setInstituteID(CommonFunction.checkNull(subList.get(6)).trim());
	          VO.setAssignType(CommonFunction.checkNull(subList.get(7)).trim());
	          VO.setAssignDate(CommonFunction.checkNull(subList.get(8)).trim());
	          VO.setDealAmount(new BigDecimal(String.valueOf(subList.get(9))));
	          ...
	          VO.setMultipleInvestor(CommonFunction.checkNull(subList.get(12)).trim());
	          VO.setAssignmentPercentage(CommonFunction.checkNull(subList.get(13)).trim());
	          logger.info(new StringBuilder().append(" savedPoolIdMakerData setLbxinstituteID:-------------").append(subList.get(13)).toString());
	          savedPoolListGrid.add(VO);
	        }
	      }
	    }
	 ...
	 public ArrayList getAuthorPoolData(PoolIdMakerVO poolIdMakerVO,int poolID) 
	 {
	 ...
	 public String  onSaveOfPoolIdAuthor(PoolIdMakerVO poolIdMakerVO,int companyId)
	 {
		logger.info("In onSaveOfPoolIdAuthor() method of PoolIDDAOImpl"); 	    
 	    ...
 	    {               		
    		...
    		in.add( 0);
    		//String date=CommonFunction.changeFormat(CommonFunction.checkNull(poolIdMakerVO.getAuthorDate1()).trim());
    		//in.add(poolIdMakerVO.getAuthorDate1());
    		in.add(poolIdMakerVO.getAuthorDate());
    		...
    		outMessages=(ArrayList) ConnectionReportDumpsDAO.callSPForHiberNate("Securitization_Pool_Authorization",in,out);
	 ...
	 public boolean uploadCsv_BankPool(HttpServletRequest request,HttpServletResponse response,String strFileName,PoolIdMakerVO  poolIdMakerVO)
	 {
	 	   ArrayList alDeleteQuery  = new ArrayList(1);
	 	   boolean status=false;
	 	   try { 
	 		   ...
	 		   String rpt1 =  ConnectionUploadDAO.singleReturn(Query);
	 		    ...
	 		    logger.info("In uploadCsv_BankPool() poolID.. "+poolIdMakerVO.getPoolID());
	 	     	 
	 	     	  KettleEnvironment.init(false); 
	 	     	  EnvUtil.environmentInit(); 
	 	     	  TransMeta transMeta = new TransMeta(rpt1.concat("\\BankPoolUpload.ktr")); 
	 	     	  Trans trans = new Trans(transMeta); 
	 	     	  	 trans.setParameterValue("filePathWithName",CommonFunction.checkNull(poolIdMakerVO.getFilePathWithName()));
	 		     	 trans.setParameterValue("maker_Date",String.valueOf(poolIdMakerVO.getMakerDate1()));
	 ...
	 public boolean uploadCsv_Repayment(HttpServletRequest request,HttpServletResponse response,String strFileName,PoolIdMakerVO  poolIdMakerVO)
	 {
	 	   ArrayList alDeleteQuery  = new ArrayList(1);
	 	   boolean status=false;
	 	   try { 
	 		  ...
	 		  logger.info("In uploadCsv_Securitization() poolID.. "+poolIdMakerVO.getPoolID());
	 		      ...
	 		      KettleEnvironment.init(false); 
	 	     	  EnvUtil.environmentInit(); 
	 	     	  TransMeta transMeta = new TransMeta(rpt1.concat("\\UploadRepayment.ktr")); 
	 	     	  Trans trans = new Trans(transMeta); 
	 	     	  // set parameter. same can be used inside steps in transformation 
	 		      	 trans.setParameterValue("filePathWithName",CommonFunction.checkNull(poolIdMakerVO.getFilePathWithName()));
	 		      	 trans.setParameterValue("maker_Date",String.valueOf(poolIdMakerVO.getMakerDate1()));
	 		      	 trans.setParameterValue("poolID",String.valueOf(poolIdMakerVO.getPoolID()));
	 		     	 trans.setParameterValue("userID",CommonFunction.checkNull(poolIdMakerVO.getMakerID()));
	 		     	 trans.execute(null);	 		// You can pass arguments instead of null. 
	 	     	     trans.waitUntilFinished(); 
	 ...
	 public boolean uploadCsv_Securitization(HttpServletRequest request,HttpServletResponse response,String strFileName,PoolIdMakerVO  poolIdMakerVO)
	 {
	 	   ...
	 	   try { 
	 		  ...
	 		  // 	String strDeleteQuery = "DELETE FROM POOL_CREATION_UPDATE_TMP WHERE MAKER_ID = '"+CommonFunction.checkNull(poolIdMakerVO.getMakerID())+"' ";
	 		 	String strDeleteQuery = "DELETE FROM SCZ_POOL_UPLOAD_TEMP WHERE pool_id = '"+CommonFunction.checkNull(poolIdMakerVO.getPoolID())+"' ";
	 		   logger.info("delete query "+strDeleteQuery);
	 		 	alDeleteQuery.add(strDeleteQuery);
	 	     	  ...
	 	     	  Trans trans = new Trans(transMeta); 
	 		     	 ...
	 		     	 trans.setParameterValue("poolID",String.valueOf(poolIdMakerVO.getPoolID()));
	 		     	 trans.setParameterValue("userID",CommonFunction.checkNull(poolIdMakerVO.getMakerID()));
	 		     	 trans.setParameterValue("makerDate",String.valueOf(poolIdMakerVO.getMakerDate1()));
	 		     	 trans.execute(null); // You can pass arguments instead of null. 
	 	     	     trans.waitUntilFinished(); 
...
//	 		Session session = HibernateSessionFactory.currentSession();
//			Transaction transaction = session.beginTransaction();
//			csmt = session.connection().prepareCall("{CALL SECURITIZATION_POOL_UPLOAD(?,?,?,STR_TO_DATE(?,'"+dateFormat+"'),STR_TO_DATE(?,'"+dateFormat+"'),?,?,STR_TO_DATE(?,'"+dateFormat+"'),?,?,?,?,?)}");
	 		
	 		
	 		Session session = HibernateSessionFactory.currentSession();
	 		 transaction = session.beginTransaction();
			csmt = session.connection().prepareCall("{CALL SECURITIZATION_POOL_UPLOAD(?,?,STR_TO_DATE(?,'"+dateFormat+"'),?,?,?,?,?)}");
	 		csmt.setString(1,"1");
	 		csmt.setString(2,CommonFunction.checkNull(poolIdMakerVO.getPoolID()));
	 		csmt.setString(3,CommonFunction.checkNull(poolIdMakerVO.getMakerDate()));
	 		csmt.setString(4,CommonFunction.checkNull(poolIdMakerVO.getFileName()));
	 		csmt.setString(5,CommonFunction.checkNull(poolIdMakerVO.getMakerID()));
	 	...
	 	/*	csmt = (CallableStatement) con.prepareCall("{CALL SECURITIZATION_POOL_UPLOAD(?,?,?,?,?,?,?,?)}");
	 		csmt.setString(1,"1");
	 		csmt.setString(2,CommonFunction.checkNull(poolIdMakerVO.getPoolID()));
	 		csmt.setString(3,CommonFunction.checkNull(poolIdMakerVO.getMakerDate1()));
	 		csmt.setString(4,CommonFunction.checkNull(poolIdMakerVO.getFileName()));
	 		csmt.setString(5,CommonFunction.checkNull(poolIdMakerVO.getMakerID()));
	 		csmt.registerOutParameter(6, java.sql.Types.VARCHAR);
	 		csmt.registerOutParameter(7, java.sql.Types.VARCHAR);
	 		...
	 		String str="SECURITIZATION_POOL_UPLOAD("+ "1,"+poolIdMakerVO.getPoolID()+", "+poolIdMakerVO.getMakerDate1()+", "+
	 				poolIdMakerVO.getFileName()+", "+poolIdMakerVO.getMakerID()+");";
	 		
	 		logger.info("The proc is : - "+ str);*/
	 	...
	 	return flag.equalsIgnoreCase("S")?"":s2;
	 }

	 
	 public String saveSecuritizationRepayment(HttpServletRequest request,PoolIdMakerVO poolIdMakerVO) {
	 	...
	 	try { 	 
	 		Session session = HibernateSessionFactory.currentSession();
			transaction = session.beginTransaction();
			csmt = session.connection().prepareCall("{CALL Securitization_RePayment_Upload(?,?,STR_TO_DATE(?,'"+dateFormat+"'),?,?,?,?,?)}");
	 		csmt.setString(1,"1");
	 		csmt.setString(2,CommonFunction.checkNull(poolIdMakerVO.getPoolID()));
	 		csmt.setString(3,CommonFunction.checkNull(poolIdMakerVO.getMakerDate()));
	 		csmt.setString(4,CommonFunction.checkNull(poolIdMakerVO.getFileName()));
	 		...
	 		s2 = csmt.getString(8);
	 		 ...
	 		 logger.info("In saveSecuritizationRepayment() flag:----"+flag);
	 		 logger.info("In saveSecuritizationRepayment() s2:----"+s2);
	 		 request.setAttribute("errorMsg", s2);		 
	 	} catch (SQLException exp) {
	 		transaction.rollback();
	 		exp.printStackTrace();
	 ...
	 public String saveSecuritizationBankUpload(HttpServletRequest request,PoolIdMakerVO poolIdMakerVO) {
	 	...
	 	try { 	 
	 		Session session = HibernateSessionFactory.currentSession();
			transaction = session.beginTransaction();
			csmt = session.connection().prepareCall("{CALL Securitization_Bank_Upload(?,?,STR_TO_DATE(?,'"+dateFormat+"'),?,?,?,?,?)}");
	 		csmt.setString(1,"1");
	 		csmt.setString(2,CommonFunction.checkNull(poolIdMakerVO.getPoolID()));
	 		csmt.setString(3,CommonFunction.checkNull(poolIdMakerVO.getMakerDate()));
	 		csmt.setString(4,CommonFunction.checkNull(poolIdMakerVO.getFileName()));
	 		...
	 		s2 = csmt.getString(8);
	 		 ...
	 		 logger.info("In saveSecuritizationBankUpload() flag:----"+flag);
	 		 logger.info("In saveSecuritizationBankUpload() s2:----"+s2);
	 		 request.setAttribute("errorMsg", s2);		 
	 	} catch (SQLException exp) {
	 		transaction.rollback();
	 		exp.printStackTrace();
	 ...
	 public ArrayList getUploadDetails(int poolID){
		 Transaction transaction=null;
		 List list=new ArrayList();
		 try { 	 
		 		Session session = HibernateSessionFactory.currentSession();
				transaction = session.beginTransaction();
				Criteria criteria =  session.createCriteria(poolIdUploadButFlagVO.class);
				criteria.add(Restrictions.eq("pool_id", poolID));
				list = criteria.list();
	 ...
	 public	 ArrayList<PoolIdMakerVO>savedPoolIdEditData(PoolIdMakerVO poolIdMakerVO,int poolID)
		{
	 ...
	 public ArrayList getPoolAddEditData(int poolID) 
		{
