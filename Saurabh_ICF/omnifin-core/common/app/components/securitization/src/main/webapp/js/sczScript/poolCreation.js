...
function generate()
{
	...
	if(cutOffDate !='')
	{
		...
		{
			alert("Invalid Cut Off Date,Only last date of month is valid,which should be " +
					"\n greater than or equal to "+dateList[0]+" but less than "+dateList[i-1]);
			return false;
		}*/
		...
		alert("Cut Off Date is Mandatory.");
		return false;
	}
					...
					var strproduct='';
				     ...
				     for(var count=0; count < product.options.length; count++) {
						...
						var strscheme='';
					     ...
					     for(var count=0; count < state.options.length; count++) {
					    	 stateList=stateList+state.options[count].value+'/';
								}
					     document.getElementById('lbxStateIds').value=stateList;
					     
					     var branchList='';
						 var branch = document.getElementById('branchList');
					     for(var count=0; count < branch.options.length; count++) {
					    	 branchList=branchList+branch.options[count].value+'/';
								}
					     
							document.getElementById('lbxBranchIds').value=branchList;
