...
<! [endif] -->	
   ...
   <html:hidden property="lbxPoolID" styleId="lbxPoolID" />
	...
	<td><bean:message key="lbl.PoolCreationDate"/></td>
			<td><html:text styleClass="text" property="poolCreationDate" styleId="poolCreationDate" onchange="checkDate('poolCreationDate')" /></td>
	     <td><bean:message key="lbl.cutOffDate"/><bean:message key="lbl.dateFormat(dd-mm-yyyy)"/></td>
	    	<td><label><html:text styleClass="text" property="cutOffDate" styleId="cutOffDate" onchange="checkDate('cutOffDate')" /></label></td>	
		</tr>
		<tr>
	<td><bean:message key="lbl.instituteID"/></td>
		   <td><html:text styleClass="text" property="instituteID" styleId="instituteID" maxlength="100"  readonly="true" tabindex="-1"/>   
   <html:hidden property="lbxinstituteID" styleId="lbxinstituteID" />
    <input type="hidden" name="hcommon" id="hcommon" />
	 ...
	 onchange="openLOVCommon(12006,'sourcingForm','poolID','userId','poolID', 'userId','','','poolName','poolCreationDate','','Y');closeLovCallonLov1();"
	  tabindex="-1"/>   
  	 <html:hidden property="lbxPoolID" styleId="lbxPoolID" />
     <input type="hidden" name="hcommon" id="hcommon" />
	 <html:button property="loanNoButton" styleId="loanNoButton" onclick="openLOVCommon(12006,'sourcingForm','poolID','userId','poolID', 'userId','','','poolName','poolCreationDate');closeLovCallonLov1();" value=" " styleClass="lovbutton"></html:button>   
     </td>
     <td><bean:message key="lbl.PoolName"/></td>
									<td><html:text styleClass="text" property="poolName" styleId="poolName"  maxlength="50"  /> </td>       
        </tr>
   ...
   <html:hidden property="lbxinstituteID" styleId="lbxinstituteID" />
		 ...
		 <tr>
           <td align="left">
            <button type="button" name="button" id="search" class="topformbutton2" title="Alt+S" accesskey="S" onclick="return searchForPoolIdMaker();"><bean:message key="button.search" /></button> 
							        <button type="button" name="button" id="new" class="topformbutton2" title="Alt+N" accesskey="N" onclick="newPoolIdMaker();"><bean:message key="button.new" /></button>
	      </td>
		</tr>
				 </table>
	      </td>
    ...
    <display:table  id="sublist"  name="poolIdMakerList" style="width: 100%" class="dataTable" pagesize="${requestScope.no}" cellspacing="1"  partialList="true" size="${poolIdMakerList[0].totalRecordSize}" requestURI="/scz_poolIdMakerBehindAction.do" >
...
<bean:message key="lbl.noDataFound" />
    ...
    <display:table  id="sublist"  name="authordetailList" style="width: 100%" class="dataTable" pagesize="${requestScope.no}" cellspacing="1"  partialList="true" size="${authordetailList[0].totalRecordSize}" requestURI="/scz_poolIdAuthorBehindAction.do" >
    <display:setProperty name="paging.banner.placement"  value="bottom"/>
    <display:setProperty name="locale.resolver" value="org.displaytag.localization.I18nStrutsAdapter"></display:setProperty>
    <display:setProperty name="locale.provider" value="org.displaytag.localization.I18nStrutsAdapter"></display:setProperty>
    <display:column property="modifyNo" titleKey="lbl.poolID"  sortable="true"  />
