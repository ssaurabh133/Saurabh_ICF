...
%>
	...
	<html:form styleId="sourcingForm" action="/monthlyProcessAction" method="post" >   
	...
	<fieldset><legend><bean:message key="lbl.monthlyProcess"/></legend>  
		...
		<tr>
			...
			<td>
		 	...
		 	<td><html:text styleClass="text" readonly="true" property="poolName" styleId="poolName"  maxlength="50"  /> </td>
		</tr>
		<tr>   
			<td><bean:message key="lbl.PoolCutOffDate"/></td>
			<td><html:text styleClass="text"  readonly="true" property="poolCreationDate" styleId="poolCreationDate1" onchange="checkDate('poolCreationDate')" /></td>
	    ...
	    <tr>
		    <td align="left">  
		    	<button type="button" name="button" class="topformbutton3" title="Alt+S" accesskey="O" onclick="checkValidForProcess();">OK</button>
			<logic:present name="s1">
				<button type="button" name="button" class="topformbutton3" title="Alt+S" accesskey="O" onclick="generateMonthlyProcess();">GenerateProcess</button>
			</logic:present>	
			<logic:notPresent name="s1">
				<button type="button" disabled="disabled" name="button" title="Alt+S" accesskey="S" >GenerateProcess</button>
			</logic:notPresent>
			</td>
		</tr>
...
<logic:present name="ref">
	<script type="text/javascript">
		if(!confirm('${ref}')){
			window.location="<%=request.getContextPath()%>/monthlyProcessBehindAction.do";	
		}
	</script>
</logic:present>
...
<logic:present name="sms">
	...
	{
		alert("<bean:message key="msg.monthlyProcessCom" />");
	  
	}
	else if('<%=request.getAttribute("sms").toString()%>'=='E')
	{
		alert("<bean:message key="msg.monthlyProcessErr" />");
