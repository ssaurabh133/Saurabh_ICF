...
public class BuyBackUploadHibImplService implements BuyBackUploadDAO {

	private static final Logger logger = Logger.getLogger(BuyBackUploadHibImplService.class.getName());
	...
	public boolean uploadCsvForBuyBack(HttpServletRequest request, HttpServletResponse response, String strFileName, BuyBackUploadVO  loanUploadVO){
		
		   boolean status=false;
	 	   try { 
	 		
	 		   String delQuery = "delete from SCZ_BUYBACK_UPLOAD_TEMP where maker_id='"+CommonFunction.checkNull(loanUploadVO.getMakerID())+"'";
	 		   ...
	 		   String rpt1 =  ConnectionUploadDAO.singleReturn(Query);
	 	     	  ...
	 	     	  TransMeta transMeta = new TransMeta(rpt1.concat("\\BuyBackCases.ktr")); 
	 	     	  Trans trans = new Trans(transMeta); 
	 	          	 trans.setParameterValue("filePathWithName",CommonFunction.checkNull(loanUploadVO.getFilePathWithName()));
	 		     	 trans.setParameterValue("user_id",CommonFunction.checkNull(loanUploadVO.getMakerID()));
	 		     	 trans.setParameterValue("maker_date",DateFormator.dmyToYMD(CommonFunction.checkNull(loanUploadVO.getMakerDate())));
	 		     	 trans.execute(null);	
	     	  ...
	     	  logger.info("In uploadCsvForBuyBack Problem is ---->"+e.getMessage());
	 	}finally {
	...
	public boolean getFilePath(HttpServletRequest request, FormFile docFile) {
	 	boolean status=false;
	 	String fileName="";
	 	String filePath="";
	 	String message="";
